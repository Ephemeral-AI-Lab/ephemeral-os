...
fn main() -> Result<()> {
    let workspace = find_sandbox_root()?;
...
fn find_sandbox_root() -> Result<PathBuf> {
    ...
    loop {
        ...
        if !dir.pop() {
            anyhow::bail!("could not find sandbox root from current directory");
        }
    }
}
...
fn crate_dirs(workspace: &Path) -> Result<Vec<PathBuf>> {
    ...
    if xtask.join("Cargo.toml").exists() && xtask.join("src").is_dir() {
        dirs.push(xtask);
    }
    dirs.sort();
    Ok(dirs)
...
fn write_crate_file_page(
  ...
  <div class="items">{items}</div>
    ...
    let html = page_shell(
        ...
        &format!(
            r#"<a class="back-link" href="../index.html">sandbox</a>
...
const REFRESH_COMMAND: &str = "cargo run --manifest-path scripts/class-inventory/Cargo.toml";
