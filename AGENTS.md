...
- Treat `eos-agent-sdk/` as the active TypeScript workspace. Use TypeScript,
...
- Run package-manager commands from `eos-agent-sdk/`. The project is a single
  package using `pnpm`; do not introduce npm, yarn, or bun lockfiles.
- Keep module boundaries explicit under `src/`: shared schemas and typed
  contracts belong in `src/contracts`, provider/model API details belong in
  `src/llm-client`, test-only helpers belong in `src/testkit` (the
  `eos-agent-sdk/testkit` subpath export) or `tests/`, and runtime code should
  not depend on test helpers.
...
- For `eos-agent-sdk/`, prefer `pnpm run typecheck`, `pnpm run lint`, and
