...
- `observability/trace/phases/README.md`
- `observability/trace/phases/phase-02-runtime-semantic-spans.md`
- `observability/trace/README.md`, focusing on safe fields, span/event names, telemetry
...
```sh
git status --short
git diff -- crates/sandbox-runtime observability/trace Cargo.lock
git diff --name-only
```
...
git diff --check -- observability/trace crates/sandbox-runtime
...
if rg -n "OccTrace(Event)" observability/trace crates/sandbox-runtime; then exit 1; else exit 0; fi
...
rg -n "sandbox-runtime-trace|internal/telemetry|remount/report|subscriber|exporter|otlp|metrics|dashboard|OccTrace(Event)" crates/sandbox-runtime observability/trace -g '*.rs' -g '*.md'
