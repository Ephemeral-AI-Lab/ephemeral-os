...
import { mintAgentRunId } from "@eos/contracts";
import type { Notifier } from "@eos/notification";

import { RunBackgroundTaskSupervisor } from "../src/background-task-supervisor.js";
import type {
  BackgroundTask,
...
function scriptedTask(
  ...
  const record: ScriptedTask = {
    task: undefined as unknown as BackgroundTask,
    finish,
    fail,
    cancelled: 0,
    completions: [],
...
describe("RunBackgroundTaskSupervisor", () => {
  it("registers a task, lists it, and removes it once onCompletion finishes", async () => {
    ...
    expect(events[0]).toEqual({
      ...
      task: expect.objectContaining({ taskId }),
    });
