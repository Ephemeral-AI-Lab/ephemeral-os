import { mintAgentRunId, type UserMessage } from "@eos/contracts";
import { RunBackgroundTaskSupervisor } from "@eos/background";
import {
  Conversation,
  RunHandle,
  runAgentLoop,
...
import { NotificationInbox, type Notifier } from "@eos/notification";
import {
  HookEngine,
  buildToolExecutor,
...
function startRun<T>(args: StartRunArgs<T>): AgentRunHandle<T> {
  ...
  // The notifier latches on finish: publishes after run end are no-ops.
  let handleRef: RunHandle<T> | undefined;
  const notifier: Notifier = {
    publish(message, opts) {
      if (handleRef?.finished === true) return;
  ...
  const supervisor = new RunBackgroundTaskSupervisor({
    ...
    // Tasks register from tool calls, strictly after the handle exists.
    emit: (event) => handleRef?.emit(event),
  });
