# PLAN v4 — Docker as Default Sandbox Provider

**Mode:** Deliberate consensus (RALPLAN-DR)
**Status:** Revised after a follow-up investigation found that several v3 claims rested on a wrong mental model of how EphemeralOS uses overlay mounts. v4 corrects that model and strips every concern that turned out to be unrelated to introducing the Docker provider. v3 prose is preserved verbatim except where explicitly revised.
...
## 0. Corrected mental model (load-bearing for v4)

Verified from primary sources:

- `kernel_mount.mount_overlay(...)` takes a **single `lowerdir: Path`**, not a list (`backend/src/sandbox/execution/overlay/kernel_mount.py:35-50`). The constructed `mount -t overlay` invocation is `lowerdir=<one_dir>,upperdir=<one_dir>,workdir=<one_dir>` — a **depth-1 overlay** every time.
- `LayerStack.prepare_workspace_snapshot` calls `view.py:195-219 materialize()`, which **hardlinks** all manifest layers into one merged destination directory BEFORE the kernel mount. The kernel never sees multiple lowerdirs.
- `AUTO_SQUASH_MAX_DEPTH = 32` (`backend/src/sandbox/occ/service.py:23`) caps the **manifest** depth, not the overlay-mount lowerdir count.
- Therefore: the util-linux 2.41 multi-lowerdir cliff documented in project memory (`overlay_depth_cap_root_cause.md`) is **not relevant** to this codebase. The kernel always sees the easy single-lowerdir regime.

Every claim in v4 about overlay capability is gated against this depth-1 reality, not the 199-layer regime.
...
4. **Capability-minimum container.** Default Docker run grants only the caps needed for `unshare -Urm` + the single-lowerdir overlay mount EphemeralOS actually constructs (`kernel_mount.mount_overlay` per `execution/overlay/kernel_mount.py:35-50`); the existing `COPY_BACKED` fallback (`execution/strategies/namespace.py:113-134`) is the safety net, not a co-equal mode.
...
1. **Minimize Daytona–Docker behavioral divergence at the host↔daemon seam.** The layer-stack perf story depends on `unshare -Urm` + a single-lowerdir overlay mount working identically inside both providers; capability granting is the load-bearing detail.
2. **Preserve overlay-based layer-stack performance.** The runtime calls `mount -t overlay` once per command exec via `subprocess.run` (`execution/overlay/kernel_mount.py:44-50`). Docker container must allow this syscall path (CAP_SYS_ADMIN + unconfined seccomp). `COPY_BACKED` fallback (`execution/strategies/namespace.py:113-134`) is the safety net, not a co-equal mode.
...
| **A.2 `--cap-add=SYS_ADMIN --security-opt seccomp=unconfined --security-opt apparmor=unconfined`** | minimum surface for user-namespace + overlay mount | Smallest cap surface that unblocks `unshare -Urm` and the kernel mount; pairs with the already-wired `COPY_BACKED` fallback for hosts that still reject it | Still grants SYS_ADMIN — review required if the runtime ever executes untrusted code. **Sufficiency is verified by §6 Step 0 against the single-lowerdir overlay shape EphemeralOS actually constructs** (`kernel_mount.mount_overlay` takes one `lowerdir: Path` per call; `LayerStack.materialize()` flattens the 32-layer manifest into one merged dir at `view.py:195-219`). If a future codebase change introduces multi-lowerdir mounts, Step 0 must be revisited. | **RECOMMENDED default, conditional on Step 0 pre-flight passing.** |
...
**Recommended:** Option A.2 — conditional on the §6 Step 0 pre-flight CI experiment proving sufficiency for the single-lowerdir overlay shape on a Linux CI host. If the pre-flight reveals A.2 is insufficient (e.g., `mount_mode == COPY_BACKED` ratio falls below the §7.3 threshold for the smoke run), the plan says **"demote A.2; expand cap set or escalate to A.1"** — re-trigger consensus loop with the demoted-A.2 finding documented.
...
**Failure:** Container runs in Docker Desktop's Linux VM with non-default UID mapping; `unshare -Urm` succeeds but the single-lowerdir overlay mount returns EPERM despite CAP_SYS_ADMIN, OR overlay mount succeeds but writes silently land in unexpected uid namespace.
...
### Scenario 2 — Host seccomp/apparmor blocks the overlay mount despite CAP_SYS_ADMIN
**Failure:** Some hosts (corp-managed Docker, hardened nodes) enforce a seccomp profile that denies the kernel mount regardless of caps; namespace_child.py exits 125 on first exec.
...
- **`tests/integration_test/test_benchmarks/test_sweevo_docker_smoke.py`** (new, gated by `EOS_SANDBOX_PROVIDER=docker` + `EOS_HAVE_DOCKER=1`): runs **one** SWE-EVO instance end-to-end on Docker.
  ...
  - **Acceptance bar (c, manifest depth not affecting mount-mode classification):** A SWE-EVO run that triggers auto-squash at depth 32 (`AUTO_SQUASH_MAX_DEPTH` per `occ/service.py:23`) still reports `mount_mode=PRIVATE_NAMESPACE` for post-squash execs. This proves the codebase's `materialize()`-then-mount design (`view.py:195-219`) keeps overlay mount at depth-1 regardless of manifest depth.
...
**Purpose:** Empirically confirm Option A.2 (`--cap-add=SYS_ADMIN --security-opt seccomp=unconfined --security-opt apparmor=unconfined`) lets the runtime perform a *single-lowerdir* overlay mount inside `unshare -Urm` — the only overlay shape EphemeralOS actually constructs.

**Action:**
- New script `backend/scripts/preflight_docker_a2_caps.sh` that, inside a Docker container started with the A.2 flag set:
  1. Verify `unshare -Urm true` exits 0.
  2. Run the project's own `detect_private_mount_namespace()` probe from `execution/strategies/namespace.py:137-152` inside the container; expect `True`.
  3. Perform one depth-1 overlay mount and umount inside `unshare -Urm`: create lowerdir + upperdir + workdir + target tmpdirs, write a marker file in lowerdir, mount, read the marker through the merged view, umount. Expect rc=0 on mount, marker readable, rc=0 on umount.
- New CI job (label: `docker-a2-preflight`) runs this script on the Linux CI host matrix; Step 1+ does not land until this job is green.

**Decision rule:**
- **All three probes pass** → A.2 is the default; proceed to Step 1.
...
- **Depth-1 mount returns non-zero** → A.2 insufficient even for the trivial overlay shape; halt.
...
**Verify:** CI job green on Linux runner; results captured in artifact.
...
3. **Docker end-to-end (strengthened from v1):** `EOS_SANDBOX_PROVIDER=docker python -m benchmarks.sweevo --instance-id <known-id> --no-register-snapshot` completes the full lifecycle — `setup_post_lifecycle` → `ensure_git` → `ensure_runtime_uploaded` → `ensure_workspace_base` returns `ready=True`. **Plus all three of:**
   ...
   - **(c) Manifest depth not affecting mount-mode classification:** A SWE-EVO run that triggers auto-squash at depth 32 (`AUTO_SQUASH_MAX_DEPTH` per `occ/service.py:23`) still reports `mount_mode=PRIVATE_NAMESPACE` for post-squash execs. This proves the codebase's `materialize()`-then-mount design keeps overlay mount at depth-1 regardless of manifest depth.
   - **Failure mode if (a), (b), or (c) misses:** abort the default flip; either demote A.2 (per §6 Step 0 decision rule) or document the host-specific cause and re-trigger consensus loop.
...
- **Do NOT** probe multi-lowerdir overlay mount behavior, the util-linux 2.41 multi-lowerdir cliff, or the 199-layer regime. The runtime never constructs that shape: `kernel_mount.mount_overlay` takes a single `lowerdir: Path` (`execution/overlay/kernel_mount.py:35-50`) and `LayerStack.materialize()` flattens manifest layers into one directory before mount (`view.py:195-219`). Layer-count tuning of the kernel mount path is unrelated to "introduce Docker provider" and out of scope.
- **Do NOT** migrate `execution/overlay/kernel_mount.py:44-50` from `subprocess.run(["mount", ...])` (mount(8)) to `mount(2)` ctypes in this plan. See ADR §Follow-ups for the (re-scoped) note.
...
2. Preserve overlay-based layer-stack performance — the single-lowerdir `mount -t overlay` call at `execution/overlay/kernel_mount.py:44-50` must remain available inside the Docker container.
...
**Why chosen:**
- Option A.2 is the minimum-cap surface that unblocks `unshare -Urm` + the single-lowerdir overlay mount EphemeralOS actually constructs; conditional on the Step 0 pre-flight CI experiment proving sufficiency. Pairs with the already-wired `COPY_BACKED` fallback for hosts that reject SYS_ADMIN.
...
- Replace `subprocess.run(["mount", ...])` in `kernel_mount.py:44-50` with `mount(2)` ctypes only if per-exec subprocess fork becomes a measured bottleneck. Layer count is NOT a driver — `materialize()` flattens the manifest before mount, so the kernel sees one lowerdir regardless of manifest depth. Sub-millisecond perf nit, not architectural.
