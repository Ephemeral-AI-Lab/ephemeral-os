# PLAN v2 — Docker as Default Sandbox Provider

**Mode:** Deliberate consensus (RALPLAN-DR)
**Status:** Revised after CRITIC_REVIEW_v1.md (applies all 9 required revisions)
...
| **A.2 `--cap-add=SYS_ADMIN --security-opt seccomp=unconfined --security-opt apparmor=unconfined`** | minimum surface for user-namespace + overlay mount | Smallest cap surface that unblocks `unshare -Urm` and `mount(2)`; pairs with the already-wired `COPY_BACKED` fallback for hosts that still reject it | Still grants SYS_ADMIN — review required if the runtime ever executes untrusted code. **Sufficiency for the full layer-stack overlay+OCC code path (199+ layer cap, `mount(2)` syscall with kernel-direct flags) is asserted but unverified.** A.2 might cover `unshare -Urm` yet silently degrade to `COPY_BACKED` on deeper kernel paths (pivot_root, devmount, fanotify under hardened seccomp). The pre-flight CI experiment in §6 Step 0 is what closes this risk: if the experiment reveals A.2 is insufficient, demote A.2 to opt-in, expand cap set, or escalate to A.1 with documented justification. | **RECOMMENDED default, conditional on Step 0 pre-flight passing.** |
...
- New script `backend/scripts/preflight_docker_a2_caps.sh` that:
  ...
  3. Inside the container, executes:
     - `unshare -Urm true` → expect exit 0.
     - The actual `detect_private_mount_namespace()` probe from `execution/strategies/namespace.py:137-152` (lift the same script body) → expect `True`.
     - A minimal layer-stack overlay mount via the same `mount(2)` syscall path used by `execution/overlay/kernel_mount.py` → expect 0 layers fail, up to ≥16 layers (matching the util-linux `mount(8)` boundary from project memory) and ideally toward the 199+ direct-syscall cap.
...
**Decision rule:**
- **All preflight steps pass** → A.2 is the default; proceed to Step 1.
- **`unshare -Urm` fails** → A.2 insufficient; demote to opt-in, document required cap delta, halt plan and re-trigger consensus loop.
- **`detect_private_mount_namespace()` returns False or `mount(2)` overlay fails at low layer counts** → A.2 insufficient for the perf-critical path; same demotion + halt.
- **Layer-mount succeeds at low counts but degrades vs. host bare-metal beyond expected overhead** → log as known caveat, proceed but tighten §7.3 perf delta to ±15% from ±25% to flag the regression.
