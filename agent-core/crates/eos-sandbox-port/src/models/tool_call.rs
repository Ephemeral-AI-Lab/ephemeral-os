...
impl ToolCallRequest {
    ...
    pub fn from_payload(payload: &JsonObject) -> Result<Self, crate::error::SandboxPortError> {
        let args = match payload.get("args") {
            None | Some(Value::Null) => JsonObject::new(),
            Some(Value::Object(map)) => map.clone(),
            Some(_) => {
                return Err(crate::error::SandboxPortError::decode(
        ...
        let invocation_id = invocation_raw.parse::<InvocationId>().map_err(|_| {
            crate::error::SandboxPortError::decode("tool-call payload missing invocation_id")
        })?;
        let intent = match payload.get("intent").and_then(Value::as_str) {
            ...
            Some(other) => {
                return Err(crate::error::SandboxPortError::decode(format!(
