...
// --- AC-eos-agent-core-04: single-place graph construction + network-url fail-fast.
...
// AC-eos-agent-core / AC9: the backend composition root reads agent-core state only
// through `AgentCoreRuntime::state_reader()`. Prove the seam returns live store
...
async fn network_database_url_fails_fast() {
    let result = AgentCoreRuntime::builder()
...
// --- AC-eos-agent-core-06: model registry seeds from provider-nested config.
...
async fn provider_models_seed_model_registry() {
    ...
    let _state = AgentCoreRuntime::builder()
...
// --- AC-eos-agent-core-09: unknown profile tool fails startup.
...
// --- AC-eos-agent-core-01: root request mints a root Task, no root workflow.
...
// --- AC-eos-agent-core-02 / -08: root success keeps the engine-stamped terminal.
...
// --- AC-eos-agent-core-03: an unfinished root fails cleanly with run_exhausted.
