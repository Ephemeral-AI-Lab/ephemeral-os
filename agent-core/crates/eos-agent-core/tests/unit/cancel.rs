...
//! live `AgentCoreCancellationApi` and marks the request `Cancelled`, idempotently.

use std::sync::Mutex;

use async_trait::async_trait;
use eos_types::{AgentCoreCancellationApi, AgentRunId, CancelError, TaskId};

use super::*;
use crate::cancel::cancel_agent_core_user_request;

/// An `AgentCoreCancellationApi` fake that records the `cancel_task` ids it receives.
#[derive(Default)]
struct RecordingAgentCoreCancellation {
    cancelled_tasks: Mutex<Vec<TaskId>>,
}

#[async_trait]
impl AgentCoreCancellationApi for RecordingAgentCoreCancellation {
...
/// A live request: cancellation reaches the request's API with the root task id
...
async fn cancel_request_reaches_live_port_and_marks_cancelled() {
    ...
    let cancellation_api = std::sync::Arc::new(RecordingAgentCoreCancellation::default());
    let _guard = services
        .cancel_registry
    ...
    assert!(
        report.had_live_run,
        "a registered cancellation API is a live run"
    );
    assert_eq!(
        cancellation_api.cancelled_tasks.lock().unwrap().as_slice(),
...
/// An already-finished request (no live cancellation API): cancellation is an idempotent
...
async fn cancel_finished_request_is_noop() {
    ...
    assert!(
        !report.had_live_run,
        "no cancellation API registered for a finished request"
    );
    let request = services
        .db
        .request_store
