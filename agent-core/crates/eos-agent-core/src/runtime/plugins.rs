...
//!
//! Runtime-local catalog modules own the declared model-facing specs. This
//! module binds those specs into real `eos-tool` executors that ensure the
//! daemon manifest for built-in plugin runtimes and then dispatch dynamic
//! `plugin.<plugin>.<op>` daemon operations.
...
use self::catalog::{plugin_package_descriptor, plugin_tool_specs, PluginToolSpec};
