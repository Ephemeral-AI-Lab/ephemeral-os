...
//! block into an empty mapping. The tool-owned skill loader reuses the
//! `eos-types` helper and `eos-agent-def` keeps its own `trim`-based raw
//! variant, so consolidating the three would change parse/error behavior.
