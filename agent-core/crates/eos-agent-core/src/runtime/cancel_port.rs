...
use eos_types::{AgentCoreCancellationApi, AgentRunApi, AgentRunId, CancelError, TaskId};
use eos_types::{AgentRunStore, JsonObject, TaskStatus, TaskStore};

/// Request-scoped cancellation implementation wired to the runner service.
#[derive(Clone)]
pub(crate) struct RuntimeAgentCoreCancellation {
    task_store: Arc<dyn TaskStore>,
    agent_run_store: Arc<dyn AgentRunStore>,
    agent_run_api: Arc<OnceLock<Arc<dyn AgentRunApi>>>,
}

impl std::fmt::Debug for RuntimeAgentCoreCancellation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RuntimeAgentCoreCancellation")
            .finish_non_exhaustive()
    }
}

impl RuntimeAgentCoreCancellation {
    pub(crate) fn new(
        task_store: Arc<dyn TaskStore>,
        agent_run_store: Arc<dyn AgentRunStore>,
        agent_run_api: Arc<OnceLock<Arc<dyn AgentRunApi>>>,
...
#[async_trait]
impl AgentCoreCancellationApi for RuntimeAgentCoreCancellation {
