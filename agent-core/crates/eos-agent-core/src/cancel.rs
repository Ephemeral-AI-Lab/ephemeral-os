...
use crate::runtime::AgentCoreRuntime;
...
/// run it owns) through the request's `AgentCoreCancellationApi`, then mark the request
/// `Cancelled`.
///
...
pub async fn cancel_agent_core_user_request(
    ...
    let cancellation_api = services.cancel_registry.get(request_id);
    let had_live_run = cancellation_api.is_some();
    if let Some(cancellation_api) = cancellation_api {
        ...
        cancellation_api.cancel_task(&root_task_id, reason).await?;
    }
    services
        .db
        .request_store
