...
//!
//! Each is a `#[repr(transparent)]` newtype over `String` (the Python IDs are
//! `UUIDv4` strings and prefixed strings such as `root-<hex16>`, so the inner
//! type is `String`, never an integer — hence not `Copy`). All are generated by
//! the `pub(crate)` [`define_id!`] codegen helper (DRY); it is an internal
//! helper, not a public extension point, and is not re-exported.
...
/// byte-identical to the Python strings. The non-empty invariant is enforced on
...
macro_rules! define_id {
    ...
    (@core $(#[$meta:meta])* $name:ident) => {
        ...
        impl $name {
            ...
            /// Consume the identifier, returning the owned inner `String`.
            #[must_use]
