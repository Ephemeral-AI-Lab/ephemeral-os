...
//! (GC-plugin-catalog-05) — `eos-runtime` binds each [`PluginToolSpec`] into a
//! real `eos_llm_client::ToolSpec` + a `ToolExecutor`, which is why this crate
//! has no `eos-tools`/`eos-llm-client` edge (GC-plugin-catalog-04).
//!
...
use crate::names::PluginToolName;

/// A catalog-native, model-facing tool-spec **source** (not an
/// `eos_llm_client::ToolSpec`). `eos-runtime` binds this into a real `ToolSpec`
