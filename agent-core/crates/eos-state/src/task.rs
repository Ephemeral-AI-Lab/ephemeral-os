...
//! `eos-agent-def`'s richer `AgentRole`) to keep `eos-state` upstream of
//! `eos-agent-def` in the dependency DAG (spec §3, GC-eos-state-05).

use schemars::JsonSchema;
use serde::{Deserialize, Serialize};

use eos_types::{AttemptId, IterationId, JsonObject, RequestId, TaskId, WorkflowId};
