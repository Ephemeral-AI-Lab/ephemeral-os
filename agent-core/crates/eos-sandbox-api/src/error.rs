...
//! `Box<dyn Error>` crosses the public boundary. `SandboxApiError` has two
...
/// constructible from any crate via [`SandboxApiError::transport`] /
/// [`SandboxApiError::decode`] so the downstream daemon-client implementor
/// (`eos-sandbox-host`) can build them.
///
/// [`SandboxTransport`]: crate::SandboxTransport
#[derive(Debug, Clone, thiserror::Error)]
#[non_exhaustive]
pub enum SandboxApiError {
...
impl SandboxApiError {
    /// Build a [`SandboxApiError::Transport`] from an optional daemon error code
    ...
    /// Build a [`SandboxApiError::Decode`] from a mismatch description.
    #[must_use]
