...
impl Submission {
    ...
    pub fn new(
        ...
        Self {
            inner: Arc::new(SubmissionInner::Live {
                task_store,
                request_store,
                attempt,
            }),
    ...
    pub(crate) fn schema() -> Self {
        Self {
            inner: Arc::new(SubmissionInner::Schema),
        }
    }

    pub(crate) fn task_store(&self) -> Result<Arc<dyn TaskStore>, ToolError> {
        match &*self.inner {
            SubmissionInner::Live { task_store, .. } => Ok(task_store.clone()),
            SubmissionInner::Schema => Err(ToolError::Internal(
    ...
    pub(crate) fn request_store(&self) -> Result<Arc<dyn RequestStore>, ToolError> {
        match &*self.inner {
            SubmissionInner::Live { request_store, .. } => Ok(request_store.clone()),
            SubmissionInner::Schema => Err(ToolError::Internal(
                "schema-only registry cannot execute tools".to_owned(),
            )),
        }
    ...
    pub(crate) fn attempt(&self) -> Result<Arc<dyn AttemptSubmissionPort>, ToolError> {
        match &*self.inner {
            SubmissionInner::Live { attempt, .. } => Ok(attempt.clone()),
            SubmissionInner::Schema => Err(ToolError::Internal(
                "schema-only registry cannot execute tools".to_owned(),
            )),
        }
...
pub struct HookServices {
    ...
    background: Option<Arc<dyn BackgroundSessions>>,
}
...
impl HookServices {
    ...
    pub fn new(runtime: &ToolRuntime) -> Self {
        Self {
            ...
            background: Some(runtime.background.clone()),
        }
    }
    ...
    /// Background-session aggregate used by hook checks.
    #[must_use]
    pub fn background(&self) -> Option<&Arc<dyn BackgroundSessions>> {
...
pub(crate) fn command_api_from_sandbox(transport: Arc<dyn SandboxTransport>) -> Arc<dyn SandboxCommandApi> {
