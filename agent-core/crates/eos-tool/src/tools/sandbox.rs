...
use eos_sandbox_port::SandboxRequestBase;
use serde::Serialize;
...
mod read_file {
    ...
    impl ToolExecutor for ReadFile {
        async fn execute(
            ...
            let result =
                match eos_sandbox_port::read_file(&*self.service.transport, sandbox_id, &request)
                    .await
                {
                    Ok(result) => result,
...
mod write_file {
    ...
    impl ToolExecutor for WriteFile {
        async fn execute(
            ...
            let result =
                match eos_sandbox_port::write_file(&*self.service.transport, sandbox_id, &request)
                    .await
                {
                    Ok(result) => result,
                    Err(err) => return Ok(ToolResult::error(err.to_string())),
...
mod edit_file {
    ...
    impl ToolExecutor for EditFile {
        async fn execute(
            ...
            let result =
                match eos_sandbox_port::edit_file(&*self.service.transport, sandbox_id, &request)
                    .await
                {
                    Ok(result) => result,
                    Err(err) => return Ok(ToolResult::error(err.to_string())),
...
mod enter_isolated_workspace {
    ...
    #[derive(Debug, Deserialize, Serialize, JsonSchema)]
    pub(super) struct EnterIsolatedWorkspaceInput {
        #[serde(default)]
...
mod exit_isolated_workspace {
    ...
    #[derive(Debug, Deserialize, Serialize, JsonSchema)]
    pub(super) struct ExitIsolatedWorkspaceInput {
...
pub(crate) fn register_schema(
    ...
    use crate::registry::{json_spec, text_spec};
    use crate::{OutputShape, ToolName};
    use outputs::{MutationOutput, ReadFileOutput};
    use schemars::schema_for;
    ...
    crate::tools::register_schema_tool(
        ...
        text_spec(
            ToolName::EnterIsolatedWorkspace,
            &enter.description,
            schema_for!(enter_isolated_workspace::EnterIsolatedWorkspaceInput),
        ),
        OutputShape::Text,
    );
    let exit = config.get(ToolName::ExitIsolatedWorkspace);
    crate::tools::register_schema_tool(
        ...
        text_spec(
            ToolName::ExitIsolatedWorkspace,
            &exit.description,
            schema_for!(exit_isolated_workspace::ExitIsolatedWorkspaceInput),
        ),
        OutputShape::Text,
    );
}
