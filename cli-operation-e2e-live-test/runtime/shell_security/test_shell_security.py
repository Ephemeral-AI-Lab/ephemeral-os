...
@pytest.fixture
def shell_security_sandbox(tmp_path):
...
@pytest.fixture
def probe(shell_security_sandbox):
    return run_probe(shell_security_sandbox)


@pytest.mark.smoke
def test_cs01_allowed_commands_still_work(shell_security_sandbox):
    for command in (
        "id -u",
        "uname -m",
        "sh -lc 'printf ok > cs01.txt && cat cs01.txt'",
    ):
        result = exec_cmd(shell_security_sandbox, command)
...
@pytest.mark.medium
def test_cs05_image_tools_are_denied(shell_security_sandbox):
    unshare_probe = exec_cmd(
        shell_security_sandbox,
    ...
    umount_probe = exec_cmd(shell_security_sandbox, "umount /workspace")
    assert umount_probe.get("status") != "ok", umount_probe


@pytest.mark.medium
def test_cs06_package_manager_still_starts(shell_security_sandbox):
    version = exec_cmd(shell_security_sandbox, "apt-get --version")
    ...
    if install.get("status") == "running":
        ...
        install = wait_command(shell_security_sandbox, command_session_id, timeout_s=180)
