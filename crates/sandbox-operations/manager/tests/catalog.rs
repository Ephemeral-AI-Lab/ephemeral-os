...
fn management_catalog_is_the_exact_public_set() {
    ...
    assert!(catalog
        .operations
        .iter()
        .all(|operation| operation.family == "management" && operation.cli.is_some()));
...
fn squash_layerstacks_keeps_the_singular_daemon_operation_internal() {
    ...
    let cli = operation.cli.expect("public CLI metadata");

    assert_eq!(cli.path, ["manager", "squash_layerstacks"]);
