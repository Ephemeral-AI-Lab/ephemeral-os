...
use anyhow::Result;
use e2e_test::{next_invocation_id, unique_suffix};
...
/// `docs/architecture/eos-sandbox/occ.html` §4.4 (Throughput Model): the single
...
fn single_overlay_exec_batches_multi_file_writes_into_one_layer() -> Result<()> {
    ...
    let before = as_i64(
        &lease.call_ok(catalog::SANDBOX_CHECKPOINT_LAYER_METRICS, json!({}))?,
        "manifest_depth",
    )?;
