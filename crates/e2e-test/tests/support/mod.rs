...
use e2e_test::{live_pool_with_config, NodeLease, NodePool};
...
pub(crate) use trace::{
    envelope_error_kind, envelope_error_kind_or_status, envelope_meta, envelope_result,
    envelope_status, has_trace_event, operation_envelope, trace_export_records, trace_record,
};

pub(crate) fn ack_trace_export(lease: &NodeLease<'_>, response: &Value) -> Result<()> {
    let Some(export_id) = response.get("export_id").and_then(Value::as_str) else {
        return Ok(());
    };
    let batch_sha256 = response
        .get("batch_sha256")
        .and_then(Value::as_str)
        .with_context(|| format!("trace export {export_id} missing batch_sha256: {response}"))?;
    let record_count = response
        .get("record_count")
        .and_then(Value::as_u64)
        .with_context(|| format!("trace export {export_id} missing record_count: {response}"))?;
    let ack = lease.call_ok(
        catalog::SANDBOX_TRACE_EXPORT_ACK,
        json!({
            "export_id": export_id,
    ...
    if ack.get("acked").and_then(Value::as_bool) != Some(true) {
        bail!("trace export ack was rejected for {export_id}: {ack}");
    }
    Ok(())
}
