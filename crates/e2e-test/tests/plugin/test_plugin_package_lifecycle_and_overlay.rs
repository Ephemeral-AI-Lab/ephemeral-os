...
#[test]
fn generic_plugin_lifecycle_reuses_package_setup() -> Result<()> {
    ...
    assert_eq!(cold["package"]["package_published"], true, "{cold}");
    assert_eq!(cold["package"]["setup_ran"], true, "{cold}");
    assert_eq!(cold["service_processes_started"], true, "{cold}");
    assert_connected_routes(&cold, &["plugin.generic.query"])?;
    ...
    assert_container_path(
        &lease,
        &format!("/eos/runtime/packages/generic/{digest}/cache/service-setup.txt"),
    )?;
    ...
    assert_eq!(warm["package"]["needs_upload"], false, "{warm}");
    assert_eq!(warm["package"]["setup_ran"], false, "{warm}");
...
fn stage_generic_package(lease: &e2e_test::NodeLease<'_>, digest: &str) -> Result<String> {
...
fn ensure_generic_oneshot_package(lease: &e2e_test::NodeLease<'_>, digest: &str) -> Result<Value> {
...
fn stage_generic_service_package(lease: &e2e_test::NodeLease<'_>, digest: &str) -> Result<String> {
    let staged = format!("/eos/scratch/uploads/plugins/generic/{digest}/upload-1/package");
    let cmd = format!(
        r#"set -eu
pkg="{staged}"
...
fn stage_generic_oneshot_package(lease: &e2e_test::NodeLease<'_>, digest: &str) -> Result<String> {
    let staged = format!("/eos/scratch/uploads/plugins/generic/{digest}/upload-1/package");
    let cmd = format!(
        r#"set -eu
pkg="{staged}"
...
fn assert_container_path(lease: &e2e_test::NodeLease<'_>, path: &str) -> Result<()> {
...
fn assert_container_absent(lease: &e2e_test::NodeLease<'_>, path: &str) -> Result<()> {
    lease
        .container()
...
fn wait_for_container_path_absent(lease: &e2e_test::NodeLease<'_>, path: &str) -> Result<()> {
...
fn read_container_file(lease: &e2e_test::NodeLease<'_>, path: &str) -> Result<String> {
