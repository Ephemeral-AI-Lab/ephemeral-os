...
fn unique_port() -> u16 {
    let hash = e2e_test::unique_suffix().bytes().fold(0_u32, |acc, byte| {
...
fn cancel(lease: &e2e_test::NodeLease<'_>, caller_id: Option<&str>, id: &str) {
...
fn isolated_loopback_service_is_not_reachable_from_peer_session() -> Result<()> {
    ...
    let marker = format!("A_ONLY_{}", e2e_test::unique_suffix().replace('-', "_"));
