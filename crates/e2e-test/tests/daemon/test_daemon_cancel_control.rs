//! `sandbox.call.cancel` response: unknown-id done response + live in-flight cancel attempt.
...
#[test]
fn live_cancel_of_started_blocking_inflight_reports_not_cancelled() -> Result<()> {
    let Some(pool) = live_pool_or_skip()? else {
        return Ok(());
    };
    let lease = pool.acquire()?;
    ...
    let already_done = as_bool(&cancel, "already_done")?;
    let cleanup_done = as_bool(&cancel, "cleanup_done")?;
    // Join before the lease drops so the background request never outlives the node.
    let _ = handle.join();
    assert!(
        !cancelled,
        "started blocking invocation cannot be aborted by generic cancel: {cancel}"
    );
    assert!(
        !already_done,
        "registered invocation is still running, not already done: {cancel}"
    );
    assert!(
        !cleanup_done,
        "cleanup cannot be complete while the blocking invocation still runs: {cancel}"
    );
    Ok(())
}
