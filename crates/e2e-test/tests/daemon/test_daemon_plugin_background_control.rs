...
fn background_plugin_operation_control() -> Result<()> {
    ...
    assert!(
        ...
        "started blocking plugin invocation cannot be aborted by generic cancel: {cancel}"
    );
    assert!(
        !as_bool(&cancel, "already_done")?,
        "background plugin invocation is registered and still running: {cancel}"
    );
    assert!(
        !as_bool(&cancel, "cleanup_done")?,
        "background plugin cleanup cannot be complete until the request returns: {cancel}"
    );
