...
use e2e_test::cas::looks_like_sha256;
...
fn commit_projects_delete_symlink_and_replacement_write() -> Result<()> {
    ...
    let lease = pool.acquire()?;
    let dir = format!("commit-projection-{}", e2e_test::unique_suffix());
...
fn rebuild_workspace_base(lease: &e2e_test::NodeLease<'_>) -> Result<(Value, Value)> {
...
fn assert_commit_to_workspace_phase(record: &trace::TraceRecord, key: &str) -> Result<()> {
...
fn assert_workspace_base_trace(record: &trace::TraceRecord) -> Result<()> {
    assert!(
    ...
    for key in [
        "sandbox.checkpoint.workspace_base.total_s",
...
fn trace_phase(record: &trace::TraceRecord, module: &str, name: &str, key: &str) -> Result<f64> {
    record
        .events
        .iter()
