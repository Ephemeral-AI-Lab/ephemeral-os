...
use trace::{TraceKind, TraceLinkKind, TraceRecord};

use crate::support::{
    ack_trace_export, array, as_bool, as_i64, as_str, has_trace_event, live_pool_or_skip, stdout,
...
fn live_trace_background_command_finalize_exports_root_linked_to_origin() -> Result<()> {
    ...
    let lease = pool.acquire()?;
    let drained = lease.call_ok(catalog::SANDBOX_TRACE_EXPORT, json!({"max_records": 64}))?;
    ack_trace_export(&lease, &drained)?;
    ...
    assert_eq!(
        finalize.request_id.as_ref().map(trace::RequestId::as_str),
        Some(request_id.as_str())
    );
    assert!(
        finalize
