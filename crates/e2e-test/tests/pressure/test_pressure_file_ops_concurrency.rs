...
fn concurrent_overlay_execs_share_lowerdir_storage_is_o1() -> Result<()> {
    ...
    for handle in handles {
        ...
        let exec_completed_in_exec_response =
            as_str(response_result(&response)?, "status")? != "running";
        // Finalize yielded ("running") execs to the finalized payload. When
        // finalization arrives through command.poll, that response sidecar belongs
        // to the poll op rather than the original exec op and does not carry the
        // command-exec resource samples.
        ...
        if exec_completed_in_exec_response {
