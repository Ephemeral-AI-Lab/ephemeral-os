...
#[test]
fn public_and_isolated_same_path_at_12() -> Result<()> {
    ...
    for level in levels {
        ...
        let body = (|| -> Result<()> {
            ...
            for (index, caller_id) in callers.iter().enumerate() {
                ...
                let isolated_client = lease.recorded_client();
                ...
                let public_client = lease.recorded_client();
...
fn exit_callers(lease: &e2e_test::NodeLease<'_>, callers: &[String]) {
    for caller_id in callers {
        let _ = lease.call(
