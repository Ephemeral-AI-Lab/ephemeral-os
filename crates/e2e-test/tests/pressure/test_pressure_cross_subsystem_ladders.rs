...
#[test]
fn ephemeral_exec_at_12() -> Result<()> {
    let Some(pool) = live_pool_or_skip()? else {
        return Ok(());
    };
...
fn run_disjoint_occ_level(lease: &e2e_test::NodeLease<'_>, level: usize) -> Result<()> {
    let barrier = Arc::new(Barrier::new(level));
    let handles: Vec<_> = (0..level)
        .map(|index| {
            let client = lease.recorded_client();
...
fn run_same_path_occ_level(lease: &e2e_test::NodeLease<'_>, level: usize) -> Result<()> {
    let barrier = Arc::new(Barrier::new(level));
    let handles: Vec<_> = (0..level)
        .map(|index| {
            let client = lease.recorded_client();
