...
pub(super) fn run_netlink<T, F, Fut>(operation: F) -> Result<T, WorkspaceProfileError>
where
    T: Send + 'static,
    F: FnOnce(Handle) -> Fut + Send + 'static,
    Fut: Future<Output = Result<T, WorkspaceProfileError>> + Send + 'static,
{
    thread::spawn(move || {
    ...
    .join()
    .map_err(|_| WorkspaceProfileError::NetworkUnavailable("netlink thread panicked".to_owned()))?
}

pub(super) async fn ensure_bridge(handle: &Handle) -> Result<u32, WorkspaceProfileError> {
...
async fn require_link_index(handle: &Handle, name: &str) -> Result<u32, WorkspaceProfileError> {
    link_index(handle, name)
        .await?
        .ok_or_else(|| WorkspaceProfileError::NetworkUnavailable(format!("link {name} not found")))
}

pub(super) async fn link_index(
    handle: &Handle,
    name: &str,
) -> Result<Option<u32>, WorkspaceProfileError> {
