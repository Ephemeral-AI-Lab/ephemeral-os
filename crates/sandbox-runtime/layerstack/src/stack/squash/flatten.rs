...
#[cfg(target_os = "linux")]
pub(crate) use linux::flatten_block_into_with_lower;

#[cfg(not(target_os = "linux"))]
pub(crate) fn flatten_block_into_with_lower(
    _staging_dir: &std::path::Path,
    _sources_newest_first: &[std::path::PathBuf],
    _lower_layers_newest_first: &[std::path::PathBuf],
) -> Result<(), crate::error::LayerStackError> {
    Err(crate::error::LayerStackError::Storage(
        "layer squash flattening is supported only on Linux".to_owned(),
    ))
}
...
mod linux {
    ...
    use crate::whiteout::{
        write_kernel_whiteout, LOGICAL_WHITEOUT_PREFIX, OPAQUE_MARKER,
        TRUSTED_OVERLAY_OPAQUE_XATTR, TRUSTED_OVERLAY_WHITEOUT_XATTR, USER_OVERLAY_OPAQUE_XATTR,
    ...
    fn classify_at(layer: &OwnedFd, name: &OsStr) -> Result<Option<SourceEntry>, LayerStackError> {
        ...
        let entry = match file_type {
            rustix::fs::FileType::CharacterDevice if stat.st_rdev as u64 == 0 => {
