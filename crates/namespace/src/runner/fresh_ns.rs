...
fn plugin_mask_paths(hidden_paths: Option<&[PathBuf]>) -> Vec<PathBuf> {
    ...
    paths.extend([
        PathBuf::from("/root"),
        PathBuf::from("/var"),
        PathBuf::from("/eos/runtime/daemon"),
    ]);
    paths.sort();
    paths.dedup();
    paths
...
mod tests {
    ...
    #[test]
    fn plugin_mask_keeps_eos_available_but_hides_daemon_runtime() {
        ...
        assert!(
            ...
            "plugin service must not see the daemon Unix socket directory"
        );
    }
}
