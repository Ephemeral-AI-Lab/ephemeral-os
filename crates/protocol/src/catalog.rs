...
impl OpFamily {
    /// Stable spelling used in `crates/operation/ops.json`.
    #[must_use]
...
/// Gateway policy for runtime-discovered `plugin.<id>.<op>` operations.
///
/// Dynamic plugin operations are not listed in [`BUILTIN_OPS`] because their
/// names are manifest-provided at runtime. Until the gateway owns a plugin
/// metadata cache, it exposes them only on the public client/operator surfaces
/// and treats them as mutating for conservative terminal-audit handling.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct DynamicPluginPolicy {
    /// Wire prefix for dynamic plugin operations.
    pub prefix: &'static str,
    /// Functional owner for audit/query metadata.
    pub family: OpFamily,
    /// Caller surface accepted by the gateway fallback.
    pub visibility: OpVisibility,
    /// Conservative mutability used by host trace persistence.
    pub mutates_state: bool,
}

/// Fallback policy for manifest-discovered dynamic plugin operations.
pub const DYNAMIC_PLUGIN_POLICY: DynamicPluginPolicy = DynamicPluginPolicy {
    prefix: "plugin.",
    family: OpFamily::Plugins,
...
declare_builtin_ops! {
    ...
    TraceRequests, SANDBOX_TRACE_REQUESTS, "sandbox.trace.requests",
    ...
    TraceShow, SANDBOX_TRACE_SHOW, "sandbox.trace.show",
    ...
    TraceVerify, SANDBOX_TRACE_VERIFY, "sandbox.trace.verify",
    ...
    PluginEnsure, SANDBOX_PLUGIN_ENSURE, "sandbox.plugin.ensure",
        Daemon, Plugins, Public, true, None, "operation.plugin.PluginEnsureInput", "operation.plugin.PluginEnsureOutput", "Ensure a plugin service is installed and running.";
    PluginStatus, SANDBOX_PLUGIN_STATUS, "sandbox.plugin.status",
        Daemon, Plugins, Public, false, None, "operation.plugin.PluginStatusInput", "operation.plugin.PluginStatusOutput", "Inspect plugin service status.";
...
mod tests {
    ...
    fn canonical_names_follow_grammar() {
        for contract in BUILTIN_OPS {
            ...
            if contract.served_by == ServedBy::Host && contract.family == OpFamily::Sandbox {
                assert_eq!(
                    contract.name.split('.').count(),
                    2,
                    "sandbox lifecycle host op {} must be sandbox.<verb>",
                    contract.name
                );
            }
        }
    ...
    #[test]
    fn dynamic_plugin_policy_is_explicit_gateway_fallback() {
        assert_eq!(DYNAMIC_PLUGIN_POLICY.prefix, "plugin.");
        assert_eq!(DYNAMIC_PLUGIN_POLICY.family, OpFamily::Plugins);
        assert_eq!(DYNAMIC_PLUGIN_POLICY.visibility, OpVisibility::Public);
        const {
            assert!(
            DYNAMIC_PLUGIN_POLICY.mutates_state,
            "gateway treats manifest-discovered plugin ops as mutating until it has manifest metadata"
            );
        }
    }
