...
#[serde(rename_all = "snake_case")]
pub enum PluginOperationIntent {
    ReadOnly,
    WriteAllowed,
    Lifecycle,
}
...
pub struct PluginOperationManifest {
    pub op_name: String,
    pub intent: PluginOperationIntent,
...
impl PluginOperationManifest {
    fn validate(&self, service_modes: &BTreeMap<&str, ServiceMode>) -> Result<()> {
        ...
        if self.intent == PluginOperationIntent::Lifecycle {
            return Err(PluginError::Manifest(
                "PluginOperationIntent::Lifecycle is reserved for sandbox lifecycle ops".to_owned(),
            ));
        }
        if self.intent == PluginOperationIntent::ReadOnly && self.service_id.is_none() {
            return Err(PluginError::Manifest(format!(
        ...
        if self.intent == PluginOperationIntent::WriteAllowed && self.auto_workspace_overlay {
