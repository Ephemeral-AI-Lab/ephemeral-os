...
//! cannot publish and does not define a publish port; `daemon` owns the
//! callback handler and routes it through its existing per-root OCC writer.
//!
//! # Build-time guarantee — no storage edge
//!
//! This crate does NOT depend on `overlay`, `layerstack`,
...
pub use manifest::{
    PluginDependencyScope, PluginManifest, PluginOperationIntent, PluginOperationManifest,
    PluginPackageManifest, PluginServiceManifest, PluginSetupManifest, PACKAGE_SHA256_MARKER,
    SETUP_SHA256_MARKER,
};
