...
use nix::unistd::Pid;
use sandbox_manager::{
    ManagerError, SandboxDaemonEndpoint, SandboxDaemonInstaller, SandboxRecord, StartedDaemon,
};
...
#[cfg(not(unix))]
fn terminate_daemon_process(_pid: u32, _stop_timeout: Duration) -> Result<(), ManagerError> {
    Err(daemon_install_error(
        "local daemon stop is unsupported on this platform".to_owned(),
    ))
}
