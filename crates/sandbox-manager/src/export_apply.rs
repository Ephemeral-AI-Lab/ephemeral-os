...
#[cfg(not(windows))]
pub use platform::ExportApplyCaps;
#[cfg(not(windows))]
pub(crate) use platform::{apply_dir_delta, write_archive, ArchiveFormat, DirApplyStats};
...
pub(crate) fn apply_dir_delta(
    _delivery: &mut impl std::io::Read,
    _dest: &std::path::Path,
    _caps: ExportApplyCaps,
) -> Result<DirApplyStats, String> {
    Err("export_changes directory apply is supported only on Unix hosts".to_owned())
}
...
pub(crate) fn write_archive(
    ...
    Err("export_changes archive rendering is supported only on Unix hosts".to_owned())
}
...
mod platform {
    ...
    fn apply_content(
        ...
        for entry in entries {
            let mut entry =
                entry.map_err(|error| map_cap_error(error, caps.max_decompressed_bytes))?;
