...
//! through [`layerstack::service::commit_direct`], the same per-root
//! service used by primary write/edit paths.

use std::path::{Path, PathBuf};

use layerstack::{CommitOptions, LayerChange, LayerPath};
...
pub(super) fn handle_callback_for_root(
    ...
    match request.op.as_str() {
        OCC_APPLY_CHANGESET_OP => handle_apply_changeset(expected_root, commit_options, request),
...
fn handle_apply_changeset(
    ...
    let base_hashes = body
        ...
        .collect::<Result<Vec<_>, PluginError>>()?;
    let result = layerstack::service::commit_direct_with_options(
        &root,
