...
use plugin::{PluginError, PluginOperationIntent, PpcDirection, PpcMessage};
...
impl PluginRuntime {
    ...
    fn dispatch_registered_route(
        ...
        if route.intent == PluginOperationIntent::ReadOnly && route.service_id.is_some() {
        ...
        if route.intent == PluginOperationIntent::WriteAllowed && route.auto_workspace_overlay {
            if let Some(outcome) =
        ...
        if route.intent == PluginOperationIntent::WriteAllowed
    ...
    fn round_trip_connected_route(
        ...
        let reply = match layer_stack_root {
            Some(expected_root) => {
                let commit_options = self.commit_options;
                client.round_trip_with_callbacks(&request, timeout, move |callback| {
                    occ_callbacks::handle_callback_for_root(
                        &expected_root,
                        commit_options,
                        callback,
                    )
                    .map_err(|err| super::PpcError::Callback(err.to_string()))
                })
            }
