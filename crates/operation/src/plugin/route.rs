...
use plugin::{PluginError, PluginOperationIntent, PluginServiceKey, ServiceMode};
...
impl PluginOperationRoute {
    pub const fn dispatch_mode(&self) -> &'static str {
        match self.intent {
            PluginOperationIntent::ReadOnly => "read_only_service",
            PluginOperationIntent::WriteAllowed if self.auto_workspace_overlay => {
                "write_allowed_oneshot_overlay"
            }
            PluginOperationIntent::WriteAllowed => "self_managed_callback",
            PluginOperationIntent::Lifecycle => "invalid_lifecycle",
        }
    }
}
