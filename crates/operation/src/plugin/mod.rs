...
//! `layerstack::service`, the same per-root writer as the primary write
...
use plugin::{PluginError, PluginManifest, PluginServiceStatus};
...
pub use self::transport_trace::{PpcTraceEvent, PpcTraceEventSink};
...
/// error (preserving the inner [`plugin::PluginError`], so the dispatcher
...
pub enum PpcError {
    ...
    #[error(transparent)]
    Protocol(#[from] plugin::wire::WireError),
...
pub struct StatusOutcome {
    ...
    pub connected_ppc_routes: Vec<String>,
    pub connected_ppc_services: Vec<String>,
    pub pending: Vec<PpcPendingStatus>,
    pub setup_failures: Vec<SetupFailure>,
    pub service_health: Vec<ServiceHealthReport>,
}

/// Pending PPC request count for one connected plugin service instance.
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize)]
pub struct PpcPendingStatus {
...
impl PluginRuntime {
    ...
    pub fn ensure(&self, input: &PluginEnsureInput) -> Result<EnsureOutcome, PluginRuntimeError> {
        ...
        let package_report = match package::ensure_package(
            input,
            parsed.manifest.as_ref(),
    ...
    pub fn status(
        ...
        let pending = state
            .service_ppc_clients
            .iter()
            .filter_map(|(service_instance_id, client)| {
                let pending_count = client.pending_count();
                (pending_count > 0).then(|| PpcPendingStatus {
