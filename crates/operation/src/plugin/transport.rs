...
use super::transport_trace::{PpcTraceEvent, PpcTraceEventSink};
use super::PpcError;

const MAX_PPC_MESSAGE_BYTES: usize = plugin::wire::MAX_PPC_MESSAGE_BYTES;
...
fn spawn_reader_thread(
    ...
    thread::Builder::new()
        .name("plugin-ppc-reader".to_owned())
...
impl PendingCalls {
    ...
    fn count(&self) -> usize {
        self.inner.lock().map(|pending| pending.len()).unwrap_or(0)
    }
