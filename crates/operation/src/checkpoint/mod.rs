...
//! plane and the storage leaves (`layerstack`, `overlay`): the daemon's
//! checkpoint op adapter -> this crate -> leaves. It owns the parts of
...
///
/// Mirrors the historical commit-to-git response fields one-to-one so the
