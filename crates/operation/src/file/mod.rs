...
pub fn read_file<B: FileBackend>(
    ...
    let read = backend.read_bytes(&path, request.max_read_bytes)?;
...
pub fn write_file<B: FileBackend>(
    ...
    let path = backend.resolve_path(&request.path)?;
    let base = backend.read_bytes(&path, request.max_file_bytes)?;
...
pub fn edit_file<B: FileBackend>(
    ...
    let total_start = Instant::now();
    let path = backend.resolve_path(&request.path)?;
    let base = backend.read_bytes(&path, request.max_read_bytes)?;
    if !base.exists {
        let mut timings = base.timings;
    ...
    for edit in &request.edits {
        ...
        match apply_search_replace(&content, &edit.old_text, &edit.new_text, edit.replace_all) {
            Ok(next) => {
                ensure_edited_file_size(next.len(), request.max_file_bytes)?;
                content = next;
            }
            Err(err) => {
                let mut timings = base.timings;
    ...
    ensure_edited_file_size(content.len(), request.max_file_bytes)?;
    let mut outcome = backend.apply(Mutation {
...
fn ensure_edited_file_size(bytes: usize, max_file_bytes: usize) -> Result<(), FileOpsError> {
    if bytes > max_file_bytes {
        return Err(FileOpsError::invalid_request(format!(
            "file too large: {bytes} > {max_file_bytes} bytes"
        )));
    }
    Ok(())
}
...
fn insert_total(timings: &mut WorkspaceTimings, verb: &str, start: Instant) {
    timings.insert(
        format!("sandbox.file.{verb}.total_s"),
