...
use crate::core::request::{require_path, require_string};
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct TraceExportAckInput {
    pub export_id: String,
    pub batch_sha256: String,
    pub record_count: usize,
}

impl TraceExportAckInput {
    pub(crate) fn parse(args: &Value) -> Result<Self, ArgsError> {
        Ok(Self {
            export_id: require_string(args, "export_id")?,
            batch_sha256: require_string(args, "batch_sha256")?,
            record_count: args
                .get("record_count")
                .and_then(Value::as_u64)
                .and_then(|value| usize::try_from(value).ok())
                .ok_or(ArgsError {
                    key: "record_count",
                    problem: crate::core::request::ArgProblem::Required,
                })?,
        })
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct TraceExportAckOutput {
    pub success: bool,
    pub export_id: String,
    pub acked: bool,
}
