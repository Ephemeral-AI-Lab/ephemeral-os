...
use super::*;
use plugin::{PluginServiceKey, PluginServiceKeyParts, RefreshStrategy, ServiceMode};

type TestResult = std::result::Result<(), Box<dyn std::error::Error + Send + Sync>>;

const PPC_CONNECT_TEST_TIMEOUT: Duration = Duration::from_secs(15);
...
#[test]
fn spawned_process_clears_daemon_auth_token_from_environment() -> TestResult {
    const CHILD_MARKER: &str = "EOS_PLUGIN_PROCESS_ENV_CLEAR_CHILD";
    const DAEMON_AUTH_TOKEN: &str = "EOS_DAEMON_AUTH_TOKEN";

    if std::env::var_os(CHILD_MARKER).is_none() {
        let status = std::process::Command::new(std::env::current_exe()?)
            .arg("spawned_process_clears_daemon_auth_token_from_environment")
            .env(CHILD_MARKER, "1")
            .env(DAEMON_AUTH_TOKEN, "must-not-leak")
            .status()?;
        assert!(status.success(), "inner env-clear regression test failed");
        return Ok(());
    }

    let root = test_socket_root("spawn-env-clear");
    let env_path = root.join("child.env");
    let spec = new_spec_with_socket_root(
        key("profile-a")?,
        vec![
            "/bin/sh".to_owned(),
            "-c".to_owned(),
            format!("/usr/bin/env > {}", env_path.display()),
        ],
        1,
        &root,
    )?;
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            return Err(std::io::Error::new(
                ErrorKind::TimedOut,
                "timed out waiting for env-capturing service process exit",
            )
            .into());
        }
        std::thread::sleep(Duration::from_millis(10));
    ...
    let child_env = std::fs::read_to_string(&env_path)?;
    assert!(
        child_env.contains("EOS_PLUGIN_SERVICE_ID=indexer\n"),
        "plugin contract env should still be present: {child_env}"
    );
    assert!(
        !child_env.contains("EOS_DAEMON_AUTH_TOKEN="),
        "daemon auth token leaked into plugin service env: {child_env}"
    );
    let _ = std::fs::remove_dir_all(root);
    Ok(())
}
...
fn spawn_connected_accepts_ppc_socket() -> TestResult {
    ...
    let (mut process, _client) =
        match spawn_connected_with_overlay(&launcher, &spec, None, PPC_CONNECT_TEST_TIMEOUT) {
            Ok(pair) => pair,
            Err(err) => {
...
fn new_spec_for_test(
    key: plugin::PluginServiceKey,
    command: Vec<String>,
    ppc_protocol_version: u32,
) -> Result<PluginProcessSpec, PluginError> {
    let socket_root = config::configs::daemon::PluginRuntimeConfig::default().ppc_root;
...
fn wait_for_socket(root: &Path) -> std::io::Result<PathBuf> {
    let deadline = Instant::now() + PPC_CONNECT_TEST_TIMEOUT;
    loop {
...
impl workspace::NsRunnerLauncher for NoLaunch {
    fn run(
        &self,
        _request: &namespace::protocol::RunRequest,
    ) -> Result<namespace::protocol::RunResult, workspace::LaunchError> {
        Err(workspace::LaunchError::Failed(
    ...
    fn spawn_detached(
        ...
        _request: &namespace::protocol::RunRequest,
        _stderr_path: &Path,
    ) -> Result<std::process::Child, workspace::LaunchError> {
        Err(workspace::LaunchError::Failed(
            "no launcher in this test".to_owned(),
        ))
    }
