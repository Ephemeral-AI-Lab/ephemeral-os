...
use operation::plugin::{read_message_bytes, PpcClient, PpcTraceEventSink};
use plugin::{PpcDirection, PpcMessage};
...
#[test]
fn ppc_client_reports_pending_count_while_request_is_in_flight() -> TestResult {
    let (client_stream, mut server_stream) = UnixStream::pair()?;
    let (request_seen_tx, request_seen_rx) = mpsc::channel();
    let (release_tx, release_rx) = mpsc::channel();
    let server = thread::spawn(move || -> TestResult {
        let request = PpcMessage::decode(&read_message_bytes(&mut server_stream)?)?;
        request_seen_tx.send(request.message_id.clone())?;
    ...
    let round_trip = thread::spawn(move || {
        request_client.round_trip(
            &PpcMessage {
                message_id: "msg-1".to_owned(),
                parent_message_id: None,
                direction: PpcDirection::Request,
    ...
    let seen_message_id = request_seen_rx.recv_timeout(Duration::from_secs(1))?;
    assert_eq!(seen_message_id, "msg-1");
    assert_eq!(client.pending_count(), 1);

    release_tx.send(())?;
    let reply = round_trip
        .join()
        .unwrap_or_else(|_| Err(std::io::Error::other("round trip thread panicked").into()))?;
    assert_eq!(reply.message_id, "msg-1");
