...
use operation::plugin::ensure::validate_plugin_caller_fields;
use operation::plugin::{
    read_message_bytes, EnsureOutcome, EnsureReady, PluginDispatchOutcome, PluginRuntime,
    PluginRuntimeError,
};
use operation::CallerId;
use plugin::{PluginError, PpcDirection, PpcMessage};
...
fn broken_ppc_drops_connected_route_then_recovers_with_restart() -> TestResult {
    ...
    assert_eq!(
        ...
        plugin::PluginServiceState::Stopped
    );
    ...
    assert_eq!(service.state, plugin::PluginServiceState::Ready);
...
impl NsRunnerLauncher for FakeNsRunnerLauncher {
    ...
    ) -> Result<namespace::protocol::RunResult, LaunchError> {
    ...
    fn spawn_detached(
        &self,
        request: &namespace::protocol::RunRequest,
