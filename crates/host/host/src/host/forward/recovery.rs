...
use crate::runtime::DaemonContainer;
...
use crate::host::registry::resolve_endpoint;
...
pub(super) fn run_recovery(attempt: &ForwardAttempt<'_>) -> Result<Value, ForwardError> {
    ...
    let endpoint = match crate::host::registry::cached_or_resolve_endpoint(attempt.record) {
        Ok(addr) => addr,
        Err(err) => {
            record_event(
                attempt,
