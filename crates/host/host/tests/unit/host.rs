...
use crate::host::drain::{drain_trace_export_once, TraceDrainTarget, TraceExportDrainer};
use crate::host::forward::{
...
use crate::host::registry::{SandboxRecord, SandboxRegistry};
use crate::host::{ForwardTraceContext, HostConfig, SandboxHost};
use crate::protocol::{
...
use crate::runtime::override_docker_command_for_tests;
...
fn direct_daemon_ops_match_catalog_contracts() {
    ...
    for (name, visibility) in [
        (crate::protocol::READY_OP, OpVisibility::Internal),
        (crate::protocol::HEARTBEAT_OP, OpVisibility::Public),
...
fn operator_trace_queries_are_audit_events_even_without_sandbox_id() -> Result<()> {
    ...
    assert_eq!(details["op"], json!("sandbox.trace.requests"));
