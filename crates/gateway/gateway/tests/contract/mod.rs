...
#[test]
fn legacy_host_aliases_still_route_during_migration() {
    let catalog = Catalog::load_builtin().expect("catalog");
    let engine = StubEngine;
    for (legacy, target) in [
        ("sandbox.acquire", "host.sandbox.acquire"),
        ("sandbox.status", "host.sandbox.status"),
        ("sandbox.trace.requests", "host.trace.requests"),
    ] {
        assert!(catalog.lookup(target).is_some(), "{target} must be canonical");
        let response = gateway::handle(
            &catalog,
            &engine,
            Surface::Operator,
        ...
        assert_ne!(kind(&response), Some("unknown_op"), "{legacy}: {response}");
    }
}
