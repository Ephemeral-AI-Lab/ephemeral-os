...
fn client_socket_refuses_non_public_ops() {
    ...
    for (op, surface, expected_forbidden) in [
        ...
        ("sandbox.trace.requests", Surface::Client, true),
        ("sandbox.trace.requests", Surface::Operator, false),
        ("sandbox.trace.verify", Surface::Client, true),
        ("sandbox.trace.verify", Surface::Operator, false),
...
fn api_error_kinds_are_produced() {
    ...
    let response = gateway::handle(
        ...
        &engine,
        Surface::Operator,
        &request("sandbox.trace.show", Some(KNOWN_SANDBOX)),
    );
    assert_eq!(
        kind(&response),
    ...
    assert_eq!(
        result(&response)["family"],
        json!(DYNAMIC_PLUGIN_POLICY.family.as_str())
    );
    assert_eq!(
        result(&response)["mutates_state"],
        json!(DYNAMIC_PLUGIN_POLICY.mutates_state)
    );
}
