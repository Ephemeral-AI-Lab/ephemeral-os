...
use protocol::catalog::{HostVerb, DYNAMIC_PLUGIN_POLICY};
use protocol::ProtocolErrorKind;
use serde_json::{json, Value};

use super::catalog::{Catalog, Visibility};
use super::engine::Engine;
use super::wire::{error_response_for, ok_response, ClientRequest};
use crate::gateway::wire::elapsed_us;
...
pub(crate) fn handle(
    ...
    let Some(entry) = catalog.lookup(&request.op) else {
        if request.op.starts_with(DYNAMIC_PLUGIN_POLICY.prefix) {
            return forward(
                engine,
                request,
                DYNAMIC_PLUGIN_POLICY.mutates_state,
                DYNAMIC_PLUGIN_POLICY.family.as_str(),
                "plugin_fallback",
                DYNAMIC_PLUGIN_POLICY.visibility.as_str(),
            );
        }
        record_route_event(
            engine,
    ...
    match entry.route {
        super::catalog::Route::Daemon => forward(
            engine,
            request,
            entry.mutates_state,
            entry.family,
