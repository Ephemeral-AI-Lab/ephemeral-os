...
fn edit_request(edits: Vec<SearchReplaceEdit>) -> EditFileRequest {
    EditFileRequest {
...
fn edit_applies_search_replace_against_base() -> TestResult {
    ...
    let outcome = edit_file(
        &backend,
        edit_request(vec![SearchReplaceEdit {
...
#[test]
fn edit_rejects_base_that_exceeds_read_limit() {
    let backend = FakeBackend::new(Some(b"abcdef".to_vec()));
    let too_large = edit_file(
        &backend,
        EditFileRequest {
            max_read_bytes: 2,
            ..edit_request(vec![SearchReplaceEdit {
    ...
    assert!(too_large.is_err(), "oversized base read is a typed error");
    assert!(
        backend.recorded.borrow().is_none(),
        "oversized base never reaches apply"
    );
}

#[test]
fn edit_rejects_result_that_exceeds_write_limit() {
    let backend = FakeBackend::new(Some(b"small".to_vec()));
    let too_large = edit_file(
        &backend,
        EditFileRequest {
            max_file_bytes: 5,
            ..edit_request(vec![SearchReplaceEdit {
                old_text: "small".to_owned(),
                new_text: "too-large".to_owned(),
                replace_all: false,
            }])
        },
    );

    assert!(too_large.is_err(), "oversized edited file is a typed error");
    assert!(
        backend.recorded.borrow().is_none(),
        "oversized edited file never reaches apply"
    );
}
