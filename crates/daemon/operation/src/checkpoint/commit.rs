...
use layerstack::{LayerStack, MergedView, WorkspaceBinding};
use overlay::{
...
pub fn commit_to_git_with_trace_recorder(
    ...
    let outcome = (|| {
        ...
        record_elapsed(
            &mut timings,
            "sandbox.checkpoint.commit_to_git.git_add_s",
            git_add_start,
        );
        ...
        record_elapsed(
            &mut timings,
            "sandbox.checkpoint.commit_to_git.git_diff_cached_s",
            diff_start,
        );
        if !has_changes {
        ...
        record_elapsed(
            &mut timings,
            "sandbox.checkpoint.commit_to_git.git_commit_s",
            commit_start,
        );
        let commit_sha = current_head(&git_dir, worktree.path(), &mut record_trace)?;
        Ok(Committed {
...
fn try_prepare_overlay_worktree(
    ...
    record_elapsed(
        timings,
        "sandbox.checkpoint.commit_to_git.overlay_mount_s",
        mount_start,
    );
...
fn prepare_projected_worktree(
    ...
    record_elapsed(
        timings,
        "sandbox.checkpoint.commit_to_git.project_worktree_s",
        project_start,
    );
    Ok(PreparedWorktree {
        path: worktree,
