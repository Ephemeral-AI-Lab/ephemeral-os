...
fn publish_changes_to_layerstack(
    root: &Path,
...
fn command_git_index_stat_refresh_is_dropped_without_conflict_or_publish() -> TestResult {
    ...
    let snapshot = service::acquire_snapshot_with_lease(&fixture.root, "git-index-stat-refresh")?;
    LayerStack::open(fixture.root.clone())?.publish_layer(&[LayerChange::Write {
        path: lp("src/other.rs")?,
        content: b"other".to_vec(),
    }])?;

    let result = publish_changes_to_layerstack(
        &fixture.root,
...
fn command_git_staged_index_rejects_whole_publish() -> TestResult {
    ...
    let snapshot = service::acquire_snapshot_with_lease(&fixture.root, "git-staged-index")?;
...
fn command_git_reflog_append_is_gated_and_rewrite_rejects() -> TestResult {
    ...
    let snapshot = service::acquire_snapshot_with_lease(&fixture.root, "git-reflog-append")?;

    let exact_noop_decisions = publish_decisions_for_manifest(
        &fixture.root,
    ...
    let rewrite = publish_changes_to_layerstack(
        &fixture.root,
        snapshot.manifest_version,
        &snapshot.layer_paths,
        &[LayerChange::Write {
...
fn command_git_reflog_prefix_extension_of_partial_record_rejects() -> TestResult {
    ...
    let snapshot =
        service::acquire_snapshot_with_lease(&fixture.root, "git-reflog-partial-prefix")?;
...
fn command_git_objects_are_gated_and_rewrites_reject() -> TestResult {
    ...
    let snapshot = service::acquire_snapshot_with_lease(&fixture.root, "git-object-rules")?;
...
fn lane_aware_publish_ignored_only_uses_direct_lww() -> TestResult {
    ...
    let first = publish_changes_to_layerstack(
        &fixture.root,
        snapshot.manifest_version,
        &snapshot.layer_paths,
        &[LayerChange::Write {
    ...
    let second = publish_changes_to_layerstack(
        &fixture.root,
        snapshot.manifest_version,
        &snapshot.layer_paths,
        &[LayerChange::Write {
...
fn opaque_dir_with_all_ignored_descendants_routes_direct() -> TestResult {
    ...
    let snapshot = service::acquire_snapshot_with_lease(&fixture.root, "opaque-all-ignored")?;
...
fn opaque_dir_with_source_descendants_validates_hidden_paths() -> TestResult {
    ...
    let snapshot = service::acquire_snapshot_with_lease(&fixture.root, "opaque-source-validation")?;
    LayerStack::open(fixture.root.clone())?.publish_layer(&[LayerChange::Write {
        path: lp("src/old.txt")?,
        content: b"theirs".to_vec(),
    }])?;
...
fn opaque_dir_with_mixed_descendant_routes_rejects_publish() -> TestResult {
    ...
    let snapshot = service::acquire_snapshot_with_lease(&fixture.root, "opaque-mixed-routes")?;
...
fn opaque_dir_with_git_descendant_rejects_as_protected() -> TestResult {
    ...
    let snapshot =
        service::acquire_snapshot_with_lease(&fixture.root, "opaque-protected-descendant")?;
...
fn opaque_dir_with_non_git_protected_descendant_rejects_as_protected() -> TestResult {
    ...
    let snapshot = service::acquire_snapshot_with_lease(&fixture.root, "opaque-non-git-protected")?;
