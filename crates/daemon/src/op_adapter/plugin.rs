...
//! `operation::plugin` outcomes, and the
...
use operation::plugin::contract::{
    ...
    PluginPendingStatusOutput, PluginStatusInput, PluginStatusOutput,
};
use operation::plugin::needs_upload_output;
use operation::plugin::route::{PluginOperationRoute, PluginProcessSpec};
use operation::plugin::PackageEnsureReport;
use protocol::catalog::DYNAMIC_PLUGIN_POLICY;
...
use operation::plugin::{
    ...
    PpcPendingStatus, ServiceHealthReport, StatusOutcome,
};

use super::to_wire_value;
use crate::trace::plugin_trace;
...
pub(crate) fn op_ensure(
    ...
    if let Err(err) = &ensure_result {
        plugin_trace::record_plugin_ensure_error_trace_events(&context, err);
    }
    let output = match ensure_result.map_err(DaemonError::from)? {
        ...
        EnsureOutcome::Ready(ready) => {
            plugin_trace::record_plugin_ensure_trace_events(&context, &ready);
...
pub(crate) fn op_status(
    ...
    plugin_trace::record_plugin_status_trace_events(&context, &outcome);
...
pub(crate) fn dispatch_registered_op(
    ...
    plugin_trace::push_stale_ppc_background_root(services.plugin.drain_ppc_trace_events());
    let outcome = services
        .plugin
    ...
    plugin_trace::record_ppc_trace_events(
        &context,
...
fn is_dynamic_plugin_op(op: &str) -> bool {
    op.starts_with(DYNAMIC_PLUGIN_POLICY.prefix)
}
...
fn status_response(outcome: &StatusOutcome) -> Value {
    to_wire_value(PluginStatusOutput {
        ...
        pending: outcome
            .pending
            .iter()
            .map(pending_status_value)
            .collect::<Vec<_>>(),
    })
}

fn pending_status_value(pending: &PpcPendingStatus) -> PluginPendingStatusOutput {
    PluginPendingStatusOutput {
        service_instance_id: pending.service_instance_id.clone(),
...
fn plugin_overlay_response_with_trace(
    ...
    plugin_trace::record_plugin_overlay_resource_stats(context, "before", before_resource_timings);
    plugin_trace::record_plugin_overlay_host_resource_stats(
        context,
        "before",
        before_resource_timings,
    );
    plugin_trace::record_plugin_overlay_started(context, op, invocation_id, overlay);
    plugin_trace::record_plugin_overlay_workspace_prepared(context, op, invocation_id, overlay);
    plugin_trace::record_plugin_overlay_mount_started(context, op, invocation_id, overlay);
    plugin_trace::record_plugin_overlay_mount_finished(context, op, invocation_id, overlay);
    plugin_trace::record_plugin_overlay_unmount_finished(context, op, invocation_id, overlay);
    plugin_trace::record_plugin_overlay_capture_started(context, op, invocation_id, overlay);
    let result = plugin_overlay_response(overlay, total_start);
    if let Ok(wire) = &result {
        plugin_trace::record_plugin_overlay_resource_stats(context, "after", &wire.timings);
        plugin_trace::record_plugin_overlay_host_resource_stats(context, "after", &wire.timings);
    }
    plugin_trace::record_plugin_overlay_capture_finished(context, op, invocation_id, overlay);
    plugin_trace::record_occ_changeset_trace_events(context, &overlay.changeset);
    match &result {
        Ok(wire) => plugin_trace::record_plugin_overlay_finished(
            context,
            op,
            invocation_id,
            overlay,
        ...
        Err(err) => plugin_trace::record_plugin_overlay_finished(
            context,
            op,
            invocation_id,
            overlay,
    ...
    plugin_trace::record_plugin_overlay_lease_release_failed(context, op, invocation_id, overlay);
