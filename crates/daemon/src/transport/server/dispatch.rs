...
use protocol::catalog::{BuiltinOp, OpVisibility, DYNAMIC_PLUGIN_POLICY};
...
fn enforce_tcp_visibility(
    ...
    let visibility = BuiltinOp::from_op_name(op)
        .map(|op| op.contract().visibility)
        .or_else(|| {
            op.starts_with(DYNAMIC_PLUGIN_POLICY.prefix)
                .then_some(DYNAMIC_PLUGIN_POLICY.visibility)
        });
