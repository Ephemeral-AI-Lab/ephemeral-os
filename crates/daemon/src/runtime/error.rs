...
impl DaemonError {
    ...
    pub fn wire_kind(&self) -> crate::wire::ErrorKind {
        ...
        match self {
            ...
            Self::Plugin(plugin::PluginError::ForbiddenInIsolatedWorkspace) => {
...
impl From<operation::plugin::PluginRuntimeError> for DaemonError {
    fn from(err: operation::plugin::PluginRuntimeError) -> Self {
        use operation::plugin::PluginRuntimeError;
        match err {
...
impl From<operation::plugin::PpcError> for DaemonError {
    fn from(err: operation::plugin::PpcError) -> Self {
        use operation::plugin::PpcError;
        match err {
            ...
            PpcError::Protocol(source) => {
                Self::Plugin(plugin::PluginError::Ppc(source.to_string()))
            }
            PpcError::Io(source) => Self::Io(source),
            PpcError::SetupFailed { message, .. } => {
                Self::Plugin(plugin::PluginError::Ensure(message))
            }
            PpcError::LockPoisoned(what) => Self::StateLockPoisoned(what),
            PpcError::Callback(message) => Self::Plugin(plugin::PluginError::Ppc(message)),
        }
    }
}
