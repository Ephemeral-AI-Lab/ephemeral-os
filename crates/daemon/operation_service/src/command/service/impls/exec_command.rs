...
mod tests {
    ...
    #[test]
    fn command_exec_resolves_workspace_session_id_with_canonical_session() {
        let fake = Arc::new(FakeWorkspaceService::new());
        let service = command_service(Arc::clone(&fake), CommandProcessStore::new());
        ...
        let output = service
            .exec_command(
                exec_input(
                    "caller-1",
                    PathBuf::from("/workspace/session"),
                    Some(canonical.workspace_session_id),
                ),
                context("caller-1"),
            )
    ...
    #[test]
    fn command_exec_rejects_unknown_workspace_session_id_before_command_allocation() {
        let fake = Arc::new(FakeWorkspaceService::new());
        let service = command_service(Arc::clone(&fake), CommandProcessStore::new());
        ...
        let error = service
            ...
            .expect_err("command service rejects unknown workspace session id");

        assert!(matches!(error, CommandServiceError::WorkspaceSession(_)));
    ...
    fn command_exec_create_failure_does_not_register_one_shot_command() {
        ...
        assert!(service
            .registry()
            .workspace_session_for(&CommandId("cmd_1".to_owned()))
            .is_none());
    }
