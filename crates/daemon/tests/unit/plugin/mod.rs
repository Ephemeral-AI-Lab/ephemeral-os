...
use layerstack::{ChangesetResult, CommitStatus, FileResult, LayerPath};
use namespace::protocol::RunResult;
use operation::{
    plugin::{
        EnsureReady, PackageEnsureReport, PluginOverlayOutcome, PluginRuntimeError,
        PluginSetupReport, PpcError, PpcPendingStatus, PpcTraceEvent, ServiceHealthReport,
        ServiceProcessStatus, StatusOutcome,
    },
    ChangedPathKind,
};
use plugin::PluginServiceState;
...
#[test]
fn dynamic_plugin_shape_uses_protocol_policy_prefix() {
    let op = format!(
        "{}generic.hover",
        protocol::catalog::DYNAMIC_PLUGIN_POLICY.prefix
    );

    assert!(super::is_dynamic_plugin_op(&op));
    assert!(!super::is_dynamic_plugin_op("sandbox.plugin.ensure"));
}
...
#[test]
fn status_response_includes_pending_ppc_counts() {
    let outcome = StatusOutcome {
        loaded_plugins: Vec::new(),
        running_service_processes: Vec::new(),
        exited_service_processes: Vec::new(),
        connected_ppc_routes: Vec::new(),
        connected_ppc_services: Vec::new(),
        pending: vec![PpcPendingStatus {
            service_instance_id: "generic:worker:profile-a".to_owned(),
            pending_count: 2,
        }],
        setup_failures: Vec::new(),
    ...
    let response = super::status_response(&outcome);

    assert_eq!(
        response["pending"],
        json!([{
            "service_instance_id": "generic:worker:profile-a",
            "pending_count": 2,
        }])
    );
}
...
fn overlay_trace_events_include_started_and_finished_facts() {
    ...
    plugin_trace::record_plugin_overlay_started(
        &context,
        "plugin.generic.write",
        "plugin-overlay-test",
        &overlay,
    );
    plugin_trace::record_plugin_overlay_mount_finished(
        &context,
        "plugin.generic.write",
        "plugin-overlay-test",
        &overlay,
    );
    plugin_trace::record_plugin_overlay_unmount_finished(
        &context,
        "plugin.generic.write",
        "plugin-overlay-test",
        &overlay,
    );
    plugin_trace::record_plugin_overlay_capture_started(
        &context,
        "plugin.generic.write",
        "plugin-overlay-test",
        &overlay,
    );
    plugin_trace::record_plugin_overlay_capture_finished(
        &context,
        "plugin.generic.write",
        "plugin-overlay-test",
        &overlay,
    ...
    plugin_trace::record_plugin_overlay_finished(
        &context,
        "plugin.generic.write",
        "plugin-overlay-test",
        &overlay,
    ...
    plugin_trace::record_plugin_overlay_lease_release_failed(
        &context,
        "plugin.generic.write",
        "plugin-overlay-test",
        &overlay,
...
fn overlay_resource_stats_events_include_before_and_after_gauges() {
    ...
    plugin_trace::record_plugin_overlay_resource_stats(&context, "before", &before);
    plugin_trace::record_plugin_overlay_host_resource_stats(&context, "before", &before);
    plugin_trace::record_plugin_overlay_resource_stats(&context, "after", &after);
    plugin_trace::record_plugin_overlay_host_resource_stats(&context, "after", &after);
...
fn plugin_overlay_response_strips_timings_but_keeps_trace_samples() -> TestResult {
    ...
    assert!(
        wire.timings.get("sandbox.plugin.overlay.total_s").is_some(),
        "{:?}",
        wire.timings
    );
    assert_eq!(
...
impl PackageTestRoots {
    fn new(name: &str) -> Result<Self, TestError> {
        let root =
            std::env::temp_dir().join(format!("plugin-package-{name}-{}", std::process::id()));
