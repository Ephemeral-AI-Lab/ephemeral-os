...
fn dispatches_layerstack_write_file_and_reads_published_bytes() -> TestResult {
    ...
    let response = daemon::dispatch(&write);
    ...
    assert_eq!(
        ...
        Value::String("direct_write".to_owned())
    );
    ...
    let response = daemon::dispatch(&read);
    assert_eq!(
        response["status"],
        Value::String("ok".to_owned()),
        "{response}"
...
fn dispatches_layerstack_edit_file_and_reads_published_bytes() -> TestResult {
    ...
    let response = daemon::dispatch(&edit);
    ...
    assert_eq!(
        result["mutation_source"],
        Value::String("direct_edit".to_owned())
    );
...
fn identical_head_write_is_idempotent() -> TestResult {
    ...
    assert_eq!(
        daemon::dispatch(&request)["status"],
        Value::String("ok".to_owned())
    );
    assert_eq!(
        daemon::dispatch(&request)["status"],
