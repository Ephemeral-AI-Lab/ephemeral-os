...
import type { ProfileWorkflowView } from "../../workflows/contract.js";

/**
 * The only app-authored workflow tool: serves one profile-visible workflow's
 * full manual. Its `name` enum is generated from the view, so an agent cannot
 * request docs for a workflow it cannot use.
 */
export function readWorkflowDocs(view: ProfileWorkflowView): ToolDefinition {
  return defineTool({
