# eos-operation Operation Contract Consolidation Spec
...
`crates/eos-command-session`, and the `eos-daemon` adapter/wire-shaping layer
...
| # | Finding | Evidence |
|---|---|---|
| F1 | The 8 per-module `ops.rs` files (`file`, `command`, `checkpoint`, `plugin`, `control`, `sandbox`, `isolation`, `workspace_run`) each hand-roll the same trio — `FAMILY_OPS`, a per-family enum, a `contract()` method — ~230 LOC with **zero consumers** anywhere in the repo. | Daemon registers from the catalog directly: `OpTable::with_builtins()` filters `BUILTIN_OPS` by `ServedBy::Daemon` (`eos-daemon/src/dispatch/dispatcher.rs:39-50`). |
| F2 | Four modules — `control/`, `sandbox/`, `isolation/`, `workspace_run/` — contain **nothing but** that scaffolding. | `src/{control,sandbox,isolation,workspace_run}/lib.rs`. |
...
| F4 | Live audit semantics are the `mutation_source` strings: `"api_write"`, `"api_edit"` (`file/direct.rs`), `"isolated_workspace"` (two producers: `file/isolated.rs` and `command/settle.rs:119`), `"overlay_capture"` (`command/settle.rs:74`). `"plugin_overlay"` is daemon-synthesized and stays daemon-owned. | Daemon contract tests assert the strings byte-for-byte. |
...
| F9 | **The plugin family already violates the v1 §9 layering rule** ("the module never sees the wire `Value`"): `PluginRuntime::ensure(&self, args: &Value)`, `ParsedEnsure::from_args(&Value)`, overlay caller-id fishing — the wire `Value` penetrates two layers into eos-operation. | `plugin/lib.rs:217`, `plugin/ensure.rs:72-101`, `plugin/overlay.rs:57-60`. |
...
| F13 | **The catalog already has external data consumers**: the gateway embeds and parses `ops.json` (`include_str!`, no crate dep) for routing + visibility; `cargo xtask check-contract` gates drift; docs generate from it. The catalog artifact — not the crate — is the cross-trust-boundary contract surface. | `eos-sandbox-gateway/src/gateway.rs:13,52-54,203-219`, `xtask/src/main.rs:149-216`. |
| F14 | **No dependency cycles; identity types don't exist below.** eos-operation sits above all substrate crates; nothing below depends on it. v1 §10 claimed caller-identity types are "owned by eos-workspace/eos-command-session" — those crates own bare `String` fields; the only identity newtype anywhere is `IsolatedWorkspaceId` (`eos-workspace/src/isolated_workspace/manager/mod.rs:26`). | Cargo graph sweep; contract inventory. |
...
| D12 | v1 §9 rule 2 ("adapter-inline parsing is the design") | **Overturned.** Control/isolation/workspace_run/checkpoint get typed input/output DTOs in eos-operation; behavior stays daemon-side. eos-operation owns the contract for every daemon-served op; the daemon implements against it. |
...
DELETED: all 8 family ops.rs; the sandbox/ module entirely (host family has no
daemon schema, D3); eos-daemon GuardedResponse + per-op hand parsing +
...
Net effect: eos-operation ≈ LOC-neutral (contract DTOs replace dead
scaffolding); eos-daemon clearly net-negative (adapters become thin generic
...
|---|---|---|
| `eos-daemon` | `dispatch/dispatcher.rs:9`, `dispatch/builtin_handlers.rs:3`, `wire/mod.rs:15` | import path only |
| `eosd` | `src/main.rs:55` (`dump-ops`) | import path only |
| `eos-e2e-test` | 50 test files + `src/pool.rs` | mechanical rename; verified by `cargo check --workspace` |
...
enrich: DispatchContext → explicit params (FileLimits, roots, registries)
   ▼
op behavior (eos-operation fn or daemon runtime) ─► Result<O::Output, FamilyError>
...
impl MutationSource {
    pub const fn as_str(self) -> &'static str { /* "api_write" | "api_edit"
...
(only `IsolatedWorkspaceId` exists, and it stays in eos-workspace).
...
| **command/contract.rs** (§11) | | | eos-operation::command |
| `sandbox.command.exec` | `ExecCommandInput { cmd, caller_id, layer_stack_root?, timeout_seconds?, yield_time_ms?, invocation_id? }` | `CommandResponse { status: CommandStatus, exit_code, stdout, stderr, command_session_id?, workspace?, metadata: Option<CommandMetadata> }` | |
...
| `sandbox.command.collect_completed` | `CollectCompletedInput { command_session_ids?, caller_id? }` | `CollectCompletedOutput { success, completions }` | |
...
`eos-command-session/src/contract.rs` op DTOs move to
`eos-operation/src/command/contract.rs`. The dependency direction is dissolved
...
```
BEFORE  eos-daemon ──▶ eos-operation ──▶ eos-command-session   substrate owns op DTOs +
              └────────────────────────────────▲               wire shaping it never constructs

AFTER   eos-daemon ──▶ eos-operation ──▶ eos-command-session   substrate consumes a
...
- `session.rs` `persist_final(response: &CommandResponse)` becomes
  ...
  `eos-operation::command` renders. This is the substrate's only DTO contact
  (`session.rs:242,295`).
...
| v1 §9 rule 2 — adapter-inline parsing "is the design, not a gap" | **Overturned** (D12) | With one parse seam, inline fishing has a better home; eos-operation owns the contract, the daemon owns the behavior. |
...
| `core/envelope.rs` shared across trust boundaries | `eos-sandbox-host` runs inside untrusted sandboxes, depends only on `anyhow/serde_json/thiserror`, and must not gain an eos-operation dep (verified). The shared artifact is `ops.json` + `contract/` fixtures (F13). Hence D3: host ops excluded from the schema layer. |
...
|---|---|---|
| 1 | Delete 8 dead `ops.rs` + the 4 scaffold-only module decls (re-added with real content in phase 7) | `cargo check -p eos-operation && cargo test -p eos-operation`; grep gate: no `FAMILY_OPS` / family-enum / `eos_operation::{control,sandbox,isolation,workspace_run}` references remain |
| 2 | Vocabulary grounding: `core/audit.rs`, `core/error.rs` (+ re-exports as `FileOpsError`/`WorkspaceApiError`), `core/id.rs`, outcome enums (`WorkspaceKind`, `MutationStatus`, `ChangedPathKind`); retype `FileBackend`; switch settle/backend literals | `cargo test -p eos-operation -p eos-daemon` (daemon contract tests pin the wire strings) |
...
| 7 | Control/isolation/workspace_run/checkpoint contracts + markers; fold `error_json` refusals into `OpError` (§13.3); delete `sandbox/` module; completeness test (28/28 schema rows vs catalog) | full e2e + fixtures; grep gate: no `error_json` / `require_arg` producers remain |
