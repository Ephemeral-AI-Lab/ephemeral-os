...
| 9 | **eos-plugin-ops** | `eos-plugin`, `eos-config`, `eos-workspace`, `eos-layerstack`, `eos-namespace` | `serde`, `serde_json`, `sha2`, `thiserror`; Linux target `nix` | plugin package publishing, service processes, PPC transport, dispatch, snapshot refresh, OCC callbacks, and oneshot overlays |
