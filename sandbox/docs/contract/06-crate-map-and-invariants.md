...
| 9 | **eos-operation** | `eos-command-session`, `eos-plugin`, `eos-config`, `eos-workspace`, `eos-layerstack`, `eos-overlay`, `eos-namespace` | `serde`, `serde_json`, `sha2`, `thiserror`; Linux target `nix` | operation modules: shared core outcome contracts, command lifecycle/runtime policy, file operation semantics, checkpoint commits, plugin package publishing, service processes, PPC transport, dispatch, snapshot refresh, OCC callbacks, and oneshot overlays |
| 10 | **eos-daemon** | `eos-protocol`, `eos-layerstack`, `eos-overlay`, `eos-occ`, `eos-isolated`, `eos-runner`, `eos-plugin`, `eos-operation` — implements + injects the port traits lower crates define, and spawns holder/runner children by binary subcommand | `serde`, `serde_json`, `sha2`, **`tokio`**, `tokio-util`, `thiserror`; Linux target `nix` | **tokio control plane** for AF_UNIX + TCP async multiplexing and PTY/session supervision |
...
`eos_operation::plugin` via the concrete OCC/LayerStack path. This is NOT contradicted
