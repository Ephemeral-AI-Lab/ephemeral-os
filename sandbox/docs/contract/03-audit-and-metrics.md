...
reimplementation (`eos-protocol` + `eos-daemon` + `eos-isolated`) MUST reproduce
...
Source: `backend/src/sandbox/daemon/audit_schema.py` (this whole module MOVES into
...
`backend/src/sandbox/audit/events.py` is a different module (the host-side
...
Source: `backend/src/sandbox/daemon/audit_buffer.py`. The daemon **never writes
...
### 2.7 Dispatcher routes (`backend/src/sandbox/daemon/rpc/dispatcher.py`)
...
Source: `backend/src/sandbox/daemon/builtin_operations.py:131-166` (function
...
Source: `backend/src/sandbox/isolated_workspace/_control_plane/pipeline_registry.py`.
...
- Plan task pointed at "`backend/src/sandbox/audit/` — the ring buffer + pull
  contract". **CORRECTION:** the ring buffer + pull contract actually lives in
  `backend/src/sandbox/daemon/audit_buffer.py`, NOT `backend/src/sandbox/audit/`.
