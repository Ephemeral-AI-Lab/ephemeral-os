...
Fixtures: `readiness_request.json`, `readiness_response.json`.
...
`<bundle>/sandbox/daemon/scripts/thin_client.py <socket> <envelope_json>`
...
Written to `sandbox/crates/eos-protocol/fixtures/envelopes/`. The import succeeds under
...
| `readiness_request.json` | §4 | host-built shape `daemon_client.py:322-329` |
...
request* fixtures are exact and deterministic.
