...
by `crates/eos-operation/ops.json` plus `contract/`.
...
1. **One entry point.** External callers reach exactly one socket, served by
   `eos-sandbox-gateway`. The per-sandbox daemons are unreachable from outside the host.
2. **Complete isolation, loose coupling.** No compiled code is shared across
   the host/box boundary. The shared artifacts are
   `sandbox/crates/eos-operation/ops.json` plus `sandbox/contract/`
...
Dependency law: `eos-sandbox-gateway → eos-sandbox-host → (std/tokio/serde only)`.
Host crates never depend on in-box crates; in-box crates never depend on host
crates. Both sides conform to `crates/eos-operation/ops.json` and `contract/`
via tests.

## 3. Wire protocol

### 3.1 Client hop (caller → eos-sandbox-gateway)
...
|---|---|---|
| `op` | yes | canonical name from `crates/eos-operation/ops.json` |
...
### 4.4 `crates/eos-operation/ops.json` schema
...
Generated by `eosd dump-ops` from `eos_operation::core::ops`, checked in, and
...
On `eos-sandbox-gateway` startup the registry is **rebuilt from docker labels**
...
`eos-sandbox-gateway` routes purely by catalog lookup:
...
`eos-sandbox-gateway` MUST NOT branch on specific op names; the only per-op data it reads
...
- `crates/eos-operation/ops.json` via `cargo run -p eosd -- dump-ops` and
