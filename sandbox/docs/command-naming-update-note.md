...
| `Session` | `eos_command::session::Session` | Command identity, caller, timeout, transcript/final paths, kill/reap state | `CommandProcess` | Public aggregate should describe the command process, not an abstract session. |
...
| `CommandSessionProcess` | `eos_command::process::CommandSessionProcess` | Low-level PTY child handle, stdin writer, reader completion, reap | private `PtyProcess` | This is the PTY implementation detail, not the command lifecycle aggregate. |
...
| `command_session_reaper_sweep()` | `eos_operation::command::runtime` | Timeout/reap sweep helper | `command_reaper_sweep()` | Reaper manages commands. |
