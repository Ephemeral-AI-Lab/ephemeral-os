...
#[test]
fn manager_starts_boxed_policy_and_counts_by_agent() -> Result<(), Box<dyn std::error::Error>> {
    let events = RecordingEvents::default();
    let manager =
        CommandSessionManager::with_event_sink(CommandSessionConfig::default(), events.clone());

    let response = manager.start_boxed(
        start_request("agent-1", "printf ok"),
        Box::new(FakePolicy::new(WorkspaceMode::Ephemeral)),
    )?;

    assert_eq!(response.status, "running");
    assert_eq!(manager.count_by_agent(Some("agent-1")), 1);
    assert_eq!(manager.count_by_agent(Some("agent-2")), 0);
    assert_eq!(manager.count_by_agent(None), 1);
    let started = lock(&events.started)?;
    assert_eq!(started.len(), 1);
    assert_eq!(started[0].agent_id, "agent-1");
...
fn terminate_finalizes_through_policy_and_parks_completion(
    ...
    let manager =
        CommandSessionManager::with_event_sink(CommandSessionConfig::default(), events.clone());
    let started = manager.start(start_request("agent-1", "cat"), policy.clone())?;
    ...
    assert_eq!(manager.count_by_agent(Some("agent-1")), 0);
...
fn cancel_can_claim_late_completion_once() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let started = manager.start(
        start_request("agent-1", "sleep 10"),
...
fn start_request(agent_id: &str, cmd: &str) -> StartCommandSession {
    StartCommandSession {
