...
//!
//! This crate is `eos-occ`- and `eos-layerstack`-free by construction — the
//! build-time no-publish guard. The daemon-resident seams (the OCC
//! single-writer publish, the snapshot lease acquire/release, the per-finalize
//! resource telemetry, and the isolated-session audit sink) are injected via
//! [`WorkspaceRunHostPorts`] so the run lifecycle stays here while the OCC
//! writer + daemon-global state stay in the daemon process.
//!
