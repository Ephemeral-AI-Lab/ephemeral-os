...
//! `plugin.<plugin>.<op>`. The intent + the `auto_workspace_overlay` flag together
//! select the [`DispatchMode`](crate::dispatch::DispatchMode) at flush time.
//!
...
/// Default for [`PluginOpRegistration::auto_workspace_overlay`]: `true`, so a
/// `WRITE_ALLOWED` handler is wrapped by the canonical overlay+OCC publish path.
...
/// One pending plugin-op registration. `handler_id` is the opaque key the warm
/// server resolves a real handler from (the Rust daemon never holds a Python
/// callable — the importlib path is replaced by the PPC warm server).
...
impl OpRegistry {
    ...
    /// names the daemon dispatcher should register. The dispatch runner is picked
    /// from each entry's [intent + flag](crate::dispatch::DispatchMode::for_registration)
    /// by the caller.
