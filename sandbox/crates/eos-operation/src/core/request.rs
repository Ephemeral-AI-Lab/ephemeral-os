...
use crate::plugin::contract::{PluginEnsureInput, PluginStatusInput};
...
impl OpRequest {
    pub fn parse(op: BuiltinOp, args: &Value) -> Result<Self, RequestError> {
        Ok(match op {
            BuiltinOp::SandboxAcquire
            | BuiltinOp::SandboxRelease
            | BuiltinOp::SandboxStatus
            | BuiltinOp::SandboxList => return Err(RequestError::NotDaemonServed(op)),
            ...
            BuiltinOp::ExecCommand => Self::ExecCommand(ExecCommandInput::parse(args)?),
