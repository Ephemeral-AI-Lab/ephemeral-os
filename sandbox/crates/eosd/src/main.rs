...
//! - `eosd ns-runner`  -> the single-threaded namespace runner in `eos-namespace::runner`.
//! - `eosd ns-holder`  -> the single-threaded namespace holder in `eos-namespace::holder`.
//!
...
//!   crash knob) — `eos_namespace::holder::NsHolderError::{CONTROL_CLOSED_EXIT,
...
///
/// Real thin call: `eos-namespace::holder` already exposes `run(readiness_fd,
...
fn run_ns_holder(mut args: std::env::Args) -> Result<()> {
    ...
    match eos_namespace::holder::run(readiness_fd, control_fd) {
        Ok(()) => Ok(()),
        Err(err) => {
            let code = match &err {
                eos_namespace::holder::NsHolderError::ControlPipeClosed => {
                    eos_namespace::holder::NsHolderError::CONTROL_CLOSED_EXIT
                }
                eos_namespace::holder::NsHolderError::UnexpectedToken => {
                    eos_namespace::holder::NsHolderError::UNEXPECTED_TOKEN_EXIT
                }
                eos_namespace::holder::NsHolderError::TestCrash => {
                    eos_namespace::holder::NsHolderError::TEST_CRASH_EXIT
                }
