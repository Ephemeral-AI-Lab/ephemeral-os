...
impl From<eos_overlay::CaptureStats> for TreeResourceStats {
    fn from(stats: eos_overlay::CaptureStats) -> Self {
        Self {
            files: stats.files,
            dirs: stats.dirs,
...
mod tests {
    ...
    fn collect_with_entry_limit_marks_real_truncation() {
        let root =
            std::env::temp_dir().join(format!("eos-workspace-tree-limit-{}", std::process::id()));
    ...
    fn collect_records_first_failing_path() {
        let root =
            std::env::temp_dir().join(format!("eos-workspace-tree-missing-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
