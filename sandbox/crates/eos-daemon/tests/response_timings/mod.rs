...
/// `eos-workspace-api`'s uncapped semantics.
#[test]
...
/// The daemon converters are exactly `eos-workspace-api`'s — one saturating
...
fn converters_match_workspace_api() {
    ...
    assert_eq!(
        ...
        eos_workspace_api::usize_to_f64_saturating(big),
    );
}
