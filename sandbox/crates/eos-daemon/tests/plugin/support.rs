...
impl TestIsolatedWorkspaceConfig {
    pub(super) fn enabled(scratch_root: &Path, workspace_root: &Path) -> Self {
        let guard = crate::adapters::workspace_run::isolated::lock_isolated_test_state();
        ...
        crate::adapters::workspace_run::isolated::configure_isolated_workspace(&config);
        Self { _guard: guard }
    }
}
...
impl Drop for TestIsolatedWorkspaceConfig {
    fn drop(&mut self) {
        crate::adapters::workspace_run::isolated::configure_isolated_workspace(
...
pub(super) fn read_ppc_request(
    ...
    let frame = eos_plugin_host::read_frame(stream)?;
