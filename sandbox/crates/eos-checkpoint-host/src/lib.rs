//! Host crate for the `commit_to_git` checkpoint pipeline.
//!
//! This is the intermediate host layer that sits *between* the storage leaves
//! (`eos-layerstack`, `eos-overlay`) and the daemon: `eos-daemon` → this crate →
//! leaves. It owns the parts of `commit_to_git` that are pure host glue —
//! pathspec policy, overlay-or-projection worktree preparation, and the git
//! staging/commit subprocess pipeline — none of which belong in the OCC single
//! writer or the daemon control plane. It gains **no** dependency toward the
//! daemon, so the DAG stays acyclic.
//!
//! The boundary is DTO-in / DTO-out: the daemon parses the request envelope and
//! hands over a typed [`CommitRequest`]; this crate returns a typed
//! [`CommitOutcome`] (or a [`CheckpointError`]) and never touches the wire
//! `Value` shape. The daemon re-maps the outcome to the protocol response.
#![forbid(unsafe_code)]
...
/// by the daemon; this crate trims, resolves them against the workspace
...
/// daemon adapter can re-emit the exact wire shape. `commit_sha` is `None` only
/// when the repository has no `HEAD` yet (re-mapped to JSON `null`).
