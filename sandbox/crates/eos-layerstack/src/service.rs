...
use crate::commit::{base_hashes_for_snapshot, ChangesetResult, CommitError, CommitWriter};
...
fn service_for_root(root: &Path) -> Result<RootService, CommitError> {
...
pub fn commit_direct(
    ...
    service_for_root(root)?.apply_changeset_with_base_hashes(
        changes,
        snapshot_version,
        true,
        base_hashes,
