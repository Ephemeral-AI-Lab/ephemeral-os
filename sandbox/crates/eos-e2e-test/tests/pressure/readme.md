...
| `pressure-bundled-sandbox-capstone` | Uses `protocol_only_bundled_sandbox_capstone`: one live protocol flow combines scaffold write, ephemeral exec publish, same-path OCC contention, squash pressure, isolated private write/discard, lease cleanup, commit-to-workspace, and generic plugin dispatch without asserting agent-loop state. | `cargo test -p eos-e2e-test --features e2e --test pressure protocol_only_bundled_sandbox_capstone -- --nocapture` | `pressure-bundled-sandbox-capstone`, `pressure-sandbox-only-boundary`, `pressure-ladder-plugin`, `pressure-isolated-cap`, `pressure-ladder-occ`, `pressure-squash-bound` |
