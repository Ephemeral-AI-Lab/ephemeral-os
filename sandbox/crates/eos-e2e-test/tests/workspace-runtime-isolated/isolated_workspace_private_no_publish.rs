...
fn isolated_write_is_discarded_on_exit() -> Result<()> {
    ...
    let read_inside = lease.call_ok(ops::SANDBOX_FILE_READ, json!({"path": path}))?;
    ...
    let read_public = lease.call_ok(ops::SANDBOX_FILE_READ, json!({"path": path}))?;
    assert!(
...
fn isolated_exec_write_is_private_and_discarded() -> Result<()> {
    ...
    let body = (|| -> Result<()> {
        ...
        let version_after = lease.call_ok(ops::SANDBOX_CHECKPOINT_LAYER_METRICS, json!({}))?
            ["manifest_version"]
            .as_i64()
