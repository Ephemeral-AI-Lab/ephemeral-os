...
//! servers on the same port collide (EADDRINUSE). Servers are bounded by
//! `timeout`/`sleep` and cleaned up on every path; the isolated session for
//! caller B is always exited (a leaked isolated lease does not self-heal).
...
fn start_server(lease: &eos_e2e_test::NodeLease<'_>, caller_id: Option<&str>, port: u16) -> Result<Value> {
    let cmd = format!(
        "mkdir -p /eos/scratch/e2e && timeout 60 python3 -m http.server {port} >/eos/scratch/e2e/srv-{port}.log 2>&1"
    );
    let mut args = json!({
        "cmd": cmd,
        "yield_time_ms": 400,
...
fn cross_mode_same_port_no_conflict() -> Result<()> {
    ...
    let body = (|| -> Result<()> {
        assert_eq!(as_str(&server_a, "status")?, "running", "{server_a}");
        ...
        let bind_b = lease.call_ok(
            ...
            json!({
                "caller_id": caller_b,
                "cmd": format!("python3 -c \"import socket,time;s=socket.socket();s.bind(('0.0.0.0',{port}));print('BOUND',flush=True);time.sleep(60)\""),
                "yield_time_ms": 600,
                "timeout_seconds": 120,
                "max_output_tokens": 500
            }),
        )?;
        assert_eq!(as_str(&bind_b, "status")?, "running", "{bind_b}");
        assert!(
