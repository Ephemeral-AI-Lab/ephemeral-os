...
- [ ] plugin-service-cleanup: Reload removes old routes, PPC clients, service snapshots, staged uploads, and worker processes. Socket-path unlink is not asserted: `sandbox/crates/eos-daemon/src/services/plugins/process.rs` removes stale socket paths before bind, while `PluginServiceProcess::teardown` only terminates the child process.
