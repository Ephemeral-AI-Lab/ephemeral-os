...
between the host side (`eos-sandbox-gateway`, `eos-sandbox-host`) and the box side
...
- `ops.json` is generated by `eosd dump-ops` (host-op entries contributed by
  a static section review-owned by `eos-sandbox-gateway`), checked in, and reviewed like
