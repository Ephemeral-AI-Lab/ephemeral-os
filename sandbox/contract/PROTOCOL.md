...
|---|---|---|
| `op` | yes | canonical name or listed alias from `ops.json` |
...
- Aliases `api.v1.read_file` (= `sandbox.file.read`) and `api.v1.heartbeat`
  (= `sandbox.call.heartbeat`) are pinned by the immutable golden fixtures and
  are **never** removed. All other aliases may sunset after consumers migrate.
- No alias may collide with a canonical name.
