...
`eos-sandbox-host`) and the box side (`eosd` / `eos-daemon`). No compiled code
...
## 1. Client hop (caller → `eos-sandbox-gateway`)
...
- `../crates/eos-operation/ops.json` is generated by `eosd dump-ops`, checked
