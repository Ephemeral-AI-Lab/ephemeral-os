...
import type { ComposeLaunchContext } from "@eos/workflow";

import type { AgentProfile } from "./agent-profile-loader.js";

export const DEFAULT_WORKFLOW_SCRIPTS_DIR = ".eos-agents/workflow/scripts";

export interface WorkflowContextScript {
  /** Absolute, validated script path. */
  scriptPath: string;
  /** Set only when `workflowScriptsDir` was explicitly overridden (§10). */
  cwd?: string;
}

/**
 * Startup validation of every planner/worker profile's
 * `workflow_context_script`: the path must resolve inside the scripts
 ...
 */
export function resolveWorkflowContextScripts(
  profiles: readonly AgentProfile[],
  scriptsDir: string,
  scriptsDirOverridden: boolean,
): Map<string, WorkflowContextScript> {
  const scripts = new Map<string, WorkflowContextScript>();
  const root = resolve(scriptsDir);
  for (const profile of profiles) {
    if (profile.agent_kind !== "planner" && profile.agent_kind !== "worker") continue;
    const raw = profile.workflow_context_script;
    if (raw === undefined) {
      throw new Error(
        `agent profile "${profile.name}" requires workflow_context_script`,
      );
    }
    const scriptPath = resolve(raw);
    if (scriptPath !== root && !scriptPath.startsWith(`${root}${sep}`)) {
      throw new Error(
        `agent profile "${profile.name}" workflow_context_script "${raw}" escapes the script root ${scriptsDir}`,
      );
    }
    if (!scriptPath.endsWith(".cjs") && !scriptPath.endsWith(".mjs")) {
      throw new Error(
        `agent profile "${profile.name}" workflow_context_script "${raw}" must be a .cjs or .mjs file`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        `agent profile "${profile.name}" workflow_context_script "${raw}" is not readable`,
        { cause: error },
      );
    }
    if (isDirectory) {
      throw new Error(
        `agent profile "${profile.name}" workflow_context_script "${raw}" is a directory`,
      );
    }
 ...
 * non-zero exit, timeout, or parse failure rejects, which the workflow
 * service records as a `context_script_error` compose failure (§2.14).
 */
export function workflowContextScriptComposer(
  scripts: ReadonlyMap<string, WorkflowContextScript>,
): ComposeLaunchContext {
  return async (agentName, input, signal): Promise<InitialUserMessage[]> => {
    const script = scripts.get(agentName);
    if (!script) {
      throw new Error(`agent profile "${agentName}" has no workflow_context_script`);
    }
