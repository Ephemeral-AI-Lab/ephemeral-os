...
describe("db guards and cancel (§16 case 10)", () => {
  it("accepts at most one mutation per entity transition under competing submissions", async () => {
    ...
    const item = (await h.tree(wf.pursuitId)).legs[0].attempts[0].workItems[0];
  ...
  it("skips a stale claimed launch after a competing cancel", async () => {
    ...
    const pursuitRow = await h.db
      .selectFrom("pursuits")
      .select("id")
      .executeTakeFirstOrThrow();
    await h.service.cancel(pursuitRow.id, "changed my mind");
    release();
    await delegated;

    expect(h.launches, "the stale launch was skipped").toHaveLength(0);
    const tree = await h.tree(pursuitRow.id);
  ...
  it("cancel cascades top-down in one transaction and resolves the terminal Cancelled", async () => {
    ...
    await h.service.cancel(wf.pursuitId, "user asked");
    await expect(wf.terminal).resolves.toEqual({
      status: "Cancelled",
      summary: "user asked",
    });
    expect(workerA.options?.signal?.aborted, "children observe the pursuit signal").toBe(
      true,
    );
    expect(workerA.options?.signal?.reason).toBe("pursuit_cancelled");

    const tree = await h.tree(wf.pursuitId);
    expect(tree.pursuit.status).toBe("Cancelled");
    expect(tree.legs[0].status).toBe("Cancelled");
    ...
    expect(JSON.stringify(await h.tree(wf.pursuitId))).toBe(before);
  });

  it("a second cancel and a cancel against a closed pursuit are no-ops", async () => {
    const h = harness();
    const wf = await h.delegate();
    await h.launches[0].submitPlanner(plannerPayload());
    ...
    await h.service.cancel(wf.pursuitId, "too late");
    const tree = await h.tree(wf.pursuitId);
    expect(tree.pursuit.status, "terminal guards hold").toBe("Success");
    await h.service.cancel(wf.pursuitId, "still too late");
    expect((await h.tree(wf.pursuitId)).pursuit.status).toBe("Success");
  });
  ...
  it("a submission landing after cancel finds terminal entities and no-ops", async () => {
    ...
    await h.launches[0].submitPlanner(plannerPayload());
    const worker = h.launches[1];
    await h.service.cancel(wf.pursuitId, "stop");
