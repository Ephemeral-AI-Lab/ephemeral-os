...
function scriptedDelegated(id: string): {
  pursuit: DelegatedPursuit;
  resolveTerminal: (terminal: PursuitTerminal) => void;
  cancelled: string[];
} {
  let resolveTerminal!: (terminal: PursuitTerminal) => void;
  const cancelled: string[] = [];
  const pursuit: DelegatedPursuit = {
    pursuitId: pursuitIdFrom(id),
...
describe("delegate_pursuit (§16 case 11)", () => {
  it("registers the session before returning and reports the pursuit id", async () => {
    ...
    expect(outcome.content).toEqual({ pursuit_id: "wf-1" });
  ...
  it("maps the pursuit terminal onto the session outcome", async () => {
    const inbox = new NotificationInbox();
    const supervisor = new BackgroundSessionSupervisor(inbox);
    const scripted = scriptedDelegated("wf-1");
  ...
  it("cancel_background_session accepts type pursuit and awaits the handle cascade", async () => {
    ...
    const outcome = await cancel.execute(
      { type: "pursuit", id: "wf-1", reason: "wrong direction" },
      ctx(),
    );
    expect(outcome.isError ?? false).toBe(false);
...
describe("per-kind submission validation (§16 case 11)", () => {
  ...
  it("a bound planner submission awaits the binding and surfaces its error for in-run correction", async () => {
    const results: SubmissionResult[] = [
      { ok: false, error: "first declaration must declare leg_goal" },
      { ok: true },
    ];
    const tool = submitPlannerOutcomeTool({
      kind: "planner",
    ...
    expect(rejected.content).toContain("leg_goal");
  ...
  it("unbound planner and worker runs keep service-free submissions", async () => {
    const planner = await submitPlannerOutcomeTool().execute(
      { summary: "plan", leg_goal: "slice", work_items: [workItem("a")] },
      ctx(),
    );
