...
- The pursuit plans the goal in legs: a planner declares each leg's focus and work items, workers execute them, and failed attempts are re-planned automatically within the attempt budget (\`max_attempts\` per leg, default 2).
...
- The pursuit may split the goal across legs itself; you do not pre-slice it.
