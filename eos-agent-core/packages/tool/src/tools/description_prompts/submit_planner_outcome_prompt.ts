export const DESCRIPTION = `Submit the final outcome of this planner run. Terminal: a successful call ends the run.
...
- \`summary\` is the one-paragraph result the caller reads first; \`payload\` optionally carries the structured plan and rides the run outcome as \`submission\`.
...
## Writing the Summary
- Lead with what the plan achieves and the approach it takes; put the structured plan itself in \`payload\`.
