...
fn main() -> Result<()> {
    let workspace = find_backend_server_root()?;
    let out_dir = workspace.join("docs/class_inventory/html");
    let mut inventory = Inventory {
        workspace: "backend-server".to_string(),
...
fn find_backend_server_root() -> Result<PathBuf> {
    ...
    loop {
        ...
        if !dir.pop() {
            anyhow::bail!("could not find backend-server root from current directory");
        }
    }
}
...
fn write_crate_file_page(
  ...
  <div class="items">{items}</div>
    ...
    let html = page_shell(
        ...
        &format!(
            r#"<a class="back-link" href="../index.html">backend-server</a>
...
const REFRESH_COMMAND: &str =
    "cd /Users/yifanxu/machine_learning/LoVC/EphemeralOS/backend-server && cargo run --manifest-path scripts/class-inventory/Cargo.toml";
