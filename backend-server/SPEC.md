...
| Obs collector moves with backend | `eos-obs-collector` lives under `backend-server/crates/` because it normalizes sandbox wire/audit data for backend stats and should not leave an `eos-protocol` edge in `agent-core`. |
...
|---|---|---|
| 0 | Relocate `eos-sandbox-host`, add `eos-sandbox-port`, move `eos-obs-collector`, and remove sandbox implementation deps from `agent-core`. | `cd agent-core && cargo check -p eos-runtime --all-targets`; dependency scan for `eos-sandbox-host` and `eos-protocol` in agent-core crates. |
