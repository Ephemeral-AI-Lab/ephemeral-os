...
| E10 | partial/pass gated | 10000 OCC-gated iterations, 0 violations, 184.80 s; direct-merge exceptions not implemented in prototype |
...
2. Mutations captured from a shell call are funneled through the changeset pipeline. Conflict-sensitive text writes/deletes go through OCC, which decides per change to **append** (commit) or **reject** (conflict); direct-merge exceptions must be explicit.
...
Every captured change routes through one of three policies, decided by path and change type. There is no caller-declared mode; classification is automatic:
...
| Symlink (`SymlinkChange`) | tracked | existence CAS |
| Symlink | gitignored | last-writer-wins |
| Opaque dir (`OpaqueDirChange`) | any | last-writer-wins (documented limitation) |
...
### E10 — OCC and direct-merge correctness

**Question:** Does the commit pipeline accept compatible commits and reject conflicting ones across the OCC-gated vocabulary, while direct-merge exceptions are explicit and bounded?
...
- Run 10k iterations across all gated combinations and a focused direct-exception matrix.

**Pass bar:** zero false-accept for OCC-gated changes (rejected change should never land), zero false-reject for compatible gated changes. Direct-merge exceptions must either be guarded or produce only documented, path-scoped last-writer-wins results. Existing OCC tests cover most semantics; this verifies they hold under the layer-stack base view.
...
11. **Direct-merge tracked types.** `BinaryChange` and `SymlinkChange` on tracked paths currently use existence/size CAS as a best-effort proxy. Should we promote them to full content-hash CAS (cost: hashing per binary) or accept best-effort? Decide after E10 measures false-accept rate on real binary edits.
