...
| **Multi-Agent Orchestration** | Coordination planner/worker toolkits, subagent dispatch, BackgroundTaskManager | Coordinator mode (env-var activated), swarm layer with subprocess spawning, filesystem mailbox IPC, git worktree isolation | **Tie** — EphemeralOS has richer primitives; OpenHarness has mature swarm IPC |
