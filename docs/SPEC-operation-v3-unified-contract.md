# operation Unified Operation Contract Spec (v3)

Status: **proposed v3** (2026-06-12) — the aggregate of
`SPEC-operation-core.md` (v2: operation schema, contract-ownership
inversion, typed vocabulary) and `SPEC-operation-unified-op-io.md`
...
`crates/command-session`, the `daemon` dispatch/adapter/wire-shaping
layer, and one attribute change in `layerstack`. Gateway, host, and
...
> **operation owns the complete typed contract of every daemon-served
...
```
                operation (the contract platform)
                ┌──────────────────────────────────────────────┐
   ...
   error        │ core/error.rs     OpError {kind,msg,details} │
                └──────────────────────────────────────────────┘
                              ▲ contract        │ typed DTOs
                              │                 ▼
                daemon (behavior + transport only)
                ┌──────────────────────────────────────────────┐
...
| Item | Why it wins |
|---|---|
| **D12 ownership inversion**: control/isolation/workspace_run/checkpoint get typed Input AND Output DTOs in operation; behavior stays daemon-side | The healthiest boundary: operation owns the contract for every daemon-served op, the daemon implements against it. unified-op-io's "runtime-state outputs stay `json!`" exemption is the lone untyped hole in an otherwise typed surface — closed here |
...
DELETED in operation: all 8 family ops.rs (229 LOC), file/port.rs,
  the eos-sandbox/ module entirely (host family has no daemon schema)
DELETED in daemon: OpTable/Handler/fn_addr_eq/boot panic, builtin_handlers.rs,
...
CHANGED in layerstack: CommitStatus drops #[non_exhaustive] (all consumers
...
builtin::dispatch(request, ctx) ── exhaustive match, sync, same spawn_blocking
   ...
   │   behavior fn (operation) or runtime service (daemon — D12 families)
   ▼
...
pub enum RequestError {
    /// Host-served catalog op rejected by the daemon parser's catalog
    /// `served_by` pre-guard before request-specific argument parsing.
...
- The five string semantics keep one owner (`ArgsError` + helpers): trimmed
  ...
  Command exec uses the top-level request `invocation_id`; args-level
  `invocation_id` is not the exec identity.
...
pub enum MutationStatus {
    ...
    // (layerstack/src/commit/mod.rs:57-80). From<CommitStatus> conversion;
...
pub struct InvocationId(String);      // heartbeat invocation_ids, cancel targets, exec request identity
...
```rust
// operation/src/core/response.rs
pub enum OpResponse {
...
```rust
// daemon/src/dispatch/dispatcher.rs — OpTable deleted
pub fn dispatch_with_context(request: &Request, context: DispatchContext<'_>) -> Value {
    ...
    let response = match BuiltinOp::from_op_name(&request.op) {
        Some(op) => match OpRequest::parse(op, &request.args, &request.invocation_id) {
...
```rust
// daemon/src/dispatch/builtin.rs — replaces builtin_handlers.rs
...
Mechanics carried from both parents. `command-session/src/contract.rs` op
DTOs move to `operation/src/command/contract.rs` (renamed per D5:
...
`command-session/tests/unit/session.rs` is **rebuilt** (it constructs the
...
| `sandbox.command.collect_completed` | `CollectCompletedInput { command_session_ids?, caller? }` | `CollectCompletedOutput { success, completions, has_more, max_completions }` | bounded oldest-first completion batch; callers repeat while `has_more` |
...
5. **Trust boundary**: zero diff to `host` and
   `gateway` in every phase. The host runs **outside** the
...
| D12 contract-ownership inversion | **Adopted** | operation owns every daemon-served contract; daemon owns behavior — the clean boundary this spec exists to draw |
...
| P4c | plugin builtins rewired (typed inputs, F9 closed); delete `OpTable`/`builtin_handlers`/legacy helpers/`request_args` string fns; rewrite dispatcher unit tests | `cargo test -p daemon`; `cargo xtask check-contract`; e2e plugin tier |
...
- **LOC**: the −242 deletion (P1) is severable and credits no design. Against
  ...
  type has a hot-path producer and consumer. daemon itself ends clearly
