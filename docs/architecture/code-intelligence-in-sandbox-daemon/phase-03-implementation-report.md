...
`RpcCiBackend` no longer raises `NotImplementedError` on business verbs —
every public method (other than `cmd`, which is Phase 4 territory) is
wired through `CiRpcClient.call(...)` with symmetric serialization
...
entries, SQLite-backed ledger, RpcCiBackend wired for every verb) is
...
| Path | LoC | Purpose |
|---|---:|---|
| `backend/tests/test_sandbox/test_code_intelligence/test_ci_storage_ledger.py` | 195 | 13 `LedgerStore` unit tests (WAL pragma, integrity rotation, interface parity, concurrency) |
| `backend/tests/test_sandbox/test_code_intelligence/test_ci_daemon_dispatch.py` | 273 | 15 daemon dispatch + bypass-guard unit tests against an in-process daemon state |
| `backend/tests/test_sandbox/test_code_intelligence/test_rpc_ci_backend_dispatch.py` | 286 | 19 `RpcCiBackend` round-trip tests with a fake `CiRpcClient` |
...
| Path | Change |
|---|---|
| `backend/src/sandbox/code_intelligence/in_sandbox/ci_storage.py` | New `LedgerStore` class (SQLite WAL, integrity-check + rotation) implementing the `EditHistoryLedger` duck-type interface |
| `backend/src/sandbox/code_intelligence/in_sandbox/ci_daemon.py` | New `_DaemonState`, daemon-resident `CodeIntelligenceService` build, full Phase 3 dispatch table, workspace-write bypass guard, SOCKET-FIRST startup |
| `backend/src/sandbox/code_intelligence/backend.py` | `RpcCiBackend` rewired through `CiRpcClient.call(...)` for every verb except `cmd`; orchestrator-side serializer helpers (`_writespec_to_dict`, `_operation_result_from_dict`, …) |
| `backend/src/sandbox/code_intelligence/service.py` | New `edit_history` kwarg threaded into `_select_backend` |
| `backend/tests/test_sandbox/test_code_intelligence/test_backend_inprocess.py` | Removed obsolete "all RPC ops raise NotImplementedError" matrix; kept the `cmd` Phase-4 sentinel and added a `rebind_sandbox` no-op test |
| `backend/tests/test_sandbox/test_code_intelligence/test_rpc_ci_backend.py` | Renamed obsolete "raises NotImplementedError" test into `cmd`-only sentinel + `rebind_sandbox` no-op coverage |
...
Orchestrator process                                  Daemon process
                                                      ┌────────────────────────────┐
RpcCiBackend.write_file([WriteSpec(...)])             │  CodeIntelligenceService    │
       │                                              │  (sandbox=None,             │
       │ _writespec_to_dict                           │   transport=None)           │
       ▼                                              │                             │
CiRpcClient.call("write_file", ...)                   │  ┌──────────────────────┐   │
       │                                              │  │ InProcessCiBackend   │   │
       │ msgpack frame over Unix socket               │  │  ─ local-FS branches │   │
       ▼                                              │  │  ─ Arbiter w/         │   │
Daytona transport.exec(python shim)  ─── RPC ───►     │  │    LedgerStore        │   │
       │                                              │  └──────────────────────┘   │
       │                                              │                             │
...
When the `Rpc` backend is selected, every business verb goes through
`CiRpcClient.call(...)` to the daemon. The daemon's
...
|---|---|---|
| **P3-001** `LedgerStore` SQLite WAL adapter | PASS | `ci_storage.py` adds `LedgerStore`. PRAGMA `journal_mode=WAL` + `synchronous=NORMAL` + `temp_store=MEMORY` + `mmap_size=64MB`. `PRAGMA integrity_check` rotates corrupt files to `ledger.corrupt.<ts>.sqlite3`. Public surface matches `EditHistoryLedger`. |
| **P3-002** Ledger unit tests | PASS | `test_ci_storage_ledger.py`: 13 tests. Round-trip, WAL pragma, schema/index check, concurrent record (10 threads × distinct rows), interface-parity by `inspect.signature`, integrity-rotation. |
| **P3-003** `edit_history` kwarg threaded through facade | PASS | `service.py:_select_backend` accepts `edit_history`. `backend.py:InProcessCiBackend.__init__` threads it into `Arbiter`. `RpcCiBackend.__init__` deliberately does NOT accept it (daemon owns the canonical ledger). 1118 baseline tests + 281 CI tests pass. |
| **P3-004** Daemon-resident `CodeIntelligenceService` | PASS | `ci_daemon.py:_DaemonState` + `_build_service` + `_populate_state`. SOCKET-FIRST: `_kick_background_index` calls `svc.symbol_index.ensure_built(wait=False)` BEFORE `asyncio.start_unix_server`. PID file written AFTER socket bind. |
...
| **P3-007** `RpcCiBackend` wired through `CiRpcClient` | PASS | Every verb except `cmd` (Phase 4) routes through `client.call(...)`. Lazy `_ensure_client` memoizes the `CiRpcClient`. Orchestrator-side serializers symmetric to daemon side. `dispose()` still calls `DaemonLauncher.shutdown()`. |
| **P3-008** Daemon dispatch unit tests | PASS | `test_ci_daemon_dispatch.py`: 15 tests. Dispatch-table presence; `status` / `query_symbols` / `write_file` / `undo_last_edit` route through `svc`; bypass guard surfaces violation in strict mode and logs in lenient mode; serializers round-trip; query ops bypass the guard. |
| **P3-009** `RpcCiBackend` round-trip tests | PASS | `test_rpc_ci_backend_dispatch.py`: 19 tests covering every public verb with a fake client. Includes the `query_symbols` cache-fallback path, `delete_file` with mixed `str | DeleteSpec` input, `warmup` bridging to `ensure_initialized`. |
...
| `pytest backend/tests/test_sandbox/test_code_intelligence/test_ci_storage_ledger.py -q` | **13 passed** |
| `pytest backend/tests/test_sandbox/test_code_intelligence/test_ci_daemon_dispatch.py -q` | **15 passed** |
| `pytest backend/tests/test_sandbox/test_code_intelligence/test_rpc_ci_backend_dispatch.py -q` | **19 passed** |
...
The original Phase 3 draft proposed shipping new `in_sandbox/ci_overlay.py`,
`ci_mutations.py`, `ci_lsp.py` files that copy or rewrite the OCC /
...
(`test_in_sandbox_drift_guard.py`) is unnecessary and has not been
written.
...
### 7.4 RPC backend keeps the Phase 1 cache fallback

`RpcCiBackend.query_symbols(query)` first attempts the daemon route. If
...
`RpcCiBackend.cmd` raising `NotImplementedError("RpcCiBackend.cmd is
...
### 7.6 `rebind_sandbox` is a no-op on the RPC side
...
handle. The RPC backend implements `rebind_sandbox(sandbox)` as a no-op.
...
### 7.8 `ci_storage.LedgerStore` shares dataclasses with the orchestrator
...
4. **`RpcCiBackend`** is fully wired except `cmd` (Phase 4 reservation).
...
| Task | File | Status |
|---|---|---|
| `IndexStore` SQLite adapter | `ci_storage.py` | Not started |
| `migrate_pickle_to_sqlite` helper | `ci_storage.py` | Not started |
| `SymbolIndex(persistence=...)` injection | `indexing/symbol_index.py` | Not started |
| Daemon `query_symbols` swap to `IndexStore` | `ci_daemon.py` | Not started |
...
| Index storage unit tests | `test_ci_storage_index.py` | Not started |
...
| Daemon lifecycle integration for the child | `ci_daemon.py` | Not started |
...
| HARD INVARIANT 5 regression against the new backend | `ci_daemon.py` | Not started |
...
4. **Serializer symmetry pays off in tests.** Writing
   ...
   `test_rpc_ci_backend_dispatch.py` exercise the serializer
...
1. **`Arbiter.__init__` already accepts `edit_history`** — no change
   ...
   `CodeIntelligenceService` → `_select_backend` → `InProcessCiBackend`.
...
```
backend/src/sandbox/code_intelligence/in_sandbox/ci_storage.py     +266 -1
backend/src/sandbox/code_intelligence/in_sandbox/ci_daemon.py      +540 -41
backend/src/sandbox/code_intelligence/backend.py                   +301 -120
backend/src/sandbox/code_intelligence/service.py                   +6 -1
backend/tests/test_sandbox/test_code_intelligence/test_ci_storage_ledger.py     +195 (new)
backend/tests/test_sandbox/test_code_intelligence/test_ci_daemon_dispatch.py    +273 (new)
backend/tests/test_sandbox/test_code_intelligence/test_rpc_ci_backend_dispatch.py +286 (new)
backend/tests/test_sandbox/test_code_intelligence/test_backend_inprocess.py     -100 +20
backend/tests/test_sandbox/test_code_intelligence/test_rpc_ci_backend.py        -8 +25
...
`test_ci_daemon_dispatch.py`) is deterministic and remains the primary
