...
> list. `ci_daemon.DISPATCH["svc_cmd"]` exists, `RpcCiBackend.cmd` routes
...
> by `test_rpc_ci_backend.py`, `test_ci_daemon_dispatch.py`, and
...
- A new `test_svc_cmd_shape_parity.py`. The Phase 4 spec listed Task 4.3
  ...
  `test_rpc_ci_backend.py::test_cmd_routes_through_daemon_and_reconstructs_namespace`
  (`backend/tests/test_sandbox/test_code_intelligence/test_rpc_ci_backend.py:186-238`)
  and
...
- Edits to `RpcCiBackend.cmd` or `handle_svc_cmd`. The wire-up is in
...
|---|---|---|
| `RpcCiBackend.cmd` raises `NotImplementedError("RpcCiBackend.cmd is reserved for Phase 4")` (Phase 3 §7.5) | **Closed.** `cmd` now routes through `svc_cmd`. | `backend/src/sandbox/code_intelligence/backend.py:564-576` |
...
| RPC backend keeps the Phase 1 cache fallback (Phase 3 §7.4) | **Closed.** Snapshot fallback retired by P35-CLEANUP. `_symbol_cache`, `_cached_file_count`, `_cached_symbol_count`, `_snapshot_bytes` removed; `_ensure_initialized_async` no longer reads pickle. | `backend.py:498` (only doc-comment reference to `_symbol_cache` remains, describing the removal); regression test `test_rpc_ci_backend.py::test_init_drops_legacy_cache_attributes` |
...
| `backend/src/sandbox/code_intelligence/backend.py` | `RpcCiBackend.cmd` (`:564-576`) — async path that ships the kwargs through `_call_async("svc_cmd", payload, timeout=…)` and reconstructs `SimpleNamespace(**(raw or {}))`. The `on_progress_line` callback is replayed once with the final stdout (matches the historical Task 4.4 decision that mid-command streaming is future transport work). |
...
| Result-shape parity test (Task 4.3) passes — every field round-trips | PASS — coverage already exists; **the spec note disowns the standalone parity-test file** | `test_rpc_ci_backend.py:186-238` + `test_ci_daemon_dispatch.py:165-238`; both round-trip the full 16-field shape. |
| `RpcCiBackend.cmd` ships args + reconstructs `SimpleNamespace` | PASS | `backend.py:564-576`; the synchronous facade also exists via `_call_async` / `run_sync` boundary. |
...
exposed a sync-bridge bug: every public RPC paid roughly 5.5 s because
...
`asyncio.run(client.call("index_refresh", …))` directly per iteration
...
FACADE. Real callers route through `RpcCiBackend._call_sync` →
...
- The 3.5/3.6 §6.4 stable-loop fix removes a sync-bridge accident that
  was inflating every public RPC by ~5.1 s of fresh-event-loop churn.
  After the fix, daemon RPCs are roughly **18× faster than the
...
- All daemon-path numbers in §6.2 / §6.3 measure public-path RPC
...
- `refresh_efficiency` JSONs (`refresh_file` p50 ≈ 5.5 s in both 2026-05-02
  ...
  `asyncio.run(client.call(…))` directly per iteration rather than
  routing through `RpcCiBackend._call_sync`, so the stable-loop fix
...
1. **True mid-command progress streaming.** `RpcCiBackend.cmd` replays
...
```
.venv/bin/pytest \
  backend/tests/test_sandbox/test_code_intelligence/test_rpc_ci_backend.py \
