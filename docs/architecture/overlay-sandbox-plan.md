...
| Decision | Choice | Why |
|---|---|---|
| Cutover | **One-shot replacement** of the git-workspace backend. Delete `git_workspace_*`, `git_diff_committer` in the same PR that lands overlay. | User directive. Git-workspace is capability-broken for gitignored deps; no value in dual-path soak. |
...
| Diff + merge location | **Inside the unshare namespace, before exit.** One sandbox-side Python script walks `upperdir`, classifies, direct-merges gitignored paths into the live workspace via the lower bind, and emits an NDJSON payload of tracked changes for the orchestrator. | Upperdir is tmpfs inside the ns; it is gone the moment the ns exits. An orchestrator-side walker would see nothing. |
...
| Whiteouts on gitignored paths | **Refuse by default.** Surface as `git_conflict_reason = "overlay_refused_gitignored_whiteout: <paths>"`. Agents that legitimately want to rebuild deps use `daytona_delete_file` (OCC-aware) or a future explicit rebuild tool. | Ecosystem-specific allowlists (`.venv/`, `node_modules/`, `target/`, `.next/`, `.gradle/`, `dist/`, …) rot. Refuse-all is the only policy that does not drift. |
| Whiteouts on tracked paths | **Emit as DELETE to OCC** (normal path). | OCC already handles this correctly via strict-base. |
| Mode-only changes on tracked paths | **Ignore.** Overlay copies up on mode change with identical content; classifier short-circuits on content equality with the SNAP base. | OCC does not track mode. Avoids spurious no-op changes. |
...
| Read isolation / live lower mutation | **No full read isolation; live changes are accepted.** Concurrent ops share live as lower; peer writes to live (via OCC commits or gitignored direct-merges) can mutate another op's active lower while its overlay is mounted. Per kernel docs this lower mutation is "undefined behavior" but explicitly "will not result in a crash or deadlock." | CodeAct is a live-workspace execution tool, not a snapshot-isolated runner. Practical surface is **weak read consistency inside the running command**: live peer changes may be visible, stale, partially visible through directory caches, or missed by a command that already read inputs. `SNAP` is only the tracked-write OCC base; it is not a read snapshot contract. Callers that require stable inputs must serialize above CodeAct or rerun. |
...
- **A `.gitignore` is strongly recommended but not required for
  ...
  tracked-route → routed through OCC. A `pip install` writing 10k files
...
│     ├── .venv/, node_modules/   (gitignored deps — present!)         │
│     ├── __pycache__/            (gitignored caches — present!)       │
│     ├── .git/                                                        │
│     └── .gitignore                                                   │
│                                                                      │
│   Per svc.cmd op:                                                    │
...
│   │       → classify each entry (whiteout? tracked? gitignored?)    ││
│   │       → direct-merge gitignored creates/modifies into live      ││
│   │         workspace through the lower bind                        ││
│   │       → emit tracked changes as NDJSON to $RUN_DIR/diff.ndjson  ││
│   │                                                                 ││
...
│    mismatch → status=aborted_version, this op's tracked writes skipped│
...
8. If any of steps 2–6 fail, raise. Tracked writes from this op are either
   ...
   direct-merged gitignored writes remain live by design.
...
direct-merge gitignored → emit NDJSON.
...
3. **Route classification** (after `.git/` REJECT and overlay-kind):
   - `git check-ignore -z --stdin` (one batch call for the surviving
     entries) says ignored → **gitignored route**.
   - Otherwise → **tracked route**.
...
- **Kind-gate first** (fail-closed):
  ...
  - non-UTF-8 content → REJECT (`overlay_non_utf8_tracked`).
...
- **whiteout** → REJECT run (`overlay_refused_gitignored_whiteout`).
- **create / modify** → atomic write via `tempfile.mkstemp` in the
  target's parent directory, then `rename` over target. Each op gets a
  unique temp filename, so concurrent writers to the same gitignored
...
writing to the same gitignored path — matches how `pip install` behaves
natively.

Mixed tracked + gitignored writes are accepted. Gitignored direct-merges are
applied inside the namespace before tracked OCC runs on the orchestrator. If
tracked OCC later aborts, the gitignored writes remain live. That is intentional
live-shell behavior, not a rollback bug. The auditor must surface this as
metadata/warning so agents know runtime state changed even though tracked source
writes did not land.
...
{"_meta": {"snap": "<sha>", "exit_code": 0,
           ...
           "tracked_changes": 3, "gitignored_changes": 39,
           "whiteouts": 0, "dotgit_rejects": 0,
           "direct_merged_bytes": 4567890}}
```

Then one object per emitted tracked change.
...
- `tracked_changed_paths`: tracked paths committed through OCC.
- `gitignored_direct_merged_paths`: gitignored paths direct-merged into live.
- `gitignored_direct_merged_count`: count of direct-merged ignored paths when
  the full path list is too large to return.
- `mixed_tracked_gitignored`: true when both routes had changes.
- `mixed_partial_apply`: true when gitignored writes landed but tracked OCC
  aborted.
- `warnings`: includes
  `"tracked changes aborted by OCC; gitignored runtime changes were already applied"`
  when `mixed_partial_apply` is true.

For downstream compatibility, `changed_paths` remains the committed tracked
...
While the command is running, peer OCC commits and gitignored direct-merges may
...
`SNAP` only protects tracked writes from this operation. It freezes the base
...
Op1: t0 SNAP1 (base A) | t2 upper writes A' | t3.5 peer lands A → C
     ...
     live still = C; upper already dead; Op1's tracked intent is skipped.
```
...
even if the tracked `requirements.txt` OCC commit aborts. This matches live
...
- Concurrent edits to **different** tracked files: OCC's
...
- `overlay.upper_files`
- `overlay.tracked_changes`
- `overlay.gitignored_changes`
- `overlay.direct_merged_bytes`
- `overlay.whiteouts_tracked` / `overlay.whiteouts_gitignored_refused`
- `overlay.dotgit_rejects`
- `overlay.upper_full_failures`
- `overlay.gitignored_changes_after_aborted_tracked` (count of ops where
  gitignored writes landed on live but tracked OCC aborted — expected under
  the accepted mixed-write contract)
- `overlay.mixed_tracked_gitignored_ops`
...
| 12 | LOW | Concurrent ops mutate each other's overlay lower (kernel docs: "undefined behavior") | **Accepted weak read semantics** — see §0 row "Read isolation / live lower mutation." CodeAct commands run against a live lowerdir, so peer writes may be visible, stale, partially visible, or missed. `SNAP` only protects tracked write commit, not command reads. If a future workload needs defined-behavior reads, serialize above CodeAct or swap to a per-op hardlink-clone lower (`cp -al $WORKSPACE_ROOT $RUN_DIR/lower`). |
| 13 | LOW | Mixed tracked + gitignored writes are non-atomic | **Accepted by contract.** The script direct-merges gitignored writes into live inside the ns, then emits tracked changes to NDJSON for orchestrator-side OCC. If OCC aborts, gitignored writes remain live while tracked writes are skipped. Coupled work like `pip install foo && echo foo >> requirements.txt` can update `.venv/` while `requirements.txt` aborts. Surface this via separated path metadata, `mixed_partial_apply=true`, a tool warning, and `overlay.mixed_partial_apply_ops`. |
...
- Unit tests against synthetic upperdir trees for every classifier
  branch (tracked add/modify/delete, gitignored create/modify, whiteout
  tracked, whiteout gitignored refused, `.git/*` reject, mode-only
  short-circuit, non-UTF-8 tracked reject, non-UTF-8 gitignored pass,
...
- E2E tests on live sandbox:
  ...
  - Concurrent peer edit to the same tracked path during overlay run aborts
    this op's tracked commit with `aborted_version`; the peer edit remains
    live (mirrors existing `test_live_daytona_occ_load.py`).
  - Mixed tracked + gitignored command with a forced tracked OCC abort still
    leaves gitignored writes live, skips tracked writes, and returns
...
4. OCC abort test passes — peer edit to the same tracked path mid-run →
   `aborted_version`; this op's tracked writes are not applied and the peer
   edit remains live.
5. Mixed tracked + gitignored partial-apply test passes — gitignored writes
   persist, tracked writes abort, and the response exposes
