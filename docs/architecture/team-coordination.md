...
- `daytona_shell` is runtime-only on coordinated lanes. File edits go through `daytona_edit_file` or `daytona_write_file`; shell/Python edit side channels such as `sed -i`, standalone `tee`, and inline Python writes are rejected before sandbox execution. Daytona shell prehooks continue to block destructive shell/git commands, package/environment mutation, and stderr suppression with `/dev/null`.
