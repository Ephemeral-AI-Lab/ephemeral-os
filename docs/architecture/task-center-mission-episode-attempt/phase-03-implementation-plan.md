...
state gates, and routes accepted submissions into `AttemptOrchestrator`.
...
9. Hard gates implemented as ordinary `ToolPreHook` instances attached through
   ...
   - `request_mission_solution` after-edit blocking,
...
11. Canonical executor request-start tool implemented as
   `request_mission_solution`, wired through the TaskCenter request /
...
`Attempt` failed and must not terminate the agent run. A successful
...
| Partial-plan ancestor gate reads caller ancestry | Phase 01, Phase 03 | Prehook walks from `Mission.requested_by_task_id` to caller harness graphs and blocks only when an ancestor caller graph has non-null `continuation_goal` | OK |
| `request_mission_solution` starts a delegated request, not failure | Phase 00, Phase 02, Phase 04 | Phase 03 exposes the canonical tool and gates it; Phase 04 fills request creation/resume behavior | OK |
...
1. Terminal tools need current task identity and graph runtime dependencies.
   ...
   metadata field.
2. `request_mission_solution` needs process-local graph runtime
   dependencies. The tool resolves the current `AttemptRuntime`, creates
   the delegated request and first segment through `MissionHandler`,
   marks the requesting executor task `waiting_mission`, and creates the
   delegated segment's initial `Attempt`.
3. Helper-agent tools are blocking helper runs rather than TaskCenter graph
   ...
   `AttemptOrchestrator.apply_*` and should not mutate graph state.
...
flowchart TD
    ...
    Valid -->|"yes"| Apply["AttemptOrchestrator.apply_*"]
...
flowchart TD
    ...
    Choice -->|"yes"| RequestStart["request_mission_solution"]
...
Helper and explorer runs are not `Attempt` DAG nodes. They are scoped to
...
notification rules inspect the live messages list passed by dispatch_rules
  ...
  |     -> remind executor that request_mission_solution is disabled
  |
...
`request_mission_solution` is a per-tool gate. It is available only while
...
2. On the next model turn, the TaskCenter reminder rule for
   `request_mission_solution` inspects its `messages` argument.
3. If its local state inference says the executor has edited, it injects a
   `<system-reminder>` telling the executor that
   `request_mission_solution` is disabled and the valid terminal choices
...
1. The agent calls `request_mission_solution`.
2. `ToolHookExecutionHelper` runs that tool's `pre_hooks`.
3. `RequestMissionBeforeEditGate` infers the same condition from the
...
flowchart TD
    ...
    SoftRule --> SoftCheck{"request_mission_solution disabled after edit?"}
    ...
    AgentChoice --> RequestStartCall["Agent calls request_mission_solution"]
    RequestStartCall --> HookRunner["ToolHookExecutionHelper runs pre_hooks"]
    HookRunner --> HardGate["RequestMissionBeforeEditGate"]
    ...
    Allow --> Handler["Run request_mission_solution handler"]
...
@tool(
    ...
    pre_hooks=(RequestMissionBeforeEditGate(),),
)
...
`notification_triggers: [request_mission_after_edit]` and resolved when
launching the executor agent. Its implementation lives in
`tools/submission/notification_triggers/request_mission_after_edit.py`. It
...
|   |-- request_mission_before_edit_gate.py  # NEW: executor request-start after-edit gate
...
|   |-- request_mission_after_edit.py   # NEW: executor request-start-disabled reminder trigger
...
|   |   |   |-- request_mission_solution.py  # NEW: canonical request-start tool
...
|-- generator/executor/agent.md              # EDIT: use request_mission_solution
...
| `request_mission_solution` | generator executor | delegated request start | `hooks/harness_role_gate.py`, `hooks/request_mission_before_edit_gate.py` | `notification_triggers/request_mission_after_edit.py` |
...
| `notification_triggers/request_mission_after_edit.py` | generator executor | Scan the notification rule's `messages` argument for edit-capable tool calls | `request_mission_solution` is disabled after the first edit |
...
class ExecutionMetadata:
    ...
    task_center_attempt_id: str | None = None  # optional consistency check
...
tools. `task_center_attempt_id` is optional metadata for diagnostics or a
...
ExecutionMetadata(
    ...
    task_center_attempt_id=launch.attempt_id,  # optional check
...
**`backend/src/task_center/attempt/runtime.py`** - edit

Expose `AttemptStore` on the runtime dependency bundle so submission tools
...
1. Read `attempt_runtime` and `task_center_task_id` from
   `ToolExecutionContextService`.
2. Load the task with `runtime.task_store.get_task(task_center_task_id)`.
3. Derive `graph_id` from `task["task_center_attempt_id"]`.
4. If optional metadata contains `task_center_attempt_id`, assert it
   matches the task row.
5. Load the graph with `runtime.graph_store.get(graph_id)`.
6. Load the segment with `runtime.segment_store.get(graph.episode_id)`.
7. Load the request with
   `runtime.request_store.get(segment.mission_id)`.
...
- `hooks/recursive_partial_plan_gate.py` and
  ...
  `Attempt.continuation_goal` values.
- `hooks/request_mission_before_edit_gate.py` and
  `notification_triggers/request_mission_after_edit.py` each scan
...
- Planner normalization and public DAG validation run in the planner tool
  handlers before calling `AttemptOrchestrator.apply_plan_submission`.
...
- Dependencies are acyclic. Reuse
  `task_center.attempt.task_graph.ordered_generator_tasks(...)` for the
...
# backend/src/tools/submission/hooks/request_mission_before_edit_gate.py
@dataclass(frozen=True, slots=True)
class RequestMissionBeforeEditGate:
    target_tool: str = "request_mission_solution"
...
| `RequestMissionBeforeEditGate` | `request_mission_solution` | Per-call `ExecutionMetadata.conversation_messages` contains an edit tool call |
...
- First edit has occurred in an executor run: remind executor that
  `request_mission_solution` is disabled and it must finish via execution
  success or failure.
...
**`backend/src/tools/submission/main_agent/generator/request_mission_solution.py`** - new
...
- Resolve `AttemptRuntime.manager_registry`, build a
  `MissionHandler`, create the delegated `Mission`, create
  its initial `Episode`, mark the requesting executor task
  `waiting_mission`, and create the initial `Attempt`.

Do not route this through `AttemptOrchestrator.apply_generator_submission`.
It is not a generator success or failure outcome.

Remove the legacy `submit_request_plan` tool surface. Phase 03 registers
`request_mission_solution` as the canonical request-start tool and wires the
...
`request_mission_solution` before making edits. After editing begins,
...
def request_has_partial_plan_ancestor(
    ...
    while True:
        ...
        if parent_task is None:
            return False
        parent_graph_id = parent_task.get("task_center_attempt_id")
        ...
        parent_segment = runtime.segment_store.get(parent_graph.episode_id)
        ...
        parent_request = runtime.request_store.get(
            parent_segment.mission_id
        )
        if parent_request is None:
            raise GraphInvariantViolation("Parent mission request is missing")
...
None of these errors should call `AttemptOrchestrator.apply_plan_submission`.
...
- `attempt_runtime` exists in metadata.
...
- task row `role` matches the expected structural role:
  `planner`, `generator`, or `evaluator`.
- graph id is derived from the task row's `task_center_attempt_id`.
- optional metadata `task_center_attempt_id`, when present, matches the
  derived graph id.
...
| Runtime | `ExecutionMetadata.task_center_attempt_id` | EDIT | Optional consistency check against graph id derived from the task row |
| Runtime | `ExecutionMetadata.attempt_runtime` | EDIT | Store + registry dependency bundle for tools |
...
| Runtime | `AttemptRuntime.graph_store` | EDIT | Graph lookup for submission-context resolution |
...
| Hook file | `hooks/request_mission_before_edit_gate.py` | NEW | Blocks complex-task request starts after edits |
...
| Notification trigger | `notification_triggers/request_mission_after_edit.py` | NEW | Executor reminder after first edit disables request start |
...
| Executor tools | `request_mission_solution` | NEW | Canonical request-start surface wired through TaskCenter request/segment lifecycle services |
...
| `test_executor_handoff_surface_uses_canonical_tool` | Executor agent definition lists `request_mission_solution` and does not list legacy `submit_request_plan` |
...
| `test_request_mission_solution_blocks_after_edit` | Edit history disables request start |
| `test_request_mission_solution_allows_before_edit` | No edit history allows gate to pass |
...
3. Implement `hooks/request_mission_before_edit_gate.py`.
...
2. Implement `notification_triggers/request_mission_after_edit.py`.
...
7. Keep executor `agent.md` on the canonical `request_mission_solution`
...
| `request_mission_solution` is blocked after executor edits | `test_request_mission_solution_blocks_after_edit` |
...
| Executor public contract uses `request_mission_solution` | agent-definition registration test |
...
The terminal tools require `task_center_task_id` and `attempt_runtime` in
...
metadata also carries `task_center_attempt_id`, it is only a consistency
check.
...
Phase 03 moves the public agent contract to `request_mission_solution` and
...
### 11g. `request_mission_solution` runtime dependencies

`request_mission_solution` needs more than graph stores: it also needs the
process-local `EpisodeManagerRegistry` and an orchestrator factory so the delegated
request's first segment can start its initial `Attempt`.

Mitigation: fail closed when `AttemptRuntime.manager_registry` is missing.
When it is present, build the delegated request through `MissionHandler` and
route the final close report back through
`AttemptOrchestrator.apply_mission_close_report`.
...
would make helper output accidentally close or fail a `Attempt`.
...
and test that helper request tools do not call `AttemptOrchestrator`.
