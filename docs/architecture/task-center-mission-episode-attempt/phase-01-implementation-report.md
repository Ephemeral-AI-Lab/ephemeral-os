...
| File | Status | Purpose |
| --- | --- | --- |
| `backend/src/db/models/mission.py` | new | `MissionRecord` SQLA row |
| `backend/src/db/models/episode.py` | new | `EpisodeRecord` SQLA row |
| `backend/src/db/models/attempt.py` | new | `AttemptRecord` SQLA row (replaces legacy) |
| `backend/src/db/models/task_center.py` | edited | drop `TaskCenterAttemptRecord` + `TaskCenterRunRecord.attempts` relationship |
...
| `backend/src/task_center/domain/mission.py` | new (DTO + status enum + close report) |
| `backend/src/task_center/domain/episode.py` | new (DTO + status + creation reason enums) |
| `backend/src/task_center/domain/attempt.py` | new (DTO + stage / status / fail-reason enums) |
...
| File | Status |
| --- | --- |
| `backend/src/db/stores/mission_store.py` | new (returns frozen DTOs) |
| `backend/src/db/stores/episode_store.py` | new |
| `backend/src/db/stores/attempt_store.py` | new |
| `backend/src/db/stores/__init__.py` | edited (register 3 new stores) |
| `backend/src/db/stores/task_center_store.py` | edited (remove `upsert_attempt`, `list_attempts_for_run`, `_serialize_attempt`) |

### Lifecycle skeleton (Wave 3)

| File | Status |
| --- | --- |
| `backend/src/task_center/mission/__init__.py` | new |
| `backend/src/task_center/mission/config.py` | new (`HarnessLifecycleConfig.default_attempt_budget = 2`) |
| `backend/src/task_center/mission/invariants.py` | new (request-level invariants) |
| `backend/src/task_center/mission/segment_manager_registry.py` | new (one-manager-per-open-segment) |
| `backend/src/task_center/mission/handler.py` | new (`MissionHandler`) |
| `backend/src/task_center/mission/segment/__init__.py` | new |
| `backend/src/task_center/mission/segment/manager.py` | new (`EpisodeManager`) |
| `backend/src/task_center/mission/segment/attempt_count.py` | new (public helper) |
| `backend/src/task_center/mission/segment/invariants.py` | new (segment-level invariants) |
| `backend/src/task_center/mission/segment/attempt/__init__.py` | new |
| `backend/src/task_center/mission/segment/attempt/orchestrator.py` | new (Phase 02 skeleton; `NotImplementedError`) |
| `backend/src/task_center/mission/segment/attempt/invariants.py` | new (graph-level invariants) |
...
| `backend/tests/task_center/domain/test_mission_dto.py` | 70 |
| `backend/tests/task_center/domain/test_attempt_dto.py` | 55 |
| `backend/tests/task_center/domain/test_episode_dto.py` | 57 |
| `backend/tests/task_center/domain/test_segment_closure_report.py` | 85 |
| `backend/tests/task_center/persistence/test_mission_store.py` | 90 |
| `backend/tests/task_center/persistence/test_episode_store.py` | 132 |
| `backend/tests/task_center/persistence/test_attempt_store.py` | 99 |
...
| `backend/tests/task_center/lifecycle/test_mission_handler.py` | 309 |
| `backend/tests/task_center/lifecycle/test_episode_manager.py` | 252 |
...
| `request_mission_solution` -> request linked to `requested_by_task_id` | `test_create_mission_links_executor` |
| Each request records segments in `episode_ids` | `test_request_records_segments_in_episode_ids` |
| `episode_ids` holds multiple `Episode` ids | `test_episode_ids_holds_multiple_segments` |
...
| Manager retry creates `Attempt` in same segment | `test_retry_creates_graph_in_same_segment` |
| Public `get_attempt_count` derived from list | `test_get_attempt_count_derived_from_list` |
| Legacy `task_center_attempt` dropped + 3 new tables created | `test_initialize_db_drops_legacy_attempt_table` |
...
```text
                              executor task E
                                    ...
                                    v
            +------- MissionHandler -------+
            |  - create_mission          |   request: open
            |  - create_initial_segment   (sequence 1)|   ↓ append id
            |                                         |
            |  spawns EpisodeManager(S1)          |   segment: open
            +------------------+----------------------+   ↓ register
                               |
                  +------------v-----------+
                  |   EpisodeManager   |
                  |  - create_initial_     |   attempt_ids = (g1,)
                  |    attempt       |       ^
                  |       |                |       | append id
                  |       v                |
                  |   AttemptOrch.    |   graph: planning -> ... -> closed
                  |   (Phase 02 fills)     |
                  +------------+-----------+
                               | on close -> handle_attempt_closed
                               v
                  +------------------------+
                  |  closure routing       |
                  |                        |
...
- `episode_ids` always grows by 1 per new segment (contiguous `sequence_no`)
- `attempt_ids` always grows by 1 per new graph (contiguous
  `attempt_sequence_no`, never crosses segments)
...
- continuation segments require predecessor `SUCCEEDED` + non-null
  `continuation_goal`
- exactly one `EpisodeManager` registered per open segment; closure
  deregisters in `try/finally`
- failed graphs require a `fail_reason`
- `attempt_count` is derived from `len(attempt_ids)` — no separate
  counter
- `'root'` creation reason is rejected
- `attempted_plan_history` carries `attempt_summary_id=None` and
...
| Item | Where | Phase |
| --- | --- | --- |
| Orchestrator runs planner / generator / evaluator | `AttemptOrchestrator` raises `NotImplementedError` | 02 |
| `submit_full_plan` / `submit_partial_plan` tool gates | `tools/submission/main_agent/planner` | 03 |
| Final-report delivery to `requested_by_task_id` | `MissionHandler.deliver_close_report=None` | 04 |
| `attempt_summary_id` / `failure_landscape` populated | `AttemptedPlanEntry` left as `None` | 06 |
