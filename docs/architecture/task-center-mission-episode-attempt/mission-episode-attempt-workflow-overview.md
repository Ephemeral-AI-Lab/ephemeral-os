# Complex Task Segmentation and Harness Graph Workflow

This document summarizes how a complex generator task is routed through the
harness graph runtime and reported back to the task that requested it.

The migration separates three concepts that were previously overloaded:

- `Mission`: the delegated complex goal requested by a generator task.
- `Episode`: one vertical continuation slice of that complex goal.
- `Attempt`: one concrete planner-produced graph execution inside one
  segment.

## Mental model

Mission progression has three axes:

| Axis | Entity | Trigger | Meaning |
| ---- | ------ | ------- | ------- |
| Request origin | `Mission` | Generator task calls `request_mission_solution(goal)` | A delegated complex goal starts for the requesting task. |
| Vertical continuation | `Episode` | Prior segment passes with non-null `continuation_goal` | The same complex request moves to its next sequential slice. |
| Horizontal retry | `Attempt` | A graph fails and segment attempt budget remains | The same segment receives a fresh planner-produced graph. |

```mermaid
flowchart TD
    E["Generator task"] -->|"request_mission_solution(goal)"| C["Mission"]
    C --> S1["Episode S1"]
    S1 --> H11["Attempt S1.H1"]
    H11 -->|"failed, attempt budget remains"| H12["Attempt S1.H2"]
    H12 -->|"passed with continuation_goal"| S2["Episode S2"]
    S2 --> H21["Attempt S2.H1"]
    H21 -->|"passed with continuation_goal = null"| Close["Close Mission"]
    Close -->|"final report"| E
```

The key rule is:

- A new `Episode` means accepted vertical continuation.
- A new `Attempt` inside that segment means retry after failure.
- The complex-task close report supplies the final result for
  `requested_by_task_id`; the original generator agent run ends at the request
  boundary.

## Layer responsibilities

| Layer | Owns | Does not own |
| ----- | ---- | ------------ |
| `MissionHandler` | Request creation and close, initial segment creation, continuation segment creation, final close report to `requested_by_task_id`. | Per-segment retry policy or graph execution. |
| `EpisodeManager` | One segment's attempt budget, next harness graph creation after failed graphs, segment close, `EpisodeClosureReport`. | Request creation, continuation segment creation, or planner/generator/evaluator execution. |
| `AttemptOrchestrator` | One `planner -> generator DAG -> evaluator` execution and graph pass/fail outcome. | Retry, continuation, or request close. |
| Agent roles | Planner, generator agent, verifier, and evaluator terminal submissions inside a graph. | Structural lifecycle decisions. |
| Context engine | Role-specific launch context, durable summaries, and detailed close-report payloads. | Lifecycle policy or source-of-truth state transitions. |

## End-to-end flow

```mermaid
sequenceDiagram
    participant E as Generator
    participant R as MissionHandler
    participant S as EpisodeManager
    participant H as AttemptOrchestrator
    participant A as Agents

    E->>R: request_mission_solution(goal)
    R->>R: create Mission
    R->>R: create Episode S1
    R->>S: spawn manager for S1
    S->>H: create Attempt S1.H1
    H->>A: spawn planner
    A->>H: submit_full_plan or submit_partial_plan
    H->>A: spawn generator DAG
    A->>H: generator/verifier terminal submissions
    H->>A: spawn evaluator after all generators pass
    A->>H: submit_evaluation_success or submit_evaluation_failure
    H->>S: report graph passed or failed
    S->>R: emit EpisodeClosureReport when segment closes
    R->>E: deliver final complex-task report
```

## Harness graph lifecycle

`AttemptOrchestrator` owns exactly one graph run. It does not inspect
attempt budget and does not create sibling graphs.

```mermaid
flowchart TD
    Start["Attempt starts"] --> Plan["planning: run planner"]
    Plan -->|"valid full plan submitted"| Gen["generating: run executor/verifier DAG"]
    Plan -->|"planner ends without valid plan"| PlannerFail["close graph failed: planner_failed"]
    Gen -->|"any generator failed or blocked after quiescence"| GenFail["close graph failed: generator_failed"]
    Gen -->|"all generators done"| Eval["evaluating: run evaluator"]
    Eval -->|"submit_evaluation_success"| Passed["close graph passed"]
    Eval -->|"submit_evaluation_failure"| EvalFail["close graph failed: evaluator_failed"]
    PlannerFail --> Segment["report graph outcome to EpisodeManager"]
    GenFail --> Segment
    Passed --> Segment
    EvalFail --> Segment
```

Generator failure waits for quiescence: failed generators block dependents,
independent siblings may finish, and the graph closes only after all generator
nodes are terminal.

## Segment decision flow

`EpisodeManager` reacts to the closed graph. It is the only layer that can
spend segment attempt budget.

```mermaid
flowchart TD
    HClose["Attempt closes"] --> Passed{"Graph passed?"}

    Passed -->|"no"| Retry{"Retry budget remains?"}
    Retry -->|"yes"| NextH["Create next Attempt in same Episode"]
    NextH --> RunNext["Run next graph through AttemptOrchestrator"]
    Retry -->|"no"| FailSeg["Close Episode failed"]
    FailSeg --> FailReport["Emit EpisodeClosureReport: attempt_plan_failed(attempted_plan_history)"]

    Passed -->|"yes"| SetCont["Set segment.continuation_goal = graph.continuation_goal"]
    SetCont --> HasCont{"continuation_goal is non-null?"}
    HasCont -->|"yes"| ContinueReport["Emit EpisodeClosureReport: success_continue(goal)"]
    HasCont -->|"no"| TerminalReport["Emit EpisodeClosureReport: terminal_success"]
```

A passed graph always closes its segment. There is no retry after a passing
graph; graph quality is enforced by the evaluator.

## Request decision flow

`MissionHandler` reacts only to `EpisodeClosureReport`.

```mermaid
flowchart TD
    Report["EpisodeClosureReport"] --> Outcome{"Outcome"}
    Outcome -->|"success_continue(goal)"| NextSeg["Create Episode N+1"]
    NextSeg --> Goal["goal = continuation_goal"]
    Goal --> NewManager["Spawn fresh EpisodeManager"]
    NewManager --> NewGraph["EpisodeManager creates initial Attempt"]

    Outcome -->|"terminal_success"| Success["Close Mission succeeded"]
    Outcome -->|"attempt_plan_failed(attempted_plan_history)"| Failed["Close Mission failed"]
    Success --> Return["Deliver final report to requested_by_task_id"]
    Failed --> Return
```

Continuation does not return to the requesting executor. It keeps the same
complex request open and creates another segment. The requesting executor sees
one final report for the whole complex request.

## Happy path

```mermaid
flowchart TD
    E["Generator decides task is non-atomic"] --> Request["request_mission_solution(goal)"]
    Request --> C["Create Mission C1"]
    C --> S1["Create Episode S1"]
    S1 --> H1["Create Attempt S1.H1"]
    H1 --> P["Planner submits submit_full_plan"]
    P --> G["Generator DAG completes successfully"]
    G --> V["Evaluator submits success"]
    V --> HP["Attempt passes"]
    HP --> SC["Episode closes with continuation_goal = null"]
    SC --> RC["Mission closes success"]
    RC --> Report["Final complex-task report is attached to requested_by_task_id"]
```

## Partial continuation path

```mermaid
flowchart TD
    P1["Planner in S1.H1 submits submit_partial_plan"] --> CG["S1.H1.continuation_goal = G"]
    CG --> Work1["Generators complete partial DAG"]
    Work1 --> Eval1["Evaluator accepts S1.H1"]
    Eval1 --> CloseS1["EpisodeManager closes S1"]
    CloseS1 --> Report["EpisodeClosureReport: success_continue(G)"]
    Report --> S2["MissionHandler creates S2"]
    S2 --> H2["EpisodeManager creates S2.H1"]
    H2 --> Work2["S2.H1 runs and passes"]
    Work2 --> Done["Request closes and reports back to requested_by_task_id"]
```

The segment inherits `continuation_goal` only from the passing graph that
closed it. Failed graphs in the same segment do not propagate their
`continuation_goal` to later graphs.

## Retry-then-pass path

```mermaid
flowchart TD
    H1["S1.H1 runs"] --> Fail["S1.H1 fails"]
    Fail --> Budget{"Retry budget remains?"}
    Budget -->|"yes"| H2["EpisodeManager creates S1.H2"]
    H2 --> Fresh["S1.H2 planner decides full or partial independently"]
    Fresh --> Pass["S1.H2 passes"]
    Pass --> Close["S1 closes using S1.H2.continuation_goal"]
    Budget -->|"no"| Exhaust["S1 closes failed and request fails"]
```

Retry history is horizontal inside the segment. The next planner receives the
failure landscape as context, but lifecycle state does not inherit the prior
failed graph's `continuation_goal`.

## Recursive mission request

Any generator agent profile can request its own mission before it edits,
provided that profile declares `request_mission_solution`. That
creates a new request, not a child segment in the outer request.

```mermaid
flowchart TD
    C1["Outer Mission C1"] --> S1["Episode S1"]
    S1 --> H1["Attempt S1.H1"]
    H1 --> E7["Generator task E7"]
    E7 -->|"request_mission_solution(goal)"| C2["Delegated Mission C2"]
    C2 --> C2S1["C2 Episode S1"]
    C2S1 --> C2H1["C2 Attempt S1.H1"]
    C2H1 --> C2Close["C2 closes"]
    C2Close -->|"final report"| E7
    E7 --> OuterContinue["Outer graph consumes E7's final report"]
```

The delegated request has its own segment chain and retry history. The outer
request sees only the close report associated with the generator task that
requested it.

## Tool and role boundaries

| Role | Scope | Main terminals |
| ---- | ----- | -------------- |
| Planner | One `Attempt` | `submit_full_plan`, `submit_partial_plan` |
| Generator direct-work profile | One graph DAG node | `submit_execution_success`, `submit_execution_failure`; may also declare `request_mission_solution` |
| Generator verifier | One graph DAG node | `submit_verification_success`, `submit_verification_failure` |
| Evaluator | Sink for one graph | `submit_evaluation_success`, `submit_evaluation_failure` |

Important gates:

- `submit_partial_plan` is blocked only when the current request was spawned,
  directly or transitively, from a caller harness graph with non-null
  `continuation_goal`; same-request continuation segments may submit partial
  plans again.
- malformed planner DAG submissions fail inline without marking the graph
  failed.
- `request_mission_solution` is blocked after the generator agent has edited.
- evaluator spawn is blocked until every generator in the current graph is
  `DONE`.
- next graph creation is blocked once the segment attempt budget is exhausted.

## Context engine boundary

The context engine composes structured context packets and summaries for each
role, but lifecycle decisions read structural state:

- planner context includes request goal, segment goal, and retry failure
  landscape when applicable;
- generator context includes the planned task spec and dependency summaries;
- evaluator context includes the graph task specification, evaluation criteria,
  and completed generator/verifier summaries;
- request close context includes the final mission summary and close report
  for `requested_by_task_id`.

Generated summaries are evidence. They do not decide whether to retry or close
the request.
