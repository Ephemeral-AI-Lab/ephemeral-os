...
| File | Status | Purpose |
| --- | --- | --- |
| `backend/src/task_center/attempt/task_graph.py` | new | Generator DAG validation, topological ordering, dependency readiness, descendant blocking, and quiescence helpers |
| `backend/src/task_center/attempt/runtime.py` | new | `AttemptRuntime`, `HarnessAgentLaunch`, and `AttemptAgentLauncher` protocol |
| `backend/src/task_center/attempt/orchestrator_registry.py` | new | Process-local active orchestrator lookup by `Attempt.id` |
| `backend/src/task_center/attempt/orchestrator.py` | edited | Full planner -> generator DAG -> evaluator state machine |
| `backend/src/task_center/attempt/factory.py` | new | Composition helper that registers and returns graph orchestrators |
| `backend/src/task_center/attempt/__init__.py` | edited | Lazy package exports for Phase 02 lifecycle helpers without circular imports |
...
| `backend/src/task_center/mission/handler.py` | edited | Accept and pass an orchestrator factory to every spawned segment manager |

### Architecture docs

| File | Status | Purpose |
| --- | --- | --- |
| `docs/architecture/task-center-mission-episode-attempt/phase-02-implementation-plan.md` | edited | Synced the delivered task module layout and actual test names after review |
...
| `backend/tests/task_center/lifecycle/test_attempt_task_graph.py` | 86 | Generator DAG and quiescence helpers |
| `backend/tests/task_center/lifecycle/test_attempt_orchestrator_registry.py` | 35 | Process-local registry behavior |
| `backend/tests/task_center/lifecycle/test_attempt_orchestrator.py` | 476 | Graph-scoped orchestration state machine |
| `backend/tests/task_center/lifecycle/test_episode_manager.py` | 318 | Manager factory seam and retry graph startup |
| `backend/tests/task_center/lifecycle/test_mission_handler.py` | 346 | Handler factory propagation and request/segment behavior |
...
- `uv run ruff check backend/src/task_center backend/tests/task_center docs/architecture/task-center-mission-episode-attempt/phase-02-implementation-plan.md`: clean
- `uv run mypy --config-file backend/mypy.ini backend/src/task_center`: clean
- Package export smoke:
  `from task_center.attempt import Attempt, AttemptOrchestratorRegistry, make_attempt_orchestrator_factory`: clean
...
| `waiting_mission` is non-terminal and keeps graph in `generating` | `test_waiting_mission_prevents_generator_quiescence` and `test_waiting_mission_is_not_quiescent_or_done` |
...
MissionHandler
  ...
  register EpisodeManager(S1, orchestrator_factory)
        |
        v
EpisodeManager.create_initial_attempt()
  insert Attempt H1
  append H1 to S1.attempt_ids
  factory(H1, handle_attempt_closed)
  orchestrator.start()
        |
        v
...
Generator terminal submissions
  ...
  waiting_mission rows stay non-terminal
        |
        v
...
_close_graph(...)
  ...
  on_graph_closed(H1)
        |
        v
EpisodeManager.handle_attempt_closed(H1)
  passed -> close segment as success or success_continue
  failed + budget -> create next Attempt in same segment
  failed + no budget -> close segment with AttemptPlanFailed
```

The orchestrator still owns only one `Attempt`. It never creates
`Mission`, `Episode`, or sibling graph rows. Segment retry
policy remains inside `EpisodeManager`.
...
- `waiting_mission` is excluded from terminal generator statuses.
...
| `task_center.attempt` package exports were missing | Added lazy `__getattr__` exports for Phase 02 helpers and an import smoke check |
...
| `request_mission_solution` delegated request spawn | complex-task request boundary | 04 |
| `AttemptOrchestrator.apply_mission_close_report` resume path | `attempt/orchestrator.py` | 04 |
| Durable final close-report delivery to `requested_by_task_id` | `MissionHandler.deliver_close_report` wiring | 04 |
...
| `failure_landscape` and `attempt_summary_id` population | segment close reports | 06 |
