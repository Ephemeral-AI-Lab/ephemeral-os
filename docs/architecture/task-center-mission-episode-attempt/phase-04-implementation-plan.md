...
Companion to
[`phase-04-mission-spawning.md`](./phase-04-mission-spawning.md).
...
`request_mission_solution(goal)` call to a delegated
`Mission`, and from the delegated request's close report back to the
...
2. An executor-profile hard gate so only the generator executor agent can call
   `request_mission_solution`, `submit_execution_success`, and
...
3. Handoff creation that avoids leaving the parent task stuck in
   `waiting_mission` when delegated graph startup fails.
4. Initial delegated request creation through `MissionHandler`, initial
   segment creation through the handler, and initial graph creation through the
   segment-owned `EpisodeManager`.
5. Continuation segment creation after `success_continue(goal)`, using the
   existing `MissionHandler.handle_segment_closed(...)` path.
6. Close-report delivery through one router that calls
   `AttemptOrchestrator.apply_mission_close_report(...)` for the
   parent graph.
7. Synchronous close-report delivery for already-closed complex requests whose
   parent task is still `waiting_mission`. Phase 04 assumes no process
...
8. `/api/db/task-center-runs/{id}/graph` backed by
   `missions -> episodes -> attempts`, with task rows
...
- Rich context packets, evidence summaries, failure landscapes, or
  `attempt_summary_id` population. Phase 06 owns that data.
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- |
| Phase 01 durable records         | `MissionRecord`, `EpisodeRecord`, and `AttemptRecord` exist under `backend/src/db/models/`; stores return frozen DTOs.       | Reuse. Add only query helpers needed by route walking.                                                     |
| Request/segment lifecycle        | `MissionHandler` creates requests and segments, emits close reports through an optional callback, and spawns `EpisodeManager`.    | Reuse. Do not create requests or segments from any other class.                                             |
| Segment retry lifecycle          | `EpisodeManager` is the only creator of graphs inside one segment and already handles retry vs segment closure.                              | Reuse. Add a narrow deferred-start seam only if needed for safe handoff startup.                            |
| Phase 02 graph orchestration     | `AttemptOrchestrator` starts planner, schedules generators, spawns evaluator, closes graphs, and keeps `waiting_mission` non-terminal. | Reuse. Harden close-report idempotency and validation.                                                      |
| Parent resume entry              | `AttemptOrchestrator.apply_mission_close_report(...)` exists and maps delegated success to parent `done`, failure to parent `failed`.  | Keep as the single parent resume method. Add idempotency behavior around it.                                |
| Phase 03 public tool             | `request_mission_solution` is registered and currently performs delegated request creation inline.                                          | Refactor the body into a coordinator. The tool should validate input and delegate.                              |
...
| Runtime store initialization     | Server initializes `TaskCenterStore`, `AgentRunStore`, and `ModelStore`; request/segment/graph stores are test-only or runtime-object-only.      | Initialize `MissionStore`, `EpisodeStore`, and `AttemptStore` in app/runtime bootstrap. |
| Graph API route                  | `persistence.py` still returns `{"attempts": []}` with `TODO(phase-04)`.                                                                   | Implement the route from the new schema.                                                                    |

Important current risk:

`request_mission_solution` currently marks the parent generator task
`waiting_mission` before `segment_manager.create_initial_attempt()`
...
```mermaid
flowchart TD
    Exec["Generator executor task E<br/>status = running"] --> Tool["request_mission_solution(goal)"]
    ...
    ParentCheck -->|"yes"| CreateReq["MissionHandler.create_mission"]
    CreateReq --> CreateSeg["MissionHandler.create_initial_segment"]
    CreateSeg --> Manager["Find registered EpisodeManager(S1)"]
    Manager --> CreateGraph["EpisodeManager creates H1<br/>inside S1<br/>without starting it yet"]
    CreateGraph --> Wait["Set parent task E<br/>waiting_mission"]
...
`MissionHandler`, `EpisodeManager`, or `TaskCenterStore`
...
```mermaid
flowchart TD
    DelegatedGraph["Delegated Attempt closes"] --> SegmentMgr["EpisodeManager.handle_attempt_closed"]
    ...
    Outcome -->|"passed + continuation_goal"| Continue["EpisodeClosureReport<br/>success_continue(goal)"]
    Outcome -->|"passed + no continuation"| Terminal["EpisodeClosureReport<br/>terminal_success"]
    Outcome -->|"failed + budget exhausted"| Failed["EpisodeClosureReport<br/>attempt_plan_failed"]

    Continue --> HandlerContinue["MissionHandler creates S_next<br/>and fresh EpisodeManager"]
    HandlerContinue --> NextGraph["EpisodeManager creates and starts S_next.H1"]
    NextGraph --> DelegatedGraph

    Terminal --> CloseOK["MissionHandler closes request success"]
    Failed --> CloseFail["MissionHandler closes request failed"]
    CloseOK --> Persist["Persist final_outcome"]
    CloseFail --> Persist
    Persist --> Router["MissionCloseReportRouter.deliver"]
    Router --> ParentOrch["Parent AttemptOrchestrator.apply_mission_close_report"]
...
`Mission` close report resumes the parent generator task.

Continuation startup is mandatory. When
`MissionHandler.handle_segment_closed(...)` receives
`success_continue(goal)`, it must create the next `Episode`, obtain the fresh
`EpisodeManager`, create that segment's initial `Attempt` through the
...
```mermaid
flowchart TD
    Close["Mission closes"] --> Build["Build MissionCloseReport"]
    Build --> Router["MissionCloseReportRouter.deliver"]
    Router --> Filter{"Parent task status"}
    Filter -->|"done or failed"| Skip["already_delivered"]
    Filter -->|"waiting_mission"| Active{"Parent orchestrator active?"}
...
```mermaid
flowchart LR
    Run["task_center_run_id"] --> Requests["MissionStore.list_for_run"]
    Requests --> Segments["EpisodeStore.list_for_request"]
    Segments --> Graphs["AttemptStore.list_for_episode"]
    Graphs --> Tasks["TaskCenterStore.list_tasks_for_attempt"]
    Tasks --> Response["/api/db/task-center-runs/{id}/graph response"]
```

The route should keep top-level `attempts` for compatibility, while also
...
| ------------------------------------------------- | ---------------------------------------- |
| `Mission` creation                     | `MissionHandler` only         |
| `Episode` creation                            | `MissionHandler` only         |
| `EpisodeManager` spawning                     | `MissionHandler` only         |
| `Attempt` creation inside a segment          | that segment's `EpisodeManager` only |
| Initial complex-task handoff orchestration      | new `ComplexTaskHandoffCoordinator`          |
| Close-report routing to parent graph              | new `MissionCloseReportRouter`       |
| Parent generator state mutation from close report | parent `AttemptOrchestrator` only   |
...
|-- request.py                         # existing DTOs; keep MissionCloseReport here
|-- handler.py                         # edit only if callback hook needs shaping
|-- handoff.py                   # NEW: ComplexTaskHandoffCoordinator
|-- close_report_delivery.py           # NEW: MissionCloseReportRouter
...
|-- executor/
|   |-- request_mission_solution.py # EDIT: delegate to handoff coordinator
...
|-- mission_store.py      # EDIT: list_for_run + closed-report queries
|-- task_center_store.py               # EDIT: compare-and-set status helper
|-- episode_store.py              # existing list_for_request
`-- attempt_store.py             # existing list_for_episode
...
|-- test_phase04_mission_handoff.py
...
### 6a. `task_center/mission/handoff.py`
...
2. Require parent status `running` and parent graph id matching the resolved
   submission context.
3. Reject if `MissionStore.list_for_executor_task(parent_task_id)`
   contains any `OPEN` request.
4. Build a `MissionHandler` with:
   - `runtime.request_store`
   - `runtime.segment_store`
   - `runtime.graph_store`
   - `runtime.manager_registry`
   - `runtime.lifecycle_config`
   - `MissionCloseReportRouter(runtime).deliver`
   - `make_attempt_orchestrator_factory(...)`
5. Call `create_mission(...)`.
6. Call `create_initial_segment(...)`.
7. Get the registered `EpisodeManager` for the new segment.
8. Create the initial delegated graph through the manager without starting its
   orchestrator yet.
9. Mark the parent task `waiting_mission` with a delegated request summary that
...
Add a single deferred-start seam to `EpisodeManager` that returns a
one-shot starter alongside the created graph. Avoids growing a second
`start_attempt(id)` method and a `created/not_started` vs `running`
state on the manager.

```python
@dataclass(frozen=True, slots=True)
class AttemptStartHandle:
    ...
    create_initial_attempt(...) for the same segment until either start()
...
class EpisodeManager:
    def create_initial_attempt(self) -> AttemptStartHandle: ...
```

Existing Phase 01/02 callers that want eager start become a one-liner:

```python
manager.create_initial_attempt().start()
```
...
close report before the parent task has entered `waiting_mission`.
...
class ComplexTaskHandoffCoordinator:
    def _compensate_failed_handoff(
        ...
        *,
        ...
        start_handle: AttemptStartHandle | None,
    ) -> None:
...
### 6b. `task_center/mission/close_report_delivery.py`
...
7. Call `orchestrator.apply_mission_close_report(report)`.
8. Return `delivered`.

### 6c. `task_center/attempt/orchestrator.py`

Edit `apply_mission_close_report(...)` in place.
...
1. Idempotency via CAS: route the parent transition through
   `task_store.set_task_status_if_current(task_id, expected_status="waiting_mission", status=...)`
   (see §6f). A `None` return means another delivery already moved the parent
   off `waiting_mission` — treat this as already-delivered and return
...
### 6d. `tools/submission/main_agent/generator/request_mission_solution.py`
...
`MissionHandler`, no `make_attempt_orchestrator_factory`,
...
| ------------------------------- | --------------------- |
| `request_mission_solution` | `executor`            |
...
```python
def list_for_run(self, task_center_run_id: str) -> list[Mission]: ...
...
| -------------------------- | ----------------------- | ------------------------- | -------------------------- | ------------------------------- |
| handoff entry              | coordinator             | `running`                 | `waiting_mission`     | `GraphInvariantViolation`       |
| handoff rollback           | coordinator             | `waiting_mission`    | `running`                  | log + swallow (already moved)   |
| close-report deliver       | parent orchestrator     | `waiting_mission`    | `done` / `failed`          | silent return (already delivered) |
...
4. For each segment, load graphs ordered by `attempt_sequence_no`.
5. For each graph, attach `TaskCenterStore.list_tasks_for_attempt(...)`.
6. Return a single nested representation plus a thin lookup index. Avoid
   ...
   `episode_id` re-embedded on each graph) — those are already implied
...
`attempts_index` is the only flat artifact, and it carries lookup keys
...
top-level `attempts` key as an alias for `attempts_index` (same
...
3. Add tests:
   ...
   - verifier profile cannot call `request_mission_solution`
...
5. Preserve existing successful delegated request behavior in
   `test_request_mission_solution_starts_delegated_request`.

### Wave 2 - Safe Startup and Compensation

1. Implement the `AttemptStartHandle` seam from §6a:
   `EpisodeManager.create_initial_attempt()` returns a one-shot handle
...
1. Add `MissionCloseReportRouter`.
2. Wire `MissionHandler.deliver_close_report` to the router from
   the coordinator-created handler.
3. Add idempotency to `AttemptOrchestrator.apply_mission_close_report`.
4. Add tests:
...
2. Add an end-to-end lifecycle test where delegated graph 1 fails, graph 2 passes
   ...
   parent.
3. Assert continuation creates a new `Episode`, while retry creates a new
   `Attempt` in the same segment.
...
1. Update `phase-04-mission-spawning.md` only if the
...
uv run pytest backend/tests/task_center/lifecycle/test_phase04_mission_handoff.py backend/tests/task_center/lifecycle/test_phase04_close_report_delivery.py -q
...
| Test                                                                            | Purpose                                                                                                |
| ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `test_executor_profile_required_for_mission`                       | Verifier-owned generator run cannot call `request_mission_solution`.                              |
...
- `request_mission_solution` creates a delegated `Mission`, its
  initial `Episode`, and its initial `Attempt` through the proper
  owners.
- The requesting executor task enters `waiting_mission` only when the
...
- Continuation creates later ordered `Episode` records and does not resume
  the parent until the whole complex request closes.
- Retry creates later `Attempt` records inside the same segment and does
  not resume the parent until the request closes.
- Closed request reports deliver idempotently while the parent task is still
  `waiting_mission` and its orchestrator is active.
- `/api/db/task-center-runs/{id}/graph` returns graph data from
  `missions`, `episodes`, and `attempts` instead of an
  empty stub.
