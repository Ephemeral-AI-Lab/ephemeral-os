...
Companion to
[`phase-02-attempt-orchestrator-lifecycle.md`](./phase-02-attempt-orchestrator-lifecycle.md).
...
single-`Attempt` execution behavior left skeletal by Phase 01.

---

## 1. Scope

Phase 02 moves one harness graph execution into `AttemptOrchestrator`:
...
1. `AttemptOrchestrator` implementation for planning, generating,
   evaluating, and graph close.
2. Process-local lookup/routing for active orchestrators by `Attempt.id`.
3. Public `AttemptOrchestrator.apply_*` methods called directly by
...
6. Task record helpers for planner, generator, and evaluator task
   lifecycle under the current `Attempt`.
...
8. Recursive complex-task request observation: when an outer generator task
   sits in `waiting_mission`, the orchestrator treats it as non-terminal
   during quiescence checks so the outer graph stays in `generating`. The
   transition into `waiting_mission` and the resume path
   (`apply_mission_close_report`) are owned by Phase 04.
9. `EpisodeManager` wiring so newly created graphs start their orchestrator
   when an orchestrator factory is configured.
10. `MissionHandler` wiring so every spawned segment manager receives
...
- Public tool hard gates for `submit_full_plan`, `submit_partial_plan`,
  `request_mission_solution`, resolver limits, and after-edit blocking
  (Phase 03).
- Generator `submit_request_plan` handling. Phase 02 must not add this legacy
  request-plan surface to `AttemptOrchestrator`; `request_mission_solution`
  is the canonical migration name, and Phase 03/04 should either reject the legacy
  stub or alias it before orchestration.
- Creation of the delegated `Mission`, the
  `apply_mission_close_report` resume entry on `AttemptOrchestrator`, and
  durable final report delivery to `requested_by_task_id` (all Phase 04).
  Phase 02 only ensures the orchestrator observes `waiting_mission` as
...
matching `AttemptOrchestrator.apply_*` method. The orchestrator owns graph
...
| --- | --- | --- | --- |
| `AttemptOrchestrator` owns exactly one graph execution | Phase 00, workflow overview, Phase 02 | Orchestrator never creates requests, segments, sibling graphs, or continuation segments | OK |
| Retry is segment-level | Phase 00/01/02 | Orchestrator closes the graph and calls `on_graph_closed`; `EpisodeManager` decides retry | OK |
| Planner success creates generator DAG | Phase 02/03 | Terminal tool handlers call `AttemptOrchestrator.apply_plan_submission` with a typed, already accepted plan; mutation then persists generator task ids | OK |
...
| Legacy request-plan is not a task outcome | Phase 00/02/04 | `submit_request_plan` never enters mutation handling; canonical delegated request start is `request_mission_solution` and is handled by request-boundary code | OK |
...
| Recursive mission is a new request, not a child segment | Phase 00/04 | Outer generator task enters `waiting_mission` via the Phase 04 spawn handler; Phase 02 keeps that status non-terminal in quiescence so the outer graph stays open until the Phase 04 resume entry fires | OK |
| `EpisodeManager` factory seam exists | Phase 01 report | Phase 02 wires the factory from handler -> manager -> graph start | OK |
...
1. The Phase 01 orchestrator constructor has mandatory arguments
   `(attempt, graph_store, on_graph_closed)`. Phase 02 should preserve
   that mandatory surface and add optional keyword-only runtime dependencies
   behind a small `AttemptRuntime` object. Existing Phase 01 tests stay
   valid because they do not call orchestrator behavior.
2. `TaskCenterTaskRecord.task_center_attempt_id` already exists and now
   semantically points at `attempts.id`. Phase 02 should reuse that column
...
```mermaid
flowchart TD
    Start["AttemptOrchestrator.start"] --> Planner["Create planner task"]
    Planner --> PlanTool["Planner submit_full_plan or submit_partial_plan"]
    PlanTool --> PlanMutation["AttemptOrchestrator.apply_plan_submission"]
    ...
    Planner -->|"planner exhausted"| PlannerFailMutation["AttemptOrchestrator.apply_planner_failure"]
    ...
    GenEvent --> GenMutation["AttemptOrchestrator.apply_generator_submission"]
    ...
    EvalEvent --> EvalMutation["AttemptOrchestrator.apply_evaluator_submission"]
...
sequenceDiagram
    ...
    participant Manager as EpisodeManager
    participant Orch as AttemptOrchestrator
    ...
    Manager->>Store: insert Attempt H1
...
`task_center_attempt_id`, fetch the registered orchestrator, and call its
...
- Terminal tools decide whether a tool call is allowed, normalize their public
  input into typed submission DTOs, and call `AttemptOrchestrator.apply_*`.
- `AttemptOrchestrator.apply_*` methods mutate graph-owned task and graph
  state.
- `AttemptOrchestrator._dispatch_ready_work()` decides which task should
  dispatch next or whether the graph should close.
- `EpisodeManager` still decides retry after the graph closes.

### 3e. Recursive Complex-Task Request Start

`request_mission_solution` is not a generator success or failure terminal.
...
```mermaid
flowchart TD
    C1["Outer Mission C1"] --> S1["C1 Episode S1"]
    S1 --> H1["C1.S1 Attempt H1"]
    H1 --> E7["Generator task E7"]
    E7 --> Req["request_mission_solution(goal)"]
    Req --> Wait["Mark E7 waiting_mission"]
    Req --> C2["Create delegated Mission C2"]
    C2 --> Link["C2.requested_by_task_id = E7"]
    C2 --> C2S1["C2 Episode S1"]
    C2S1 --> C2H1["C2.S1 Attempt H1"]
...
adds focused files under the existing `attempt/` lifecycle package.
...
|   |-- test_attempt_task_graph.py
|   |-- test_attempt_orchestrator_registry.py
...
- Phase 03 public terminal tools validate their own inputs and then call
  `AttemptOrchestrator.apply_*` with `PlannerSubmission`,
...
**`backend/src/task_center/attempt/runtime.py`** - new
...
from task_center.attempt.graph import Attempt
...
class AttemptAgentLauncher(Protocol):
    ...
    AttemptOrchestrator.apply_* with validated submission DTOs.
    """
...
class AttemptRuntime:
    ...
    def task_center_run_id_for_graph(self, graph: Attempt) -> str: ...
```
...
Attempt.episode_id
  -> Episode.mission_id
  -> Mission.task_center_run_id
```

The orchestrator stays in-process and ephemeral. `AttemptRuntime` is only
a dependency bundle, not durable state. `AttemptStore` remains a mandatory
constructor dependency on `AttemptOrchestrator`; do not also store it on
`AttemptRuntime`.
...
**`backend/src/task_center/attempt/task_graph.py`** - new
...
**`backend/src/task_center/attempt/orchestrator_registry.py`** - new
...
class AttemptOrchestratorRegistry:
    ...
    def __init__(self) -> None:
        self._by_graph_id: dict[str, AttemptOrchestrator] = {}
...
one `AttemptOrchestrator.apply_*` entry with a typed submission DTO.
...
- Resolve the active `AttemptOrchestrator` from the current task's
  `task_center_attempt_id`.
- Call the matching `AttemptOrchestrator.apply_*` method.
- End the agent run when the terminal call succeeds.

`AttemptOrchestrator` public entry responsibilities:
...
**`backend/src/task_center/attempt/orchestrator.py`** - edit
...
class AttemptOrchestrator:
    ...
    # `apply_mission_close_report(report)` ships in Phase 04 alongside the
    # `request_mission_solution` spawn handler.
...
adds `apply_mission_close_report(report)` for delegated-request resume.
...
def _on_graph_closed_with_deregister(self, graph_id: str) -> None:
    ...
    AttemptOrchestratorRegistry before notifying the segment."""
```
...
- Terminal tool handlers call `AttemptOrchestrator.apply_*` directly after
...
- Generator tasks are persisted with `task_center_attempt_id = graph.id`.
...
- The Phase 04 spawn handler — not the orchestrator — transitions the
  outer generator task to `waiting_mission` when
  `request_mission_solution` is accepted. The orchestrator only
  observes `waiting_mission` as a non-terminal generator status during
  quiescence checks. The matching resume entry
  (`apply_mission_close_report`) ships in Phase 04.
...
**`backend/src/task_center/attempt/validation.py`** - edit

Add:

```python
def assert_graph_stage(
    graph: Attempt, expected: AttemptStage
) -> None: ...


def assert_graph_not_closed(graph: Attempt) -> None: ...
...
def assert_task_belongs_to_graph(task: dict, graph: Attempt) -> None: ...


def assert_generator_task_for_submission(task: dict, graph: Attempt) -> None: ...


def assert_evaluator_task_for_submission(task: dict, graph: Attempt) -> None: ...
```

All raise `GraphInvariantViolation`.

### 5j. EpisodeManager wiring
...
OrchestratorFactory = Callable[
    [Attempt, GraphClosedCallback], "AttemptOrchestrator"
]
```
...
```python
def _start_orchestrator_if_configured(self, graph: Attempt) -> None:
...
self._segment_store.append_attempt_id(segment.id, graph.id)
...
### 5k. MissionHandler wiring

**`backend/src/task_center/mission/handler.py`** - edit
...
Store it and pass it to each `EpisodeManager` in `_spawn_segment_manager`:

```python
manager = EpisodeManager(
    episode_id=segment.id,
...
The passed factory is responsible for registry registration. `EpisodeManager`
...
task_store.upsert_task(
    ...
    summaries=[],
    needs=[],
    task_center_attempt_id=graph.id,
...
- Persist `graph_store.set_stage(graph.id, AttemptStage.GENERATING)`.
- Launch pending tasks whose `needs` are empty, marking each launched task
  `running`.

### 6c. Generator submission handling

`AttemptOrchestrator.apply_generator_submission` handles both executor
...
`request_mission_solution` is **not** a `GeneratorSubmission`. The
orchestrator must not accept it. The Phase 04 spawn handler owns this
flow:

1. Generator executor calls `request_mission_solution(goal)`.
2. Phase 04 spawn handler validates the call, creates the delegated
   `Mission` with `requested_by_task_id` set to the outer
...
The outer `AttemptOrchestrator` is not in the loop for steps 1-3. It
observes the outer task as `waiting_mission` on the next
...
Phase 02's only job here is to keep `waiting_mission` non-terminal in
...
(`apply_mission_close_report`) and its wiring through
`MissionHandler.deliver_close_report` ship in Phase 04. The
terminal tool gate (Phase 03), not the orchestrator, decides which generator
agent profiles may call `request_mission_solution`.
...
graph_store.set_stage(graph.id, AttemptStage.EVALUATING)
agent_launcher.launch(...)
```
...
| --- | --- | --- | --- | --- |
| Planner exhausted | `failed` | `planner_failed` | runtime synthesizes `PlannerFailureSubmission`; calls `AttemptOrchestrator.apply_planner_failure(submission)` -> `_close_graph(...)` | immediate |
...
`AttemptOrchestrator._close_graph(status, fail_reason)` is the single
...
5. Deregister from `AttemptOrchestratorRegistry`.
6. Call `on_graph_closed(graph_id)` so `EpisodeManager` reacts.

No other orchestrator method should call `graph_store.close(...)` directly.
The orchestrator must not call
`EpisodeManager.create_next_attempt` or inspect segment attempt
budget.
...
| Persistence | `TaskCenterStore.list_tasks_for_attempt` | EDIT | Graph-scoped task lookup |
...
| Runtime | `AttemptAgentLauncher` | NEW | Testable launch protocol |
| Runtime | `AttemptRuntime` | NEW | Dependency bundle for orchestrator behavior |
| Lifecycle | `AttemptOrchestratorRegistry` | NEW | Process-local graph -> orchestrator lookup |
| Lifecycle | `AttemptOrchestrator.start` | EDIT | Bootstrap planner |
| Lifecycle | `AttemptOrchestrator.apply_*` | EDIT | Direct submission mutation surface (`apply_plan_submission`, `apply_planner_failure`, `apply_generator_submission`, `apply_evaluator_submission`). `apply_planner_failure` is called by the runtime; the others by terminal tool handlers. `apply_mission_close_report` ships in Phase 04. |
| Lifecycle | `AttemptOrchestrator._dispatch_ready_work` | EDIT | Launchability, generator quiescence, evaluator spawn |
| Lifecycle | `AttemptOrchestrator._close_graph` | EDIT | Single close site that calls `graph_store.close(...)` and `on_graph_closed` |
| Lifecycle | `AttemptOrchestrator.apply_planner_failure` | NEW | Runtime-synthesized planner failure entry; marks planner task `failed` and delegates to `_close_graph(planner_failed)` |
| Lifecycle | `EpisodeManager._start_orchestrator_if_configured` | EDIT | Start graph orchestrator after graph creation |
| Lifecycle | `MissionHandler.__init__` | EDIT | Accept/pass orchestrator factory |
...
| `test_list_tasks_for_attempt_filters_by_graph_id` | Lookup uses `task_center_attempt_id` |
...
| `test_waiting_mission_prevents_generator_quiescence` | Outer graph stays in `generating` while a generator task is `waiting_mission` (Phase 04 owns the transition; Phase 02 only verifies the quiescence observation) |
...
1. Add `runtime.py` with `AttemptRuntime`,
   `HarnessAgentLaunch`, and `AttemptAgentLauncher`.
...
2. Implement `AttemptOrchestrator.start`.
3. Implement `AttemptOrchestrator.apply_plan_submission`.
4. Implement `AttemptOrchestrator.apply_planner_failure` (runtime
...
5. Implement generator submission handling, dependency scheduling, and
   quiescence close.
6. Implement `waiting_mission` observation in quiescence checks
...
1. Thread `orchestrator_factory` through `MissionHandler`.
2. Start orchestrators from `EpisodeManager._create_graph` when configured.
...
uv run pytest backend/tests/task_center/lifecycle/test_attempt_orchestrator.py -q
uv run pytest backend/tests/task_center/lifecycle/test_attempt_task_graph.py -q
...
| No retry path is implemented inside `AttemptOrchestrator` | `test_orchestrator_never_creates_retry_graph` |
| Retry remains delegated to `EpisodeManager` | `test_failed_graph_with_budget_starts_next_graph_orchestrator` |
| Orchestrator close reports to `EpisodeManager` via callback | `test_apply_evaluator_success_closes_graph_passed` |
...
| `waiting_mission` keeps the outer graph in `generating` and out of evaluator spawn | `test_waiting_mission_prevents_generator_quiescence` |
...
The `AttemptAgentLauncher.launch(...)` protocol is synchronous in Phase 02.
...
`MissionHandler`. Phase 02 should not remove it. The "final report
...
docs use the canonical `request_mission_solution` name. Phase 02 should
...
`waiting_mission` is a graph-local pause, not a generator terminal. If it
...
### 11i. `AttemptFailReason.PLANNER_FAILED`

Phase 02 uses `AttemptFailReason.PLANNER_FAILED` (literal value
...
- [Task Center Harness Migration - Phase Index](../task-center-mission-episode-attempt.md)
- [Complex Task Segmentation and Harness Graph Workflow](./mission-episode-attempt-workflow-overview.md)
- [Phase 00 - Target Architecture](./phase-00-target-architecture.md)
- [Phase 01 - Complex Task Request and Harness Graph Model](./phase-01-mission-episode-attempt-model.md)
...
- [Phase 02 - Harness Graph Orchestrator Lifecycle](./phase-02-attempt-orchestrator-lifecycle.md)
