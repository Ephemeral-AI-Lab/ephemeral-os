...
`Mission` / `Episode` / `Attempt` lifecycle model.
...
`MissionHandler`. Per-segment retry and segment close belong to
`EpisodeManager` (one per segment). Planner, generator, and evaluator
execution belong to `AttemptOrchestrator`.
...
- `AttemptedPlanEntry` (under
  ...
  `attempt_id`, `attempt_sequence_no`, `task_specification`,
  `evaluation_criteria`, `fail_reason`, and two Phase 06 slots:
  `attempt_summary_id: str | None` and
  ...
  pins the contract.
- `EpisodeManager._build_attempted_plan_history()` returns the ordered
  tuple of `AttemptedPlanEntry`s for every closed graph in the segment,
  preserving `attempt_sequence_no` order via
  `AttemptStore.list_for_episode`. Phase 06 only needs to fill the
...
- The new request / segment / graph stores expose every walk the context
  engine needs:
  `MissionStore.get` / `list_for_executor_task`,
  `EpisodeStore.list_for_request` / `get_by_sequence`,
  `AttemptStore.list_for_episode` / `get_by_sequence`.
- Closure routing is already in place — `EpisodeManager` reads from
  `AttemptStore.list_for_episode` to assemble history, so Phase 06
  can hook a `record_attempt_summary` call into the orchestrator's
  close path (Phase 02) and into segment close without rewriting history
  assembly.

**Phase 06 wires:**

- `ContextEngine.record_attempt_summary(attempt_id)` produces
  a `attempt_summary_id` and a `failure_landscape` payload per
  closed graph; those IDs back-fill `AttemptedPlanEntry` when the segment
  closes.
- `record_episode_summary(episode_id)` is called by
  `EpisodeManager` on segment close.
- `record_mission_summary(mission_id)` is called by
  `MissionHandler` on request close, before
...
| Mission request | `Mission.goal` | Created from `request_mission_solution(goal)` and becomes the direct request-level goal. |
| Task segment | `Episode.goal` | For segment 1 this starts from the request goal. For segment 2+ it equals the previous segment's `continuation_goal`. |
| Harness graph | `Attempt.task_specification`, `Attempt.evaluation_criteria`, and `Attempt.continuation_goal` | Emitted by the planner through `submit_full_plan` (continuation_goal null) or `submit_partial_plan(continuation_goal)`. Set per graph; not inherited by later graphs in the same segment. |
...
The `goal` passed to `request_mission_solution(goal)` is the source of
truth for the mission request and the initial segment. Later segment goals
...
> `Episode.task_specification` and `Episode.task_summary` are
...
> `EpisodeStore.close_succeeded`.

> **Amendment (US-017b — entry-graph carve-out).**
> An **entry segment may have zero `Attempt` rows.** The entry
> executor is launched in graph-less mode: its task row carries
> `task_center_attempt_id=None`, and the segment row's
> `attempt_ids` is empty. Lifecycle events for the entry task are
> received by `EntryTaskController` (peer to `composer` on
> `AttemptRuntime`) instead of a `AttemptOrchestrator`. The
...
> `final_attempt_id` field is widened to `str | None`; `None`
...
class ContextEngine:
    ...
    async def record_attempt_summary(
        self, attempt_id: str
    ) -> AttemptSummary: ...

    async def record_episode_summary(
        self, episode_id: str
    ) -> EpisodeSummary: ...

    async def record_mission_summary(
        self, mission_id: str
    ) -> ComplexTaskSummary: ...
```
...
- `prior_episode_specification` *(new in flexible-composition plan v4)* — per-prior-segment block carrying `Episode.task_specification`,
...
| Field | Meaning |
| ----- | ------- |
| `attempt_id` | Owning harness graph. |
...
`EpisodeManager` retries and launches a later graph. It should distinguish
...
| Field | Meaning |
| ----- | ------- |
| `episode_id` | Closed segment. |
| `mission_id` | Owning request. |
...
| `final_attempt_summary_id` | Summary for the harness graph that produced the final segment outcome: the passing graph for success, or the final attempted failed graph for `attempt_plan_failed`. |
...
Produced when a mission request closes.

Required fields:

| Field | Meaning |
| ----- | ------- |
| `mission_id` | Closed request. |
| `requested_by_task_id` | Executor task receiving the final report. |
| `goal` | Request goal from `request_mission_solution(goal)`. |
| `outcome` | Succeeded, failed, or cancelled. |
| `final_segment_id` | Segment that produced the final outcome. |
| `final_attempt_id` | Harness graph that produced the final outcome; `None` only when the final outcome came from the graph-less entry segment. |
...
For normal graph-mode complex-task requests, `final_attempt_id` names the
...
Planner context is built for one `Attempt`.

Required blocks:

- `mission_goal`: `Mission.goal`.
- `episode_goal`: current `Episode.goal`.
- `continuation_instruction`: present for segment 2+ and derived from the
  previous segment's `continuation_goal`.
- `failed_attempt_landscape`: present when this graph follows an earlier failed
  graph in the same segment and derived from those earlier failed harness graphs.
- `prior_episode_summary`: present for segment 2+ and derived from previous
  segments' closing summaries.

The planner may receive parent executor background, but it must be marked as
background. It must not override the mission request goal.

The planner of a graph launched after a `EpisodeManager` retry decision
...
The evaluator should judge the current harness graph, not the full mission
...
When a mission request closes, `MissionHandler` asks the context
engine for a close packet for `requested_by_task_id`.

Required blocks:

- `close_report`: succeeded, failed, or cancelled.
- `mission_goal`: the original request goal.
- `mission_summary`: final result and ordered segment digest.
...
Retry is horizontal inside one `Episode` and only follows a failed graph.
A planner launched after `EpisodeManager` retries receives the same request
...
- stores the final mission summary before reporting to the requesting
  executor task.
...
- inserting and loading mission summaries,
- listing harness graphs by `segment_id` and `attempt_sequence_no`,
...
8. Connect `AttemptOrchestrator` role launches to context packets.
...
- Complex-task close context returns final mission summary to
...
- Summaries exist at task, harness graph, segment, and mission levels.
