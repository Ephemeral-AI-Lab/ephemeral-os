...
| --- | --- | --- |
| `submit_request_plan` removed from executor terminals | Done | Negative asserts at `test_agent_markdown.py:36`, `test_submission_tool_registration.py:34`, `test_attempt_orchestrator.py:536` |
...
| Attempt-row child-graph retry | Replaced by `EpisodeManager` retry inside same segment | `test_retry_creates_graph_in_same_segment`, `test_failed_graph_with_budget_creates_next_graph` |
| `final_attempt_id` in `EpisodeClosureReport` | **Kept** (design doc explicitly preserves it as event payload) | n/a |
...
| --- | --- |
| `request_mission_solution` creates `Mission` | `test_request_mission_solution_starts_delegated_request`, `test_create_mission_links_executor` |
| Close report becomes the requesting executor task result | `test_router_delivers_success_to_waiting_parent`, `test_mission_close_report_success_resumes_waiting_generator` |
| `MissionHandler` is sole creator/closer for requests and `Episode` records | `test_handle_segment_closed_*`, `test_handler_passes_orchestrator_factory_to_spawned_manager` |
| `EpisodeManager` is sole `Attempt` creator inside its segment + sole `EpisodeClosureReport` emitter | `test_failed_graph_with_budget_creates_next_graph`, `test_passing_graph_*`, `test_smoke_*` |
| Request links to `requested_by_task_id` | `test_create_mission_links_executor` |
| Initial `Episode` creation | `test_initial_segment_has_sequence_one_and_initial_reason`, `test_initial_segment_creates_graph_sequence_1` |
| Initial `Attempt` creation | `test_initial_segment_creates_graph_sequence_1`, `test_creating_initial_graph_twice_raises` |
...
| Manager retry creates `Attempt` N+1 inside same segment | `test_retry_creates_graph_in_same_segment`, `test_delegated_retry_waits_until_final_graph` |
...
| Continuation creates `Episode` N+1 with `goal` inherited from passing graph's `continuation_goal` | `test_continuation_segment_inherits_continuation_goal`, `test_smoke_success_continue_then_terminal`, `test_delegated_continuation_waits_until_final_segment` |
...
| `request_mission_solution` from generator inside in-flight graph | `test_request_mission_solution_starts_delegated_request`, `test_handoff_creates_request_segment_graph_and_marks_parent_waiting` |
...
- `test_mission_has_no_retry_budget_field` — assert
  `Mission` has no `retry_budget` / `attempt_budget` / `max_retries`
...
`docs/architecture/task-center-mission-episode-attempt/phase-05-implementation-report.md`
...
- **Q1 (retry budget):** Lives on `Episode`, not `Mission`.
  `Mission` does not retry. Fixed runtime default
...
- **Q2 (planner exhaustion):** Runtime dispatches `PlannerFailureSubmission`
  ...
  `AttemptFailReason.PLANNER_FAILED`. Already implemented + tested.
...
| Public executor contract = `request_mission_solution` + `submit_execution_{success,failure}` | §1b row "request_mission_solution from generator…" + §2a regression |
