...
| --- | --- | --- |
| Major | `request_mission_solution` implements delegated request creation and parent resume behavior even though the Phase 03 plan explicitly defers that body to Phase 04 | Phase boundary is no longer coherent, and a failure during delegated graph startup can leave the parent generator task in `waiting_mission` after the tool returns an inline error |
...
| `backend/src/task_center/attempt/runtime.py` | edited | Exposes `graph_store`; also adds Phase-04-adjacent manager registry and lifecycle config dependencies |
| `backend/src/task_center/attempt/orchestrator.py` | edited | Adds `apply_mission_close_report`, which is Phase-04-adjacent resume behavior |
...
| `backend/src/tools/submission/hooks/request_mission_before_edit_gate.py` | new | Blocks executor request start after edit-capable tool use |
...
| `backend/src/tools/submission/notification_triggers/request_mission_after_edit.py` | new | Executor reminder after edits disable request start |
...
| Generator executor | implemented | `submit_execution_success`, `submit_execution_failure`, `request_mission_solution` |
...
- `uv run pytest backend/tests/task_center/lifecycle/test_attempt_orchestrator.py backend/tests/task_center/lifecycle/test_integration_phase02.py -q`: **18 passed**
...
| Executor public contract uses `request_mission_solution` | Agent markdown and registration tests |
...
- No failure-path test proves `request_mission_solution` leaves parent generator state unchanged when delegated graph startup fails.
...
Harness agent run
  |
  v
QueryContext.task_center_task_id + ExecutionMetadata.attempt_runtime
  |
  v
Tool prehooks
  - HarnessRoleGate
...
request_mission_solution
  |
  v
create delegated Mission
  |
  v
create initial Episode and EpisodeManager
  |
  v
mark parent generator task waiting_mission
  |
  v
create initial delegated Attempt and start its orchestrator
  |
  v
mission close report -> parent orchestrator.apply_mission_close_report
```
...
- `submit_partial_plan` is prehook-blocked when the current request was spawned,
  ...
  `continuation_goal`.
- `request_mission_solution` is prehook-blocked after edit-capable tool
...
`request_mission_solution` now creates delegated complex-task requests,
...
- `backend/src/tools/submission/main_agent/generator/request_mission_solution.py:110`
  starts the built-in delegated request path when no injected handler is present.
- `backend/src/tools/submission/main_agent/generator/request_mission_solution.py:134`
  marks the parent task `waiting_mission` before delegated graph startup.
- `backend/src/task_center/attempt/orchestrator.py:199` adds parent
  resume behavior through `apply_mission_close_report`.

This is more than scope drift. In the current handler, the parent generator task
is marked `waiting_mission` before the delegated graph is created. If delegated
...
| Item | Where | Phase |
| --- | --- | --- |
| Durable restart recovery for active orchestrator lookup | `AttemptOrchestratorRegistry` / cutover runtime | 05 |
| Production launch metadata wiring beyond direct `run_ephemeral_agent` stamping | real `AttemptAgentLauncher` implementation | 05 |
...
| Full delegated complex-task spawning and request-start review, if the current early implementation remains | `request_mission_solution`, `apply_mission_close_report` | 04 |
