...
| File | Status | Purpose |
| --- | --- | --- |
| `backend/src/task_center/mission/handoff.py` | new | `ComplexTaskHandoffCoordinator` — orchestrates executor → delegated request handoff with parent-task CAS, deferred orchestrator startup, and compensation rollback |
| `backend/src/task_center/mission/close_report_delivery.py` | new | `MissionCloseReportRouter` — single synchronous delivery path to the active parent orchestrator |
...
| File | Status | Purpose |
| --- | --- | --- |
| `backend/src/task_center/segment/manager.py` | edited | `AttemptStartHandle` deferred-start seam; `create_initial_attempt` now returns the handle so the coordinator can write parent waiting state between graph creation and orchestrator startup; startup failures close the just-created graph as `startup_failed` |
| `backend/src/task_center/mission/handler.py` | edited | Continuation startup is now mandatory: `handle_segment_closed`'s `SuccessContinue` branch creates and starts the next segment's initial graph, and falls back to a tested failure-close path if startup raises |
| `backend/src/task_center/attempt/orchestrator.py` | edited | `apply_mission_close_report` is now CAS-idempotent; `start()` owns registry registration and startup rollback |
| `backend/src/task_center/attempt/factory.py` | edited | Factory now constructs orchestrators only; registration happens inside `AttemptOrchestrator.start()` so failed startup can deregister atomically |
| `backend/src/task_center/attempt/graph.py` | edited | Added `startup_failed` fail reason for graphs that fail before a planner launch completes |
| `backend/src/db/stores/task_center_store.py` | edited | Added `set_task_status_if_current` plus bulk `list_tasks_for_attempts` for parent-task CAS and graph read-model assembly |
| `backend/src/db/stores/mission_store.py` | edited | Added `list_for_run` and the package-private `_cancel_for_compensation` |
| `backend/src/db/stores/episode_store.py` | edited | Added package-private `_cancel_for_compensation` and bulk `list_for_requests` |
| `backend/src/db/stores/attempt_store.py` | edited | Added bulk `list_for_episodes` |
| `backend/src/tools/submission/main_agent/generator/request_mission_solution.py` | edited | Refactored to thin tool: validate input, resolve submission context, delegate to `ComplexTaskHandoffCoordinator.start`. No handler/factory/store imports remain |
...
| `backend/src/server/app_factory.py` | edited | Initialize and pass `MissionStore`, `EpisodeStore`, `AttemptStore` to `create_persistence_router` |
| `backend/src/server/routers/persistence.py` | edited | `/api/db/task-center-runs/{id}/graph` delegates to the TaskCenter graph read model, attaches `tasks` per graph, and emits an id-only `attempts_index` |

### New tests

| File | Lines | Coverage |
| --- | ---: | --- |
| `backend/tests/task_center/lifecycle/test_phase04_mission_handoff.py` | 311 | Coordinator happy path, startup-failure rollback (parent → running, request and segment → cancelled), started-orchestrator cleanup, duplicate-open-request rejection, non-running parent rejection |
...
| `backend/tests/task_center/lifecycle/test_episode_manager.py` | edited | Adapted to the new `create_initial_attempt()` → `AttemptStartHandle` API |
...
| `backend/tests/task_center/lifecycle/test_mission_handler.py` | edited | Same |
...
- `uv run pytest backend/tests/task_center/lifecycle/test_phase04_mission_handoff.py backend/tests/task_center/lifecycle/test_phase04_close_report_delivery.py backend/tests/task_center/lifecycle/test_phase04_continuation_retry.py -q` — clean
...
| --- | --- |
| `request_mission_solution` creates a delegated request, initial segment, and initial graph through the proper owners | `test_handoff_creates_request_segment_graph_and_marks_parent_waiting`, `test_request_mission_solution_starts_delegated_request` |
| Parent generator only enters `waiting_mission` when delegated startup succeeds (or with a tested rollback) | `test_handoff_startup_failure_leaves_parent_running`, `test_handoff_startup_failure_closes_started_graph_and_deregisters_orchestrator` |
| Delegated success becomes parent's final success | `test_router_delivers_success_to_waiting_parent`, `test_request_mission_solution_return_updates_outer_generator` |
...
| Continuation creates ordered later `Episode` records and does not resume the parent until close | `test_delegated_continuation_waits_until_final_segment` |
| Retry creates later `Attempt` records inside the same segment and does not resume parent until close | `test_delegated_retry_waits_until_final_graph` |
...
| Executor/verifier profile gate enforced on the right tools | `test_executor_profile_required_for_mission`, `test_executor_profile_required_for_execution_terminals`, `test_verifier_profile_required_for_verification_terminals` |
...
ComplexTaskHandoffCoordinator.start
  ...
  - Reject if any open Mission exists for the parent
  - Build MissionHandler (with router as deliver_close_report)
  - create_mission → create_initial_segment
  - manager.create_initial_attempt() → StartHandle (no orchestrator yet)
  - CAS parent: running → waiting_mission (records waiting summary)
...
Delegated graph runs through planning → generating → evaluating → close
  |
  v
EpisodeManager.handle_attempt_closed
  ...
  |
  v
MissionHandler.handle_segment_closed
  - terminal_success / attempt_plan_failed → close_mission
  - success_continue(goal) → create_continuation_segment + start its initial
    graph (with handler-level failure-close fallback)
  |
  v
MissionHandler.close_mission
  - persists final_outcome
  - delivers MissionCloseReport via MissionCloseReportRouter
  |
  v
MissionCloseReportRouter.deliver
  - Already DONE/FAILED parent → already_delivered (idempotent)
  - WAITING parent + active orchestrator → orchestrator.apply_mission_close_report
...
- The single creator of `Mission` and `Episode` records is
  `MissionHandler`. The single creator of `Attempt` records
  inside a segment is that segment's `EpisodeManager`.
...
- Final close-report persistence goes through `MissionCloseReport`; the
  handler no longer open-codes the JSON field names separately.
- `request_mission_solution` rejects whenever the parent already has an
  open delegated `Mission`.
...
- `AttemptStartHandle.start()` and `cancel()` are one-shot and
  mutually exclusive: a second call to either raises.
- `AttemptOrchestrator.start()` owns active-orchestrator registration and
...
- `apply_mission_close_report` is idempotent on the parent task: a
  second delivery against an already-resumed parent returns silently.
- `request_mission_solution` carries hard gates for: structural
...
### 7a. Import-isolation in `request_mission_solution.py`
...
```text
from task_center.mission.handoff import (
...
`waiting_mission`. This means even a duplicate delivery into a graph that
...
`MissionHandler._start_continuation_segment` no-ops when the
...
### 7e. `attempts` legacy alias dropped

Per the plan's note that the snapshot has no frontend client, the placeholder
`{"attempts": []}` payload is gone. The route returns
`missions` (nested) plus `attempts_index` (id-only flat
lookup) only.
...
| Rich helper-agent context packets, evidence summaries, failure landscapes, `attempt_summary_id` | Phase 06 | 06 |
...
- `make_attempt_orchestrator_factory` registers the orchestrator with
...
- Close-report delivery is intentionally synchronous and process-local in this
  ...
  `AttemptOrchestrator`; restart-safe replay from persisted request,
