...
| File | Lines | Coverage |
| --- | ---: | --- |
| `backend/tests/task_center/lifecycle/test_phase05_no_legacy_artifacts.py` | 60 | Static source regression checks: no `submit_request_plan`, `RETRY_ON_FAILURE`, `retry_after_partial`, or standalone `ROOT` token in `backend/src`; `Mission` has no request-level retry budget field |
...
| File | Status | Purpose |
| --- | --- | --- |
| `docs/architecture/task-center-mission-episode-attempt/phase-05-implementation-plan.md` | existing companion plan | Records the Phase 05 coverage map, remaining work, and definition of done |
| `docs/architecture/task-center-mission-episode-attempt/phase-05-workflows-and-cutover.md` | existing design doc | Records resolved retry-budget and planner-exhaustion questions |
| `docs/architecture/task-center-mission-episode-attempt/phase-05-implementation-report.md` | new | This implementation report |
...
| `EpisodeManager` is sole `Attempt` creator inside its segment and sole `EpisodeClosureReport` emitter | `test_failed_graph_with_budget_creates_next_graph`, `test_passing_graph_*`, `test_smoke_*` |
...
| Continuation creates `Episode` N+1 with inherited continuation goal | `test_continuation_segment_inherits_continuation_goal`, `test_smoke_success_continue_then_terminal`, `test_delegated_continuation_waits_until_final_segment` |
...
| `Mission` has no request-level retry budget | `test_mission_has_no_retry_budget_field` |
...
- Phase 06 context-engine work: rich helper-agent context, evidence summaries,
  and `attempt_summary_id` population.
