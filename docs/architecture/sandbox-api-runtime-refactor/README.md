...
| 3 | 2 | [`raw_exec` primitive](./step-03-slice-2-raw-exec.md) | Public `sandbox.api.raw_exec` over the adapter; importer-allowlist test. |
| 4 | 3 | [Runtime scaffolding](./step-04-slice-3-runtime-scaffolding.md) | `daemon/` → `runtime/`; add `bundle.py`, `setup_orchestrator.py`, `entrypoint.py`. |
| 5 | 4 | [OCC peer relocation](./step-05-slice-4-occ-relocation.md) | `mutations/` → `sandbox/occ/`; add `edit_pipeline`, `write_pipeline`. |
| 6 | 5b | [Overlay peer relocation](./step-06-slice-5b-overlay-relocation.md) | `overlay/` → `sandbox/overlay/`; add `shell_pipeline`. |
