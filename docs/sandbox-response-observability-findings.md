...
Scope: read-only scan of `eos-sandbox/crates` for response-visible logging, tracing, audit, timing, CPU, disk, memory, mount, LayerStack, and overlay status surfaces.
...
All daemon responses that contain a `timings` object are finalized through `eos-sandbox/crates/daemon/src/dispatch/dispatcher.rs`.
...
Source: `daemon/src/dispatch/dispatcher.rs`.

## Command Responses

Command session elapsed time originates in `eos-sandbox/crates/command-session/src/session.rs` as `ReapedCommand.elapsed_s`. `eos-sandbox/crates/operation/src/command/service.rs` carries it into command settlement, and `eos-sandbox/crates/operation/src/command/settle.rs` renders the final response.
...
| `timings.sandbox.command.exec.dispatch_total_s` | Same elapsed value for dispatch compatibility. |
| `timings.sandbox.command.exec.total_s` | Included for isolated command settlement. |
...
`namespace/src/runner/fresh_ns.rs` records runner-side `workspace.mount_s` and `workspace.tool_s`. Plugin overlay response shaping aliases those to `command_exec.mount_workspace_s` and `command_exec.run_command_s`.

## Resource Timings

Resource gauges and counters are response-visible through `timings`, mainly from `operation/src/command/settle.rs` and the daemon helper `daemon/src/runtime/response.rs`.
...
Direct file APIs are enriched at the daemon boundary in `daemon/src/op_adapter/files.rs`, so `sandbox.file.read`, `sandbox.file.write`, and `sandbox.file.edit` can expose the same resource gauges for direct LayerStack routes. Isolated file APIs are intentionally thinner because publication is deferred and private.

## LayerStack And OCC Status

OCC commit timing is produced by `eos-sandbox/crates/layerstack/src/commit/worker.rs` and is copied into command, file, plugin overlay, and callback responses.
...
`sandbox.checkpoint.layer_metrics` is implemented in `daemon/src/op_adapter/checkpoint.rs` and returns top-level metrics, not `timings` keys:
...
| `timings.sandbox.checkpoint.commit_to_git.overlay_mount_s` | Overlay mount time when overlay mode succeeds. |
| `timings.sandbox.checkpoint.commit_to_git.project_worktree_s` | Projection time when fallback mode is used. |
| `timings.sandbox.checkpoint.commit_to_git.git_add_s` | Git add duration. |
| `timings.sandbox.checkpoint.commit_to_git.git_diff_cached_s` | Cached diff check duration. |
| `timings.sandbox.checkpoint.commit_to_git.git_commit_s` | Git commit duration when a commit is created. |
| `timings.sandbox.checkpoint.commit_to_git.total_s` | Total commit pipeline duration. |
...
Isolated workspace responses are shaped in `daemon/src/op_adapter/isolation.rs`; lifecycle work is in `workspace/src/isolated_workspace/manager/lifecycle.rs`.
...
| PTY transcript | `command-session/src/process.rs` writes timestamp-prefixed command output to a transcript file. |
| Final command response | `command-session/src/session.rs` writes pretty JSON final response for crash recovery. |
| CLI stdout/stderr | `eosd`, `gateway`, and e2e helpers use limited `println!` / `eprintln!` for version, usage, and process messages. |
...
6. Keep overlay health represented through `worktree_mode`, `workspace.mount_s`, `sandbox.checkpoint.commit_to_git.overlay_mount_s`, manifest depth, storage bytes, and auto-squash telemetry rather than reintroducing a separate pre-mount overlay depth guard.
...
| Area | Test file |
|---|---|
| Command timing and changed paths | `eos-sandbox/crates/e2e-test/tests/ephemeral_workspace/test_ephemeral_workspace_overlay_exec.rs` |
| Cgroup/process memory gauges | `eos-sandbox/crates/e2e-test/tests/pressure/test_pressure_resource_report.rs` |
| Commit-to-git overlay mode and timing | `eos-sandbox/crates/e2e-test/tests/layerstack/test_eos_layerstack_git_overlay_commit.rs` |
| Isolated no-publish and teardown audit | `eos-sandbox/crates/e2e-test/tests/workspace-runtime-isolated/readme.md` |
| LayerStack depth/storage metrics | `eos-sandbox/crates/e2e-test/tests/layerstack/*` and `eos-sandbox/crates/e2e-test/tests/pressure/*` |
