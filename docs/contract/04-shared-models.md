...
> **Historical contract:** this document is retained for migration evidence from
> the Python runtime. The live Rust operation catalog and DTO checks now live in
> `crates/protocol`, `crates/operation`, and `crates/operation/ops.json`.
...
1. **Request dataclass** (`backend/src/eos-sandbox/shared/models.py`) — typed input the
...
2. **Wire request `args`** — what actually crosses to the daemon, built field-by-field by
   the `api/tool/<verb>.py` wrappers (`backend/src/eos-sandbox/api/tool/`). The daemon-side
   primitives (`backend/src/eos-sandbox/shared/tool_primitives/<verb>.py`) read fields out of
...
> Verb routing stays in `daemon` dispatch: `read`/`write`/`edit` fast-path → `eos-occ`;
...
| `shell` | **ephemeral overlay pipeline** | `daemon` + `eos-runner` + `overlay` |
| `glob` | **ephemeral overlay pipeline** | `daemon` + `eos-runner` + `overlay` |
| `grep` | **ephemeral overlay pipeline** | `daemon` + `eos-runner` + `overlay` |
...
is `apply_search_replace` in `backend/src/eos-sandbox/shared/edit_apply.py` (§5), shared by
...
`backend/src/eos-sandbox/shared/edit_apply.py:21-48`. Single source of truth for
