...
2. Watcher: finalize → `complete(id,status,exit)` BEFORE `promise.resolve` → `on_terminal`. resolved ⟹ terminal entry exists; yield drops `wait_for_completed_record`. `complete` never touches `V` (race-free vs late `attach`).
3. Running read = file `transcript_window`+`Running`; terminal read = `resolved()` (non-consuming)+file; `is_finished()` decides.
...
```
cargo fmt --check && cargo test -p sandbox-runtime-namespace-execution && cargo test -p sandbox-runtime-command && cargo test -p sandbox-runtime --tests && cargo check -p sandbox-daemon
cargo clippy -p sandbox-runtime-namespace-execution -p sandbox-runtime-command -p sandbox-runtime -p sandbox-daemon --all-targets --no-deps -- -D warnings
rg -n "CommandProcessStore|CommandLaunchDriver" crates/sandbox-runtime/operation/src/command || echo "gone ✓"; git diff --check
```
Done when every spec §16 DoD + §14 matrix holds. Report LOC via `git diff --numstat`.
