...
- [ ] Session `file_read` requests looping concurrently with an in-session shell
      ...
      namespace.
- [ ] `destroy_workspace_session` racing an `exec_command` still running in that
      same session.
...
- [ ] Blame key resolution and unaudited paths: sessionless `file_write`
...
- [ ] Same-owner coalescing: sessionless `file_write` seeds a 6-line file
...
- [ ] Line-shift on insert and delete: seed a 6-line file via `file_write`
      ...
      untouched line.
- [ ] `replace_all` multi-site attribution: seed a 9-line file (request A)
...
- [ ] [complex] 50-round insert/delete ladder from many actors: seed a 10-line
...
- [ ] [complex] Owner turnover across 30 wholesale generations: issue 30
...
- [ ] Live-session changes are invisible to blame: after a sessionless
...
- [ ] Capture insertion shifts without reassigning: sessionless `file_write`
      ...
      positions.
- [ ] Capture deletion mints no ownership: seed a 4-line file via `file_write`
...
- [ ] [complex] Deep prepend history across 20 captures: seed a 5-line file
