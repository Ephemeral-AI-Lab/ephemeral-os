...
| `API.md` | The public op reference. **Generated** from `../crates/operation/ops.json` via `cargo run -p xtask -- gen-docs`; `check-contract` fails when stale. |
...
`../crates/operation/ops.json` plus `../contract/` (`PROTOCOL.md` +
...
`contract/`, or `sandbox-architecture-7-to-9_SPEC.md` instead.
