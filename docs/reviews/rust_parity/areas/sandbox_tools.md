# Rust Parity Review — Sandbox tools (command_exec, write_stdin, write, edit, multi-edit)

Area key: `sandbox_tools` (domain: sandbox)
Report scope: the `tool_api` public API facade, agent-facing tool wrappers, command/session
substrate, and the terminal-pair allocator.

## Ground truth

Python ground truth (authoritative for dynamics/constants/ordering):
- API facade verbs: `backend/src/sandbox/api/tool/{read,write,edit}.py`,
...
- Agent-facing tool wrappers + registry: `backend/src/tools/sandbox/_lib/registry.py`,
  `backend/src/tools/sandbox/{edit_file,multi_edit,write_file,read_file,exec_command,write_stdin}/`.
...
binary (`sandbox/crates/eos-daemon`, `eos-runner`). Therefore the in-sandbox find/replace OCC and
command-session execution have NO live Python counterpart; their ground truth is the **wire
contract** (the daemon-response envelope the Python `sandbox.api.tool` facade parses) plus
`backend/src/sandbox/shared/edit_apply.py` / `shared/models.py` (referenced by PORT comments).
Raw glob/grep file-walk is no longer a Rust contract at any layer. This is an intentional change,
not a missing dynamic.

## Rust mapping

| Concern | Rust anchor |
| --- | --- |
| API facade verbs | `agent-core/crates/eos-sandbox-api/src/tool_api/{read,write,edit,command,control}.rs` |
...
| Agent-facing tool wrappers + registration | `agent-core/crates/eos-tools/src/tools/sandbox`; names in `agent-core/crates/eos-tools/src/core/name.rs` |
...
|---|---|---|---|---|---|---|
| 1 | Tool set named exactly for current Rust: read_file, write_file, edit_file, multi_edit, exec_command, write_stdin | accepted scope change | none | Legacy Python also exposed `glob`/`grep` | `agent-core/crates/eos-tools/src/core/name.rs`; `.eos-agents/tools`; `eos-protocol/src/ops.rs`; `eos-daemon/src/dispatcher.rs`; `eos-runner/src/request.rs` | Rust intentionally removed `glob`/`grep` from the model-facing registry and raw daemon/protocol/runner surface |
...
| 5 | grep + glob semantics (patterns, output shape) preserved | closed by full removal | none | Legacy Python agent wrappers existed | `agent-core/crates/eos-tools/src/core/name.rs`; `.eos-agents/tools`; `eos-protocol/src/ops.rs`; `eos-daemon/src/dispatcher.rs`; `eos-runner/src/request.rs` | Not a Rust parity contract after removal from all Rust tool/protocol/daemon/runner layers |
...
| C1 | Per-verb timeout constants + dispatch grace arithmetic | match | none | `api/timeouts.py:5-22` | `timeouts.rs:13-29` | READ=WRITE=60, EDIT=20, default_cmd=60, grace=30, dispatch=cmd+grace; GLOB/GREP timeouts deleted with the raw search surface |
...
| C6 | DEFAULT_GLOB_LIMIT / MAX_FILE_BYTES constants | closed by full removal | none | (Python primitive deleted in 37c13f3db) | `eos-protocol/src/models.rs` | `DEFAULT_GLOB_LIMIT` was deleted with the raw search DTOs; `MAX_FILE_BYTES` remains as the write/edit cap |
...
### D3 (CLOSED BY FULL REMOVAL) — namespace-runner grep line-number prefix no longer applies

Evidence:
- `sandbox/crates/eos-runner/src/tool_primitives.rs` was deleted.
...
Why it matters: it no longer matters to Rust parity because `grep` is removed from the Rust tool
registry, API facade, protocol DTOs, daemon op table, and runner verb set.

### D4 (CLOSED BY FULL REMOVAL) — raw glob grammar no longer applies
...
Why it matters: it no longer matters to Rust parity because `glob` is removed from the Rust tool
registry, API facade, protocol DTOs, daemon op table, and runner verb set.
