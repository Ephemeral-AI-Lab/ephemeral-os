...
**Status:** Slice 1 ✅ landed & verified · Slice 2 ✅ implemented & verified (Linux-target compile/clippy-clean — after a post-review fix to two stale `CommandSession` test fixtures that had broken the `aarch64-unknown-linux-musl` test gate; macOS host tests pass) · Slice 3 ✅ landed (`terminate` flag, `\x03`→cancel de-escalation, expanded descriptions, "stderr always empty" doc, `default_tool_specs.snap` re-pinned).
...
> - **Slice 2 — Daemon sense-2 (`eos-daemon/src/command.rs`): IMPLEMENTED.** `Child`-in-session, one idempotent `try_finalize(publish)` (the two former detached finalizer threads deleted), the unified `wait_for_yield` (quiet-after-output early return) shared by `exec_command`/`write_stdin`, the `CommandSessionReaper` (timeout backstop + unpolled finalize) wired in `server.rs`, conservative `recover_orphaned_command_sessions` startup scan, the UTF-8 carry-over decode fix (pure helper, unit-tested on every host), the `CommandWorkspaceKind` enum, and the `CommandWorkspace → EphemeralCommandWorkspace` rename. **Verification:** compiles + `clippy`-clean on `aarch64-unknown-linux-musl` (the real daemon target) and the macOS host (non-Linux stubs); the pure carry-over helper is runtime-tested. The syscall-bound paths (PTY child reaping, `killpg`, lease release, OCC publish) are **compile-verified only** — they need a Linux runtime to exercise. The OCC/lease/capture finalize **bodies are byte-identical** to the prior passing daemon; only the orchestration around them changed. Orphan recovery deliberately does **not** `killpg` (pgids are not persisted; a restarted daemon could signal a reused PID). **Post-review fixes (this pass):** (1) the two stale `#[cfg(target_os="linux")]` `CommandSession` test fixtures were updated for the four new fields (`child`/`workspace`/`finalized`/`timeout_deadline`) via a single `test_command_session()` helper — they had silently broken the Linux test/clippy gate while macOS gated them out; (2) the UTF-8 carry-over helper (renamed `utf8_consumable_prefix_len`) now branches on `Utf8Error::error_len()` so a genuinely **invalid** byte is consumed lossily (U+FFFD) instead of wedging the carry buffer and withholding all model output until EOF — only an *incomplete trailing* multibyte is carried (added an invalid-byte unit test). **Note on §2.5:** `reader_done` is **retained** (not deleted as the struct-diff shows) — it is the output-reader drain handshake (`completed_session_stdout` waits on it before finalizing), orthogonal to the deleted finalizer.
> - **Slice 3 — Tool polish: COMPLETE.** Both the daemon-side `terminate: bool` teardown channel (D7) in `write_stdin` **and** the full agent-core tool-side have landed: `WriteStdinInput.terminate` plumbed end-to-end, the `\x03`→cancel escalation dropped (a `\x03` rides through as ordinary stdin and the daemon raises SIGINT; teardown is the explicit `terminate` flag), expanded `exec_command`/`write_stdin` descriptions, the "stderr always empty (PTY merges stderr into stdout)" doc, and `default_tool_specs.snap` re-pinned (byte-matches the current specs). A regression test (`write_stdin_ctrl_c_does_not_escalate_to_cancel`) guards the de-escalation. (The "deferred" note below was written before this landed and is retained only as history.)
>
