...
| LOW | sandbox | sandbox_tools | `api.exec_command.dispatch_total_s` timing dropped; grep/glob primitive semantics unpinned | `command.py:60-61` | no `dispatch_total` in agent-core; `tool_primitives.rs:233-248` | **FIXED:** command finalization emits `api.exec_command.dispatch_total_s` and `command_exec.total_s`; grep/glob request fields are pinned in the architecture page and schema snapshots. | Done. |
...
| sandbox_tools | sandbox | 9 match / HIGH `write_stdin` disparity CLOSED / 1 unproven | **unproven:** C6 glob/file-byte constants; grep/glob semantics divergence-without-anchor |
...
| ☑ | sandbox / [sandbox_tools](areas/sandbox_tools.html) | LOW | **Done.** `exec_command` finalization now emits `api.exec_command.dispatch_total_s` alongside `command_exec.total_s`; the grep/glob wire contract is pinned in `docs/architecture/tools/sandbox.html` and the `eos-sandbox-api` request schema snapshots. | sandbox_tools N1 · `command.rs`, `schema_snapshot.rs`, `tools/sandbox.html` |
