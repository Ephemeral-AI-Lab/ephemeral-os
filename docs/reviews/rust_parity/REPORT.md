...
- **Headline real bugs / missed dynamics (most likely to bite, ranked):**
  ...
  3. **Background command sessions now have the per-request heartbeat and parent-exit
     cancellation path**; the former `write_stdin` daemon op-name / retry-set
     mismatch is closed by the real-`eosd` empty-response proof below.
  ...
  7. **`write_stdin` wire/retry mismatch and live replay proof are closed** — the
     client uses `api.v1.write_stdin`, the host fail-closed retry set includes
     that stdin-write op, and `write_stdin_empty_response_from_real_eosd_is_not_replayed`
     proves the double-apply case against a real `eosd`: the daemon applies stdin
     once, the thin-client response is forced to exit 98, and the host performs no
     daemon spawn or replay (sandbox_tools D1/D2 = daemon_protocol D1).
...
| HIGH | sandbox | sandbox_tools / daemon_protocol | `write_stdin` source contract and retry-gating | `transport.py:17`; daemon `dispatcher.rs:139` | **FIXED:** `ops.rs` maps `DaemonOp::ExecStdin` to `api.v1.write_stdin`; daemon registers `api.v1.write_stdin`; live `write_stdin_empty_response_from_real_eosd_is_not_replayed` proves the real-`eosd` double-apply case. | CLOSED for source contract and live replay proof. | Done. |
| HIGH | sandbox | sandbox_tools / daemon_protocol | Host empty-response fail-closed set for stdin writes | `host/daemon_client.py:619-624` | **FIXED:** `daemon_client.rs` fail-closes the canonical `api.v1.write_stdin` op; the live proof forces an empty response after the real daemon applies stdin and observes no spawn/replay. | CLOSED for retry set and real-daemon replay behavior. | Done. |
...
| HIGH | sandbox | plugins | Rust plugin facade landed; native runtime payload/ensure path still missing | `host_dispatch.py:77-225`; `install.py:155-468` | **SOURCE-FIXED:** the Rust facade now points at a host-staged, sandbox-Python PPC service wrapper under `/eos/daemon/plugins/catalog/lsp/runtime/ppc_service.sh`; host lifecycle uploads the trusted bridge/runtime files alongside `eosd`; the wrapper runs `python3 -m plugins.runtime_bridge.ppc_service` from `/eos/daemon` and uses only image-provided Pyright or the existing `/eos/plugin-packages/lsp/{node.tar.xz,pyright.tgz}` artifact contract. No Python `call_plugin`/`install.py`, no untrusted `setup.sh`, no `PYTHONPATH`. | Source gap closed by Rust tests; existing live benchmark still needs to be run through the repo's normal Docker Python environment with `EOS_LIVE_E2E_IMAGE=sweevo-dask__dask-10042`. | Keep the Rust-native staged runtime path; live gate should use the existing benchmark/env contract, not a new setup path. |
...
| MEDIUM | sandbox | plugins | Manifest identifier validator diverges from `_PLUGIN_NAME_RE` (accepts `my-plugin`, rejects op-names Python accepts) | `op_registry.py:78,109` | **FIXED:** manifest `plugin_id` uses the faithful Python plugin-name rule; `op_name` is only required to be non-empty at the manifest boundary. | CLOSED by `eos-plugin` manifest/service tests. | Done. |
| MEDIUM | sandbox | plugins | Auto-overlay WRITE trigger has no producer; Python auto-selected the overlay write path by intent | `op_registry.py:230-235` | **FIXED as fail-fast validation:** `WRITE_ALLOWED + auto_workspace_overlay=true` is rejected unless its service is `oneshot_overlay`; LSP write ops stay self-managed with `auto_workspace_overlay=false`. | CLOSED for silent-misconfiguration risk; no default overlay fallback added. | Done. |
| MEDIUM | sandbox | perf | `acquire_snapshot` took the exclusive storage-writer lock; Python used a lighter process-local lock | `stack.py:108-135` (`self._lock` RLock only) | **SOURCE-FIXED:** `StorageWriterLockLease` now has `shared()` for snapshots and reentrant `exclusive()` for writers; `LayerStack::acquire_snapshot` uses `shared()`. Phase 3 bench now seeds a base-only fixture and folds `space.gate_pass` into the live gate. | Source-level contention gap closed by storage-lock tests; live Phase 3/3T throughput re-baseline and first space-gate artifact still pending. | Re-run the N-concurrent live throughput matrix; the same run should produce D1 `space.gate_pass` evidence. |
...
| MEDIUM | sandbox | provider_network | Docker provider seam must be reachable from production runtime composition | n/a | **FIXED:** `app_state.rs:411-428` seeds one `ProviderRegistry` with `DockerProviderAdapter::connect()`, shares it through one `DaemonClient`, and passes `sandbox.docker.default_snapshot` into `RequestSandboxProvisioner::with_default_snapshot`. | CLOSED: NF-2 reachability gap resolved; `registry.rs:69-85` keeps first-call-wins locally without a bootstrap singleton. | Done. Verified by `cargo check -p eos-runtime -p eos-sandbox-host --all-targets`, `cargo test -p eos-sandbox-host --lib`, `cargo test -p eos-runtime missing_model_registry_does_not_fail_startup --lib`, scoped Clippy, and `cargo machete`. |
...
| MEDIUM→LOW | agent-core | model_provider | `next_seq` off-by-one: Rust first seq=0 vs Python first seq=1 | `prompt_report_recorder.py:33-35` | `prompt_report.rs:90-95` | **FIXED:** `PromptReportRecorder::next_seq` now pre-increments; the prompt report golden starts at seq 1. | Done. |
...
| LOW | sandbox | ephemeral_workspace | Command-session prepare-error path leaks the overlay run_dir (no RAII cleanup) | `lifecycle.py:100-103` | **FIXED:** `prepare_command_session` now arms `RunDirCleanup` after `overlay_run_dirs` and disarms it only after a session is spawned. | CLOSED for the rare prepare-error scratch-dir leak. | Done. Verified by `cargo check -p eos-daemon --all-targets` and host `cargo clippy -p eos-daemon --all-targets -- -D warnings`; Linux cross-target check is blocked before `eos-daemon` by `eos-isolated`/`rtnetlink`. |
...
| LOW | sandbox | provider_network | First-call-wins provider seed and Docker-only provider selection | `bootstrap.py:40-53`; `sandbox.py:41` | **FIXED/accepted:** `registry.rs:69-85` makes default seeding first-call-wins locally; `error.rs:22-23` reports non-Docker provider selection as `expected docker`. | CLOSED as Rust Docker-only contract; no `OnceCell` bootstrap sentinel needed. | Done; covered with `registry::default_seed_is_first_call_wins` and `registry::selects_provider_from_config`. |
| LOW | agent-core | workflow_lifecycle / deferred_goal_depth | `is_nested_workflow` single-hop check vs full ancestry walk + cycle guard | `workflow_depth.py:10-49` | **FIXED:** `WorkflowControlPort::workflow_depth` walks parent workflow ancestry and cycle-guards malformed trees. | CLOSED by the deferred-goal-depth remediation row below. | Done. |
| LOW | agent-core | context_engine | `<terminal_tool_selection>` block format diverges (colon rows + extra "Pick exactly one" header) | `registry.py:120` + `skill_message.py:30-33` | **FIXED:** `terminal_selection_block` now renders through `render_tool_instruction(..., SelectionGuidance)` and drops the extra header. | CLOSED; prompt bytes now use the shared terminal-catalog shape. | Done. Verified by `cargo test -p eos-workflow terminal_selection_uses_terminal_catalog_format --lib`, scoped `cargo check`, and scoped Clippy. |
| LOW | sandbox | sandbox_tools | `api.exec_command.dispatch_total_s` timing dropped; grep/glob primitive semantics unpinned | `command.py:60-61` | no `dispatch_total` in agent-core; old `tool_primitives.rs` deleted | **FIXED:** command finalization emits `api.exec_command.dispatch_total_s` and `command_exec.total_s`; grep/glob were removed from `eos-tools`, `.eos-agents`, `eos-sandbox-api`, `eos-protocol`, `eos-daemon`, and `eos-runner`, so no Rust raw daemon search contract remains. | Done. |
| LOW | sandbox | daemon_protocol | `cancel` skips bounded 5s cleanup-wait (`cleanup_done` inverted); `runtime.*` timings stubbed 0.0; `internal_error` now carries `details.error_id` | `builtin_operations.py:182-198`; `dispatcher.py:122-131,198-212` | `dispatcher.rs:300-317,234-243,3089-3106` | **FIXED:** dispatcher responses attach measured `runtime.boot_to_dispatch_s`, `runtime.dispatch_s`, and server-supplied `runtime.read_request_s`; `api.v1.cancel` waits up to 5s for deregistration; `internal_error.details.error_id` is generated. | Done. |
| LOW | agent-core | model_provider | Neutral `Reasoning` block serializes `type:"reasoning"` not `"thinking"`; empty tool_use id fails stream | `message.py:30-34,189-190` | `message.rs:62-67`; `anthropic.rs:218-227` | **CLOSED:** reasoning compatibility is covered by decode + prompt-report golden rows; empty provider tool ids remain an accepted Rust fail-fast behavior because `ToolUseId` is a non-empty typed id and first-party providers send ids. | No synthetic id minting. |
| LOW | agent-core | terminal_tools / query_engine / budget_notifications | `terminal_not_submitted` failure message drops structured counts | `loop.py:50-57` | **FIXED:** `terminal_not_submitted_message` now includes `tool_calls_used`, `text_only_no_terminal_turns`, `tool_call_limit`, and `hard_ceiling`. | CLOSED for string/log parity; event shape was already matching. | Done. Verified by `cargo test -p eos-engine hard_ceiling_exit_terminal_not_submitted --lib`, scoped `cargo check`, and scoped Clippy. |
| LOW | agent-core | budget_notifications | Profile `notification_triggers` never merged (planner loses `nested_planner_deferral_disabled` reminder) | `factory.py:382-387` | `factory.rs:144` hardcodes defaults | CONFIRMED low (only planner populates the field); D3 placement REFUTED to none | **Deferred by scope decision:** ignore profile notification triggers for now; default budget/terminal reminders remain covered. |
...
| LOW(cov) | agent-core | persistence_state | Live migration `0001_initial.sql` never diffed against canonical schema; message snapshot is self-referential | `parity/sqlite/schema.sql` | **PARTIAL-FIXED:** `eos-db` integration now introspects the executed migration with `PRAGMA table_info`, and FK cascade is covered end-to-end. The message-schema snapshot remains a fixture-vs-fixture caveat. | Migration coverage gap closed; residual NF-1 is not a live `eos-db` behavior break. | Done for migration/FK coverage. Verified by `cargo test -p eos-db --test integration`, scoped `cargo check`, and scoped Clippy. |
...
- write_stdin op-name + fail-closed set (`tool_api/command.rs:76` /
  `eos-sandbox-host/.../daemon_client.rs:603` vs `transport.py:17` /
  `host/daemon_client.py:619-624`; daemon `dispatcher.rs:139`) are fixed
  and live-proofed by `write_stdin_empty_response_from_real_eosd_is_not_replayed`.
...
- `acquire_snapshot` exclusive lock concurrency regression (`stack.rs:343-344` vs
  `stack.py:108-135`) is source-fixed; live re-baseline pending. MEDIUM.
...
| **ephemeral upperdir→OCC merge & discard** (ephemeral_workspace + occ) | `ephemeral_workspace/pipeline.py` + `occ/commit_transaction.py` | **MATCH.** Per-call fresh overlay, upperdir captured and published through the shared OCC writer, lease released and run_dir discarded; verifier chased "writes land in upperdir" into the ns-runner and confirmed. The prepare-error run_dir leak is now fixed by `RunDirCleanup`; the remaining caveat is the benign `atomic=true` batching nuance (verifier REFUTED the silent-success claim). |
...
| ephemeral_workspace | sandbox | High fidelity; investigator HIGH REFUTED → lows | atomic-flag silent-success refuted; prepare-error run_dir leak FIXED |
| isolated_workspace | sandbox | High fidelity; both MEDIUMs REFUTED → none; lows | sampler/orphan-reaper lows |
| sandbox_tools | sandbox | Model-facing sandbox tool surface simplified / HIGH `write_stdin` disparity CLOSED | grep/glob removed from the agent-facing Rust tool registry and raw daemon/protocol/runner surface |
| daemon_protocol | sandbox | Faithful; HIGH fail-closed set CLOSED; lows | accurate, well-anchored |
| plugins | sandbox | Source path closed; live benchmark proof still pending | Rust facade, sandbox-Python runtime artifact, manifest validation, auto-overlay fail-fast guard, and caps are source-verified; run the existing Docker plugin benchmark with `EOS_LIVE_E2E_IMAGE=sweevo-dask__dask-10042` for live closure evidence |
| provider_network | sandbox | Units faithful; NF-2 production wiring closed; only Linux runtime proof remains | **unproven (Linux):** ns-holder/setns/fresh-ns/nft runtime behavior (source-only on darwin) |
| perf | sandbox | Complexity properties match; 1 MEDIUM (snapshot lock) | D1 space-property gate added to Phase 3 bench; **unproven live:** first `space.gate_pass` report still pending; shell-pre-mount squash absent |
...
| context_engine | agent-core | 4/4 invariants match; D1 prompt-block FIXED | D4 CLOSED by persistence_state 2e |
...
| persistence_state | agent-core | High fidelity; migration/FK coverage FIXED, tests green | Empty-`task_id` drop remains low; NF-1 message snapshot remains a fixture caveat |
...
6. ~~sandbox_tools C6 glob/file constants + grep/glob primitive semantics.~~
   **Closed by full removal:** grep/glob were removed from the model-facing Rust
   tool registry and raw daemon/protocol/runner surface.
7. Space-complexity property now has a Phase 3 benchmark source gate, but no
   refreshed live report has proven `space.gate_pass` yet (perf D1, shared gap).
...
5. **Done:** `write_stdin` op-name, fail-closed retry set, and the live
   double-apply proof have landed (sandbox_tools D1/D2, daemon_protocol D1).
   `write_stdin_empty_response_from_real_eosd_is_not_replayed` forwards the first
   stdin write to a real `eosd`, forces the thin-client empty-response exit 98,
   and verifies one client call, no daemon spawn, and one payload line.
...
12. Overlay remount peel-loop (overlay D1); re-baseline the source-fixed
    `acquire_snapshot` shared-lock change and the new Phase 3 space gate
    (perf D1/D2); run the existing Docker plugin benchmark for live LSP/Pyright
    evidence after the source-closed plugin runtime path (plugins D1/D6).

**P3 — observability / telemetry parity (no control-flow impact):**
13. Squash audit D3 and plugin response/caller caps are fixed; remaining P3
    items are workflow `task.*` audit events (attempt_harness D8);
    `dispatch_total_s` timing (sandbox_tools N1);
    `internal_error` uuid + `runtime.*` timings (daemon_protocol); prompt-report seq
    + reasoning-block golden row (model_provider D5/D7). Persistence migration
    introspection + FK cascade coverage is now fixed in `eos-db`.
...
>   - **Effect on this report's items:** a forward redesign layered on the audited spine. Together with the subagent/background lanes it closes the Phase-1 *background-supervisor-in-loop* item (`query_engine D1` / `background_supervisor §4/§5`). It **supersedes the command-session finalizer architecture** behind the ephemeral `command.rs` run_dir anchor (§2/§5 ephemeral_workspace LOW) and the `write_stdin` flat-sleep. It did **not** by itself prove the `write_stdin` live replay check (Phase 2, `sandbox_tools D1/D2`); that proof is now covered by `write_stdin_empty_response_from_real_eosd_is_not_replayed`.
...
**Status:** CLOSED. All HIGH/MED rows are remediated; the former LOW rows are now
either implemented (`exec_command` timing, runtime timings, cancel cleanup wait,
prompt-report seq/golden coverage) or closed as accepted Rust-native behavior
(fail-fast malformed provider `tool_use` ids).

| ✓ | Area | Sev | Fix & verify | Disparity · anchor |
|---|------|-----|--------------|--------------------|
| ☑ | sandbox / [sandbox_tools](areas/sandbox_tools.html) ⊕ [daemon_protocol](areas/daemon_protocol.html) | HIGH | Fix `write_stdin` source contract: rename the client wire op `api.v1.exec_stdin` → `api.v1.write_stdin` **and** set the host fail-closed retry set to that canonical stdin-write op (renaming alone re-opens a replay double-apply). **Landed:** `ops.rs` wire string + `daemon_client.rs` fail-closed set (`api.v1.write_stdin`) + pinning tests (`daemon_op_wire_strings`, `empty_response_gating_matches_python_set`, `exec_stdin_request` snapshot) green; live `write_stdin_empty_response_from_real_eosd_is_not_replayed` forwards the first stdin write to a real `eosd`, forces the thin-client empty-response exit 98, and verifies the host does not spawn/replay and the payload is applied exactly once. | sandbox_tools D1/D2, daemon_protocol D1 · `tool_api/command.rs:76`, `daemon_client.rs:603` |
| ☑ | sandbox / [sandbox_tools](areas/sandbox_tools.html) | LOW | **Done.** `exec_command` finalization now emits `api.exec_command.dispatch_total_s` alongside `command_exec.total_s`; grep/glob were removed from the model-facing tool registry, API facade, protocol DTOs, daemon op table, and runner verb set. | sandbox_tools N1 · `command.rs`, `eos-tools`, `tools/sandbox.html` |
| ☑ | sandbox / [daemon_protocol](areas/daemon_protocol.html) | LOW | **Done.** `internal_error.details.error_id` is generated; dispatcher responses attach measured `runtime.boot_to_dispatch_s` / `runtime.dispatch_s` / server-supplied `runtime.read_request_s`; `api.v1.cancel` waits up to 5s for the target invocation to deregister before reporting `cleanup_done`. | daemon_protocol · `dispatcher.rs`, `server.rs`, `invocation_registry.rs` |
...
| ☑ | agent-core / [model_provider_prompt](areas/model_provider_prompt.html) | LOW | **Done/accepted.** Prompt reports now pre-increment seq and the golden includes a `Reasoning` block. The default empty `tool_use` id path is deliberately not ported: Rust keeps `ToolUseId` non-empty and fails malformed provider streams fast; first-party providers send ids. | model_provider D5/D7 · `prompt_report.rs`, `message.rs`, `anthropic.rs` |
...
| ✓ | Area | Sev | Fix & verify | Disparity · anchor |
|---|------|-----|--------------|--------------------|
| ☑ | sandbox / [plugins](areas/plugins.html) | HIGH | **Done.** `lsp.*` tools register from `eos-plugin-catalog` and dispatch through the Rust daemon. Host lifecycle uploads the trusted sandbox-Python PPC bridge/runtime files under `/eos/daemon`; the LSP manifest command is `/eos/daemon/plugins/catalog/lsp/runtime/ppc_service.sh`; the wrapper runs sandbox `python3` from `/eos/daemon` with no `PYTHONPATH`, no Python `call_plugin`/`install.py`, and no catalog `setup.sh` execution. Pyright setup uses host-uploaded `/eos/plugin-packages/lsp/{node.tar.xz,pyright.tgz}` and installs the runtime under `/eos/env/eos-node22`. Docker `put_archive` now preserves `/eos` destinations even with the `/eos` tmpfs. *Verified:* `cargo test -p eos-sandbox-host runtime_artifact`, `cargo test -p eos-runtime lsp_executor_ensures_manifest_before_dispatch`, Python compile for the staged bridge/modules, and live `EOS_LIVE_E2E_IMAGE=sweevo-dask__dask-10042` `bench_rust_daemon_plugin.py` gate pass in `bench/rust-daemon-plugin-closure-20260604-rerun4.json` (PPC bridge, Pyright LSP read/rename/write paths, isolated plugin gate, and cleanup all pass). | plugins D1/D6 · runtime payload/ensure + live PPC/LSP gate closed |
| ☑ | sandbox / [plugins](areas/plugins.html) | MED | **Done.** Manifest `plugin_id` now uses `^[A-Za-z_][A-Za-z0-9_]*$`; `op_name` is only non-empty. *Verified:* `cargo test -p eos-plugin`. | plugins (manifest) · `manifest.rs`, `service.rs` |
| ☑ | sandbox / [plugins](areas/plugins.html) | MED | **Done.** Daemon/plugin validation rejects `WRITE_ALLOWED + auto_workspace_overlay=true` unless the referenced service is `oneshot_overlay`; LSP write ops remain self-managed with `auto_workspace_overlay=false`. *Verified:* `cargo test -p eos-plugin`; `cargo test -p eos-daemon plugin`. | plugins (auto-overlay) · `manifest.rs`, `mod.rs` |
| ☑ | sandbox / [plugins](areas/plugins.html) | LOW | **Done.** PPC replies above 8 MiB are rejected before JSON parse, and plugin caller/audit fields reject NUL, non-string, and >256-char values at the daemon boundary. *Verified:* `cargo test -p eos-daemon plugin`. | plugins D7 · `mod.rs` |
...
| ◐ | sandbox / [perf](areas/perf.html) | MED | **Source fixes landed.** `acquire_snapshot` now uses a shared/read storage guard while publish/squash/release/commit keep the reentrant exclusive guard. The Phase 3 live bench now seeds a base-only fixture, samples LayerStack disk usage, and folds `space.gate_pass` into the top-level gate. Verified with storage-lock overlap/exclusion tests, full `eos-layerstack` lib tests, scoped Clippy, Python compile, and a synthetic space-gate evaluator check. **Remaining:** live N-concurrent throughput re-baseline with first `space.gate_pass` artifact. | perf D1/D2 · `bench_rust_daemon_phase3.py::{seed_space_fixture,sample_layer_stack_space,evaluate_space_accounting}`, `stack.rs::acquire_snapshot`, `storage_lock.rs::{shared,exclusive}` |
| ☑ | sandbox / [provider_network](areas/provider_network.html) | MED | **Done — production provider wiring simplified.** `AppStateBuilder::build` seeds Docker when the host transport/provisioner defaults are needed, shares the registry through one `DaemonClient`, and wires the configured Docker `default_snapshot` into fresh sandbox creation. The first-call-wins behavior lives in `ProviderRegistry::set_default`; the redundant async context-preparer shim was removed. *Verified:* `cargo check -p eos-runtime -p eos-sandbox-host --all-targets`, `cargo test -p eos-sandbox-host --lib`, `cargo test -p eos-runtime missing_model_registry_does_not_fail_startup --lib`, `cargo clippy -p eos-runtime -p eos-sandbox-host --all-targets -- -D warnings`, and `cargo machete --with-metadata crates/eos-sandbox-host crates/eos-runtime crates/eos-sandbox-api crates/eos-tools`. | provider_network NF-2 · `app_state.rs:411-428`, `registry.rs:69-85`, `provisioning.rs:63-75` |
...
| ✓ | Area | Sev | Fix & verify | Disparity · anchor |
|---|------|-----|--------------|--------------------|
| ☑ | agent-core / [context_engine](areas/context_engine.html) | MED | **Done.** `<terminal_tool_selection>` now uses the shared terminal catalog renderer (`render_tool_instruction(..., SelectionGuidance)`), producing backtick rows, blank-line separation, and no extra "Pick exactly one" header. *Verified:* `cargo test -p eos-workflow terminal_selection_uses_terminal_catalog_format --lib`, scoped `cargo check`, and scoped Clippy. | context_engine D1 · `composer.rs::terminal_selection_block` |
| ☑ | agent-core / [budget_notifications](areas/budget_notifications.html) | LOW | **Deferred by scope decision.** Profile `notification_triggers` remain intentionally ignored for now; no active remediation. Default budget tiers and terminal reminders stay covered. | budget_notifications · `factory.rs:144` |
| ☑ | agent-core / [persistence_state](areas/persistence_state.html) | LOW | **Done for migration/FK coverage.** `eos-db` integration now opens the executed migration, checks every table with `PRAGMA table_info`, and keeps the FK cascade test green. The message-schema snapshot caveat remains documented but is not a live `eos-db` behavior break. *Verified:* `cargo test -p eos-db --test integration`, scoped `cargo check`, and scoped Clippy. | persistence_state D2 · `integration.rs::migrations_create_schema` |
...
empty-`task_id` outcome-drop loud error. Fixed in this pass: context-engine
terminal catalog bytes, persistence migration/FK coverage, command-session
prepare-error `RunDirCleanup`, and `terminal_not_submitted` structured counts.
Already fixed by earlier deferred-goal-depth work: `is_nested_workflow`
ancestry-walk + cycle guard.
