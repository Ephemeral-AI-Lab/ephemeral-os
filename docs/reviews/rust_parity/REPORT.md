...
- **Headline real bugs / missed dynamics (most likely to bite, ranked):**
  ...
  7. **`write_stdin` wire/retry mismatch and live replay proof are closed** — the
     ...
     the stdin ops, and `write_stdin_empty_response_from_real_eosd_is_not_replayed`
...
- write_stdin op-name + fail-closed set (`tool_api/command.rs:76` /
  `eos-sandbox-host/.../daemon_client.rs:603-604` vs `transport.py:17` /
...
12. Overlay remount peel-loop (overlay D1); re-baseline the source-fixed
    ...
    (perf D1/D2); plugin runtime
    payload/ensure + manifest validator/setup-trust verification (plugins D1/D6).
