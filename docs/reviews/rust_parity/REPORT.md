...
| ✓ | Area | Sev | Fix & verify | Disparity · anchor |
|---|------|-----|--------------|--------------------|
| ◐ | sandbox / [plugins](areas/plugins.html) | HIGH | **Source closed — live proof pending.** `lsp.*` tools register from `eos-plugin-catalog` and dispatch through the Rust daemon. Host lifecycle now uploads the trusted sandbox-Python PPC bridge/runtime files under `/eos/daemon`, the LSP manifest command is `/eos/daemon/plugins/catalog/lsp/runtime/ppc_service.sh`, and the wrapper runs sandbox `python3` from `/eos/daemon` with no `PYTHONPATH`, no Python `call_plugin`/`install.py`, and no catalog `setup.sh` execution. Pyright setup uses the existing image-provided binary or `/eos/plugin-packages/lsp/{node.tar.xz,pyright.tgz}` artifact contract. *Verified:* `cargo test -p eos-sandbox-host runtime_artifact`, `cargo test -p eos-runtime lsp_executor_ensures_manifest_before_dispatch`, Python compile for the staged bridge modules. *Pending:* existing live benchmark through the repo Docker Python environment with `EOS_LIVE_E2E_IMAGE=sweevo-dask__dask-10042`. | plugins D1/D6 · source-closed runtime payload/ensure; live gate pending |
