...
12. Overlay remount peel-loop (overlay D1); re-baseline the source-fixed
    `acquire_snapshot` shared-lock change (perf D2); plugin runtime
...
| ◐ | sandbox / [perf](areas/perf.html) | MED | **Source fix landed.** `acquire_snapshot` now uses a shared/read storage guard while publish/squash/release/commit keep the reentrant exclusive guard. Verified with storage-lock overlap/exclusion tests, full `eos-layerstack` lib tests, and scoped Clippy. **Remaining:** live N-concurrent throughput re-baseline. | perf D2 · `stack.rs::acquire_snapshot`, `storage_lock.rs::{shared,exclusive}` |
