# operation Unified Op I/O Spec — Catalog-Derived `OpRequest`/`OpReply`
...
Relationship to `SPEC-operation-core.md` (v1): supersedes its §9/§10
...
AFTER
  bytes ─ decode ─ Envelope (unchanged)
        ...
        ─ OpRequest::parse(op, &args, invocation_id) -> typed variant
        │                                                        ── core/request.rs, ONE parse site
        │     Err(Args)            ─ family→channel map ─ OpReply::{Refused | Error}
        │     Err(NotDaemonServed) ─ structured host-boundary forbidden error
...
├── command/
│   ├── contract.rs              NEW  op DTOs moved from command-session +
...
DELETED ENTIRELY: control/  eos-sandbox/  isolation/  workspace_run/  (scaffold-only)
DELETED IN daemon: dispatch table (OpTable/Handler/fn_addr_eq/boot panic),
...
| `request.rs`: `OpRequest`, payloads, `ArgsError`, parse helpers, `DEFAULT_CALLER_ID` | `daemon/src/runtime/request_args.rs:10-86` + parse blocks in 7 `op_adapter/*` modules + `op_adapter/mod.rs:44-50` | The wire-args vocabulary IS the op contract; one parse site, compile-time totality |
...
| `outcome.rs::MutationWire` + `WorkspaceMutationOutcome::into_wire` | `daemon/src/runtime/response.rs:94-131` + `op_adapter/files.rs:251-274` | v1 §9 rule 3's "one core-owned shape" finally rendered by core, not a daemon-private struct |
...
| `command/contract.rs` (not core, typed against core) | `command-session/src/contract.rs` | Kills the v1-F6 stringly round-trip; substrate consumes a pre-rendered `Value` only |
...
`OpRequest` is **not** `#[non_exhaustive]`: daemon (a foreign crate to
operation) must match it exhaustively — that match IS the totality
...
```rust
// daemon/src/dispatch/dispatcher.rs
pub fn dispatch_with_context(request: &Request, context: DispatchContext<'_>) -> Value {
    ...
    let reply = match BuiltinOp::from_op_name(&request.op) {
        Some(op) => match OpRequest::parse(op, &request.args, &request.invocation_id) {
            ...
            // Host-served names: catalog-derived daemon boundary error.
...
process-global `operation::command::command_session_config()` (NOT
...
The op DTOs move from `command-session/src/contract.rs` (the substrate
...
- Substrate re-cut: `persist_final(final_wire: &serde_json::Value)`; the
  caller in `operation::command` renders. `tail_lines` widens to `pub` but
  **stays in command-session** (`transcript.rs` consumes it; moving it
...
- `command-session/tests/unit/session.rs` is **rebuilt**: it constructs
...
// as_str(): "direct_write" / "direct_edit" / "isolated_workspace" / "overlay_capture"
...
`ops.json` (byte-identical), `host` keeps its independent envelope
...
`host` runs **outside** the sandbox (v1 §10 inverts this); the
no-dep rule survives on its real legs — no compiled code across the boundary,
and operation's Linux dep tree cannot live in a darwin host binary.
...
| 8 | `FileOpsError`/`WorkspaceApiError` → `core::OpError` re-exports; `kind` live | ~10 operation files mechanical; daemon `#[from]` survives via re-export; error-equality assertions now compare `kind` (named test churn) | none (Display = message only) |
| 9 | Command DTOs relocate; `CommandResponse{workspace, metadata}` → `settled: Option<CommandMetadata>` + `WorkspaceMode`; `persist_final(&Value)`; `tail_lines` pub (stays substrate); `CommandSessionError` stays substrate; `u64_to_f64_saturating` home fixed | substrate 3 files + **session unit tests rebuilt**; operation `command/*`; daemon command adapter + `tests/unit/command` (constructs `CommandResponse` field-by-field) | canonical-equal; `final.json` bytes identical; conflict-key membership pinned by the additive fixture |
...
|---|---|---|
| P1 | Delete 8 `ops.rs`, 4 scaffold modules, `file/port.rs`; prune `src/lib.rs` + family lib.rs decls | `cargo check/test -p operation`; rg gates (no `FAMILY_OPS`/family enums/`operation::{control,sandbox,isolation,workspace_run}`/`file::port`); `cargo xtask check-contract` |
...
out of workspace (correct ownership, ~6 lines, out of scope — and its
