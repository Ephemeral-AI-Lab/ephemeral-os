...
| P1. `eos-workspace-api` command policy contract prep | TODO |  | Add object-safe trait bounds and prepared path fields. |
| P2. Workspace policy crates updated to new prepared DTO | TODO |  | `eos-ephemeral-workspace` and `eos-isolated-workspace` compile against updated contract. |
| P3. New `eos-command-session` crate with fake-policy tests | TODO |  | No daemon wiring yet. Dependency guard must pass. |
| P4. Daemon manager wiring and event sink | TODO |  | Wire `exec_command`, `write_stdin`, cancel, collect, count, reaper. |
| P5. Remove old daemon runtime modules and mode enum | TODO |  | Delete drained compatibility code. |
| P6. Focused Rust verification | TODO |  | New crate, daemon command, isolated tests, clippy. |
