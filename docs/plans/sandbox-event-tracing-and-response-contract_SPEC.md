...
| Phase 06 - Debt deletion | Complete | Phase 06 closeout passed on 2026-06-13 after deleting the mixed-wire host/e2e response classifiers, daemon `ErrorResponse` wire branch, gateway host-route `json!` response envelopes, stale generated inventory response docs, and clippy debt surfaced by the workspace gate. Verification: `CARGO_TARGET_DIR=/tmp/eos-phase06-target cargo test --workspace`, `CARGO_TARGET_DIR=/tmp/eos-phase06-target cargo clippy --workspace --all-targets -- -D warnings`, `cargo fmt --check`, `git diff --check`, focused stale-surface greps for removed classifier/error-response names and public response-contract debt returned no matches, and `cargo run --manifest-path scripts/class-inventory/Cargo.toml` regenerated 15 crate inventories. Remaining `serde_json::Value`, `timings`, and `success` hits are bounded contract/detail/projection/test code or lower-level timing carriers for trace/resource event generation, not daemon-host audit payload contracts or public response-shape branches. |
| Phase 07 - TypeScript mirror | Pending | |
...
| --- | --- |
| `host.protocol` | request_received, request_persisted, forward_started, forward_finished, forward_failed, response_missing, uncertain_outcome, trace_degraded |
...
| `host.transport` | connect_started/finished/failed, retry_scheduled, endpoint_refreshed, endpoint_refresh_failed, fallback_chain_started, exec_client_started/finished/failed, daemon_respawn_started/finished/failed, request_written/write_failed, response_read/read_failed, empty_response, decode_failed, sidecar_decode_failed, sidecar_ingest_failed |
| `daemon.transport` | accepted, read_started, read_finished, auth_checked, decoded, response_write_started/finished/failed, response_shutdown_failed, shutdown_finished |
| `daemon.boot` | config_loaded, listen_bound |
...
| `layer_stack` | binding_loaded, snapshot_acquired, snapshot_lease_used {manifest_version, manifest_depth}, lease_released, lease_release_failed, manifest_read, manifest_validated {manifest_version, manifest_depth, active_lease_count}, publish_layer_finished {manifest_version_before/after, published_layer_count}, commit_to_workspace_finished {manifest_version, duration_s}, auto_squash_started/finished {error on failure}, auto_squash_skipped {reason} |
| `occ` | commit_started, validate_groups_finished, worker_handoff, worker_batch_finished, conflict_detected {path, reason, observed_version?, observed_state?} per conflicting file, commit_finished |
...
| `command` | prepared, spawned, yielded, stdin_written {bytes, wait_ms, waited_for_output}, progress_read, wait_finished {status, completed, yield_time_ms, duration_ms}, cancelled, timed_out, exit_taken {kill_reason, signal}, finalized, artifact_written/failed, final_persisted, final_persist_failed, transcript_failed, completion_buffer_evicted {command_id, seq, max_entries}, changed_paths_recorded {changed_path_count, changed_paths (bounded head), changed_paths_truncated}, response_meta {status, exit_code, workspace, success, changed_path_count}, advance_finished {ActiveCommandAdvance background-root summary} |
| `isolated_workspace` | enter_started, holder_started, network_configured {dns_fallback_applied, previous_first_nameserver?}, status_read, exit_started, teardown_phase_finished (×4; kill_holder carries {holder_was_alive, exit_status, signal?}), exited {mountinfo_scan_error?}, workspace_evicted {IdleWorkspaceEvict background-root summary}, recovery_started/finished {manager_json_error?, orphan_cleanup_error?} |
...
| `checkpoint` | worktree_mode_selected {mode}, git_command_finished {argv_summary, exit_code, stderr_tail}, workspace_base_finished {action, created, reset, duration_s}, commit_to_git_finished {committed, worktree_mode, manifest_version, manifest_root_hash, path_count, duration_s} |
...
- [x] Step 0 lands a shape-aware host/gateway decoder:
  ...
  any family flips.
- [x] Shared e2e trace support lands before family flips:
...
- [x] Shared helpers decode `OperationEnvelope`, extract `trace_id` and
...
Phase 06 is also Complete as of 2026-06-13, and Phase 07 is Pending.
```
...
- [x] Delete remaining migration/debt surfaces: `is_success`, `error_kind`,
  `OpResponse`, `merge_runner_timings`, flat timing/resource helpers, quirk
  serializers, and `json!` response envelopes at public response/migration
  surfaces. The v1 flattening adapter is already deleted; remaining lower-level
  timing maps are internal trace/resource event inputs.
- [x] `SpanKind`/event vocabulary exhaustiveness is the only timing/resource
  drift guard.
- [x] JSON trace payload helpers remain only for human exports/projections, not
  daemon-host audit payloads.
- [x] Generated inventories/readmes no longer describe timing-key contracts.
- [x] `cargo test --workspace`
- [x] `cargo clippy --workspace --all-targets -- -D warnings`
- [x] Focused public response-debt greps for
  `ErrorResponse|response_classification|ResponseShape|ResponseClassification|success:false|merge_runner_timings`
  and `timings\.|merge_runner_timings|\bis_success\(|\berror_kind\(|OpResponse|success\s*:`
  over E2E/host/gateway/daemon-wire surfaces return no legacy contract uses.
  The broad all-crates pattern still finds domain DTO `success` fields and
  internal timing maps used as trace/resource event inputs.
- [x] `rg -n "serde_json::Value" sandbox/crates/eos-daemon sandbox/crates/eos-sandbox-host sandbox/crates/eos-sandbox-gateway sandbox/crates/eos-operation` shows only bounded contract/detail/export/projection, decoding, and test code.
- [x] Update the progress tracker with Phase 06 evidence and mark Phase 06
  `Complete`.

Phase gate:

```text
Phase 06 is Complete: migration-only adapters and public flat
timing/resource response surfaces are gone, focused grep gates prove no legacy
contract use remains, and workspace test/clippy gates pass. Phase 07 is
unblocked and Pending.
```
