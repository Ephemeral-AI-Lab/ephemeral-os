...
| Phase 04 - Subsystem events and resource stats | In progress | Checkpoint worktree-mode and git command-step events are implemented through request-scoped daemon sidecar events; `cargo test -p eos-operation -p eos-daemon` passed on 2026-06-12 for the checkpoint slice. |
