...
- Renaming `continuation_goal` (field, column, JSON key) — "continuation" is tier-agnostic.
...
| Column `continuation_goal` | **unchanged** (Principle 2; "continuation" is a relationship) |
...
| `_core/infra.py:175-185` (`assert_continuation_episode_predecessor`) | `f"Continuation requires predecessor episode {previous.id!r} to be SUCCEEDED, not {previous.status}"` / `f"Continuation requires predecessor episode {previous.id!r} to have a continuation_goal; none was recorded"` | `f"Continuation requires predecessor iteration {previous.id!r} to be SUCCEEDED, not {previous.status}"` / `f"Continuation requires predecessor iteration {previous.id!r} to have a continuation_goal; none was recorded"` |
...
- **Option E — Rename `continuation_goal` to `next_iteration_goal`.** Rejected; "continuation" describes the relationship, not the tier.
...
3. **`continuation_goal` rename:** Re-evaluate `continuation_goal` → `next_iteration_goal` only if a future iteration-pattern variant (e.g. parallel iterations) makes the relational-vs-tier distinction less clear.
