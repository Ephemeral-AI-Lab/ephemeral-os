...
**6.6.** `tests/mock/sandbox/concurrency/test_background_shell_lifetime.py` (NEW — Planner F.17 / Scenario D.1; revised after user directive — background PERMITTED in both modes; unified `WorkspacePipeline.run_tool_call` dispatch):

Per the revised Phase 2 §3.1 design: all four background-shell verbs (`shell_launch` / `shell_reap` / `shell_poll` / `shell_cancel`) route through `WorkspacePipeline.run_tool_call`. Lifecycle ownership differs by mode (ephemeral: per-`ShellJob` overlay; iws: shared session overlay), but the protocol is the same.

- **Sub-test A (ephemeral happy path):** issue `api.shell.launch` against an ephemeral agent. Assert `success=True`, `job_id` returned, overlay handle registered in `EphemeralPipeline._background_jobs[job_id]`. Verify `run_tool_call` returned BEFORE the child process exits (i.e. the call did NOT block on the shell). Issue `api.shell.reap`; assert child exit captured, OCC commit landed (source=`overlay_capture` per Q5), overlay destroyed, `_background_jobs` entry removed.
- **Sub-test B (ephemeral OCC conflict at reap — Q1):** launch a background shell at snapshot V0 that writes `/testbed/foo`. Concurrently issue a foreground `write_file` to `/testbed/foo` that commits at V1. Then reap. Assert the reap result is `success=False, conflict=ConflictInfo(...)` — best-effort OCC surfaces the conflict; the agent decides whether to retry. Verify overlay was still destroyed (no leak even on conflict).
- **Sub-test C (ephemeral cancel discards):** launch; before child exits, issue `api.shell.cancel`. Assert child PG killed, overlay destroyed, NO OCC commit (the upperdir state was discarded). Assert `_background_jobs` entry removed; `lease.released` audit event fires exactly once.
- **Sub-test D (interleaved foreground + background in ephemeral):** launch a background job touching path A; while it runs, issue a foreground `write_file` to path B; reap the background job. Assert (i) both touch different overlays (different lease_ids), (ii) both commits land in OCC with the expected ordering, (iii) `_destroy_with_lease_guard` ran exactly once per handle (per-handle-lock TOCTOU fix from §3.1 holds), (iv) foreground write was visible in main_workspace BEFORE the background reap completed.
- **Sub-test E (iws happy path — shared session overlay):** enter iws; launch background job; issue an iws foreground `write_file`; reap the background job. Assert (i) both ops used the SAME `OverlayHandle` (from `_sessions[agent_id]`), (ii) reap returned with `changed_paths` observability but NO OCC commit (iws never commits), (iii) `_session_jobs[agent_id][job_id]` was populated then removed. Exit iws; assert all changes discarded.
- **Sub-test F (iws drain at exit):** enter iws; launch background job that sleeps longer than `grace_s`. Issue `api.isolated_workspace.exit(grace_s=0.5)`. Assert exit WAITS ~0.5s, then SIGTERM/SIGKILLs the child PG; `evicted_background_jobs: 1` in `ExitIsolatedWorkspaceResult.phases_ms`; overlay destroyed.
- **Sub-test G (Q4 mode-mix rejection):** in ephemeral mode, launch a background job (don't reap). Call `enter_isolated_workspace(agent_id)`. Assert `LifecycleError(kind="ephemeral_jobs_in_flight", details={"job_ids": ["shell-<uuid>"]})`. Reap the background job. Re-attempt `enter`; assert success.
- **Sub-test H (unified-protocol assertion):** static check — assert that all four `shell_*` verb names appear ONLY through `EphemeralPipeline.run_tool_call` and `IsolatedPipeline.run_tool_call` (NOT as public pipeline methods). Walk `EphemeralPipeline.__dict__` and confirm no public method matches `^(launch|reap|poll|cancel)_background_job$`. Confirms Principle 4 — `WorkspacePipeline` protocol stays at one method.
- **Counter-test:** ephemeral foreground shell (verb=`"shell"`, args without `background`) succeeds normally (no `_background_jobs` registration; per-call lifecycle).
...
- §10 NEW — Background-shell policy (revised after user directive): PERMITTED in both modes via the unified `WorkspacePipeline.run_tool_call(req)` protocol. Document: (a) the four verbs (`shell_launch`/`shell_reap`/`shell_poll`/`shell_cancel`) and their wire-RPC names (`api.shell.launch` etc.), (b) overlay-lifetime ownership (ephemeral = per-`ShellJob`; iws = shared session overlay), (c) OCC commit policy (ephemeral reap commits with source=`"overlay_capture"`, best-effort; iws reap NEVER commits — discards at exit), (d) cross-mode rejection (`enter_isolated_workspace` returns `ephemeral_jobs_in_flight` if agent has live ephemeral background jobs), (e) the 5 design-decision references (Q1 best-effort, Q2 pipeline-owned, Q3 unbounded, Q4 reject-enter, Q5 overlay_capture).
...
- ✅ `concurrency/` tier covers OCC source-tag coalescing on all 4 helper sites (api_write batches, overlay_capture doesn't), `_wire_handle` ordering invariant, concurrent disjoint-path writes, same-path conflict resolution, destroy-under-asyncio-interleaving (D.2), background-shell lifetime in BOTH modes (D.1; 8 sub-tests A–H covering ephemeral launch/reap/conflict/cancel/interleaved-foreground + iws shared-overlay/drain + Q4 mode-mix rejection + unified-protocol assertion), AND a 10-step interleaved E2E sequence.
...
- ✅ `docs/sandbox/api_surface.md` exists with all 11 sections (including pass-through table, background-shell policy, `WorkspaceSession` deferral note).
