...
3. **Multi-engine / engine-restart Q4 split-brain.** Engine-layer `BackgroundTaskManager.count_by_agent` is engine-local in-process state; it cannot see another engine's in-flight tasks. Two concrete failure modes:
   ...
   - **Mitigation enacted:** `enter_isolated_workspace` consults the daemon via a lightweight `api.v1.inflight_count(agent_id)` RPC and merges its count with the engine-local count before deciding Q4 rejection. The §6 heartbeat remains a batched liveness refresh for known invocation ids; orphan cleanup is TTL-driven, and stale entries remain visible to Q4 until the cancelled coroutine reaches `finally` and deregisters. Pre-Phase-2.5 deployment precondition: "one engine per sandbox" is documented in §14 as a current limitation; the cross-engine RPC defends against forward incompatibility, not today's deployment.
