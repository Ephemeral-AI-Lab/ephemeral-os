...
This document is the **overview**. The detailed plan lives in three phase documents:
...
2. [`unify_sandbox_workspace_phase2.md`](unify_sandbox_workspace_phase2.md) — **Unification** (per-call ephemeral pipeline, persistent isolated pipeline, lifecycle host API, agent tools; substantive)
3. [`unify_sandbox_workspace_phase3.md`](unify_sandbox_workspace_phase3.md) — **Test migration & documentation**
...
| **`ephemeral_workspace`** | Per-tool-call execution context (foreground) OR per-background-job execution context (`shell_launch → shell_reap`). | OCC merge into `main_workspace` at end of foreground call OR at `shell_reap`. | **Foreground: created per tool call, destroyed at end of call (full lifecycle inline in `run_tool_call`). Background-shell: created at `shell_launch`, owned by the `ShellJob`, destroyed at `shell_reap`/`shell_cancel`.** Overlay lifetime is owner-defined — usually the tool call, but a `ShellJob` may own it longer. | `run_tool_call` only (1 method). Background-shell ops are verbs (`shell_launch` / `shell_reap` / `shell_poll` / `shell_cancel`) dispatched internally — same protocol, different lifecycles. |
...
### Background-shell policy

(Critic must-fix #4 / Architect F.3 / Planner D.1; revised after user directive — background shells are permitted in BOTH modes; overlay lifetime is owner-defined.)

| Mode | Background-shell verbs (`shell_launch` / `shell_reap` / `shell_poll` / `shell_cancel`) |
|---|---|
| Ephemeral | **PERMITTED.** `shell_launch` creates an overlay and a `ShellJob`; the job retains the `OverlayHandle` and lease beyond `run_tool_call`'s return. `shell_reap` waits for child exit, captures upperdir, OCC-commits (source=`"overlay_capture"`), then destroys the overlay. `shell_cancel` SIGTERM/SIGKILLs the child and destroys the overlay with NO commit. `shell_poll` checks status without affecting overlay state. The pipeline tracks in-flight jobs in `_background_jobs: dict[str, ShellJob]`. |
| Isolated | **PERMITTED.** Overlay persists across the session; background jobs run within it (no separate per-job overlay). `shell_launch` / `shell_reap` / `shell_poll` / `shell_cancel` operate against the session overlay. `exit` drains in-flight background jobs up to `grace_s`, then force-kills survivors and reports `evicted_background_jobs` count. Discards upperdir; no commit. |

**Unified-protocol design (user directive):** all four background-shell verbs go through the same `WorkspacePipeline.run_tool_call(req)` protocol as foreground verbs — they're routed internally based on `req.verb`. The pipeline preserves Principle 4 ("ONE method") and aligns foreground+background under one interface; the difference is purely *which lifecycle the verb triggers internally* (foreground = per-call create/destroy; `shell_launch` = create + register `ShellJob`; `shell_reap`/`shell_cancel` = look up handle by `job_id`, capture/commit-or-discard, destroy; `shell_poll` = look up handle, return status only).

**OCC conflict policy at reap (Q1):** best-effort. The job's changes apply against the snapshot it launched at (V0). If head has advanced to V1 by reap time AND the change paths overlap with what landed between V0 and V1, OCC returns a conflict; the agent sees a `conflict` result and decides whether to retry. No hidden replay; matches today's shell semantics.

**Registry ownership (Q2):** pipeline-owned. `EphemeralPipeline._background_jobs` holds ephemeral jobs (agent-scoped). `IsolatedPipeline` jobs are session-scoped (the iws handle owns its jobs). Each pipeline's `__init__` runs `startup_gc` to reap orphans from a previous daemon process. Global `daemon/service/shell_job.py::ShellJobRegistry` is removed; its responsibilities split across the two pipelines.

**Concurrency cap in ephemeral (Q3):** unbounded. An agent self-regulates how many ephemeral background jobs it has in flight. Documented risk: a buggy agent loop launching many jobs consumes leases + upperdir disk. Mitigation is observability, not enforcement — `_background_jobs` size exported as a gauge for alerting.

**Mode-mix policy (Q4):** if an agent has live ephemeral background jobs when it calls `enter_isolated_workspace`, the enter returns `LifecycleError(kind="ephemeral_jobs_in_flight", details={"job_ids": [...]})`. The agent must `shell.reap` or `shell.cancel` them first. Keeps the mental model clean: one workspace mode at a time.

**OCC source-tag (Q5):** background-shell commits always use `source="overlay_capture"` (no coalescing; cross-path atomicity). Shell can touch arbitrary paths; coalescing two background-shell commits would be unsafe. Recoverable later if measurements show single-path coalescing matters.
...
- **6 tool ops** with static `Intent` metadata:
  ...
  - `Intent.WRITE_ALLOWED` (shell-specific shape): `shell` — keeps its own signature for `cancel_event`, `pgrp`, background `job_id`, TTL reap
...
1. **Workspace lifecycle is owner-defined.** The `OverlayHandle` is owned by whoever needs it longest. For foreground tool calls in ephemeral mode, that's the `run_tool_call` invocation (full lifecycle inline). For `shell.launch` in ephemeral mode, ownership transfers to the `ShellJob`; the overlay survives until `shell.reap` or `shell.cancel`. For isolated mode, the iws session owns the overlay (created at `enter`, destroyed at `exit`); background jobs run inside the session and don't create separate overlays. The shapes are intentionally different — the model is "owner-defined lifecycle," not "fixed per-call lifecycle" — and Phase 2 §3.1 wires this explicitly. (Revised from "mode-defined" after user directive to allow background shells in both modes.) Pipelines are named `EphemeralPipeline` / `IsolatedPipeline` — "Pipeline" connotes "orchestrates workspace-scoped execution context." We considered `Executor` / `Context` (Planner C.5); kept `Pipeline` for symmetry with the two-class story and to avoid the `concurrent.futures.Executor` overload (rename is a separate plan if revisited).
...
11. **O(1) lowerdir disk; upperdir is owner-defined.** Overlayfs natural sharing — lowerdir is the layer stack, shared across calls/sessions, no copy. Upperdir is transient and its lifetime tracks the owning entity: **O(parallel calls + in-flight background jobs) in ephemeral mode** (each foreground call creates and destroys an upperdir at call boundaries; each `shell.launch` creates an upperdir that lives until `shell.reap`/`shell.cancel`), **O(mutations-per-session) in isolated mode** (upperdir grows until exit, then discarded; background jobs share the session upperdir). Planner B.5 caught the earlier wording that conflated lowerdir/upperdir.
...
Implements the per-call ephemeral pipeline and persistent isolated pipeline. Adds the unified tool-op dispatch and the lifecycle host API + agent-callable tools. Deletes the iws tool-op surface and the in/out-of-workspace branching. **Includes TOCTOU fix, host-path denylist, background-shell policy, OCC 4-helper source-tag threading.**
...
- `EphemeralPipeline.{launch,reap,poll,cancel}_background_job` — background-shell lifecycle owned by the `ShellJob` (overlay survives past `run_tool_call`); pipeline-local `_background_jobs: dict[str, ShellJob]` + `_handle_locks` reuse
- `EphemeralPipeline.startup_gc()` reaps orphan ephemeral background jobs from a prior daemon process (replaces global `daemon/service/shell_job.py::startup_gc`)
- `IsolatedPipeline.run_tool_call` + `enter` + `exit`; **`enter` rejects with `ephemeral_jobs_in_flight` if the agent has live ephemeral background jobs**; **`exit` drains in-flight background shells up to `grace_s`, reports `evicted_background_jobs`**
...
- **`sandbox/lifecycle/` host-side coroutines** (NEW top-level package — replaces the rejected `sandbox/api/lifecycle/` naming per Critic must-fix #6; `sandbox/api/` stays client-side only)
...
**Ships:**
- Updated happy_path tests using agent-level lifecycle tools (imports `sandbox.lifecycle.*`, NOT `sandbox.api.*`)
...
- `concurrency/` tier (OCC source-tag coalescing on all 4 helper sites; `_wire_handle` ordering invariant; **destroy-under-asyncio-interleaving**; **background-shell lifetime — ephemeral launches return job_id and survive `run_tool_call`, reap commits, cancel discards; iws drains at exit; enter rejects when ephemeral jobs in flight**; **10-step interleaved E2E**)
...
- `docs/sandbox/api_surface.md` — 11 sections including pass-through table, background-shell policy, `WorkspaceSession` deferral note, namespace-child-boundary diagram, `open_no_follow` per-component-walk explanation, deployment-precondition reference
- Updated blast-radius doc (reflects `sandbox/lifecycle/` + extracted `manager.py` modules), PLAN.md (9 tiers), CHANGELOG
...
- **Inserting host-side coroutines into `sandbox/api/`.** Rejected: `sandbox/api/` already houses client-side wire artifacts; same-package opposite-sides-of-wire is confusing. Resolved instead via new top-level `sandbox/lifecycle/` package (Critic must-fix #6).
...
- **Background shells in ephemeral mode with rejected `background=true`.** ~~Originally accepted~~ **Reversed after user directive.** Earlier rejection rationale ("per-call lifecycle would survive `run_tool_call` return; breaks Principle 1") narrowed the model unnecessarily. The corrected framing: overlay lifetime is owner-defined (Principle 1, revised). For background shell, the `ShellJob` owns the handle from `shell.launch` to `shell.reap`/`shell.cancel`, holding the lease + upperdir across the multi-RPC protocol. The pipeline tracks in-flight jobs in `_background_jobs: dict[str, ShellJob]`. This is essentially what `daemon/service/shell_job.py::ShellJob` already does today; the unified design just makes the lifetime ownership explicit at the pipeline boundary instead of leaving it global. Trade-offs accepted: ephemeral now has two overlay-lifetime shapes (per-call foreground and per-job background); Principle 1 loses its "fixed per-call" form. Mitigations: pipeline-owned registry (Q2), best-effort OCC at reap with conflict surfaced to agent (Q1), unbounded but gauge-observed (Q3), enter-iws rejects when ephemeral jobs in flight (Q4), `"overlay_capture"` source-tag for cross-path atomicity (Q5).
...
- Phase 2: substantive PR; new audit class; new agent-level tools; iws tool-op RPCs deleted atomically; thin daemon handlers; OCC source-tag plumbed through 4 helpers; OverlayHandle idempotency wired with per-handle `asyncio.Lock` TOCTOU fix; `tool_primitives.file_ops.open_no_follow` chokepoint enforced; host-path denylist landed; **background-shell pipeline ownership wired (ephemeral `_background_jobs` registry, `launch`/`reap`/`poll`/`cancel` methods, `startup_gc` orphan sweep, enter-iws rejection on in-flight ephemeral jobs)**; `sandbox/lifecycle/` package replaces rejected `sandbox/api/lifecycle/`. ≤8 atomic commits.
...
1. **Background-shell overlay leaked or destroyed before reap** (Phase 2 §3.1 / Planner D.1; revised after user directive):
   - **Fails when:** ephemeral mode permits `background=true`; the `ShellJob` owns the `OverlayHandle` from `shell.launch` through `shell.reap`. If `_background_jobs` registration races with `_destroy_with_lease_guard` from a sibling foreground call (sharing nothing — different handles, different leases — but reusing the same per-pipeline `_handle_locks` dict), a registration window can leave the handle owned-by-nobody for a few `await` ticks; conversely, if `shell.cancel` and `shell.reap` race, double-destroy or destroy-before-capture can fire.
   - **User-visible:** silent corruption: a `shell.reap` returns success but no `changed_paths` because the upperdir was destroyed before capture; or a `shell.launch` returns success but the next `shell.poll` returns `job_not_found` because the registry entry was overwritten.
   - **Leading indicator (specific signal):** Tier 8 soak's audit JSONL shows a `shell_job.launched` event without a matching `shell_job.reaped` OR `shell_job.cancelled` event within the job's max-timeout window; OR a `release_lease` event fires before the matching `shell_job.reaped` event for the same lease_id. Either ordering inversion is the canary.
   - **Mitigation enacted:** registration is atomic (lock acquired before the `await overlay.create`); `_destroy_with_lease_guard` is the single destroy chokepoint and is keyed on the same per-handle lock; `cancel`/`reap` race is resolved by check-cancelled-then-capture pattern; Phase 3 §6.6 sub-tests A–D pin launch-survives-call, reap-commits, cancel-discards, interleaved foreground+background. Without these, the soak audit signature WILL fire under load.
