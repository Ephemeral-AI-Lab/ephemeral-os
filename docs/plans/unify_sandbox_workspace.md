...
11. **O(1) lowerdir disk; upperdir is owner-defined.** Overlayfs natural sharing — lowerdir is the layer stack, shared across calls/sessions, no copy. Upperdir is transient and its lifetime tracks the owning entity: **O(parallel calls + in-flight background tasks) in ephemeral mode** (foreground calls create and destroy an upperdir at call boundaries; background calls hold the same per-call upperdir until the engine-owned asyncio task completes or is cancelled through `api.v1.cancel(request_id)`), **O(mutations-per-session) in isolated mode** (upperdir grows until exit, then discarded; background tasks share the session upperdir and are drained by `sandbox.isolated_workspace.lifecycle.exit_isolated_workspace`). Planner B.5 caught the earlier wording that conflated lowerdir/upperdir.
