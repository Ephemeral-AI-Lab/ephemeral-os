...
| `phase-04.6-agent-runtime-e2e_SPEC.md` | Completed | Test-only, additive: `packages/agent-runtime/e2e/` live e2e module over the configured `codex_coding_plan` entry (real SSE, real model) - agent-loop fundamentals (terminal submission, mocked-tool round-trip, `max_turns`, composed live auth rejection as `provider_error`), background supervisor + agent tools (subagent round-trip with `session_settled` wake + transcript read; submission guard with live-model recovery + `cancel_background_session` recording `model_cancelled`), interrupt/wake/steer (mid-tool interrupt with every `tool_use` answered, boundary steer redirect, steer waking an auto-wait park), and engine-direct `AgentEvent`/SSE golden ordering + interrupt-salvage restart over `outcome.llm`; the credential probe runs through the runtime's own `loadLlmClientRegistry`, so absent/stale codex auth clean-skips; no `src/` changes in any package; Rust `agent-core/` remains live and unchanged. | `pnpm exec vitest run --config vitest.e2e.config.ts packages/agent-runtime/e2e` live on this machine: 4 files, 11/11 green (~63s, `gpt-5.5` medium); `EOS_LLM_CLIENTS_PATH=/nonexistent` clean-skips all 11; `pnpm run check` unaffected (282 unit tests); coverage matrix + edge-case disposition in the spec §5-§6; `git diff --stat -- agent-core` stays empty. |
