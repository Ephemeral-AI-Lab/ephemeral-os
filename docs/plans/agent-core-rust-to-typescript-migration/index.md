...
| `phase-04.5-agent-runtime_SPEC.md` | Proposed | `@eos/agent-runtime` composition root (stub package already renamed): `AgentType`-parameterized `startRun` for main/subagent/advisor runs with ordered `initialMessages`, agent profile resolution, per-run inbox/supervisor wiring, run registry, agent tool runtime calls backed by `startRun`, JSONL transcript writer + event broadcaster, hook config loading, disposal cascade; external execution backends and backend-specific tools stay deferred; Rust `agent-core/` remains live and unchanged. | `pnpm run check` in `eos-agent-core/` including the Phase 04.5 §13 integration suite (subagent round-trip with auto-wait, advisor ask, disposal cascade, hook script over `transcript_path`, event broadcast isolation); `git diff --stat -- agent-core` stays empty. |
