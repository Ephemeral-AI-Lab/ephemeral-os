...
| `phase-04-tool-framework_SPEC.md` | Proposed | Additive `@eos/tool` package (flat Zod tool contract, per-call pipeline, batch executor, external-only hooks, five tool families with per-family service injection — folder per family, file per tool) plus an engine restructure (`tools.ts`/`tool-runner.ts` removed behind one injected `ToolExecutor` port; engine-owned generic `NotificationInbox` + `BackgroundSupervisor` with spawn-site session handles and dispose-on-finish; batch-result normalization, terminal exit with `submission`, auto-wait); contracts gain `AgentKind` + 4 branded ids + `ToolCallResult`; testkit gains the happy sandbox fakes; Rust `agent-core/` remains live and unchanged. | `pnpm run check` in `eos-agent-core/` including the Phase 04 §15 suite (pipeline order, relocated batch-runner suite, hook exit-code protocol with spawned scripts, supervisor lifecycle, `exec_command` promotion, executor assembly, engine normalization); ported Phase 03 loop suite stays green; `git diff --stat -- agent-core` stays empty. |
| `phase-04.5-agent-runtime_SPEC.md` | Proposed | `@eos/agent-runtime` composition root (rename of the `@eos/runtime` stub): per-run queue/supervisor wiring, run registry, real `AgentRunPort` via `startRun` recursion, JSONL transcript writer + event fan-out, hook config loading, disposal cascade; real sandbox/workflow backends stay deferred; Rust `agent-core/` remains live and unchanged. | `pnpm run check` in `eos-agent-core/` including the Phase 04.5 §13 integration suite (subagent round-trip with auto-wait, advisor ask, command promotion, disposal cascade, hook script over `transcript_path`); `git diff --stat -- agent-core` stays empty. |
