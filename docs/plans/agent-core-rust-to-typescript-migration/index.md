...
| `phase-04-tool-framework_SPEC.md` | Proposed | Additive `@eos/tool` package (flat Zod tool contract, per-call pipeline, external-only hooks, generic background supervisor keyed by native session refs, push notification queue, five tool families over ports) plus bounded engine seam growth (terminal-solo batch policy, terminal exit with `submission`, per-turn tool specs, `NotificationSource` + auto-wait); contracts gain `AgentKind` + 3 branded ids; testkit gains the happy sandbox fakes; Rust `agent-core/` remains live and unchanged. | `pnpm run check` in `eos-agent-core/` including the Phase 04 §15 suite (pipeline order, hook exit-code protocol with spawned scripts, supervisor lifecycle, `exec_command` promotion, toolset assembly); Phase 03 suite stays green; `git diff --stat -- agent-core` stays empty. |
