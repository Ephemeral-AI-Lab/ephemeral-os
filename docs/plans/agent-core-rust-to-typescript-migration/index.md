...
| `phase-04.6-agent-runtime-e2e_SPEC.md` | Baseline completed; expansion in progress | Test-only, additive: `packages/agent-runtime/e2e/` live e2e module over the configured `codex_coding_plan` entry (real SSE, real model). The baseline covers agent-loop fundamentals, background supervisor + agent tools, interrupt/wake/steer, and engine-direct `AgentEvent`/SSE ordering. The expanded E2E-12..60 matrix now adds large-scale tool pressure, hooks/notifications, subagent fanout, cancellation timing, recursive cancellation, and isolation targets; the first expanded fanout shard covers parallel `run_subagent` launches, child failure settlement, nested notification ownership, and `cancel_background_session` cancelling one child while a sibling completes. The credential probe runs through the runtime's own `loadLlmClientRegistry`; no `src/` changes in any package; Rust `agent-core/` remains live and unchanged. | Baseline observed: `pnpm exec vitest run --config vitest.e2e.config.ts packages/agent-runtime/e2e` live on this machine: 4 files, 11/11 green (~63s, `gpt-5.5` medium), and absent credentials clean-skip all 11. Expanded shard observed: `subagent-fanout.e2e.ts` 4/4 green (46.87s) and absent credentials clean-skip 4/4. Current local checks: `pnpm run check` (282 unit tests) and `git diff --check` clean. Remaining expanded rows are still target work before the expansion is complete. |
