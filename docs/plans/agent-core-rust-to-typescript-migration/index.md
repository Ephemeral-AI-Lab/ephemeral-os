...
| `phase-04-tool-framework_SPEC.md` | Proposed | Additive `@eos/tool` package (flat Zod tool contract, per-call pipeline, batch executor, external-only hooks, generic background supervisor keyed by native session refs, push notification queue, five tool families with per-family service injection) plus an engine tool-boundary restructure (`tools.ts`/`tool-runner.ts` removed behind one injected `ToolExecutor` port; batch-result normalization, terminal exit with `submission`, `NotificationSource` + auto-wait); contracts gain `AgentKind` + 3 branded ids + `ToolCallResult`; testkit gains the happy sandbox fakes; Rust `agent-core/` remains live and unchanged. | `pnpm run check` in `eos-agent-core/` including the Phase 04 §15 suite (pipeline order, relocated batch-runner suite, hook exit-code protocol with spawned scripts, supervisor lifecycle, `exec_command` promotion, executor assembly, engine normalization); ported Phase 03 loop suite stays green; `git diff --stat -- agent-core` stays empty. |
