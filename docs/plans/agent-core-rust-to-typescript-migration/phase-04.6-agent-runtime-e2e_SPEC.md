...
6. **Agent-tool scope is `run_subagent` + cancelling an agent run.**
   Cancellation of a background subagent run is exercised through
...
| E2E-09 | `interrupt-steer` :: steer wakes a parked run | 3-wake+steering, 2, 4-agent | park observed as a bare-text assistant turn with a background session (auto-wait); the steer wakes it; the model cancels the sleeper (`model_cancelled` in its transcript) and submits; `turns` stays bounded - parking consumed no provider calls | 04 §15.3, 03 §9 `waitForSteer`, 04.5 §13.5 |
