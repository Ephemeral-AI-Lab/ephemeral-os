...
- ✅ Live verification:
  ...
  passed (`1 passed in 47.42s` on the latest rerun). The generated Rust plugin
  report `.omc/results/rust-daemon-plugin-generic-20260602T025157Z-90949.json`
  ...
  harness process count went from `8` to `0`, connected PPC routes and services
  were empty, and `running_service_processes` was empty after cleanup.
- ✅ Durable benchmark refresh:
  `.omc/results/plugin-refresh-strategies-20260602T025157Z-90949.json` / `.md`
  recommend `workspace_snapshot_refresh`; p95 refresh `6.275 ms` vs
  `commit_to_workspace` p95 `4.593 ms`; raw workspace watch without
...
- 🟡 Remaining plugin scope: broader AV-10 LSP parity beyond the representative
  ...
  crash-recovery matrix beyond the covered status health probe, closed PPC
