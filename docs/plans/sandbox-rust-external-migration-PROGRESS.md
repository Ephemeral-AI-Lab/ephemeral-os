...
- ✅ Connected read-only plugin routes can now round-trip over the daemon PPC
  ...
  read-only calls serialize on that shared client instead of falling back to
  deferred dispatch, and failed PPC I/O removes the broken stream from
  connected-route status. Registered routes without a connected client still
  return the structured deferred response.
...
- ✅ Live Rust-runtime generic plugin coverage landed in the same pytest path.
  ...
  and publish `plugin.generic.runtime_bridge_apply` through its
  mounted-workspace callback,
  ...
  Latest artifact
  SHA:
  `dfb9334466e2a32365e944e74e3a809acdb68d39808ba7d595e6e61d52dfc960`.
- ✅ Live verification:
  ...
  passed (`1 passed in 49.31s` on the latest rerun). The generated Rust plugin
  report `.omc/results/rust-daemon-plugin-generic-20260602T033206Z-57542.json`
  ...
  harness process count went from `10` to `0`, connected PPC routes and services
  were empty, and `running_service_processes` was empty after cleanup.
- ✅ Durable benchmark refresh:
  `.omc/results/plugin-refresh-strategies-20260602T033206Z-57542.json` / `.md`
  recommend `workspace_snapshot_refresh`; p95 refresh `6.087 ms` vs
  `commit_to_workspace` p95 `4.567 ms`; raw workspace watch without
...
- 🟡 Remaining plugin scope: broader AV-10 LSP parity beyond the representative
  ...
  live separately,
  true operation-level out-of-order multiplexing if required, and a broader
  crash-recovery matrix beyond the covered status health probe,
