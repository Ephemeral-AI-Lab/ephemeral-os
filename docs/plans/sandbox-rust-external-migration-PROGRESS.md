...
| 3 — write/publish + shell/search + plugin (HIGH risk) | OCC/LayerStack publish, PPC | 🟡 direct `write_file`/`edit_file` publish flows through routed `eos-occ`; `api.v1.shell`/`glob`/`grep` overlay paths, background registry/control ops, PPC framing/no-OCC plugin edge, LayerStack squash/GC, and CP-4s live evidence are in place; direct-argv/no-shell is >70% faster, but string-shell CP-4s + plugin warm-server/live gates remain |
...
- Continue CP-4s optimization from `bench/phase3-rust-daemon-amd64.json`: raw-argv/no-shell `api.v1.shell` now clears the 70% target (`command_exec.run_command_s` p50 `15.379 ms` vs required `<=95.468 ms`) and host-wall p50/p95 is `29.957/30.910 ms`; string-shell no-op host-wall still beats Phase 1 (`311.042/325.259 ms` p50/p95 vs `361.567/373.759 ms`) and mount/host-delta/memory gates pass, but the `bash -lc` process segment p50 is `295.056 ms`, not the required `<=238.671 ms`. The next optimization must either reduce the string-shell-compatible segment without changing semantics, or explicitly redefine CP-4s around a documented raw-argv call path.
