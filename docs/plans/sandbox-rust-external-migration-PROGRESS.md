...
- Add a daemon memory profile to CP-4s: sample `/proc/<pid>/smaps_rollup` before load, during load, and after drain. Use daemon PSS as the primary "total daemon memory" metric; also report RSS, private memory (`Private_Clean + Private_Dirty`), non-gating `VmSize`, cgroup `memory.current`, and child shell/tool PSS/RSS separately if collected.
