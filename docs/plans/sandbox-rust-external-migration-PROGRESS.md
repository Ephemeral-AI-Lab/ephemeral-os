...
- Add the CP-4s shell hot-path gate before Phase 3 closeout: benchmark `api.v1.shell` no-op (`true` / `bash -lc true`), small-write, and representative search in `sweevo-dask__dask-10042:latest`; emit p50/p95/max plus per-sample phase timings for host dispatch/TCP, daemon dispatch, snapshot, overlay mount, process/shell, upperdir capture, OCC publish, audit, and cleanup.
- Add active daemon memory measurement to CP-4s: sample daemon-process RSS before load, during load, and after drain; report idle-before, active p50/p95, active peak, idle-after, and child shell/tool RSS separately if collected.
- Shell optimization pass criteria: no-op shell host-wall p50/p95 must be no worse than Phase 1 CP-2b (`361.567 ms` p50, `373.759 ms` p95 in the same image); the process/shell segment must improve by at least 25% versus Phase 1 `runner_tool_ms` p50 (`318.228 ms`) before we call it optimized; overlay mount p95 stays <= `5 ms`; TCP/daemon dispatch p95 stays <= `5 ms`; Rust daemon active RSS p95/peak must be no worse than the CP-0 Python daemon active RSS p95/peak under the same load, and idle-after must return within max(10%, 2 MiB) of idle-before; direct-argv/no-shell fast-path numbers, if added, are reported separately from shell-compatible semantics.
