...
- ✅ Live verification:
  ...
  passed (`1 passed in 73.77s` on the latest rerun). The generated Rust plugin
  report `.omc/results/rust-daemon-plugin-generic-20260602T022610Z-10996.json`
  ...
  before reply`, `plugin.generic.crash_probe` removed from connected routes,
  `crash_harness.state == "stopped"` with the same error recorded in
  `last_error`, hung-service timeout `expected_failure=true` with
  ...
  `recover_harness.restart_count == 1`, final manifest version `20`, final
  ...
  harness process count went from `7` to `0`, connected PPC routes and services
  were empty, and `running_service_processes` was empty after cleanup.
- ✅ Durable benchmark refresh:
  `.omc/results/plugin-refresh-strategies-20260602T022610Z-10996.json` / `.md`
  recommend `workspace_snapshot_refresh`; p95 refresh `9.818 ms` vs
  `commit_to_workspace` p95 `5.911 ms`; raw workspace watch without
...
- 🟡 Remaining plugin scope: broader AV-10 LSP parity beyond the representative
  ...
  `apply_code_action` + `format_document` path. Current Pyright live artifacts route document
  ...
  commands; positive execute-command coverage still requires a broader provider
  or harness that advertises commands if it becomes an AV-10 target,
  true operation-level out-of-order multiplexing if required, and broader crash recovery beyond
  the covered status health probe, closed PPC stream, PPC timeout fail-closed,
