...
- ✅ Live isolated exit inspection rerun passed in Docker/dask with then-current amd64 artifact SHA `6f94b650023186b9b4e282d20ad1bd0cd53b97c44759c313547c47f158ebecf6`: `bench/phase3t-rust-isolated-inspection-docker-20260601.json` (`run_id=local-f4d71d9fd6bb`) passed 51/51 checks. The target image lacked `ip` and `nft`; cgroup was writable; enter/status/list_open succeeded; finite command and PTY writes stayed private and unpublished; namespace veth/default route were visible; non-forced exit returned `active_pty_sessions`; force exit cancelled the real PTY and left no active/stale PTY ids; inspection reported `holder_pid=224`, `ns_fd_count=4`, `active_leases_after=0`, `mountinfo_reference_count_after=0`, `cgroup_exists_after=false`, scratch/upper/workdir removed, host veth removed, holder process gone, and the JSONL exit audit mirrored the inspection payload.
