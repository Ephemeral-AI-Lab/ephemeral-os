...
- ✅ `eos-daemon` now registers `api.plugin.ensure` and `api.plugin.status`.
  ...
  Focused verification: `cargo test -p eos-daemon plugin` (`5 passed`).
...
- ✅ Live verification:
  ...
  passed (`1 passed in 12.55s` on the latest rerun).
...
- 🟡 Remaining plugin scope: process-backed PPC behind registered `plugin.*`
  routes, self-managed callbacks through the same OCC writer, daemon
  remount/restart execution for `workspace_snapshot_refresh`, non-LSP dummy
  service parity, Pyright/LSP parity, and AV-10.
...
## DONE (verified 2026-06-01, all checks re-run independently)
