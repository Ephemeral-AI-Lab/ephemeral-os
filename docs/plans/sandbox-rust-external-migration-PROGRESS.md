...
**Last updated:** 2026-05-31 · **Phase:** 1 implementation landed locally; Linux musl build/clippy green; CP-2/AV runtime gates still pending in the dask container.
...
| 1 — ns-runner (fresh-ns) | `eos-runner` unshare→mount→exec | 🟡 Rust fresh-ns + raw mount implementation landed; container gates pending |
...
- ✅ `xtask package` implemented for `eosd-linux-{amd64,arm64}`: default builder is `rust-lld` (`cargo` with `RUSTFLAGS=-C linker=rust-lld`), with optional `cargo`/`cross`; writes binary-only `SHA256SUMS`, `protocol_version`, per-artifact JSON manifests, and optional minisign `.minisig` signatures. Both target artifacts package locally (`amd64` SHA `c81993538d4cfb6425e1a00f91d38d0a85dd07a1706907c3b07db6faf5a5629e`, `arm64` SHA `6edbe7bdc7bb4d6414b2b331d58857b1ce55bcf61bd391f34f34b36bdba716c6`) and both have local Docker upload/run verification.
...
- ✅ `bench/local-eosd-amd64-upload.json` captured the amd64 handoff: `sandbox/dist/eosd-linux-amd64` (683,328 bytes, static PIE) uploaded to `/tmp/eosd-local/eosd` in `8.121 ms`; readback SHA256 matched `c81993538d4cfb6425e1a00f91d38d0a85dd07a1706907c3b07db6faf5a5629e`; mode `0755`; direct exec returned `eosd 0.1.0`; target `rustc`/`cargo` absent.
- ✅ `bench/local-eosd-arm64-upload.json` captured the arm64 handoff: `sandbox/dist/eosd-linux-arm64` (597,848 bytes, static aarch64 ELF) uploaded to `/tmp/eosd-local/eosd` in `8.444 ms`; readback SHA256 matched `6edbe7bdc7bb4d6414b2b331d58857b1ce55bcf61bd391f34f34b36bdba716c6`; mode `0755`; direct exec returned `eosd 0.1.0`; target `rustc`/`cargo` absent.
...
- ✅ `eos-runner` fresh-ns mode now performs `setsid`, `unshare(NEWUSER|NEWNS)`, root uid/gid map setup, private mount propagation, overlay mount guard acquisition, shell command execution with cwd/env policy, timeout kill, and `RunResult` JSON construction.
...
- ⬜ Not yet claimed: CP-2a/CP-2b, AV-1 verb parity, and AV-3 cancel/teardown in the dask container. The implementation is build-verified, not runtime-gate closed.
...
- Runtime-gate the landed fresh path in the dask container: CP-2a (≥20× runtime-init vs CP-0) + CP-2b (no e2e regression) + AV-1 shell/fresh-ns result shape + AV-3 timeout/teardown. Toggle `EOS_SANDBOX_RUNTIME=rust` once the host dispatch fork exists.
- Remaining scope clarification: setns mode stays Phase 3.5, and broad daemon host dispatch stays Phase 2. Current `eosd ns-runner` is an executable request/response subcommand for the fresh path, not a full daemon runtime cutover.
