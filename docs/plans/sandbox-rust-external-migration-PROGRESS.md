...
**Last updated:** 2026-06-01 · **Phase:** 3 is closed at the structural core boundary: direct write/edit publish routes through `eos-occ`; shell/search overlay daemon paths, background registry/control ops, PPC framing/no-OCC plugin edge, LayerStack squash/GC, and CP-4s structural live evidence are in place. Deferred terminal/session implementation and high-risk parity/load gates are exported to Phase 3T: non-login Bash `exec_command`/`write_stdin_exec_command`, typed background/subagent controls, process-backed plugin PPC, CP-4/CP-5, AV-3/AV-4/AV-7/AV-10, and the §7 differential/property gates.
...
| 3T — terminal sessions + deferred Phase 3 gates | non-login Bash shell/session tools, typed background/subagent controls, plugin PPC execution, CP-4/CP-5/AV gates | ⬜ **next:** replace model-facing raw argv with `/bin/bash --noprofile --norc -c <cmd>` shell strings; implement PTY sessions and stdin/progress/cancel; finish process-backed plugin PPC; close CP-4t, CP-4, CP-5, AV-3, AV-4, AV-7, AV-10, and §7 differential/property |
...
### E. Phase 3T — ordered implementation and deferred gate closeout
1. **Replace model-facing shell with non-login Bash.** Implement shell-format command strings executed as `/bin/bash --noprofile --norc -c <cmd>` with no `/bin/sh`, login Bash, host-shell, or raw-argv fallback. Set the command environment explicitly, including the Dask fast path `PATH=/opt/miniconda3/envs/testbed/bin:/opt/miniconda3/bin:$PATH`.
2. **Implement finite `exec_command(tty=false)`.** Use the overlay path for every sample: LayerStack lease, overlay upper/work allocation and mount, Bash execution, descendant cleanup after Bash exits, capture, OCC publish/discard, cleanup, and lease release. Detached `nohup ... &` descendants must not escape the finite boundary.
3. **Implement PTY shell sessions.** `exec_command(tty=true)` keeps PTY, process group/cgroup, output ring, overlay lease, upper/work dirs, and cleanup state until terminal state. Add `write_stdin_exec_command`, `check_shell_progress`, and `cancel_exec_command` on top of that session lifecycle.
4. **Replace generic background tools with typed surfaces.** Retire model-facing `shell background=true`, `check_background_task_result`, `wait_background_tasks`, and `cancel_background_task`; keep the internal manager for ownership, heartbeats, timeout cleanup, reminders, shell sessions, subagents, and isolated-workspace lifecycle gates.
5. **Finish plugin PPC execution.** Implement process-backed warm-server spawn/round-trip, READ_ONLY out-of-process dispatch, WRITE_ALLOWED eosd-owned overlay+OCC wrapping, and self-managed plugin OCC callback over PPC. MF-1 remains load-bearing: plugin callbacks route through the same per-root OCC writer and storage lock as primary publishes.
6. **Close the deferred Phase 3 gates in the new order.** First CP-4t non-login Bash shell/session correctness and latency, then AV-3 process-tree/session cancellation, then AV-4 audit drop-free under shell load, then AV-10 PPC plugin parity, then AV-7 forward/back on-disk parity, then CP-4 throughput/contention with non-login Bash shell verbs plus plugin self-managed interleave, then CP-5 cache-lock contention, then §7 differential/property contention.
7. **Refresh architecture docs only where surfaces change.** If tool names, terminal-session lifecycle, background identifiers, or plugin-dispatch ownership change, update the smallest affected `docs/architecture` page alongside the implementation.
