...
**Last updated:** 2026-05-31 · **Phase:** 3 direct write/edit publish now has routed `eos-occ` validation; shell/search overlay daemon paths, background registry/control ops, PPC framing/no-OCC plugin edge, LayerStack squash/GC, and CP-4s live harness/evidence are in place; `api.v1.shell` raw argv is retained as the fast internal daemon primitive, while the model-facing terminal target is native container `/bin/bash` + PTY through `exec_command`/`write_stdin` with no `/bin/sh` fallback; plugin warm-server/live gates remain.
...
| 3 — write/publish + shell/search + plugin (HIGH risk) | OCC/LayerStack publish, PPC | 🟡 direct `write_file`/`edit_file` publish flows through routed `eos-occ`; `api.v1.shell`/`glob`/`grep` overlay paths, background registry/control ops, PPC framing/no-OCC plugin edge, LayerStack squash/GC, and CP-4s live evidence are in place; raw argv is the internal daemon hot path, and the model-facing terminal contract moves to native container `/bin/bash` + PTY via `exec_command`/`write_stdin`; plugin warm-server/live gates remain |
...
- ⬜ This is not Phase 3 closeout: CP-4s raw-argv live performance and load evidence is now green, and native Bash+PTY viability is measured, but full `exec_command`/`write_stdin` implementation, CP-4t proof, and CP-4/CP-5 proof are still open. Daemon cancel still needs AV-3 process-tree cleanup proof under live shell/background load. Plugin process-backed warm-server dispatch/self-managed callback, AV-7 forward/back on-disk parity, AV-10 plugin parity, and the §7 differential/property contention gates remain open.
...
- CP-4s raw-argv live evidence is green: the harness now seeds the overlay lower layer from live `/testbed`, proves `/opt/miniconda3/envs/testbed/bin/python` plus `conda run -n testbed python` can import Dask `2023.3.2` through `api.v1.shell` raw argv under the mounted overlay, and records 1/3/5/10 concurrent command waves. Fresh `bench/phase3-rust-daemon-amd64.json` run `local-c30761120c9b` cleared the 70% target (`command_exec.run_command_s` p50 `16.296 ms` vs required `<=95.468 ms`) and host-wall p50/p95 was `31.477/31.831 ms`; concurrent no-op host p95 was `31.783/42.414/71.444/108.253 ms` at 1/3/5/10, and concurrent unique `touch` host p95 was `32.088/43.931/74.553/140.142 ms`. This remains the internal daemon hot path and conda/Dask overlay proof, not the model-facing terminal parser.
- CP-4t is the next shell target and must not skip overlay: implement `exec_command({cmd, tty})` and `write_stdin({session_id, chars})` on top of native container `/bin/bash` with no `/bin/sh` or host-shell fallback. Every CP-4t latency and correctness sample must include LayerStack snapshot lease, overlay upper/work allocation and mount, capture, OCC publish/discard, cleanup, and lease release. `tty=false` is a blocking foreground command using `/bin/bash --noprofile --norc -c <cmd>` and releases the overlay lease after exit/capture/publish. `tty=true` opens a PTY-backed Bash session, returns a `tty_session_id` after the initial yield if still live, and keeps the overlay lease, upperdir/workdir, cgroup/process group, PTY master, and output ring until the whole process tree is terminal. Empty `write_stdin` polls; non-empty `write_stdin` writes to the PTY. Prompt/input reminders are observational from PTY output/idleness/process state, not command-string classification. Missing `/bin/bash` is a readiness/setup failure for the shell feature, never a fallback to `/bin/sh`.
...
- Shell optimization pass criteria: argv no-op host-wall p50/p95 must be no worse than Phase 1 CP-2b (`361.567 ms` p50, `373.759 ms` p95 in the same image); argv process segment must be at least 70% faster than Phase 1 `runner_tool_ms` p50 (`318.228 ms`); overlay mount p95 stays <= `5 ms`; TCP/daemon dispatch p95 stays <= `5 ms`; Rust daemon active PSS p95/peak must be no worse than the CP-0 Python daemon active PSS p95/peak under the same load, and idle-after PSS must return within max(10%, 2 MiB) of idle-before. If `smaps_rollup` is unavailable, RSS is the fallback gate and the report must say so.
- Gate: CP-4s argv hot-path + CP-4 (final-workspace-state hash) + the **§7 differential/property tests under contention** (NOT fixtures) + AV-1c byte-identity + AV-7 forward/back on-disk parity + AV-10 plugin parity. Needs the Python differential harness.
