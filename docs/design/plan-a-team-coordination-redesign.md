...
│   5. Main loop ends (stop or tool_call_limit)         │
│   6. Post-run re-prompt with posthook tools only      │
│      → submit_plan(tasks=[...], rationale="...")      │
│      → executor maps tool call → Plan domain object   │
└─────────────────────────────────────────────────────┘
```
...
submit_plan tool execution
  ...
  │
  └── 6. Return ToolResult
        (executor maps tool_input → Plan domain object)
```
...
Executor.run_forever()
  ...
  │   │     # query.py loop runs. Posthook tools NOT available.
  │   │     # Agent works, posts notes, reads context.
  │   │
  │   ├── 4. _run_post_run(task, defn, ctx)     ← ALWAYS RUNS
  │   │     │
  │   │     ├── runner.run() with posthook tools only
  │   │     │     Agent re-prompted to call exactly one terminal tool
  │   │     │     Executor maps tool_name/tool_input → domain object
  │   │     │
  │   │     └── Returns: Plan | Summary | Retry | Replan | Blocker
  │   │
...
Developer encounters transient failure
  → main loop ends
  → post-run re-prompt: agent calls request_retry(reason="sandbox timeout")
  → executor maps tool call → RetryRequest
...
Developer realizes task is mis-scoped
  → main loop ends
  → post-run re-prompt: agent calls request_replan(reason="auth is 3 services, need separate tasks")
  → executor maps tool call → ReplanRequest
  ...
  → Replanner's post-run: calls submit_replan(add_tasks=[...], cancel_ids=[...])
...
2. Post summary note to Task Center (for siblings to read)
3. Return `ToolResult`

Posthook tools are **not** available during the main query loop. They are
only exposed in the post-run re-prompt phase via `runner.run()`.
The executor maps `result.tool_name` / `result.tool_input` directly to
domain objects (`Plan`, `ReplanPlan`, `RetryRequest`, `ReplanRequest`,
`BlockerDeclaration`), so the tools do not write to metadata.

### 7.2 Post-Run Re-Prompt

After the main query loop completes (for any reason), the executor always
runs a post-run phase. It re-prompts the agent with **only** posthook tools
and the conversation history from the main loop. The agent must call exactly
one terminal tool. The executor maps the tool call to a domain object:

| `tool_name` | Domain type |
|-------------|-------------|
| `post_note` | `AgentResult(summary=...)` |
| `submit_plan` | `AgentResult(submitted_plan=Plan(...))` |
| `add_tasks` / `cancel_and_redraft` / `submit_replan` | `AgentResult(submitted_replan=ReplanPlan(...))` |
| `request_retry` | `RetryRequest` |
| `request_replan` | `ReplanRequest` |
| `declare_blocker` | `BlockerDeclaration` |

### 7.3 Post-Run Phase — Guaranteed Submission
...
```
                  await self.runner(defn, ctx)
                     ...
                     runner returns (any reason)
                             ...
                             ▼
                  ┌─────────────────────────────┐
                  │  _run_post_run(task,defn,ctx) │
                  │  ALWAYS RUNS                  │
                  │                               │
                  │  runner.run()        │
                  │    with posthook tools only    │
                  │         │                     │
                  │    agent calls one tool        │
                  │         │                     │
                  │    executor maps               │
                  │    tool_name → domain object   │
                  └─────────────────────────────┘
                             │
...
| Loop ended because | Post-run result |
|----|----|
| Model returned stop (no tool calls) | Post-run re-prompts → agent calls posthook tool |
| `tool_call_limit` exhausted | Same — post-run is a separate invocation |
| Runner exception | Executor catches, calls `dispatcher.fail()` |

**Rationale:** The post-run phase is a separate `runner.run()` call
with only posthook tools. The agent's main loop cannot call submission tools
(they are not registered). This cleanly separates the work phase from the
submission phase and ensures every completed agent produces a typed result.
