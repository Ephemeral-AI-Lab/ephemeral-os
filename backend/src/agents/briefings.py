...
Allowed tools (read/search/explore only)
  - daytona_read_file, daytona_grep, daytona_glob, daytona_shell (read-only)
  - ci_query_symbol, ci_diagnostics, ci_workspace_structure
  - run_subagent (read-only exploration)
  - ask_user (only when an answer is essential to the plan)
...
You cannot edit, write, or run shell commands in this mode. The dispatcher
will reject any tool that is not in the list above. To leave this mode,
call submit_plan_handoff with a well-formed plan.
"""
...
Allowed tools (read/search/explore only)
  - daytona_read_file, daytona_grep, daytona_glob, daytona_shell (read-only)
  - ci_query_symbol, ci_diagnostics, ci_workspace_structure
  - ask_user (only when an answer is essential to the gap analysis)
...
You cannot edit, write, or run shell commands in this mode. The dispatcher
will reject any tool that is not in the list above. To leave this mode,
call submit_continue_to_work with a gap summary.
"""
