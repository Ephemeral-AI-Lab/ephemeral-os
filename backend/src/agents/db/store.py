    ...
    def clone(self, source_name: str, new_name: str) -> AgentDefinitionRecord:
        with self._sf() as db:
            ...
            clone_record = AgentDefinitionRecord(
                ...
                permission_mode=source.permission_mode, max_turns=source.max_turns,
                tools=source.tools, disallowed_tools=source.disallowed_tools,
                toolkits=source.toolkits, skills=source.skills or [],
                mcp_servers=source.mcp_servers, hooks=source.hooks,
                color=source.color, background=source.background,
                initial_prompt=source.initial_prompt, memory=source.memory,
                isolation=source.isolation, subagent_type=new_name,
