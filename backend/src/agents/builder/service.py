...
class AgentBuilderService:
    ...
    def record_to_definition(record: AgentDefinitionRecord) -> AgentDefinition:
        return AgentDefinition(
            ...
            system_prompt=record.system_prompt, tools=record.tools,
            disallowed_tools=record.disallowed_tools, model=record.model,
            effort=record.effort, permission_mode=record.permission_mode,
            max_turns=record.max_turns, skills=record.skills or [],
            toolkits=record.toolkits or [], mcp_servers=record.mcp_servers,
            hooks=record.hooks, color=record.color, background=record.background,
            initial_prompt=record.initial_prompt, memory=record.memory,
            isolation=record.isolation, subagent_type=record.subagent_type,
            source="user",
        )
    ...
    def _record_to_response(record: AgentDefinitionRecord) -> AgentDefinitionResponse:
        return AgentDefinitionResponse(
            ...
            effort=record.effort, permission_mode=record.permission_mode,
            max_turns=record.max_turns, tools=record.tools,
            disallowed_tools=record.disallowed_tools, toolkits=record.toolkits,
            skills=record.skills or [], mcp_servers=record.mcp_servers,
            hooks=record.hooks, color=record.color, background=record.background,
            initial_prompt=record.initial_prompt, memory=record.memory,
            isolation=record.isolation, subagent_type=record.subagent_type,
    ...
    def create_agent(self, data: AgentDefinitionCreate) -> AgentDefinitionResponse:
        ...
        record = AgentDefinitionRecord(
            ...
            effort=data.effort, permission_mode=data.permission_mode,
            max_turns=data.max_turns, tools=data.tools,
            disallowed_tools=data.disallowed_tools, toolkits=data.toolkits,
            skills=data.skills or [], mcp_servers=data.mcp_servers,
            hooks=data.hooks, color=data.color, background=data.background,
            initial_prompt=data.initial_prompt, memory=data.memory,
            isolation=data.isolation, subagent_type=data.subagent_type,
