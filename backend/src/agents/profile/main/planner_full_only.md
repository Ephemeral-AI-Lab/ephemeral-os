...
**Partial planning is disabled in this goal.** A caller attempt in this goal's ancestry has already submitted a partial plan, so the only valid terminal in this attempt is `submit_full_plan`. Plan an attempt whose tasks fully cover `Current Iteration`. You cannot defer remainder work via a continuation goal. If the iteration goal feels too large, narrow scope inside `Current Iteration`'s bounds and submit a full plan for the narrowed slice; you do not control later iterations.
