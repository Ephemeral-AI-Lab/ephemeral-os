...
def _read_unshare_grandchild_pid(unshare_pid: int) -> int | None:
    try:
        text = Path(f"/proc/{unshare_pid}/task/{unshare_pid}/children").read_text(encoding="utf-8")
    except OSError:
        return None
    tokens = text.split()
    if not tokens:
...
class _LinuxNamespaceRuntime:
    ...
    def spawn_ns_holder(self, handle: IsolatedWorkspaceHandle, *, setup_timeout_s: float) -> int:
        ...
        proc = subprocess.Popen(
            [
                ...
                sys.executable,
                "-m",
                "sandbox.isolated_workspace.scripts.ns_holder",
                str(r_holder),
                str(c_holder),
            ],
    ...
    async def configure_dns(self, handle: IsolatedWorkspaceHandle, *, fallback_dns: str) -> bool:
        user_fd = handle.ns_fds.get("user")
        mnt_fd = handle.ns_fds.get("mnt")
        if user_fd is None or mnt_fd is None:
            return False
    ...
    def signal_net_ready(self, handle: IsolatedWorkspaceHandle, *, setup_timeout_s: float) -> None:
...
def _read_linux_memavailable_kb() -> int:
    try:
...
def _expect_line(fd: int, prefix: bytes, *, timeout_s: float) -> None:
    ...
    while b"\n" not in buf:
        if time.monotonic() > deadline:
            raise IsolatedWorkspaceError(
                "setup_timeout",
                f"ns_holder did not signal {prefix!r}",
        ...
        if not chunk:
            raise IsolatedWorkspaceError(
                "setup_failed",
                "ns_holder closed pipe before signaling",
            )
        buf += chunk
    if not buf.startswith(prefix):
        raise IsolatedWorkspaceError(
            "setup_failed",
            f"unexpected ns_holder signal: {buf!r}",
        )
