...
async def shell(sandbox_id: str, request: ShellRequest) -> ShellResult:
    ...
    if _is_read_only_pipeline(request.command) and request.stdin is None:
...
def _is_read_only_pipeline(command: str) -> bool:
...
def _contains_option_with_value(tokens: tuple[str, ...], options: frozenset[str]) -> bool:
    ...
    while idx < len(tokens):
        ...
        if token in options:
            return True
        if "-o" in options and token.startswith("-o") and token != "-":
            return True
