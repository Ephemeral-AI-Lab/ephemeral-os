...
class StorageUnavailable(Exception):
    """Raised when the OCC state directory cannot be created or written."""
...
def _confine(state: Path, name: str) -> Path:
    ...
    if target == state_real:
        raise StoragePathEscape(f"target {target} resolves to the state dir itself")
    if state_real not in target.parents:
        raise StoragePathEscape(f"path {target} escapes state dir {state_real}")
    return target
