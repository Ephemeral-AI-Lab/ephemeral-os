...
class ShellJobRegistry:
    ...
    def _run_strategy(self, job: ShellJob) -> None:
        try:
            ...
            process_result = run_in_namespace(
                spec=spec,
                request=job.request,
    ...
    def _build_layout(self, job: ShellJob) -> LayerPathsLayout:
        scratch_root = command_exec_scratch_root(Path(job.storage_root))
        handle = job.handle
        if handle.layer_paths is None:
            raise RuntimeError("background shell snapshot did not include layer paths")
        layer_storage_root = ""
...
def shell_launch(args: dict[str, Any]) -> dict[str, Any]:
    ...
    result = get_shell_job_registry().launch(
        request=request,
        overlay=overlay,
...
def shell_poll(args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        snapshot = get_shell_job_registry().poll(job_id)
    except ShellJobNotFound:
...
def shell_cancel(args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        result = get_shell_job_registry().cancel(job_id, reason=reason)
    except ShellJobNotFound:
        return _job_not_found(job_id)
...
async def shell_reap(args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        payload = await get_shell_job_registry().reap(
            job_id,
...
def shell_metrics(args: dict[str, Any]) -> dict[str, Any]:
    del args
    snapshot = get_shell_job_registry().metrics()
    return {
        "success": True,
