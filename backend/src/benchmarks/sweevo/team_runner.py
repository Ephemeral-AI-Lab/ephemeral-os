...
def _build_root_prompt(instance: SWEEvoInstance, repo_dir: str) -> str:
    ...
    return (
        ...
        f"- release notes are intentionally omitted from the root planner prompt; rely on "
        f"the declared skills plus live repository evidence instead of replaying the changelog.\n"
        f"- Per-layer plan cap: {max_plan_size} items.\n"
        f"- Stable SWE-EVO workflow policy lives in the declared skills for this run.\n"
        f"- Recommended first-ready frontier cap: the submitted root plan must stay within the "
        f"runtime cap of {max_plan_size} total tasks.\n"
        f"- Use that runtime cap as a budget, not as a fixed graph recipe; that does not mean "
        f"the whole submitted graph should stop at that many items.\n"
        f"- When wide surface remains, do not hand the whole remaining surface to only the "
        f"initial developers; each distinct ready owner slice must still receive its own "
        f"developer lane or expandable child planner.\n"
        f"- Developers must not inspect dependency/version metadata unless live failure evidence "
        f"proves the benchmark issue is actually in dependency or packaging state.\n"
