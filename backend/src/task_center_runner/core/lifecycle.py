...
``run_pipeline`` (Phase 4) calls four hooks on a single ``LifecycleHooks``
object:
...
- ``after_run(ctx, report)`` — once after the recorder is disposed; may
  mutate ``report.lifecycle_extras`` (e.g. SweevoLifecycle writes
  ``sweevo_result``; ScenarioLifecycle writes ``hook_results``)
