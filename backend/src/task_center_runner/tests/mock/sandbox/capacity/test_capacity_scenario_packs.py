...
def test_planner_validation_capacity_scenarios_encode_rejection_cases() -> None:
    ...
    blank_deferred_goal = SCENARIO_REGISTRY[
        "planner_validation.blank_deferred_goal"
    ]().planner_response(_ctx())
    assert blank_deferred_goal.tool.name == "submit_planner_outcome"
    assert blank_deferred_goal.args["deferred_goal_for_next_iteration"].isspace()
