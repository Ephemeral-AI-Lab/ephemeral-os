...
The design property: ``prepare_workspace_snapshot(materialize=False)``
returns layer paths that point at the SAME shared layer-stack files for
every concurrent reader. If a future PR flips ``materialize=True``, each
...
``manifest_root_hash`` AND the daemon emits ``materialize=false`` in
every enter event.
"""
...
async def test_lowerdir_layer_paths_shared_across_concurrent_handles(
    ...
    try:
        ...
        # Every enter event must carry materialize=false (snapshot-share tripwire).
        ...
        for ev in enter_events:
            ...
            assert payload.get("materialize") is False, ev
    finally:
        for agent in _AGENTS:
