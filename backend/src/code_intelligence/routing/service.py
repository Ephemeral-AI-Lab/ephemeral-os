...
"""
    ...
    def _rename_symbol_plans_many_same_file_fast(
        ...
        base_by_path = self._cached_content_or_read_many(
    ...
    def _cached_content_or_read_many(
        self,
        paths: Sequence[str],
    ) -> dict[str, tuple[str, bool]]:
        cached_content = getattr(self.symbol_index, "cached_content", None)
        tree_cached_content = getattr(self.tree_cache, "cached_content", None)
        base_by_path: dict[str, tuple[str, bool]] = {}
        missing: list[str] = []
        for path in paths:
            content = cached_content(path) if callable(cached_content) else None
            if content is None and callable(tree_cached_content):
                content = tree_cached_content(path)
            if content is None:
                missing.append(path)
            else:
                base_by_path[path] = (content, True)
        if missing:
            base_by_path.update(self._content.read_many(missing, allow_missing=True))
        return base_by_path
