...
from code_intelligence.routing.telemetry import record_overlay_op
...
class OverlayAuditor:
    ...
    async def _ensure_script_uploaded(self, sandbox: Any) -> None:
        ...
        async with self._script_upload_lock:
            ...
            target = posixpath.join(_RUN_DIR_PREFIX, "overlay_run.py")
            setup_cmd = (
                f"mkdir -p {shlex.quote(_RUN_DIR_PREFIX)} && "
                f"python3 -c {shlex.quote('import base64,sys,pathlib; pathlib.Path(sys.argv[1]).write_bytes(base64.b64decode(sys.argv[2]))')} "
