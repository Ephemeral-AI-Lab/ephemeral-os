...
- :class:`code_intelligence.atlas.store.AtlasStore` — transactional SQLAlchemy store.
- :class:`code_intelligence.atlas.model.ProjectAtlasRecord` / ``ProjectAtlasChunkRecord``
  — ORM tables registered on ``db.base.Base``.
- :mod:`code_intelligence.atlas.freshness` — ledger + content-hash staleness checks.
- :mod:`code_intelligence.atlas.identity` — stable ``project_key`` derivation.
...
def __getattr__(name: str):
    if name == "project_key_for":
        return import_module("code_intelligence.atlas.identity").project_key_for
    if name in {
        "changes_since_chunk",
        "hash_paths_under",
        "is_chunk_fresh",
        ...
        return getattr(import_module("code_intelligence.atlas.freshness"), name)
    if name in {"AtlasChunk", "AtlasStore"}:
        return getattr(import_module("code_intelligence.atlas.store"), name)
    if name in {"ProjectAtlasChunkRecord", "ProjectAtlasRecord"}:
        return getattr(import_module("code_intelligence.atlas.model"), name)
