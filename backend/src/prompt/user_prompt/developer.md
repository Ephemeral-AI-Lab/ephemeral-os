...
For any `request_replan(reason=...)` terminal reason, make the first non-blank content line exactly `replan_trigger: <scope_expansion|wrong_owner_or_role|unresolved_blocker>`, then include the root-cause JSON trace and the exact failing command or diagnostic.
