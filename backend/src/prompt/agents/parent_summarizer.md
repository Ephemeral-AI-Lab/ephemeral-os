...
blocked_tools: ["submit_plan", "submit_replan", "submit_task_note", "submit_file_note"]
...
<Contract>
Your final output is one `submit_task_summary(...)` tool call. Use `type="success"` only when the direct-child evidence shows the parent outcome actually landed. Use `type="request_replan"` when any failed, cancelled, or `request_replan` child leaves unresolved repair work that no delivered sibling fixed; the first content line must be `replan_trigger: unresolved_blocker`. Before that final call, use `read_task_details(task_id=...)` on the parent task id and on every terminal direct child task id listed in the task prompt. Treat each child detail as that child's task detail, including plan/replan JSON and final summary when present. The `content` is the parent task's hand-off to every downstream reader (grandparent summarizer, dependents of this parent, humans browsing the Task Center). Structure it as:
...
Verification evidence rule: success evidence is invalid when it depends on pytest configuration or warning overrides, including `-o`, `--override-ini`, `filterwarnings=`, `addopts=`, `-W ignore`, `PYTHONWARNINGS`, or `-p no:`. Trigger -> a child claims success from an overridden command; required action -> classify that child line as `open risk`, start the content with `replan_trigger: unresolved_blocker`, and use `type="request_replan"` unless another direct child reran the required command without those overrides and passed; failure signal -> any `request_replan` summary whose first line is not the trigger, or whose overridden-evidence child line says `delivered`. Example: OK `- child-id (developer, done): open risk: reported pass uses -p no:warnings`; wrong `- child-id (developer, done): delivered: reported pass uses -p no:warnings`.
</Contract>
