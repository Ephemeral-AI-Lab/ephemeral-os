...
from ephemeralos.memory import (
    add_memory_entry,
...
from ephemeralos.services.session_storage import get_project_session_dir, load_session_snapshot
from ephemeralos.skills import load_skill_registry
from ephemeralos.tasks import get_task_manager

if TYPE_CHECKING:
    from ephemeralos.state import AppStateStore
...
def create_default_command_registry() -> CommandRegistry:
    ...
    async def _version_handler(_: str, context: CommandContext) -> CommandResult:
        del context
        try:
            version = importlib.metadata.version("ephemeralos")
        except importlib.metadata.PackageNotFoundError:
            version = "0.1.0"
        return CommandResult(message=f"EphemeralOS {version}")
    ...
    async def _resume_handler(args: str, context: CommandContext) -> CommandResult:
        from ephemeralos.services.session_storage import list_session_snapshots, load_session_by_id
    ...
    async def _init_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        if not claudemd.exists():
            claudemd.write_text(
                ...
                "- Use EphemeralOS tools deliberately.\n"
        ...
        for relative, content in (
            (
                ...
                "# Project EphemeralOS Config\n\nThis directory stores project-specific EphemeralOS state.\n",
            ),
            (
        ...
        if not created:
            return CommandResult(message="Project already initialized for EphemeralOS.")
    ...
    async def _login_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        save_settings(settings)
        return CommandResult(message="Stored API key in ~/.ephemeralos/settings.json")
    ...
    async def _privacy_settings_handler(_: str, context: CommandContext) -> CommandResult:
        ...
        lines = [
            ...
            "- storage: local files under ~/.ephemeralos and project .ephemeralos",
        ]
        return CommandResult(message="\n".join(lines))
    ...
    async def _release_notes_handler(_: str, context: CommandContext) -> CommandResult:
        ...
        return CommandResult(
            message=(
                "# Release Notes\n\n"
                "- Web UI is now the default `oh` interface.\n"
    ...
    async def _output_style_handler(args: str, context: CommandContext) -> CommandResult:
        tokens = args.split(maxsplit=1)
        if not tokens or tokens[0] == "show":
            current = context.app_state.get().output_style if context.app_state is not None else "default"
            return CommandResult(message=f"output-style: {current}")
        if tokens[0] != "set" or len(tokens) != 2:
            return CommandResult(message="Usage: /output-style set STYLE")
        output_style = tokens[1]
        available = {style.name for style in load_output_styles()}
        if output_style not in available:
            return CommandResult(message=f"Unknown output style: {output_style}")
        if context.app_state is not None:
            context.app_state.set(output_style=output_style)
        return CommandResult(message=f"Output style set to {output_style}")

    async def _keybindings_handler(_: str, context: CommandContext) -> CommandResult:
        state_bindings = context.app_state.get().keybindings if context.app_state is not None else None
        keybindings = state_bindings if state_bindings else load_keybindings()
        if not keybindings:
            return CommandResult(message="No keybindings configured.")
        return CommandResult(
            message="\n".join(
                f"{key}={value}" for key, value in sorted(keybindings.items())
            )
        )

    registry.register(SlashCommand("help", "Show available commands", _help_handler))
    registry.register(SlashCommand("exit", "Exit EphemeralOS", _exit_handler))
    registry.register(SlashCommand("clear", "Clear conversation history", _clear_handler))
    registry.register(SlashCommand("version", "Show the installed EphemeralOS version", _version_handler))
    ...
    registry.register(SlashCommand("init", "Initialize project EphemeralOS files", _init_handler))
    ...
    registry.register(SlashCommand("keybindings", "Show configured keybindings", _keybindings_handler))
    ...
    registry.register(SlashCommand("release-notes", "Show recent EphemeralOS release notes", _release_notes_handler))
    ...
    registry.register(SlashCommand("output-style", "Set output style", _output_style_handler))
    return registry
