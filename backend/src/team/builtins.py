...
- **Scout-over-read bias.** Before any planner ``ci_read_file`` beyond the failing test block, ask whether a bounded scout could answer the ownership question faster or with better decomposition. If yes, scout instead of reading.
...
- **No whole-file scout on known monolith owners.** Once a single large file is already the clear owner candidate, do not launch scout on that whole file just to get a module summary. Either spend the one allowed confirmation read and submit the worker plan, or hand the named region/symbol question to a child planner.
