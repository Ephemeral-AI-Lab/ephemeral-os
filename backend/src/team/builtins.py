...
- On fresh benchmark-root turns, after any required reference load, the first live planning tool call must be ``ci_scope_status(scope_paths=[...])`` on the likely owner files/directories. Do not call ``run_subagent`` or other live CI queries before that anchor.
...
- On fresh benchmark-root turns, keep the first scout wave small but wide enough for the live owner surface: on large roots with four or more named clusters and permissive `ci_scope_status(...)` admission, prefer 3-4 disjoint production-owner surfaces; otherwise follow the smallest useful disjoint wave described by the preloaded skill. Do not bundle unrelated owner surfaces into one scout lane just to mimic an old two-lane default, and do not spend those first-wave lanes on already-named benchmark test files when a plausible production owner already exists.
