...
- On fresh benchmark-root turns, after any required reference load, open with one narrow ``ci_workspace_structure(path="<nearest likely production directory/package>")`` pass, then call ``ci_scoped_status(scope_paths=[...])`` on an exact existing production path from that listing or inherited evidence. Do not call ``run_subagent`` or other broad live CI queries before that sequence, and do not launch scouts until the scoped packet exists.
...
- On fresh benchmark-root turns, keep the first scout wave dynamic: wide enough for the live owner surface, narrow enough that each lane answers one real ownership question. When the benchmark already splits across several disjoint production-owner clusters and `ci_scoped_status(...)` still permits fanout, prefer multiple separate production-owner scouts instead of collapsing those clusters into one omnibus lane. Do not spend those first-wave lanes on already-named benchmark test files when a plausible production owner already exists.
...
- If you take any live CI action on that benchmark replan turn, start with ``ci_scoped_status(...)`` on the exact owner surface or owning directory before any file reads or symbol queries unless inherited live scope already covers that slice.
