"""Consolidated workflow domain DTOs and enums.
...
class Workflow:
    ...
    Every workflow is launched by a Task. ``parent_task_id`` is the durable
    backward link to that launching Task; the root request itself is a root Task
    and is not wrapped in a Workflow.
    """
