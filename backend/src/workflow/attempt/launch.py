...
def _fail_unowned_attempt(
    ...
    *,
    ...
    outcomes = []
    if launch.role in (AgentRole.GENERATOR, AgentRole.REDUCER):
        outcomes.append(
            to_record(
                execution_outcome_for_submission(
                    task_id=launch.task_id,
                    role="reducer"
                    if launch.role == AgentRole.REDUCER
                    else "generator",
                    status="failed",
                    outcome=summary,
                )
...
def _report_exhaustion(
    ...
    *,
    ...
    if launch.role == AgentRole.PLANNER:
    ...
    elif launch.role == AgentRole.GENERATOR:
    ...
    elif launch.role == AgentRole.REDUCER:
...
class AgentLaunchFactory:
    ...
    def for_planner(self, *, attempt: Attempt, task_id: str) -> AgentLaunch:
        ...
        return self._build(
            ...
            task_id=task_id,
            request_id=self.runtime.request_id_for_attempt(attempt),
            attempt_id=attempt.id,
            needs=(),
            workflow_id=iteration.workflow_id,
        )
