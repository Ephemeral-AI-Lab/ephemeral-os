...
- Must keep commands repo-root-relative. Inside `daytona_codeact`, run them as direct `result = shell("...", timeout=N)` calls; never prepend guessed `cd /workspace`, `cd /home/user`, use Python process wrappers, or append `2>&1`.
