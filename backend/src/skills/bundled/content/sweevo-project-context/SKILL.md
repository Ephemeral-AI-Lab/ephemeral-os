...
- **Exact existing paths only.** Planner and replanner payloads must stay anchored on exact existing checkout-relative paths from the live repo or inherited evidence. If a cited file path is missing in the live checkout, treat that as an owner-map mismatch and re-anchor before assigning work.
...
- At the submitted benchmark root, keep at most two validators total. Pair them with concrete developer lanes only; residual child-planner branches emit their own validators after decomposition.
- Child benchmark plans inherit that same cap. When a residual branch decomposes into three or more concrete developer lanes, keep validator items to at most two total and cover the remaining concrete lane(s) with a sibling validator command or later branch-level guardrail instead of emitting a third validator.
...
- If that adjacent production/export owner sits outside the lane's assigned `owned_files`, the developer should report `scope_mismatch` with the exact missing import/export and likely owner path instead of editing the test file or broadening the lane.
...
- The default large-root benchmark shape for this repo is: two critical developer lanes, one residual child planner lane, and one validator lane. Smaller roots may still use one dominant developer lane plus one residual child planner lane and one validator lane. Flatten the residual lane into direct developers only when each residual owner is already bounded without more planning.
