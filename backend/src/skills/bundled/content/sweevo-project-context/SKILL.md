...
- **Fresh benchmark roots should stay live-first.** On the first root planning pass, prefer `ci_scope_status(scope_paths=[...])` plus fresh scouts over `atlas_lookup`. Use Atlas only on resume/replan or after the current turn already established a live scout-backed owner map and still needs cross-run structural reuse.
- **Fresh benchmark roots ground live ownership before scouting.** After any required reference load, the opening live planning step should be either `ci_scope_status(scope_paths=[...])` on the likely owner files/directories, or one narrow `ci_workspace_structure(path="<nearest likely production directory/package>")` pass when the exact production path is still only a hypothesis. After that narrow structure pass, call `ci_scope_status(...)` on an exact existing production path before broader queries or scout launch.
...
- At any submitted benchmark plan level, keep validators paired with the concrete developer lanes they actually verify and stay within the active runtime validator cap for that plan. Child benchmark plans inherit that same cap. Residual child-planner branches should usually emit their own validators after decomposition, except for the rare planner-submission-artifact check.
