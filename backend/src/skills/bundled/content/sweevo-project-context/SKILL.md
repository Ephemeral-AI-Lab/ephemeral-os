...
- At any submitted benchmark plan level, keep validators paired with the concrete developer lanes they actually verify. Child benchmark plans should keep validators branch-local and risk-weighted instead of emitting one validator per developer or recreating an umbrella validation layer. Residual child-planner branches should usually emit their own validators after decomposition, except for the rare planner-submission-artifact check.
- An expandable child planner is not a readiness barrier for descendant code verification. If a validator command names residual benchmark tests or runtime checks that the child branch will decompose, that validator belongs inside the child branch or behind the concrete descendant developers, not directly on the expandable planner item.
