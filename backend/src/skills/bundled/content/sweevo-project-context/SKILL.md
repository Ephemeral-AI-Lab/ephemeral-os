...
- **Benchmark commands should stay repo-root-relative.** Prefer `python -m pytest dask/...` and similar checkout-relative commands. Do not emit `cd /home && ...`, `cd /workspace && ...`, or other absolute-root wrappers unless the command truly needs a real subdirectory under the injected repo root.
...
- **Fresh benchmark roots should stay live-first.** On the first root planning pass, prefer one narrow `ci_workspace_structure(...)` pass followed by `ci_scoped_status(scope_paths=[...])` plus fresh scouts over `atlas_lookup`. Use Atlas only on resume/replan or after the current turn already established a live scout-backed owner map and still needs cross-run structural reuse.
- **Fresh benchmark roots ground live ownership before scouting.** After any required reference load, start with one narrow `ci_workspace_structure(path="<nearest likely production directory/package>")` pass, then call `ci_scoped_status(scope_paths=[...])` on an exact existing production path from that listing before broader queries or scout launch.
...
- When a guessed owner stem is close but wrong, recover the exact live sibling filename instead of preserving the alias. Do not shorten `compatibility.py` to `compat.py`, `configuration.py` to `config.py`, or similar prompt-shaped guesses when live structure already names the real file.
- Prompt-named benchmark test files are symptom evidence, not default implementation ownership. Do not emit root developer lanes whose `owned_files` contain only those tests unless live evidence says the slice truly belongs to test/support infrastructure. Otherwise keep the test path in `owned_failures`, anchor the lane on a confirmed production/export surface, or park the unresolved cluster behind a child planner.
...
- File-level fallback applies to retry commands too. If a child planner or worker payload only inherited a benchmark test file plus a cluster label or prose hint, keep `owned_failures`, `reproduction`, `verification`, and validator `verify` on that exact test file path until a live runtime artifact confirms the exact `::pytest_node` suffix byte-for-byte.
- Representative node ids are evidence, not automatic retry commands. When a planner keeps a deduped subset of exact parametrized ids in `owned_failures`, it should still keep `reproduction`, `verification`, and validator `verify` file-scoped until a live worker packet proves that one concrete node still collects in the current checkout.
- Exact-file benchmark evidence should stay exact-file for validation too. Do not widen a validator from `dask/tests/test_compatibility.py` to a repo-wide keyword sweep like `pytest dask/ -k compatibility` unless live evidence proves that broader slice is the smallest meaningful regression net and it will not introduce unrelated collection/runtime surfaces.
- If a child planner inspects the referenced test file and the inherited exact pytest node is absent there, treat that node as stale benchmark evidence. Keep the branch on the exact test file path or report `benchmark_surface_mismatch`; do not invent a same-stem production helper, public symbol, or replacement node from the missing test name.
- At any submitted benchmark plan level, keep validators paired with the concrete developer lanes or whole child branches they actually verify. Child benchmark plans should keep validators branch-local and risk-weighted instead of emitting one validator per developer or recreating an umbrella validation layer. A validator may depend on an expandable child planner only when you intentionally want a whole-branch barrier on that child branch's full descendant subtree.
- A dependency on an expandable child planner now waits for the full descendant subtree rooted at that planner item, not just decomposition. If a validator command names residual benchmark tests or runtime checks that the child branch will decompose, decide explicitly whether you want that whole-branch barrier or narrower validators behind the concrete descendant developers.
...
- If the natural root task set would crowd the runtime `max_plan_size`, regroup adjacent sibling work into one or more expandable child-planner items until the submitted level is comfortably within budget.
...
- For broad SWE-EVO roots, choose the graph shape dynamically from the mapped owner surface: sometimes that is several direct developer lanes, sometimes one or more downstream child planners, and validators only where they materially reduce uncertainty. Do not force a fixed lane recipe.
...
- When one dominant owner cluster is already mapped and the remaining named failures span several smaller modules, keep the root graph decomposed by ownership: use however many concrete residual developer lanes and downstream expandable child planners the live owner graph warrants. Do not keep scouting just to flatten every residual into the root plan.
- For residual child-planner branches, keep `owned_files`, `owned_failures`, and `expansion_hint` grounded on exact existing checkout-relative paths where live evidence already confirms them. If the source owner is still unresolved, say so explicitly and anchor the branch on the exact failing test file plus the confirmed candidate directory. Do not guess nonexistent production files such as `utils_dataframe.py` or truncated paths such as `dask/dataframe/s`.
- Residual child planners must apply the same ownership rule as root planners: an existing benchmark `tests/...` path is not, by itself, a child developer lane's edit surface. Keep benchmark tests in `owned_failures`; use `owned_files` only for confirmed production/support owners or, when still unresolved, a confirmed candidate package/directory.
- Treat `owned_files` as a grounded edit surface, not a hypothesis bucket. Keep missing guessed owners out of `owned_files`. unresolved production guesses belong in `expansion_hint` or `notes` until live structure confirms an exact existing path.
...
- Child planners follow the same rule for near-stem aliases: if live structure resolves `compatibility.py`, do not keep planning against guessed siblings like `compat.py` or `_compat.py`.
...
- A passing `py_compile`, clean LSP result, or source readback is not a passing retry target. If the exact runtime retry command never turns green, report the blocker; do not declare the lane fixed from structural checks alone.
...
- A missing named pytest node is terminal for that developer lane. Do not keep debugging the broader test file, run `-k` sweeps for nearby failures, or substitute a different failing test from the same module unless a live planner/replanner packet has already retargeted the work.
- A missing pytest node name is not design guidance for new product code. If the payload names `test_dataframe_overlap` and no such node exists, do not invent `dataframe_overlap()` or a similar helper from that stale node stem unless live runtime evidence separately names that symbol as missing.
...
- If a product bug manifests as pytest import-time warning/config parsing fallout, keep the developer lane on the product import/export surface first. Do not retarget the lane to `setup.cfg`, `pytest.ini`, `pyproject.toml`, or warning filters unless live evidence proves the config file itself owns the regression.
- In particular, when the failing lane already owns a compatibility/import/export module and pytest now dies while parsing warning filters, assume the product module changed import-time warning behavior until a direct read proves otherwise. That failure mode is not a license to patch benchmark runner config first.
...
- If the dominant cluster is already mapped to one owner file or one tightly-coupled owner pair, keep that lane tightly bounded. Cover everything else with separately bounded residual developer lanes and/or residual child planners according to the remaining owner surface.
- The default large-root benchmark shape for this repo is a small set of concrete developer lanes, one or more residual child-planner lanes when ownership stays broad, and only the validation that those concrete branches actually need. Start with the concrete developer lanes and residual child-planner branches the live owner surface justifies, then attach only the validation that those concrete branches actually need.
