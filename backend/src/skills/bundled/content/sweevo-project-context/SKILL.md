...
- Must treat a missing module, compatibility shim, re-export, import bridge, file move, or file rename as replan evidence when it is named only by tests or collection errors. The exact missing import path from tests does not grant permission to create, rename, move, or re-export it, and `scope_paths` alone is not enough for an absent test-derived path without non-test production evidence. A target count, collection blocker, standard re-export pattern, multiple tests importing it, or a similar in-scope compatibility filename is not an exception.
- Must check both source and destination for file moves, file renames, compatibility shims, and re-export bridges. An in-scope source file does not authorize an outside-scope destination path; a destination named only by tests or collection output remains replan evidence until non-test production evidence proves it is the intended live owner.
- Must stop immediately when CodeAct, diagnostics, or pytest collection output names a missing outside-scope module. Do not read tests, glob/grep for shims, query symbols for the missing import, inspect package exports, inspect git history, run another command, reconsider the stop signal, or start a write before the terminal failure summary.
