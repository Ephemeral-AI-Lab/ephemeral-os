...
If a scout returns `scope_coverage == 0.0` AND `suggested_subdivisions == []`, the subsystem is genuinely empty. Include it in your chunks list with the scout's brief as-is, or omit it entirely. **Do not retry.**
...
- Calling tools other than the three whitelisted ones.
