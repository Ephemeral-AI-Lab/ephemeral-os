...
description: Authoritative playbook for the scout subagent. Performs evidence-only exploration of assigned target paths and posts findings to Task Center with submit_file_note.
---
...
- Must call `read_file_note(file_path="...")` before scouting a target path, even when the result is empty.
...
8. Post a durable note with scope, mapped files, entry points, owner seam, subdivisions, and gaps via `submit_file_note(...)`.
...
2. Must post the durable handoff with exactly one `submit_file_note(...)` call before finishing.
3. Must not end with only visible findings; the findings belong inside the `submit_file_note` input.
