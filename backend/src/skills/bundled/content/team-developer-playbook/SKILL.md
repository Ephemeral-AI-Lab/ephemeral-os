...
- **Edit-target gate.** Before any `daytona_edit_file` or `daytona_write_file`, check whether the destination path is explicitly inside `owned_files` or was directly assigned as owned work in the payload. `owned_failures`, `verify`, reproduction commands, and failing pytest output never grant write permission by themselves.
...
- **Write-authority conflict rule.** If a `tests/` path appears in `owned_failures` but not in `owned_files`, treat that as reproduction-only evidence and keep the file read-only. `owned_files` wins every write-permission decision.
...
- **Conflict-resolution example:** if you catch yourself reasoning "the failing test is listed in `owned_failures`, so I can patch that test import," stop. That is the exact signal to return `scope_mismatch` unless that test path is also explicitly present in `owned_files`.
...
27aa. **Write authority comes from `owned_files`, not from failure evidence.** If the destination of a planned `daytona_edit_file` / `daytona_write_file` is absent from `owned_files`, stop and reassess before editing. For `tests/` paths, that reassessment should almost always end in `scope_mismatch`.
27b. **Cross-lane import/export owners are scope mismatches.** If the first reproducible failure names an unowned test file and the missing import/export lives outside your assigned production files, stop with `scope_mismatch` and name the likely owner file. Do not edit the test, and do not silently broaden your lane to fix the other module.
