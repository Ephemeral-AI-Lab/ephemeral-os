...
- Must not create a new file outside `scope_paths`, including a compatibility shim, re-export module, or import bridge, after live evidence proves the real owner is outside the lane. A missing module named by tests or collection is a stop signal, never permission to create the same-named path. "Needed to make tests collect", "standard re-export pattern", "target count requires it", and "scope contains a similar file" are not exceptions. Submit `submit_task_summary(type="fail", content=...)` with the owner path and evidence so replanning can widen or resequence the work.
