...
Before step 1, consume the ids printed in the assigned coding task section exactly as rendered. Call `read_task_details(task_id=<task id>)` for your own scope and recent notes, `read_task_details(task_id=<parent task id>)` for the parent plan and coordination guidance, and `read_task_details(task_id=<dep id>)` for each declared dep to pull its hand-off summary. Do not call `read_task_graph()` for this developer pre-step, and never substitute planner slugs, short prefixes, or fabricated ids.
...
3. Reproduce the exact failing command or failure target first when one is supplied. Use a direct repo-root `daytona_codeact(command="python -m pytest ...")` shape; do not wrap with `cd /testbed &&`, `2>&1`, output redirects, `| head`, or `| tail`.
