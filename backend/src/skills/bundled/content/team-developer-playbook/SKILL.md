...
- Never use git/workspace archaeology, `daytona_bash`, or generic `edit_file`, `write_file`, or `read_file` on a benchmark lane.
...
- Example: a public API documents `option_backend=...`, the value arrives in `**kwargs`, but the compute helper still reads only the legacy flag.
  Treat the option-normalization or helper handoff as the first failing boundary. Prove the handoff once, then patch that path or replan.
  Do not keep re-reading tests, re-grepping the package, or narrating the same parameter flow after it is already established.
...
6. Never guess missing nodes, files, or public symbols from stale names, and never use git status/history/diff commands to argue that a sibling failure was pre-existing.
