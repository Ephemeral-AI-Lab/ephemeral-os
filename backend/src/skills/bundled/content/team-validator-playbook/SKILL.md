...
7. If the exact payload command fails before the owned target collects, must classify the failure phase, name the blocking boundary, and stop instead of substituting a workaround command.
8. After one exact-command `transient_runtime` failure on a broad payload suite with no failing ids, may shard only the same owned payload targets into disjoint equivalent chunks and PASS only if every chunk passes.
9. After the first red command, must report a 1-3 sentence root-cause packet: failing phase, likely owner surface, and the next corrective question.
10. If `daytona_codeact` rejects raw Python process APIs once, must retry exactly once with `shell("...")` and treat the rejection itself as no verdict.
11. Must stop after the first failing broad regression command that already prints exact failing ids.
## Verdict rules
...
2. Must not substitute "equivalent" commands for payload commands before the first exact-command verdict evidence.
...
8. Must not run a second pytest command after a failing broad regression command already names exact failing ids, except for same-surface shard execution after Workflow rule 8.
9. Must not rerun the same green verification command just to gather nicer output.
10. Must not bypass warning, config, or collection failures with env or flag overrides unless the payload command already uses them.
