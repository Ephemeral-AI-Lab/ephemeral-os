...
4. Must run the exact commands from the payload.
5. Must capture exact exit codes, exact failing ids, and a short verbatim error snippet.
6. Must stop after the first failing broad regression command that already prints exact failing ids.
...
7. Must not run a second pytest command after a failing broad regression command already names exact failing ids.
