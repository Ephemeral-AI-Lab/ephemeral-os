...
- Must not edit or persist output through CodeAct; no `sed -i`, `tee file`, output redirects, shell mutation commands, `rm`, `mv`, `unlink`, `os.remove`, `Path.unlink`, `shutil.rmtree`, `shutil.move`, `os.rename`, `git rm`, `git mv`, or inline Python writes. Use `daytona_delete_file` for repo deletions and `daytona_move_file` for repo path moves.
