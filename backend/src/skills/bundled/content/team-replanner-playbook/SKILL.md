...
- Context: `read_task_note(scope="sibling", paths=[...], keyword="...")` before fresh archaeology, `read_task_graph(scope="parent")` for DAG overview, `context_changed_since()` before final corrective submit. Blocked: `submit_task_note`.

## Workflow

1. Read the validator packet. Preserve exact failing ids, failure type, exit code, error snippet, and inherited owner files.
2. Check the Active Blockers section first. If an `assessing` or `fixing` blocker already overlaps your intended `root_cause_paths`, do not call `declare_blocker(...)`; call `submit_task_plan(new_tasks=[...])` with `deps=[fix_task_id]` instead.
...
7. Choose exactly one action: shared repeated failure across subtrees with no active blocker -> `declare_blocker(...)`; otherwise -> `submit_task_plan(...)` with `new_tasks` for corrective work and `remove_tasks` for stale siblings.
...
7. Never call `declare_blocker(...)` when the Active Blockers section already lists an overlapping ASSESSING/FIXING blocker; use `submit_task_plan(new_tasks=[...], deps=[fix_task_id])` instead.
8. End with exactly one of `submit_task_plan(...)` or `declare_blocker(...)`.
9. Use `draft_task_plan(...)` to preview and validate your plan before committing with `submit_task_plan(...)`. Both tools are available during the main query loop.
10. **Two-phase corrective plan for layered failures.** When step 6 identifies a layered failure, never emit a single corrective task scoped only to the visible errors. Instead emit via `submit_task_plan(new_tasks=[...])`:
