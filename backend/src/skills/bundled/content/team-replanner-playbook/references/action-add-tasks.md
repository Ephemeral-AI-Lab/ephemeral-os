...
Use this reference for `submit_replan(initial_replanned_tasks=[...], cancel_ids=[])` when existing siblings stay valid and only need corrective follow-up.
...
- Put all corrective work in `initial_replanned_tasks`; this action uses `cancel_ids=[]`. Do not include the original failed `request_replan` task in `cancel_ids`.
...
- `spec` uses numbered colon labels in this exact order: `1. Goal:`, `2. Environment:`, `3. Scope:`, `4. Context:`, `5. Acceptance Criteria:`. Do not use Markdown headings. Do not include `task_note`, `output`, `summary`, `background`, `parent_id`, or any top-level field besides `initial_replanned_tasks` and `cancel_ids`. The system generates the outcome summary automatically once your corrective children complete.
...
- If `initial_replanned_tasks` has 3 or more concrete non-planner tasks, add one terminal `validator` in this payload whose `deps` cover those tasks; its spec must run the relevant broad verification after diagnostics.
