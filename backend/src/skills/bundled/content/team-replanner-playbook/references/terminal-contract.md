...
`2. Task Details:` must name classification, diagnostics decision for `unresolved_blocker`, root-cause mechanism or unresolved trace gap, production scope, sibling/cancel handling, dependency context, and evidence/uncertainty.
...
`3. Acceptance Criteria:` must name concrete verification commands or pytest ids and require reporting command output, exit codes, changed behavior, and residual risk.
...
| 8 | Every spec uses `1. Goal:`, `2. Task Details:`, `3. Acceptance Criteria:`. |
| 9 | Every spec with `Classification: unresolved_blocker` also includes `Diagnostics decision: trivial_direct_replan` or `Diagnostics decision: deep_diagnostics` inside `2. Task Details:`. |
