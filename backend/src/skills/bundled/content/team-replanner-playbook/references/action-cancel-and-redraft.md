...
Use this reference for `submit_replan(new_tasks=[...], cancel_ids=[...])` only when one or more stale non-terminal direct siblings other than the original failed `request_replan` task must be replaced after the replanner playbook's trigger gate found a valid trigger. Cancelling a sibling cascades to its subtree automatically; replacements go in `new_tasks` as direct children of this replanner.
...
- Do not replace a failed same-scope developer with another developer, validator, planner, or replanner just to continue unfinished work in the same owner file.
- Do not replace a failed task with a benchmark-test edit task because the failure packet suggests the test is wrong; use a live production boundary or submit an empty replan when no production owner is justified.
