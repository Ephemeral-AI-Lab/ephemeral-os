...
5. Inspect fresh scouts with `check_background_progress(...)` before any `wait_for_background_task(...)`; after a full wave, default to `check_background_progress(task_id="all")` once before waiting.
6. Scouts post findings to the Task Center after their work completes. After the wave, planners must `read_task_note(paths=[...])`.
