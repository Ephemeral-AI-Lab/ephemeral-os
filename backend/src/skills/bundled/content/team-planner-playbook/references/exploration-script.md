...
7. On repeated work, if one canonical owner is already exact and same-run reuse is empty, call `read_notes(paths=[...])` to check for existing findings before relaunching that scout.
...
9. After the wave, `read_notes(paths=[...])`; if `context_changed_since()` or a scope-change warning says the layer moved, refresh only stale slices.
