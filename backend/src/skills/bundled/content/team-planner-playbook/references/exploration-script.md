...
- Never use scouts to locate or correct benchmark test path mismatches; benchmark tests stay in the evidence ledger while scouts map production owners.
...
- Never treat a structure listing that shows `pkg/io/foo/` as confirmation for a guessed `pkg/io/foo.py`; the guessed exact file is disproved unless live symbol/import/note evidence confirms it.
...
5. On cold CI, keep unresolved work on stable directories/packages and let scouts confirm exact production files. Failing benchmark tests remain evidence only.
...
8. If a guessed exact leaf is disproved, including by a directory/nested-file structure listing for the same owner family, delete it and keep the last confirmed parent boundary until note review.
