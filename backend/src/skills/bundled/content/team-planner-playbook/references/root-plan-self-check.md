...
- Example: root explorers map `pkg/io/hdf.py`, `pkg/io/parquet/`, `pkg/groupby.py`, and five small exact files.
...
- Example: the opener was cold and one branch is still only confirmed as `dask/dataframe/`.
  Keep that branch expandable. Do not turn it into `dask/dataframe/utils_dataframe.py` only because the benchmark file name looks similar.
- Example: one scout says `pkg/utils_dataframe.py` does not exist.
  Remove that exact leaf. Do not relabel ownership to `tests/test_utils_dataframe.py`; keep `pkg/` broad or omit the branch.
