...
2. **Spec format:** `spec` uses numbered colon labels in exact order, each at the start of its own line with body text on that same line: `1. Goal:`, `2. Task Details:`, `3. Acceptance Criteria:`. Do not combine labels on one line, put body text on the next line, or use Markdown headings.
