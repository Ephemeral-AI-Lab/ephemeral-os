...
- Missing modules, compatibility shims, re-export modules, and import bridges named only by tests or collection errors are not live owner paths. Do not put them in `scope_paths` unless non-test production evidence proves the absent file is the intended repository surface.
