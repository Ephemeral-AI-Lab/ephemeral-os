...
- Example: three ready misc slices plus one terminal check.
  Emit developer items like `compat_fix`, `config_cli_fix`, and `utils_fix`, plus one validator with top-level `deps` on those ids. Do not emit `agent_name: "fix_compat_make_bytes_tuple_version"` or `kind: "developer"`.
- Example: one ready HDF lane plus residual parquet work.
  Emit `{"agent_name":"developer","local_id":"hdf_fix","kind":"atomic","payload":{...}}` and `{"agent_name":"team_planner","local_id":"parquet_child","kind":"expandable","payload":{...}}`. Do not flatten payload keys like `owned_files` or `verification` beside `agent_name`.
...
- Example: terminal validator for `compat_fix` and `config_fix`.
  Emit `{"agent_name":"validator","local_id":"validate_misc","kind":"atomic","deps":["compat_fix","config_fix"],"payload":{"verify":"pytest ..."}}`. Do not place `deps` under `payload`, and do not duplicate the validator block later in the same `items` list.
