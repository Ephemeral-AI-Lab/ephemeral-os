...
- The terminal validator's top-level `deps` field must list every same-layer non-validator sibling id.
- Include child `team_planner` decomposition lanes in validator `deps`, not only direct `developer` lanes.
...
4. **Tool deps**: the validator task item's `deps` array must contain every sibling task id it validates, including child planner lanes such as `plan-parquet` or `plan-groupby`.
