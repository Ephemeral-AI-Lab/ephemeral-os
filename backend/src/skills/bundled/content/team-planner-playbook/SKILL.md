...
2. Fresh benchmark root: start with a narrow `ci_workspace_structure(path="<nearest likely production directory/package>", max_depth<=4)` pass, then call `ci_scoped_status(...)` on an exact existing production path from that listing or inherited evidence. Do not open with root-wide `ci_workspace_structure()`, `ci_query_symbols(...)`, or other broad live CI queries before that sequence completes.
...
7a. Any benchmark planner level: only use a `::pytest_node` suffix in `owned_failures`, `reproduction`, `verification`, or validator `verify` commands when that exact node id was explicitly confirmed by the prompt, inherited artifact, or live runtime evidence. If the child branch only has a file path plus a cluster label or prose hint, keep every benchmark reference at the exact checkout-relative test file path.
7b. If the benchmark evidence is still file-scoped, keep validator `verify` commands and planner-provided verification commands on that exact checkout-relative test file path. Do not widen to repo-wide keyword sweeps such as `pytest pkg/ -k compatibility` unless live evidence proves the broader slice is the minimal relevant regression net and it does not introduce unrelated collection surfaces.
7c. Broad parametrized clusters are not a license to pick one "representative" node for retry commands. Even when `owned_failures` preserves exact node ids as evidence, keep planner-supplied `reproduction`, `verification`, and validator `verify` commands on the exact benchmark test file path until a live runtime artifact proves that specific node still collects in the current checkout.
8. Benchmark roots: keep root validators paired with the concrete developer lanes or whole child branches they actually verify. A validator may depend directly on an expandable residual child-planner branch when you intentionally want a whole-branch barrier for that branch's full descendant subtree, but keep that shape deliberate because it serializes behind the entire branch.
8a. A dependency on an expandable child planner now waits for the full descendant subtree rooted at that planner item, not just the planner's JSON submission step. If a validator command names benchmark tests, modules, or runtime checks for work that the child planner will decompose, decide explicitly whether you want a whole-branch barrier on that subtree or narrower leaf-local validators inside the child branch.
...
18ab. Cluster summaries are not exact retry targets. A phrase like `test_read_json`, `test_cli_*`, or "utils helpers" does not authorize a `::node` suffix in downstream commands. Use the exact benchmark test file path until a runtime artifact or inherited packet proves the full node id byte-for-byte.
18ac. A copied exact node id is still stale if the current checkout cannot collect it. When a cluster came from a broad parametrized family and no live runtime artifact has re-confirmed one concrete node in this checkout, keep downstream retry commands file-scoped rather than freezing a guessed "representative" node into the payload.
18aa. Keep command-bearing payload keys canonical and minimal. Use standard fields such as `reproduction`, `verification`, or `verify` with exact checkout-relative benchmark paths. Benchmark shell commands should normally be repo-root-relative, for example `python -m pytest dask/...`, not `cd /home && python -m pytest ...` or `cd /workspace && ...`. Do not invent ad hoc command fields like `retries` to smuggle in guessed pytest paths.
...
18da. If a validator `deps` points at an expandable child planner, its `verify` payload must be about the child planner artifact itself, not descendant code work. A broad pytest command over residual test files is proof that the validator is misplaced and must move into the child branch.
...
5. As soon as the mapped owner graph is sufficient to cover the known failure surface, emit the plan instead of waiting for a prettier template. Let the live owner graph choose the shape: sometimes that is several direct developer lanes, sometimes one or more downstream child planners, and sometimes a mix.
5a. Root validation must stay attached only to the concrete developer lanes it actually verifies, and any residual child-planner validation belongs inside the child branch after that branch is decomposed.
...
- Even when the benchmark test file itself exists, do not forward that `tests/...` path as the child developer lane's `owned_files` unless live evidence says test/support infrastructure is the real owner. Existing benchmark tests stay in `owned_failures`; child `owned_files` should still resolve to a production/export surface or, if unresolved, to a confirmed candidate package/directory.
...
41. **Benchmark residuals must stay decomposed.** When one dominant source-owner cluster is mapped and the remaining named failures span multiple smaller modules, keep those residuals in separately owned developer lanes or downstream expandable child planners instead of flattening them into one omnibus "small failures" lane or reopening the dominant cluster.
...
- At the benchmark root, let the live owner graph choose the shape once ownership is clear. Sometimes that means several direct developer lanes, sometimes one or more residual child planners, and sometimes both. Keep validators attached only where concrete verification value exists, and do not force a fixed dominant-lane/residual-planner template.
...
- In that shape, either:
  ...
  - depend on an expandable sibling only when you intentionally want that validator to wait for the entire descendant subtree rooted at the child planner.
- A dependency on an expandable sibling waits for the full descendant subtree rooted at that planner item. It does not unblock after decomposition alone.
- Do not create a root validator whose scope is primarily the residual child-planner branch (`val_core`, `val_remaining`, `val_small_residuals`, etc.) unless you intentionally want a whole-branch barrier there. Prefer child-branch validators when you want narrower guardrails without serializing the entire branch behind one root validator.
