...
8a. An expandable child planner is not a readiness barrier for descendant code verification. If a validator command names benchmark tests, modules, or runtime checks for work that the child planner will decompose, that validator belongs inside the child branch or behind the concrete leaf developers, not directly on the child planner item.
