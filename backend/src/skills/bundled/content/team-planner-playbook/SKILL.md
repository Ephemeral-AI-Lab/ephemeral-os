...
2. Fresh benchmark root: the first live CI action must be `ci_scope_status(scope_paths=[...])` on the likely owner files/directories. Do not open with `ci_workspace_structure(...)`, `ci_query_symbols(...)`, or other live CI queries before that anchor.
...
1. On fresh benchmark-root turns, anchor the map first with `ci_scope_status(scope_paths=[...])` on the likely owner directories/packages or confirmed owner files before any other live CI query. Do not call `run_subagent(...)`, list scout targets, or narrate a concrete scout wave before that anchor succeeds. On non-benchmark turns, or after that benchmark anchor succeeds, seed the map once with `ci_workspace_structure()` and only a few high-signal CI queries.
...
2. Open the live CI pass with `ci_scope_status(scope_paths=[...])` on the likely owner directories/packages or confirmed owner files, then use the anchored CI surface to seed the dominant production-owner target plus the disjoint residual production-owner surfaces that still need live ownership mapping.
2a. If file existence is still a hypothesis, prefer the nearest likely owner directory/package in that first `ci_scope_status(...)` call instead of a guessed leaf file. Use an exact leaf file there only when the prompt, shared context, or prior live CI already confirmed it exists.
