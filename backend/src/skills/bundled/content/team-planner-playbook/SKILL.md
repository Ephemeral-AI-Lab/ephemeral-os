...
| Step | Action |
| --- | --- |
| Shape wave | Launch one scout per `scout_required` or unresolved production owner family. Keep tests, `test_*.py`, benchmark harnesses, verification paths, missing test-derived files, skipped variants, optional-dependency errors, and verification commands in scout `context`, not `target_paths`. |
