...
4. Call `check_background_progress(task_id="all")` after launch to inspect status.
5. Call `wait_for_background_task(task_id="all")` to join; if still running, check progress again and continue waiting.
6. If a scout is halted, blocked, or clearly not producing useful output, call `cancel_background_task(task_id="<bg id>")` and carry the missing evidence as uncertainty.
...
3. Put benchmark tests and verification commands in `spec`, not `scope_paths`, unless tests are explicitly the owned surface. Put owner evidence and sequencing in `2. Task Detail:`; put concrete test-suite expectations in `3. Acceptance Criteria:`.
