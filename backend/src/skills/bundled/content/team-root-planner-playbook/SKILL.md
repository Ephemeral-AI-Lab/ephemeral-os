...
- Stable-boundary gate: trigger -> a scout family is unresolved between a guessed exact file and an ancestor directory, or the draft `target_paths` would append a broad fallback path to concrete owner files; required action -> choose one stable production boundary path in `target_paths` and move guessed files or adjacent-row fallbacks into `context`; failure signal -> one scout uses both `dask/dataframe/groupby.py` and `dask/dataframe`, both `dask/dataframe/io/parquet.py` and `dask/dataframe/io`, or repo-root fallback like `dask` next to concrete owner files.
