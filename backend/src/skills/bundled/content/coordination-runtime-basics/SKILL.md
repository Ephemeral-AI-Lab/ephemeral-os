...
- If the verifier finds failures and the runtime exposes `request_replan()`, it should call that tool. When the tool is absent, the verifier should emit the same structured triage in its FAIL summary so a later coordinator or human turn can revise the plan explicitly.
