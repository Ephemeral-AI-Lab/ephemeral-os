...
The 4 stress tests in `stress/` are deselected from the regular live-tier
run because each one is long-running (100-cycle loops, public internet,
N=5 maximum-load fan-out, or an idle disk bound). They share the same Docker provider
...
share a session-level sweevo container across a tight enter/exit loop and
internet-bound package install).
...
The closure session (see `NEXT-AGENT-NOTES.md`) made two changes that
touch soak-test invariants. Re-validate these specifically the first
time you run soak after pulling those commits:

1. **`test_rapid_create_destroy_cycle`** — 100 enter/exit cycles. The
...
2. **`test_5_concurrent_isolated_workspaces`** — relies on
