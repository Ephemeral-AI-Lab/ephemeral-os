...
sandbox-heavy task/request workflows.
...
EOS_SWEEVO_INSTANCE=dask__dask_2023.3.2_2023.4.0 \
uv run pytest -q \
  backend/src/test_runner/tests/mock/sandbox/project_build/test_complex_project_build_smoke.py
```
...
EOS_SWEEVO_INSTANCE=dask__dask_2023.3.2_2023.4.0 \
uv run pytest -q \
  backend/src/test_runner/tests/mock/sandbox/project_build/test_complex_project_build_smoke.py::test_complex_project_build_smoke
```
...
## task/request Mock Tests

task/request mock tests focus on workflow correctness rather than load testing.
...
Run one task/request test file:

```bash
EOS_SWEEVO_INSTANCE=dask__dask_2023.3.2_2023.4.0 \
uv run pytest -q backend/src/test_runner/tests/mock/request/test_focused_scenarios.py
```

Run one parametrized task/request case:

```bash
EOS_SWEEVO_INSTANCE=dask__dask_2023.3.2_2023.4.0 \
uv run pytest -q \
  'backend/src/test_runner/tests/mock/request/test_focused_scenarios.py::test_focused_reference_scenario_runs[pipeline.initial_workflow]'
```

## Real-Agent Tests

Real-agent tests run live LLM agents through the task/request runner. They require
...
EOS_SWEEVO_INSTANCE=dask__dask_2023.3.2_2023.4.0 \
uv run pytest -q \
  backend/src/test_runner/tests/real_agent/test_real_agent.py::test_real_agent_resolves_canonical_instance
```
...
The task/request-side benchmark implementation lives in:

- `backend/src/test_runner/benchmarks/sweevo/run.py`
- `backend/src/test_runner/benchmarks/sweevo/eval.py`
- `backend/src/test_runner/benchmarks/sweevo/run.py`

The CLI entrypoint is:

- `backend/src/test_runner/benchmarks/sweevo/__main__.py`
...
```bash
uv run pytest --collect-only -q backend/src/test_runner/tests/mock/sandbox
```
