...
ROLE_TEMPLATES: MappingProxyType[str, str] = MappingProxyType({
    ...
    "developer": (
        ...
        "`daytona_codeact` probe a direct "
        "`daytona_codeact(command=\"...\", timeout=N)` call, never a Python "
        "wrapper, `subprocess`, or `2>&1`.\n"
    ...
    "reviewer": (
        ...
        "on benchmark lanes, call them as direct "
        "`daytona_codeact(command=\"...\", timeout=N)` probes, never "
        "`subprocess` or `2>&1`.\n"
