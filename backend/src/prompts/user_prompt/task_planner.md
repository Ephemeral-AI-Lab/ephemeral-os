...
0. Before scouts, CI, note reads, or plan drafting, consume the ids above exactly as rendered: call `read_task_details(task_id=<your task id>)`, call `read_task_details(task_id=<your parent task id>)`, then call `read_task_graph()` to enumerate same-parent siblings. Call `read_task_details(task_id=<sibling id>)` for any sibling whose scope, dependency ordering, or validator coverage could affect this plan. Never substitute planner slugs, short prefixes, or fabricated ids for the header-provided ids.
1. Please read the assigned planner task and inherited context from those task-detail and graph reads.
