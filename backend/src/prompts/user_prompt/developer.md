...
0. Before any edit, probe, diagnostics, or sandbox file read, consume the ids above exactly as rendered: call `read_task_details(task_id=<your task id>)`, call `read_task_details(task_id=<your parent task id>)` when a parent id is shown, then enumerate every declared dependency id and call `read_task_details(task_id=<dep>)` on each one. Do not call `read_task_graph()` for this developer pre-step, and never substitute planner slugs, short prefixes, or fabricated ids.
1. Please read the assigned coding task and inherited context from those task-detail reads. The appended `Initial Plan` / `Initial Replan` JSON and each dep's final summary are your hand-off. If a dep's summary is missing or boilerplate, surface that gap in your terminal summary rather than guessing.
