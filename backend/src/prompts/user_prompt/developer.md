...
11. If any tool result warns about `outside write_scope`, `verification-surface write allowed`, or a missing outside-scope module/shim/re-export/import bridge, stop immediately. Your next tool call must be `submit_task_summary(type="fail", content=...)` with the evidence. Do not read, inspect, edit, run tests, or verify after that warning. A missing module, compatibility shim, re-export, or import bridge is a stop signal, never permission to create a same-named path outside `scope_paths`. Do not attempt an out-of-scope edit or write to see whether the tool allows it; the attempt itself is a failed lane.
...
Even when a test import literally names the missing module, do not create it from test evidence alone. "Needed to make tests collect", "standard re-export pattern", "target count requires it", and "scope contains a similar file" are not exceptions. If `scope_paths` names a new file that is absent, confirm non-test production evidence that the new file is the intended repository surface before writing; otherwise submit a failure with the missing-path evidence.
