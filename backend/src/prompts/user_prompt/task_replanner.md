...
0. Before scouts, CI, note reads, diagnosis, or corrective planning, consume the ids above exactly as rendered: call `read_task_details(task_id=<your task id>)`, call `read_task_details(task_id=<your parent task id>)`, call `read_task_details(task_id=<failed task id>)`, then enumerate every declared dependency id and call `read_task_details(task_id=<dep>)` on each one. Then call `read_task_graph()` to enumerate same-parent siblings; call `read_task_details(task_id=<sibling id>)` for any sibling you may preserve, cancel, or rewire. Never substitute planner slugs, short prefixes, or fabricated ids for the header-provided ids.
1. Please read the assigned replanning task and failure context from those task-detail and graph reads.
2. Use the failed task detail to pull the failed task's spec, status, scope_paths, failure reason, completion summary, and recent notes. Call `read_task_details(task_id="...")` once per relevant sibling or dependent task you need. Only fall back to `read_file_note(file_path="...")` when you need path-based search across the notes stream. After that, use CI tools such as `ci_workspace_structure(...)`, `ci_query_symbol(...)`, or `ci_diagnostics(...)` as needed.
...
6. For each new task spec, use exactly this section order with colon labels: `1. Goal:`, `2. Environment:`, `3. Scope:`, `4. Context:`, `5. Acceptance Criteria:`. Each label must start its own line and have body text on that same line, e.g. `1. Goal: Fix ...\n2. Environment: Use ...`; not `1. Goal:\nFix ...` and not all labels on one line. Do not use Markdown headings such as `## Goal`.
