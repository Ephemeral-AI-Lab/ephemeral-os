...
13. Submit the final corrective plan with `submit_replan(initial_replanned_tasks=[...], cancel_ids=[...])`. Every new task must include a short `description`. Do not author a prose summary — the system generates the outcome summary automatically once your children complete. `cancel_ids` may only target non-terminal **direct siblings** with the same parent as this replanner; cascade handles their subtrees. If an id appears only in `read_task_graph(scope="global")`, has a different `parent_id`, or is nested under another task, omit it from `cancel_ids`. Put replacement work in `initial_replanned_tasks` so downstream work remains blocked on this replanner until recovery completes. Do not include `task_note`, `output`, `summary`, `background`, `parent_id`, or any other top-level fields.
