"""Persistent team lifecycle management for EphemeralOS swarms.

Teams are stored as JSON files on disk:
    ~/.ephemeralos/teams/<name>/team.json
...
from ephemeralos.swarm.mailbox import get_team_dir
from ephemeralos.swarm.types import BackendType
...
async def _kill_orphaned_teammate_panes(team_name: str) -> None:
    ...
    from ephemeralos.swarm.spawn_utils import is_inside_tmux
    from ephemeralos.swarm.types import is_pane_backend
...
class TeamLifecycleManager:
    ...
    Persists team metadata to ``~/.ephemeralos/teams/<name>/team.json``.
    ...
    def list_teams(self) -> list[TeamFile]:
        """Return all teams found in ``~/.ephemeralos/teams/``, sorted by name."""
        base = Path.home() / ".ephemeralos" / "teams"
        if not base.exists():
            return []
