...
TEAMMATE_COMMAND_ENV_VAR = "EPHEMERALOS_TEAMMATE_COMMAND"
...
def get_teammate_command() -> str:
    ...
    Resolution order:
    1. ``EPHEMERALOS_TEAMMATE_COMMAND`` environment variable — allows the
       operator to point at a specific binary or wrapper script.
    2. The ``ephemeralos`` entry-point on PATH (installed package mode).
    3. The current Python interpreter running the ``ephemeralos`` module
    ...
    entry_point = shutil.which("ephemeralos")
    if entry_point:
        return entry_point
...
def build_inherited_env_vars() -> dict[str, str]:
    ...
    Always includes ``EPHEMERALOS_AGENT_TEAMS=1`` plus any provider/proxy
