"""Permission sync protocol for leader-worker coordination in EphemeralOS swarms.
...
Paths:
    ~/.ephemeralos/teams/<teamName>/permissions/pending/<id>.json
    ~/.ephemeralos/teams/<teamName>/permissions/resolved/<id>.json
"""
...
def get_permission_dir(team_name: str) -> Path:
    """Return ~/.ephemeralos/teams/{teamName}/permissions/"""
...
async def get_leader_name(team_name: str | None = None) -> str | None:
    ...
    """
    from ephemeralos.swarm.team_lifecycle import read_team_file_async
