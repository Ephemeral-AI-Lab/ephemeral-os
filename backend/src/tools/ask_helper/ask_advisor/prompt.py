...
def get_ask_advisor_description() -> str:
    return (
        ...
        f"\n"
        f"Use this when:\n"
        f"- You're about to call a terminal tool (e.g., `{SUBMIT_GENERATOR_OUTCOME_TOOL_NAME}`,\n"
        f"  `{SUBMIT_REDUCER_OUTCOME_TOOL_NAME}`, `{SUBMIT_PLANNER_OUTCOME_TOOL_NAME}`) and you want a\n"
        ...
        f"  eval).\n"
        f"- Fixing problems — the advisor only audits and cannot edit. Reducer\n"
        f"  agents may apply trivial inline fixes themselves via\n"
        ...
        f"- NOT callable inside an isolated workspace. Because every terminal\n"
        f"  requires a prior advisor approval, terminal submission is impossible\n"
        f"  while isolated — call `exit_isolated_workspace` first, then\n"
        f"  `{ASK_ADVISOR_TOOL_NAME}` and submit your terminal.\n"
