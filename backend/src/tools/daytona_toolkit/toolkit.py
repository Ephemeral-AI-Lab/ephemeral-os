...
def _build_instructions(*, include_codeact: bool) -> str:
    ...
    return (
        ...
        "- `daytona_delete_file` — delete a file by validating the target and submitting one audited bash command; set "
        "`recursive=true` for a folder tree. "
        "Use this instead of `rm` in CodeAct; the shell policy blocks `rm` for that reason.\n"
        "- `daytona_move_file` — move a file by validating source/destination and submitting one audited bash command; set "
        "`recursive=true` when moving a folder tree. "
        "Use this instead of `mv` in CodeAct. By default `dst` must not exist; pass `overwrite=true` to replace it.\n"
        f"{codeact_line}\n"
        "**Execute**\n"
