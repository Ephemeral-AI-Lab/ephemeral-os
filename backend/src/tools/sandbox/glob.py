...
@tool(
    name="glob",
    description=(
        "Enumerate workspace files matching a glob pattern under the sandbox's "
        "leased snapshot. Uses Python fnmatch semantics (NOT POSIX rg --glob; "
        "no brace-expansion). The leading-dot VCS directories (.git/.svn/.hg/"
        ".bzr/.jj/.sl) are excluded. Results are capped at 100 paths — set "
        "`path` to narrow scope when truncated. Symlinks are listed but not "
        "followed. Prefer this over `shell` with find/ls for structured output."
    ),
