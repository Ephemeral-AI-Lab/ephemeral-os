...
@tool(
    name="grep",
    description=(
        "Regex-scan workspace file contents under the sandbox's leased "
        "snapshot. Uses Python `re` (NOT PCRE2). VCS directories (.git/.svn/"
        ".hg/.bzr/.jj/.sl) are excluded. Files larger than 10 MB and non-UTF-8 "
        "files are skipped silently. Output is capped at 20 KB total content "
        "and `head_limit` entries (default 250; 0 = unlimited). Three modes: "
        "`files_with_matches` (default), `count`, `content`. Prefer this over "
        "`shell` with grep — cheaper, structured, and routed read-only."
    ),
