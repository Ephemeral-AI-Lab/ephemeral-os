...
from tools._names import SUBMIT_GENERATOR_OUTCOME_TOOL_NAME
...
editing, finish what you can and use `{SUBMIT_GENERATOR_OUTCOME_TOOL_NAME}` instead.

## Do Not Use This Tool When
- The task is bounded and doable — just do it.
- You're stuck on an environment or dependency issue — that's
  `{SUBMIT_GENERATOR_OUTCOME_TOOL_NAME}` with `status="failed"`, not a decomposition request.
