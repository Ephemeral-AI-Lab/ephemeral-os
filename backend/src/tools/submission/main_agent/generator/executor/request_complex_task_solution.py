...
async def request_complex_task_solution(
    ...
    *,
    ...
    try:
        ...
        nested_request = request_handler.create_complex_task_request(
            task_center_run_id=submission_context.task["task_center_run_id"],
            requested_by_task_id=submission_context.task_center_task_id,
            goal=clean_goal,
        )
        nested_segment = request_handler.create_initial_segment(
            complex_task_request_id=nested_request.id,
        )
        ...
        segment_manager = manager.get(nested_segment.id)
        if segment_manager is None:
            raise GraphInvariantViolation(
                f"TaskSegmentManager {nested_segment.id!r} was not registered."
            )
        submission_context.runtime.task_store.set_task_status(
            ...
            summary={
                ...
                "summary": "Waiting on nested complex task solution.",
                "payload": {
                    "complex_task_request_id": nested_request.id,
                    "initial_segment_id": nested_segment.id,
                    "goal": clean_goal,
                },
            },
        )
    ...
    return ToolResult(
        output=(
            "Started nested complex task request "
            f"{nested_request.id} for this executor task."
        ),
        metadata={
