...
def collect_schema_toolkits(
    *,
    ...
    toolkits = [create_toolkit(name, ctx) for name in sorted(list_factories())]
...
def _format_tool(tool: BaseTool, *, include_descriptions: bool) -> list[str]:
    ...
    lines.append(
        f"    input: {_format_model(tool.input_model, include_descriptions=include_descriptions)}"
    )
    lines.append(
        f"    output: {_format_model(tool.output_model, include_descriptions=include_descriptions)}"
    )
    return lines


def _format_model(model: type[BaseModel], *, include_descriptions: bool) -> str:
    if issubclass(model, RootModel):
        description = _clean_description(model.__doc__ or "")
        suffix = f" - {description}" if include_descriptions and description else ""
