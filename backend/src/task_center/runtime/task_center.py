"""TaskCenter — request-scoped store + creation primitives for the task graph.
...
- the ``_create_<role>`` primitives + ``_open_graph`` primitive (Stage 1 of
  the four-role / recursive-orchestrator restructure)
...
under :mod:`task_center.harness_agents`. The :class:`Orchestrator` class in
:mod:`task_center.runtime.orchestrator` is a graph-scoped facade that calls
these primitives.
"""
...
class TaskCenter:
    """Request-scoped store + dispatcher created by the server runtime."""
    ...
    # ------------------------------------------------------------------ #
    # Stage 1: creation primitives                                       #
    #                                                                    #
    # Each ``_create_<role>`` adds a single ``Task`` of that role to the #
    # store. They take exactly the fields needed to construct that role  #
    # (no input synthesis, no graph creation). Composers (in             #
    # ``Orchestrator`` and ``RunController``) call them.                 #
    # ------------------------------------------------------------------ #
    ...
    def _open_graph(
        ...
        *,
        ...
        """Create a new ``HarnessGraph`` rooted at ``root_task_id``.

        The planner Task is expected to have been created by the caller via
        :meth:`_create_planner` before calling this, since the graph stores
        the planner id in both ``planner_task_id`` (legacy) and ``planner``
        (Stage 1 slot).
        """
        root_task = self._graph.get(root_task_id)
        graph = HarnessGraph(
...
__all__ = ["SpawnFunc", "TaskCenter", "TaskSummary"]
