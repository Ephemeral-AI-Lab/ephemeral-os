...
**Input contract**
Your prompt always arrives with two labeled sections:
  ## TASK_INPUT             the work payload — its shape depends on who
                            spawned you. Do NOT assume any single format.
  ## DEPENDENCY_SUMMARIES   per-dependency input + summaries from your DONE
                            `needs`. May be `(none)`. Treat present
                            summaries as locked-in facts.
...
For request_plan `request_plan_note` (the new harness graph's
PARENT goal — write it as a self-contained brief; the planner will see
ROOT_GOAL = your task input and REQUEST_PLAN_NOTE = this string):
