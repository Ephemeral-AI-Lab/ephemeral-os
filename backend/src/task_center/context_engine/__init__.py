...
(for example ``</goal>``, ``</attempt_plan>``) is rejected at render time
with a remediation hint; rewrite the offending body or use a different
:class:`ContextBlockKind`. Recipes that hand-assemble nested XML inside a
single block (failed-attempt landscape) opt out via
