"""``planner_closes_goal`` recipe — context for a leaf planner spawn.

The ``planner_closes_goal`` variant is resolved when a planner is delegated a
...
PLANNER_CLOSES_GOAL_ID = "planner_closes_goal"
_REQUIRED_FIELDS = frozenset({"goal_id"})


def _planner_closes_goal_build(
...
PLANNER_CLOSES_GOAL_RECIPE = ContextRecipe(
    id=PLANNER_CLOSES_GOAL_ID,
    required_scope_fields=_REQUIRED_FIELDS,
    build=_planner_closes_goal_build,
)
