...
Emits the graph's ``task_specification`` and ``evaluation_criteria`` as
``priority=required`` blocks, plus one ``completed_task_summary`` per
generator task in the graph.
"""
