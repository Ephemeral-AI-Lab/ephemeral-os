...
|---:|---|---|---|
| 4 | `occ/test_commit_transaction.py` **(+ race)** | `sandbox.occ.stage.transaction` | atomicity; rollback on failure; idempotent retry. Race: N=4 concurrent commits → atomic per commit, no partial visible state |
| 5 | `occ/test_orchestrator.py` **(+ race)** | `sandbox.occ.router` | happy/conflict/abort flows; restart recovery. Race: N=4 orchestrators interleave → conflict detection deterministic |
...
| 9 | `occ/test_direct_route.py` **(+ race)** | `sandbox.occ.stage.direct` | empty changeset; 10k-path changeset; no contention. Race: N=8 direct commits disjoint paths → all succeed, no lock blow-up |
| 10 | `occ/test_gated_route.py` **(race-by-definition)** | `sandbox.occ.stage.gated` | first-commits-wins; both-reject; partial overlap (test is inherently concurrent — confirm spec, not added) |
...
| 17 | `layer_stack/test_changes_aggregation.py` **(+ race)** | `sandbox.layer_stack.layer_change` | dedup; ordering; rename pairs; out-of-order writes. Race: N=8 concurrent producers → dedup invariant holds, ordering deterministic per-path |
