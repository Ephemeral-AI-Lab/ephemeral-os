...
def test_planner_skill_has_explicit_conditional_reference_loading() -> None:
    ...
    assert (
        "before loading `plan-json-contract` or `task-planning-decomposition`, must complete at least one scout wave"
        in planner
    )
...
def test_developer_and_validator_skills_explain_when_to_load_references() -> None:
    ...
    assert (
        "never rewrite the verify import or binding just because the path looks stale" in developer
    )
...
def test_scout_playbook_keeps_missing_targets_missing() -> None:
    ...
    assert (
        'End with raw JSON like `{"summary":"This file defines compatibility helpers."' in scout_ref
    )
