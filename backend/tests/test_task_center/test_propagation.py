...
def test_chain_closes_all_four_in_walk_order() -> None:
    ...
    b (evaluator, closes_for=a)   <- submit_continue_to_work
    ...
    """
    a = _t("a", role="executor", closes_for=None, status=Status.AWAITING)
    b = _t("b", role="evaluator", closes_for="a", status=Status.AWAITING)
    c = _t("c", role="executor", closes_for="b", status=Status.AWAITING)
...
def test_awaiting_status_flips_to_done() -> None:
    """Mid-chain AWAITING tasks flip to DONE just like leaves."""
    parent = _t("p", role="executor", closes_for=None, status=Status.AWAITING)
