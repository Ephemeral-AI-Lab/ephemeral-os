...
@pytest.mark.asyncio
async def test_run_subagent_rejects_team_planner_non_scout_targets_early():
    class _StubConfig:
        cwd = Path("/tmp")
        session_id = "session_abc"
    ...
    ctx = ToolExecutionContext(
        cwd=Path("/tmp"),
        metadata={"session_config": _StubConfig(), "agent_name": "team_planner"},
    )
    ...
    assert result.is_error is True
    assert "caller 'team_planner' may dispatch only 'scout'" in result.output
...
async def test_run_subagent_forces_promotion_for_root_benchmark_planner_scout(monkeypatch):
    ...
    root = WorkItem(
        ...
        payload={"fail_to_pass": ["tests/test_auth.py::test_login"]},
    )
    run.root_work_item_id = root.id
    run.dispatcher.graph[root.id] = root
    monkeypatch.setattr(
        "team.runtime.registry.get",
        lambda team_run_id: run if team_run_id == "TR1" else None,
    )
...
async def test_run_subagent_rejects_team_scout_artifact_with_invalid_field_types(monkeypatch):
    ...
    monkeypatch.setattr(
        "team.context.scout_briefings.store_stable_scout_artifact",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("invalid scout brief should not be stored")
        ),
    )
    monkeypatch.setattr(
        "team.context.scout_briefings.auto_promote_scout_briefing",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("invalid scout brief should not be promoted")
        ),
    )
