"""A8 — Audit recorder must not import plan-mode storage or client packages.

Per `.planning/coding_plan_mode_plan.md` v8 §A8, the audit recorder is the
load-bearing privacy boundary for plan-mode OAuth tokens. If
``recorder.py`` were to import any of:

* ``providers.clients.coding_plan.*`` (any plan-mode client module), or
...
plan-mode object inside the recorder could leak a token literal into a
JSONL audit file. The static-graph guard enforces that no such reach
...
plan-mode-aware logic. recorder.py must NEVER follow them into plan-mode
territory.
"""
...
def test_recorder_does_not_import_plan_mode_packages() -> None:
    """recorder.py must not import any plan-mode client module or model_store.

    Failure surface: token-leak via ``__repr__`` / ``asdict`` /
    str-coercion of a plan-mode client object accidentally captured in
    a recorder record. See plan A8.
    """
    ...
    if offenders:
        pytest.fail(
            "Audit recorder must not import plan-mode packages "
            "(A8 token-leak guard). Offenders:\n"
            + "\n".join(offenders)
            + "\n\nFix: move the offending import to a different module or "
            "restructure so the recorder accepts plan-mode-related data "
            "only as opaque record dataclasses, never as live client "
            "objects."
        )
