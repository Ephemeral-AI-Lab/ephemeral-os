...
from sandbox.runtime import health_handlers, layer_stack_server, occ_server
...
def test_runtime_ready_bound_workspace_returns_ready(tmp_path: Path) -> None:
...
def test_runtime_ready_unbound_root_fails_closed(
    tmp_path: Path,
) -> None:
    stack = tmp_path / "layer-stack"
...
def test_runtime_ready_reports_data_plane_failure(
    tmp_path: Path,
...
def test_runtime_ready_reports_incomplete_data_plane_backend(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class _Backend:
...
def test_runtime_ready_reports_mutation_gate_failure(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class _Backend:
...
def test_runtime_ready_reports_incomplete_mutation_gate_backend(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class _Backend:
...
def test_runtime_ready_reports_explicit_workspace_mount_mode(tmp_path: Path) -> None:
    response = health_handlers.runtime_ready(
        {"layer_stack_root": (tmp_path / "stack").as_posix()}
    )
