...
def test_bootstrap_helper_uploads_by_sandbox_id() -> None:
    from sandbox.host.bootstrap import bootstrap_in_sandbox_runtime
    ...
    with patch("sandbox.host.bootstrap.ensure_runtime_uploaded", new=fake_upload):
        asyncio.run(
...
def test_bootstrap_helper_raises_on_runtime_upload_failure() -> None:
    ...
    with patch(
        "sandbox.host.bootstrap.ensure_runtime_uploaded",
        new=fail_upload,
...
def test_run_runtime_bootstrap_skips_when_workspace_unresolvable() -> None:
    from sandbox.host.bootstrap import run_runtime_bootstrap
    ...
    with patch(
        "sandbox.host.bootstrap.bootstrap_in_sandbox_runtime",
        new=boom,
    ):
...
def test_ensure_workspace_base_skips_when_workspace_missing() -> None:
    ...
    with patch("sandbox.host.bootstrap.call_daemon_api") as call:
...
def test_start_upload_submits_future_and_invokes_helper() -> None:
    ...
    from sandbox.host.bootstrap import (
...
def test_finish_upload_noop_when_future_none() -> None:
    from sandbox.host.bootstrap import finish_runtime_bundle_upload
