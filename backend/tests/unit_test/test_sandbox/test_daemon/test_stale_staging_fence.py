...
def test_stale_staging_fence_removes_old_dirs_and_keeps_fresh_dirs(
    ...
    monkeypatch.setattr(workspace_server, "_DAEMON_STARTED_AT", started_at)
    ...
    result = workspace_server.fence_stale_staging(tmp_path / "stack")
...
def test_stale_staging_fence_second_call_is_idempotent(
    ...
    first = workspace_server.fence_stale_staging(tmp_path / "stack")
    second = workspace_server.fence_stale_staging(tmp_path / "stack")
...
def test_get_layer_stack_manager_fences_once_per_resolved_root(
    ...
    monkeypatch.setattr(workspace_server, "fence_stale_staging", fake_fence)

    manager_a = workspace_server.get_layer_stack_manager(tmp_path / "stack")
    manager_b = workspace_server.get_layer_stack_manager(tmp_path / "stack")
