...
def test_stale_staging_fence_removes_old_dirs_and_keeps_fresh_dirs(
    ...
    result = layer_stack_runtime.fence_stale_staging(tmp_path / "stack")
...
def test_stale_staging_fence_second_call_is_idempotent(
    ...
    first = layer_stack_runtime.fence_stale_staging(tmp_path / "stack")
    second = layer_stack_runtime.fence_stale_staging(tmp_path / "stack")
...
def test_get_layer_stack_manager_fences_once_per_resolved_root(
    ...
    monkeypatch.setattr(layer_stack_runtime, "fence_stale_staging", fake_fence)

    manager_a = layer_stack_runtime.get_layer_stack_manager(tmp_path / "stack")
    manager_b = layer_stack_runtime.get_layer_stack_manager(tmp_path / "stack")
