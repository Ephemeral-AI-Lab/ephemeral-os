"""``api.find_files`` / ``api.search_content`` daemon handler tests."""
...
from sandbox.daemon.handler.search import (
    DEFAULT_GLOB_LIMIT,
...
def test_find_files_basic_glob_returns_sorted_matches(tmp_path: Path) -> None:
    ...
    result = _find_files_sync(_args(stack, pattern="*.py"))
...
def test_find_files_subpath_filter_restricts_scope(tmp_path: Path) -> None:
    stack, workspace = _seed_workspace(
        tmp_path,
        files={
            "outer.py": "o",
            "pkg/inner.py": "i",
    ...
    result = _find_files_sync(_args(stack, pattern="*.py", path="pkg"))

    assert sorted(result["filenames"]) == ["pkg/inner.py", "pkg/nested/leaf.py"]


def test_find_files_excludes_vcs_directories(tmp_path: Path) -> None:
    stack, _ = _seed_workspace(
        tmp_path,
        files={
            ".git/config": "x",
...
def test_find_files_missing_pattern_raises(tmp_path: Path) -> None:
    stack, _ = _seed_workspace(tmp_path, files={"a.txt": "a"})

    with pytest.raises(ValueError, match="pattern is required"):
        _find_files_sync(_args(stack))


def test_find_files_acquires_and_releases_lease(tmp_path: Path) -> None:
    stack, _ = _seed_workspace(tmp_path, files={"a.txt": "a"})
    services = build_occ_backend(stack)
    before = services.manager.active_lease_count()

    _find_files_sync(_args(stack, pattern="*.txt"))

    assert services.manager.active_lease_count() == before


def test_find_files_releases_lease_when_validation_rejects_path(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(ValueError, match="search path must be inside the workspace"):
        _find_files_sync(_args(stack, pattern="*.txt", path="/etc"))

    assert services.manager.active_lease_count() == before


def test_find_files_releases_lease_when_scan_raises_mid_flight(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The try/finally guard must release the lease when iter_paths raises
    after the lease has been acquired. Without this, a transient layer-stack
    storage error would leak a pinning lease and grow active_lease_count
...
def test_search_content_files_with_matches_mode(tmp_path: Path) -> None:
    stack, _ = _seed_workspace(
        tmp_path,
        files={
            "a.py": "hello world\n",
    ...
    result = _search_content_sync(_args(stack, pattern="hello"))
...
def test_search_content_count_mode_returns_per_file_counts(tmp_path: Path) -> None:
    stack, _ = _seed_workspace(
        tmp_path,
        files={
...
def test_search_content_content_mode_emits_filename_line_pairs(tmp_path: Path) -> None:
    stack, _ = _seed_workspace(
        tmp_path,
        files={
            "a.py": "alpha\nhello world\nbeta\n",
...
def test_search_content_case_insensitive_flag(tmp_path: Path) -> None:
    stack, _ = _seed_workspace(
        tmp_path,
...
def test_search_content_glob_filter_narrows_files(tmp_path: Path) -> None:
    stack, _ = _seed_workspace(
        tmp_path,
        files={
            "a.py": "needle\n",
...
def test_search_content_head_limit_truncates(tmp_path: Path) -> None:
...
def test_search_content_content_cap_at_twenty_kb(tmp_path: Path) -> None:
...
def test_search_content_skips_files_larger_than_max(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    ...
    assert result["timings"]["api.search_content.skipped_large"] >= 1


def test_search_content_skips_non_utf8_files(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    ...
    assert result["timings"]["api.search_content.skipped_binary"] >= 1


def test_search_content_rejects_invalid_output_mode(tmp_path: Path) -> None:
...
def test_search_content_rejects_invalid_regex(tmp_path: Path) -> None:
    stack, _ = _seed_workspace(tmp_path, files={"a.txt": "x"})

    with pytest.raises(ValueError, match="invalid regex pattern"):
        _search_content_sync(_args(stack, pattern="("))


def test_search_content_releases_lease_when_scan_raises_mid_flight(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The try/finally guard must release the lease when iter_paths raises
    after the lease is held. Mirrors the find_files mid-scan regression."""
    stack, _ = _seed_workspace(tmp_path, files={"a.txt": "needle"})
    services = build_occ_backend(stack)
    before = services.manager.active_lease_count()
    ...
    with pytest.raises(RuntimeError, match="synthetic mid-scan failure"):
        _search_content_sync(_args(stack, pattern="needle"))

    assert services.manager.active_lease_count() == before


def test_search_content_excludes_vcs(tmp_path: Path) -> None:
    stack, _ = _seed_workspace(
        tmp_path,
        files={
            ".git/HEAD": "needle\n",
    ...
    result = _search_content_sync(_args(stack, pattern="needle"))

    assert ".git/HEAD" not in result["filenames"]
    assert "src/main.py" in result["filenames"]


def test_search_handler_does_not_call_occ_client(tmp_path: Path) -> None:
    """Read-only by construction: search.py must not touch the OCC mutation gate.

    Matches the AC: ``grep -rn 'occ_client\\.' src/sandbox/daemon/handler/search.py``
    returns zero hits — i.e. no attribute/call access on the OCC client.
    """
    import sandbox.daemon.handler.search as search_module

    source = Path(search_module.__file__).read_text(encoding="utf-8")
    # Strip docstrings/comments by checking only meaningful code lines.
    code_lines = [
        line for line in source.splitlines()
        if line.strip() and not line.strip().startswith(("#", '"', "'"))
    ]
    code = "\n".join(code_lines)
    assert "occ_client." not in code, "search.py must not access occ_client"
    assert "OccClient" not in code, "search.py must not reference OccClient"
    assert ".commit_" not in code, "search.py must not call commit_* methods"
    assert ".apply_" not in code, "search.py must not call apply_* methods"
