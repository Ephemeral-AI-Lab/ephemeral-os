...
``recipes/planner.py:_current_iteration_goal_child``, so drift
...
class _FakeWorkflow:
    def __init__(self, *, id: str, workflow_goal: str):
        self.id = id
...
def test_current_iteration_goal_child_pairs_metadata_and_group_attrs():
    ...
    block = _current_iteration_goal_child(
        _FakeIteration(id="i7", sequence_no=7, iteration_goal="iter 7")
    )
...
def test_iter1_iteration_goal_body_uses_identity_marker():
    ...
    block = _current_iteration_goal_child(
        _FakeIteration(id="i1", sequence_no=1, iteration_goal="iter 1 goal")
    )
...
def test_iteration_no_invariant_holds_for_every_sequence_no(seq_no: int):
    block = _current_iteration_goal_child(
        _FakeIteration(id="i", sequence_no=seq_no, iteration_goal="g")
    )
    _assert_invariant([block])
...
def test_invariant_catches_planted_drift():
    ...
    block = _current_iteration_goal_child(
        _FakeIteration(id="i", sequence_no=3, iteration_goal="g")
    )
...
def test_goal_iteration_blocks_full_frame_invariant():
    ...
    workflow = _FakeWorkflow(id="g", workflow_goal="overall goal")
    prior = _FakeIteration(
        id="i1",
        sequence_no=1,
        iteration_goal="iter 1 goal",
        outcomes=json.dumps(
            [to_record(Outcome(local_id="t", status="success", outcome="prior summary"))]
        ),
    )
    current = _FakeIteration(id="i2", sequence_no=2, iteration_goal="iter 2 goal")
