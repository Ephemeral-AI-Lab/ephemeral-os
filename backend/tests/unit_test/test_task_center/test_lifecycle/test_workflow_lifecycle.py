...
def test_create_workflow_links_executor(
    ...
    assert workflow.requested_by_task_id == "executor-1"
...
def test_workflow_records_iterations_in_iteration_ids(
    ...
    """Phase 01 exit: each workflow records created iterations in iteration_ids."""
    workflow = workflow_lifecycle.create_workflow(
        task_center_run_id=task_center_run_id,
...
def test_iteration_ids_holds_multiple_iterations(
    ...
    """Phase 01 exit: iteration_ids can hold multiple Iteration ids for one workflow."""
    workflow = workflow_lifecycle.create_workflow(
        task_center_run_id=task_center_run_id,
        origin=WorkflowOrigin.task(task_id="t1"),
        goal="g1",
...
def test_handle_iteration_closed_terminal_success_closes_workflow_succeeded(
    workflow_lifecycle, workflow_store, iteration_store, task_center_run_id
):
    workflow = workflow_lifecycle.create_workflow(
        task_center_run_id=task_center_run_id,
...
def test_handle_iteration_closed_attempt_plan_failed_closes_workflow_failed(
    workflow_lifecycle, workflow_store, task_center_run_id
):
    workflow = workflow_lifecycle.create_workflow(
        task_center_run_id=task_center_run_id,
...
def test_no_legacy_entry_creation_reason_in_lifecycle(workflow_lifecycle, task_center_run_id):
    ...
    # Indirect: workflow-lifecycle driven iteration creation only ever uses INITIAL or
