...
def test_create_goal_links_executor(
    ...
    """Phase 01 exit: delegated goal links to requested_by_task_id."""
    goal = goal_lifecycle.create_goal(
    ...
    persisted = goal_store.get(goal.id)
...
def test_goal_records_iterations_in_iteration_ids(
    ...
    iteration, _ = goal_lifecycle.create_initial_iteration_with_coordinator(goal_id=goal.id)
    refreshed = goal_store.get(goal.id)
...
def test_continuation_iteration_inherits_deferred_goal(
    ...
    iteration1, _ = goal_lifecycle.create_initial_iteration_with_coordinator(goal_id=goal.id)
    ...
    iteration2, _ = goal_lifecycle.create_deferred_iteration_with_coordinator(
...
def test_continuation_iteration_only_from_succeeded_predecessor_with_goal(
    ...
    with pytest.raises(TaskCenterInvariantViolation):
        goal_lifecycle.create_deferred_iteration_with_coordinator(previous_iteration=iteration1)
    ...
    with pytest.raises(TaskCenterInvariantViolation):
        goal_lifecycle.create_deferred_iteration_with_coordinator(previous_iteration=iteration1_no_goal)
...
def test_open_iteration_coordinators_enforces_unique_per_iteration(
    ...
    goal_lifecycle.create_initial_iteration_with_coordinator(goal_id=goal.id)
