...
``start_request(config=...)`` argument needing ``.cwd``, a
...
class _StubStores:
    ...
    def get_request(self, *_args: Any, **_kwargs: Any) -> dict:
        return {"status": "done"}

    def list_tasks_for_request(self, *_args: Any, **_kwargs: Any) -> list[dict]:
        return [
            {"status": "done"},
            {"status": "done"},
            {"status": "failed"},
...
def stubbed_engine(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(engine_module, "start_request", _stub_start)
    ...
    class _StubRecorder:
        ...
        def bind_request_id(self, _request_id: str) -> None:
            return None
