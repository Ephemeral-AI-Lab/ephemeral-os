...
def test_scoped_gate_ignores_edits_inside_target_paths(tmp_path) -> None:
    """Edits to files inside the plan's target set must not trip the gate.

    Per-file base-hash checks already validate those files; the gate
    exists to catch *foreign* Python edits that could have added new
    references outside the plan.
    """
    import time
    ...
    # Simulate an edit to a file that IS part of the plan (target set).
    svc.arbiter.record_edit(
        file_path=str(a),
        edit_type="edit",
        old_hash="",
        new_hash=content_hash("x = 1\n"),
    )
    ...
    result = svc.commit_many_against_base(
        ...
        plan_target_paths=frozenset([str(a)]),
    )
    assert result.success is True
    assert result.status == "committed"
...
def test_scoped_gate_aborts_on_foreign_python_edit(tmp_path) -> None:
    """A Python edit outside the plan's target set aborts the batch."""
    import time
    ...
    plan_captured_at = time.time()
    # Foreign Python edit lands after plan time — gate must fire.
    svc.arbiter.record_edit(
        file_path=str(other),
        edit_type="write",
        old_hash="",
    ...
    # File untouched by the aborted batch.
    assert a.read_text(encoding="utf-8") == "x = 1\n"


def test_scoped_gate_ignores_foreign_non_python_edit(tmp_path) -> None:
    """Non-Python edits (e.g. markdown, json) don't trip the gate."""
    import time
