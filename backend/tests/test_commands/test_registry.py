...
async def test_version_context_and_share_commands(tmp_path: Path, monkeypatch):
    ...
    assert "EphemeralOS" in version_result.message

    context_command, context_args = registry.lookup("/context")
    context_result = await context_command.handler(context_args, context)
    assert "EphemeralOS" in context_result.message or "interactive agent" in context_result.message
...
async def test_init_and_bridge_commands(tmp_path: Path, monkeypatch):
    ...
    assert (tmp_path / ".ephemeralos" / "memory" / "MEMORY.md").exists()
...
@pytest.mark.asyncio
async def test_mcp_commands_report_richer_state(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("EPHEMERALOS_CONFIG_DIR", str(tmp_path / "config"))
    monkeypatch.setenv("EPHEMERALOS_DATA_DIR", str(tmp_path / "data"))
    settings = Settings(
        mcp_servers={
