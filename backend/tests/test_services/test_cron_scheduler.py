...
from ephemeralos.services.cron_scheduler import (
    _jobs_due,
    append_history,
    execute_job,
    load_history,
...
def _tmp_dirs(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    data_dir.mkdir()
    logs_dir.mkdir()
    monkeypatch.setattr("ephemeralos.services.cron_scheduler.get_data_dir", lambda: data_dir)
    monkeypatch.setattr("ephemeralos.services.cron_scheduler.get_logs_dir", lambda: logs_dir)
    # Also redirect the cron registry used by the scheduler
    monkeypatch.setattr(
        "ephemeralos.services.cron.get_cron_registry_path",
...
class TestExecuteJob:
    ...
    async def test_timeout_job(self) -> None:
        with patch("ephemeralos.services.cron_scheduler.asyncio.wait_for") as mock_wait:
            import asyncio
            ...
            with patch(
                "ephemeralos.services.cron_scheduler.asyncio.create_subprocess_exec",
...
class TestSchedulerLoop:
    ...
    async def test_once_mode_fires_due_job(self) -> None:
        ...
        from ephemeralos.services.cron import upsert_cron_job

        upsert_cron_job({"name": "test-once", "schedule": "* * * * *", "command": "echo fired"})

        # Force next_run to the past so it's immediately due
        from ephemeralos.services.cron import load_cron_jobs, save_cron_jobs
