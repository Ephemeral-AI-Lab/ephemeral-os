...
# ===========================================================================
# Test 4: Long integration suite with early termination — the crown jewel.
#
# The test suite runs for ~150 seconds even when it fails. It logs progress
# incrementally to /tmp/long_suite.log. It has three bugs in the config that
# surface at different phases. Crucially, after hitting a FATAL marker the
# suite keeps running (buggy behavior) — the agent must detect the FATAL
# in the log file and cancel the background task to save time, rather than
# waiting out the full 150 seconds per iteration.
#
# No guidance is given in the prompt — the agent must discover this workflow.
# ===========================================================================
...
@pytest.mark.skipif(not EvalAgent.has_all(), reason="API + Daytona both required")
class TestSupernovaLongSuiteEarlyCancel:
    ...
    @pytest.fixture(scope="class")
    def sandbox(self):
        sb = create_test_sandbox("nova-long")
        yield sb
        delete_test_sandbox(sb["id"])
