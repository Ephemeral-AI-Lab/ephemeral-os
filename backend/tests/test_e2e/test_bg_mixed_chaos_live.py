...
class TestCancelAndRelaunch:
    ...
    async def test_cancel_fix_relaunch(self, sandbox):
        ...
        result = await agent.invoke(
            ...
            "6. Create /workspace/fix_applied.txt with 'config_v2' using daytona_write_file\n"
...
class TestBgErrorFgRecovery:
    ...
    async def test_detect_bg_error_recover_in_fg(self, sandbox):
        ...
        result = await agent.invoke(
            ...
            "5. The background build failed! Create /workspace/error_report.txt "
...
class TestFullPipelineSimulation:
    ...
    async def test_cicd_pipeline_mixed_bg_fg(self, sandbox):
        ...
        result = await agent.invoke(
            ...
            "7. Create /workspace/changelog.txt with 'v2.0 release' using daytona_write_file\n\n"
...
class TestNotificationDrivenWorkflow:
    ...
    async def test_bg_completion_triggers_next_phase(self, sandbox):
        ...
        result = await agent.invoke(
            ...
            "4. Create /workspace/build_artifact.txt with 'build_v1_hash_abc123' "
            ...
            "Phase 3 (final):\n"
            "8. Create /workspace/deploy_manifest.txt with 'deploy_ready: true' "
            "using daytona_write_file\n"
            "9. Run 'cat /workspace/build_artifact.txt && cat /workspace/deploy_manifest.txt' "
            "in foreground\n"
        ...
        assert not result.has_errors, \
            f"Pipeline errors: {[e.output[:200] for e in result.error_events]}"
