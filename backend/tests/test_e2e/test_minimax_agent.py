...
async def test_agent_write_and_read_file(sandbox_id):
    ...
    result = await agent.invoke(
        ...
        f"then read it back using daytona_codeact: cat /workspace/minimax_test.txt"
    )
...
async def test_agent_grep_search(sandbox_id):
    ...
    await agent.invoke(
        "Use daytona_codeact to run: echo 'GREP_TARGET_MINIMAX' > /workspace/searchable.txt"
    )
    ...
    tool_started = result.tools_started()
    tool_names = [ev.tool_name for ev in tool_started]
    assert any("daytona_grep" in t or "daytona_codeact" in t for t in tool_names), (
...
async def test_agent_glob_find(sandbox_id):
    ...
    await agent.invoke(
        "Use daytona_codeact to run: touch /workspace/glob_test_1.txt /workspace/glob_test_2.txt"
    )
...
async def test_agent_multi_step_pipeline(sandbox_id):
    ...
    result = await agent.invoke(
        ...
        "2. Use daytona_codeact to run: python3 /workspace/pipeline.py\n"
        "3. Report the output"
    )
...
async def test_tool_started_has_correct_structure(sandbox_id_events):
    ...
    result = await agent.invoke("Use daytona_codeact to run: echo 'STRUCTURE_OK'")
    ...
    for ev in tool_started:
        ...
        name = ev.tool_name
        if name == "daytona_codeact":
            assert "command" in tool_input, f"daytona_codeact missing 'command': {tool_input}"


@pytest.mark.asyncio
async def test_tool_completed_has_output(sandbox_id_events):
    agent = create_eval_agent(sandbox_id=sandbox_id_events, system_prompt=MINIMAX_AGENT_PROMPT)
    result = await agent.invoke("Use daytona_codeact to run: echo 'OUTPUT_CHECK'")
