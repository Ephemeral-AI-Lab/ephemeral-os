"""Comprehensive integration tests for all previously untested EphemeralOS features.

Run with: python -m pytest backend/tests/test_untested_features.py -v --tb=short -x
Or standalone: python backend/tests/test_untested_features.py
...
async def test_hooks_command_block():
    ...
    from ephemeralos.hooks.schemas import CommandHookDefinition
    from ephemeralos.hooks.executor import HookExecutor, HookExecutionContext
...
async def test_hooks_in_agent_loop():
    ...
    from ephemeralos.engine.stream_events import AssistantTextDelta, ToolExecutionStarted, ToolExecutionCompleted
    from ephemeralos.permissions.checker import PermissionChecker
    from ephemeralos.permissions.modes import PermissionMode
    from ephemeralos.tools.base import ToolRegistry
    from ephemeralos.tools.bash_tool import BashTool
...
async def test_skills_load():
    ...
    from ephemeralos.skills.loader import load_user_skills
...
""")
        ...
        import ephemeralos.skills.loader as sl
...
async def test_plugins_load():
    ...
    from ephemeralos.plugins.loader import load_plugin
...
async def test_memory_lifecycle():
    ...
    from ephemeralos.memory.manager import list_memory_files, add_memory_entry, remove_memory_entry
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        import ephemeralos.memory.paths as mp
        orig = mp.get_project_memory_dir
        mem_dir = Path(tmpdir) / ".ephemeralos" / "memory"
        mem_dir.mkdir(parents=True, exist_ok=True)
        mp.get_project_memory_dir = lambda cwd: mem_dir

        # Also patch entrypoint
        import ephemeralos.memory.manager as mm
        ...
        p2 = add_memory_entry(tmpdir, "project-goal", "Building an AI agent framework called EphemeralOS")
...
async def test_config_settings():
    ...
    return (
        ...
        and config_dir.name == ".ephemeralos"
    )
...
async def test_commands_registry():
    ...
    from ephemeralos.commands.registry import (
    ...
    # Default registry
    from ephemeralos.commands.registry import create_default_command_registry
...
async def test_worktree_real_git():
    ...
    from ephemeralos.swarm.worktree import WorktreeManager
...
async def test_mcp_types():
    ...
    from ephemeralos.mcp.types import McpStdioServerConfig, McpToolInfo, McpConnectionStatus
...
async def test_config_paths():
    ...
    # All should be under ~/.ephemeralos
    all_under_home = all(".ephemeralos" in str(p) for p in paths.values())
...
async def test_combined_hooks_skills_agent():
    ...
    from ephemeralos.engine.query import QueryContext, run_query
    from ephemeralos.engine.messages import ConversationMessage
    from ephemeralos.engine.stream_events import AssistantTextDelta, ToolExecutionStarted
    from ephemeralos.permissions.checker import PermissionChecker
    from ephemeralos.permissions.modes import PermissionMode
    from ephemeralos.tools.base import ToolRegistry
    from ephemeralos.tools.bash_tool import BashTool
...
async def test_full_swarm_autoagent():
    ...
    from ephemeralos.engine.query import QueryContext
    from ephemeralos.api.client import AnthropicApiClient
    from ephemeralos.config.settings import PermissionSettings
    from ephemeralos.permissions.checker import PermissionChecker
    from ephemeralos.permissions.modes import PermissionMode
    ...
    import ephemeralos.swarm.mailbox as mb
    import ephemeralos.swarm.team_lifecycle as tl
