...
def test_context_for_includes_active_blockers_section_when_provider_set():
    tc = _tc()
    ...
    async def _provider():
        return [
            SimpleNamespace(
                ...
                root_cause_paths=["backend/src/db/session.py"],
                reason="async session context broken",
                fix_task_id="t_42",
            )
        ]

    tc.set_blocker_provider(_provider)
    task = _task("work-1", task="replan failed work")
    ctx = _run(tc.context_for(task))

    assert "## Active Blockers (in-progress)" in ctx
    assert "blk_abc" in ctx
    assert "fixing" in ctx
    assert "backend/src/db/session.py" in ctx
    assert "async session context broken" in ctx
    assert "t_42" in ctx


def test_context_for_omits_active_blockers_section_when_none():
    tc = _tc()
...
def test_context_for_survives_blocker_provider_exception():
    tc = _tc()

    async def _bad_provider():
        raise RuntimeError("db unreachable")

    tc.set_blocker_provider(_bad_provider)
...
def test_two_replanners_with_active_blocker_see_fix_task_id():
    """Both replanners facing the same blocker receive the same fix_task_id
    in their context, which is the mechanical precondition for the skill rule
    that tells them to use ``add_tasks(deps=[fix_task_id])`` instead of
    ``declare_blocker``."""
    tc = _tc()
    ...
    async def _provider():
        return [
            SimpleNamespace(
                id="blk_shared",
                status=SimpleNamespace(value="assessing"),
                root_cause_paths=["pkg/shared_config.py"],
                reason="load_defaults export removed",
                fix_task_id="t_resolver_1",
            )
        ]
    ...
    ctx_a = _run(tc.context_for(replanner_a))
    ctx_b = _run(tc.context_for(replanner_b))

    for ctx in (ctx_a, ctx_b):
        assert "## Active Blockers (in-progress)" in ctx
        assert "blk_shared" in ctx
        assert "pkg/shared_config.py" in ctx
