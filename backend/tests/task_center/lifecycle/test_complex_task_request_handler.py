...
def test_create_complex_task_request_links_executor(
...
def test_continuation_segment_inherits_continuation_goal(
    ...
    seg2 = handler.create_continuation_segment(previous_segment=seg1_succeeded)
...
def test_handle_segment_closed_terminal_success_closes_request_succeeded(
    handler, request_store, segment_store, task_center_run_id
):
    req = handler.create_complex_task_request(
        task_center_run_id=task_center_run_id,
...
def test_handle_segment_closed_attempt_plan_failed_closes_request_failed(
    handler, request_store, task_center_run_id
):
    req = handler.create_complex_task_request(
        task_center_run_id=task_center_run_id,
...
def test_handle_segment_closed_success_continue_creates_continuation(
    handler, request_store, segment_store, task_center_run_id
):
    req = handler.create_complex_task_request(
        task_center_run_id=task_center_run_id,
...
def test_handle_segment_closed_deregisters_manager(
...
def test_continuation_segment_only_from_succeeded_predecessor_with_goal(
    ...
    with pytest.raises(GraphInvariantViolation):
        handler.create_continuation_segment(previous_segment=seg1)
    ...
    with pytest.raises(GraphInvariantViolation):
        handler.create_continuation_segment(previous_segment=seg1_no_goal)
...
def test_segment_manager_registry_enforces_unique_per_segment(
    ...
    handler.create_initial_segment(complex_task_request_id=req.id)
    # Calling create_initial_segment again should fail because the request now
    # has segment 1 — sequence_no 1 is no longer the contiguous next.
    with pytest.raises(GraphInvariantViolation):
        handler.create_initial_segment(complex_task_request_id=req.id)


def test_close_complex_task_request_delivers_close_report_when_callback_set(
    ...
    handler.create_initial_segment(complex_task_request_id=req.id)
    handler.close_complex_task_request(
