"""Phase 3 — RpcCiBackend now routes business verbs through CiRpcClient.

These tests inject a fake :class:`CiRpcClient` whose ``call`` method returns
canned responses, then assert that each :class:`RpcCiBackend` method
...
class _FakeClient:
    """Minimal stand-in for :class:`CiRpcClient` used by these tests."""
...
def _make_backend(response_map: dict[str, Any]) -> tuple[RpcCiBackend, _FakeClient]:
    backend = RpcCiBackend(
        sandbox_id="sb-test",
        workspace_root="/ws",
        transport=object(),  # type: ignore[arg-type]
    )
    client = _FakeClient(response_map)
    backend._client = client  # bypass the real CiRpcClient
...
def test_query_symbols_uses_daemon_when_initialized() -> None:
    ...
    assert client.calls[0][0] == "query_symbols"
...
def test_query_symbols_propagates_daemon_error() -> None:
    ...
    class _BrokenClient:
        async def call(self, op: str, args: Any | None = None) -> Any:
...
def test_write_file_serializes_specs() -> None:
    backend, client = _make_backend({"write_file": _operation_result_payload()})
...
def test_edit_file_serializes_specs() -> None:
    backend, client = _make_backend({"edit_file": _operation_result_payload()})
...
def test_delete_file_supports_str_or_spec() -> None:
    backend, client = _make_backend({"delete_file": _operation_result_payload()})
...
def test_move_file_serializes_specs() -> None:
    backend, client = _make_backend({"move_file": _operation_result_payload()})
...
def test_warmup_calls_ensure_initialized(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_ensure(self: RpcCiBackend, wait: bool = True) -> bool:
        called.append(True)
        return True

    monkeypatch.setattr(RpcCiBackend, "ensure_initialized", fake_ensure)
    backend = RpcCiBackend(
        sandbox_id="sb",
        workspace_root="/ws",
        transport=object(),  # type: ignore[arg-type]
