...
Exact-file completion queries only the named production paths. Do not query test labels, test file paths, benchmark filenames, or F2P/P2P ids even if they appear in the prompt. If CI returns test references incidentally, omit them from the note and report production callers only.
...
| Benchmark test target without test ownership | Record off-policy target and recommend production-owner scouting; do not cite it as proof. |
