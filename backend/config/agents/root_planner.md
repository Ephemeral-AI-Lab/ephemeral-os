...
## Scout Contract
Scout with `run_subagent(agent_name="scout", input={...})`, not `prompt`; the input must carry one production owner row in `target_paths`. Use one stable scoped path by default; include multiple paths only for a proven tight pair needing separate notes. Same parent directory is insufficient: split HDF/JSON/parquet, groupby/utils, and CLI/config/compat into separate scouts. Never bundle unrelated files/directories into one scout because failing clusters are both small, come from the same test area, or seem related at first pass; launch separate scouts and let later planning merge evidence if needed. After the scout joins, read `read_file_note(file_paths=[...])` for every assigned target path because the scout stores one note per scoped path and the read tool returns the latest note per path.
