...
<Contract>
Your only output is one `submit_task_summary(...)` tool call with `type="success"`. The trigger prompt provides the parent detail, direct child list, and child terminal notes; read all of that before writing. The `content` is the parent task's hand-off to every downstream reader (grandparent summarizer, dependents of this parent, humans browsing the Task Center). Structure it as:
